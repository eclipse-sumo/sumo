...
class SumoSimulation(object):
    ...
    def __init__(self, cfg_file, step_length, host=None, port=None, sumo_gui=False, client_order=1):
        ...
        self.spawned_actors_person = set()
        self.destroyed_actors_person = set()

        # Traffic light manager.
        self.traffic_light_manager = SumoTLManager()

        self.buildings = self.getPolygons()

    
    def getPolygons(self):
        temp = [ ]
        for i in traci.polygon.getIDList():
            if "building" in traci.polygon.getType(i):
                temp2 = []
                for x  in traci.polygon.getShape(i):
                    x = list(x)
        ...
        if v :
            traci.vehicle.subscribe(actor_id, [
             ...
             traci.person.subscribe(actor_id, [
                ...
                traci.constants.VAR_LENGTH, traci.constants.VAR_WIDTH, 
                traci.constants.VAR_POSITION, traci.constants.VAR_ANGLE, 
                traci.constants.VAR_SPEED,  traci.constants.VAR_VEHICLE
            ])

    @staticmethod
    def unsubscribe(actor_id,v=True):
        """
    ...
    def get_actor(actor_id,v=True):
        ...
        if v:
            ...
            color = [ x/255 for x in results[traci.constants.VAR_COLOR] ]
            ...
            extent = [length / 2.0, width / 2.0, height / 2.0]

            return SumoActor(type_id, vclass, location, signals, extent, color)
        else:
            ...
            location = list(results[traci.constants.VAR_POSITION])
            location.extend([0])
            rotation = [0, results[traci.constants.VAR_ANGLE], 0.0]
            #transform = carla.Transform(carla.Location(location[0], location[1], location[2]),
            #                           carla.Rotation(rotation[0], rotation[1], rotation[2]))

            
            extent = [length / 2.0, width / 2.0, 0]

            return SumoActor(type_id,"person",  location, 1,  extent,(1,1,0,1))
    ...
    def tick(self):
        ...
        self.spawned_actors_person = set(traci.simulation.getDepartedPersonIDList())
        self.destroyed_actors_person = set(traci.simulation.getArrivedPersonIDList())
