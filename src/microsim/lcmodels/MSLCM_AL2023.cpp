...
// ===========================================================================
MSLCM_AL2023::MSLCM_AL2023(MSVehicle& v) :
    MSAbstractLaneChangeModel(v, LaneChangeModel::AL2023),
...
MSLCM_AL2023::~MSLCM_AL2023() {
    changed();
}


void
MSLCM_AL2023::initDerivedParameters() {
...
bool
MSLCM_AL2023::debugVehicle() const {
    return DEBUG_COND;
}


int
MSLCM_AL2023::wantsChangeSublane(
    int laneOffset,
...
void
MSLCM_AL2023::setOwnState(const int state) {
...
void
MSLCM_AL2023::updateSafeLatDist(const double travelledLatDist) {
...
double
MSLCM_AL2023::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
    gDebugFlag2 = DEBUG_COND;
...
double
MSLCM_AL2023::_patchSpeed(double min, const double wanted, double max, const MSCFModel& cfModel) {
    if (wanted <= 0) {
        return wanted;
    }
...
void*
MSLCM_AL2023::inform(void* info, MSVehicle* sender) {
...
void
MSLCM_AL2023::msg(const CLeaderDist& cld, double speed, int state) {
    assert(cld.first != 0);
...
double
MSLCM_AL2023::informLeader(int blocked,
                           int dir,
...
void
MSLCM_AL2023::informFollower(int blocked,
                             int dir,
...
double
MSLCM_AL2023::informLeaders(int blocked, int dir,
...
void
MSLCM_AL2023::informFollowers(int blocked, int dir,
                              const std::vector<CLeaderDist>& blockers,
                              double remainingSeconds,
                              double plannedSpeed) {
    // #3727
...
double
MSLCM_AL2023::getExtraReservation(int bestLaneOffset) const {
    if (bestLaneOffset < -1) {
        return 20;
...
double
MSLCM_AL2023::getPosLat() {
    return myVehicle.getLateralPositionOnLane() + mySigmaState;
}

int
MSLCM_AL2023::computeSublaneShift(const MSEdge* prevEdge, const MSEdge* curEdge) {
...
int
MSLCM_AL2023::_wantsChangeSublane(
    int laneOffset,
    LaneChangeAction alternatives,
    const MSLeaderDistanceInfo& leaders,
    const MSLeaderDistanceInfo& followers,
...
int
MSLCM_AL2023::slowDownForBlocked(MSVehicle** blocked, int state) {
...
bool
MSLCM_AL2023::isBidi(const MSLane* lane) const {
...
void
MSLCM_AL2023::updateExpectedSublaneSpeeds(const MSLeaderDistanceInfo& ahead, int sublaneOffset, int laneIndex) {
...
double
MSLCM_AL2023::forecastAverageSpeed(double vSafe, double vMax, double gap, double vLeader) const {
...
double
MSLCM_AL2023::computeSpeedGain(double latDistSublane, double defaultNextSpeed) const {
...
CLeaderDist
MSLCM_AL2023::getLongest(const MSLeaderDistanceInfo& ldi) const {
    int iMax = -1;
    double maxLength = -1;
...
bool
MSLCM_AL2023::tieBrakeLeader(const MSVehicle* veh) const {
...
CLeaderDist
MSLCM_AL2023::getSlowest(const MSLeaderDistanceInfo& ldi) {
    int iMax = 0;
...
int
MSLCM_AL2023::checkBlocking(const MSLane& neighLane, double& latDist, double maneuverDist, int laneOffset,
                            const MSLeaderDistanceInfo& leaders,
                            const MSLeaderDistanceInfo& followers,
                            const MSLeaderDistanceInfo& /*blockers */,
                            const MSLeaderDistanceInfo& neighLeaders,
...
int
MSLCM_AL2023::checkBlockingVehicles(
...
void
MSLCM_AL2023::updateCFRelated(const MSLeaderDistanceInfo& vehicles, double foeOffset, bool leaders) {
...
bool
MSLCM_AL2023::overlap(double right, double left, double right2, double left2) {
    assert(right <= left);
    assert(right2 <= left2);
...
int
MSLCM_AL2023::lowest_bit(int changeReason) {
...
MSLCM_AL2023::StateAndDist
MSLCM_AL2023::decideDirection(StateAndDist sd1, StateAndDist sd2) const {
...
LaneChangeAction
MSLCM_AL2023::getLCA(int state, double latDist) {
...
int
MSLCM_AL2023::checkStrategicChange(int ret,
                                   const MSLane& neighLane,
                                   int laneOffset,
                                   const MSLeaderDistanceInfo& leaders,
                                   const MSLeaderDistanceInfo& neighLeaders,
...
bool
MSLCM_AL2023::mustOvertakeStopped(const MSLane& neighLane, const MSLeaderDistanceInfo& leaders, const MSLeaderDistanceInfo& neighLead,
...
double
MSLCM_AL2023::computeGapFactor(int state) const {
    return (state & LCA_STRATEGIC) != 0 ? MAX2(0.0, (1.0 - myPushy * (1 + 0.5 * myImpatience))) : 1.0;
}


int
MSLCM_AL2023::keepLatGap(int state,
                         const MSLeaderDistanceInfo& leaders,
                         const MSLeaderDistanceInfo& followers,
                         const MSLeaderDistanceInfo& blockers,
                         const MSLeaderDistanceInfo& neighLeaders,
...
void
MSLCM_AL2023::updateGaps(const MSLeaderDistanceInfo& others, double foeOffset, double oldCenter, double gapFactor,
...
double
MSLCM_AL2023::getWidth() const {
    return myVehicle.getVehicleType().getWidth() + NUMERICAL_EPS;
}


double
MSLCM_AL2023::computeSpeedLat(double latDist, double& maneuverDist, bool urgent) const {
...
MSLCM_AL2023::getDesiredAlignment() const {
...
void
MSLCM_AL2023::commitManoeuvre(int blocked, int blockedFully,
                              const MSLeaderDistanceInfo& leaders,
                              const MSLeaderDistanceInfo& neighLeaders,
                              const MSLane& neighLane,
                              double maneuverDist) {
...
double
MSLCM_AL2023::commitFollowSpeed(double speed, double latDist, double secondsToLeaveLane, const MSLeaderDistanceInfo& leaders, double foeOffset) const {
...
double
MSLCM_AL2023::getSafetyFactor() const {
    return 1 / ((1 + 0.5 * myImpatience) * myAssertive);
}

double
MSLCM_AL2023::getOppositeSafetyFactor() const {
    return myOppositeParam <= 0 ? std::numeric_limits<double>::max() : 1 / myOppositeParam;
}


std::string
MSLCM_AL2023::getParameter(const std::string& key) const {
...
void
MSLCM_AL2023::setParameter(const std::string& key, const std::string& value) {
    double doubleValue;
    try {
...
double
MSLCM_AL2023::getLeftBorder(bool checkOpposite) const {
...
double
MSLCM_AL2023::getVehicleCenter() const {
    if (isOpposite()) {
...
double
MSLCM_AL2023::getNeighRight(const MSLane& neighLane) const {
    if (isOpposite()) {
...
bool
MSLCM_AL2023::preventSliding(double maneuverDist) const {
...
bool
MSLCM_AL2023::wantsKeepRight(double keepRightProb) const {
    return keepRightProb * myKeepRightParam > MAX2(myChangeProbThresholdRight, mySpeedGainProbabilityLeft);
}


bool
MSLCM_AL2023::saveBlockerLength(double length, double foeLeftSpace) {
...
bool
MSLCM_AL2023::outsideEdge() const {
