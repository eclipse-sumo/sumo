...
// thresholds
// ===========================================================================
#define GAP_THRESHOLD_SPEEDCTRL 150
#define GAP_THRESHOLD_GAPCTRL 120
...
// method definitions
// ===========================================================================
MSCFModel_ACCFric::MSCFModel_ACCFric(const MSVehicleType* vtype) :
    MSCFModel(vtype),
...
MSCFModel_ACCFric::~MSCFModel_ACCFric() {}


double
MSCFModel_ACCFric::followSpeed(const MSVehicle* const veh, double speed, double gap2pred, double predSpeed, double predMaxDecel, const MSVehicle* const /*pred*/) const {
	double speedFactor = 1.3;// / veh->getLane()->getFrictionCoefficient();// (-1. / 5 * veh->getLane()->getFrictionCoefficient()) + 6. / 5.;																																		  
    const double desSpeed = speedFactor * MIN2(veh->getLane()->getSpeedLimit(), veh->getMaxSpeed());
    const double vACC = _v(veh, gap2pred, speed, predSpeed, desSpeed, true);
    const double vSafe = speedFactor * maximumSafeFollowSpeed(gap2pred, speed, predSpeed, predMaxDecel);
...
double
MSCFModel_ACCFric::stopSpeed(const MSVehicle* const veh, const double speed, double gap, double decel) const {
...
double
MSCFModel_ACCFric::getSecureGap(const MSVehicle* const /*veh*/, const MSVehicle* const /*pred*/, const double speed, const double leaderSpeed, const double /* leaderMaxDecel */) const {
...
double
MSCFModel_ACCFric::insertionFollowSpeed(const MSVehicle* const v, double speed, double gap2pred, double predSpeed, double predMaxDecel, const MSVehicle* const /*pred*/) const {
//#ifdef DEBUG_ACC
//        std::cout << "MSCFModel_ACCFric::insertionFollowSpeed(), speed="<<speed<< std::endl;
//#endif
...
double
MSCFModel_ACCFric::interactionGap(const MSVehicle* const /* veh */, double /* vL */) const {
    /*maximum radar range is ACC is enabled*/
    return 250;
}

double MSCFModel_ACCFric::accelSpeedControl(double vErr) const {
    // Speed control law
    return mySpeedControlGain * vErr;
}

double MSCFModel_ACCFric::accelGapControl(const MSVehicle* const /* veh */, const double gap2pred, const double speed, const double predSpeed, double vErr) const {
...
// Gap control law
    double gclAccel = 0.0;
	//spacing gain factor Lee 1999?
	//double spacingFactor = (-5./4. * veh->getLane()->getFrictionCoefficient()) + 9./4.;
	//double spacingFactor = veh->getLane()->getFrictionCoefficient();
	double spacingFactor = 1.5;							
    double desSpacing = myHeadwayTime * speed  * spacingFactor;;
...
double
MSCFModel_ACCFric::_v(const MSVehicle* const veh, const double gap2pred, const double speed,
...
#ifdef DEBUG_ACC
    if (DEBUG_COND) {
        std::cout << SIMTIME << " MSCFModel_ACCFric::_v() for veh '" << veh->getID() << "'\n"
...
MSCFModel*
MSCFModel_ACCFric::duplicate(const MSVehicleType* vtype) const {
    return new MSCFModel_ACCFric(vtype);
}
