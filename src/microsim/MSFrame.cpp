...
MSFrame::fillOptions() {
    ...
    oc.doRegister("fcd-output.write-empty", new Option_Bool(false));
    oc.addDescription("fcd-output.write-empty", "Output", TL("Write null rows for timesteps with no vehicles in the FCD output (Parquet only)"));
