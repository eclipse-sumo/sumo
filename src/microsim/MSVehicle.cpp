...
MSVehicle::Manoeuvre::entryManoeuvreIsComplete( MSVehicle* veh, const SUMOTime currentTime)
{
    ...
    if (currentStop->parkingarea == nullptr)
        ...
        {
            // TODO? it is possible that whilst manoeuvering a space becomes available so we
            //  end up in a different lot - we could actually check that here and adjust timing
            return false;
        }
        else // manoeuvre complete
        {
            // in case we end up in a different lot - reset the angle for exit - to allow recompute
