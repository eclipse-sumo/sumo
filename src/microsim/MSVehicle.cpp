...
MSVehicle::adaptToLeader(const std::pair<const MSVehicle*, double> leaderInfo,
    ...
    if (leaderInfo.first != 0) {
        ...
        if (lastLink != nullptr) {
            /// @fixme: setting gComputeLC is workaround until implementing #11265
            MSGlobals::gComputeLC = true;
            const double futureVSafe = cfModel.followSpeed(this, lastLink->accelV, leaderInfo.second, leaderInfo.first->getSpeed(), leaderInfo.first->getCurrentApparentDecel(), leaderInfo.first);
