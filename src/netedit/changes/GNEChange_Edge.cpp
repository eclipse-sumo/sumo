...
GNEChange_Edge::~GNEChange_Edge() {
    ...
    if (myEdge->unreferenced()) {
        ...
        WRITE_DEBUG("Deleting unreferenced " + toString(myEdge->getTagProperty().getTag()) + " '" + myEdge->getID() + "' GNEChange_Edge");
        delete myEdge;
    }
}
...
GNEChange_Edge::undo() {
    if (myForward) {
        // show extra information for tests
        WRITE_DEBUG("Removing " + toString(myEdge->getTagProperty().getTag()) + " '" + myEdge->getID() + "' from " + toString(SUMO_TAG_NET));
        // delete edge from net
        myNet->deleteSingleEdge(myEdge);
    } else {
        // show extra information for tests
        WRITE_DEBUG("Adding " + toString(myEdge->getTagProperty().getTag()) + " '" + myEdge->getID() + "' from " + toString(SUMO_TAG_NET));
