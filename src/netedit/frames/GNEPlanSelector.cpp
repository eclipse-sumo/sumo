...
FXDEFMAP(GNEPlanSelector) TagSelectorMap[] = {
    FXMAPFUNC(SEL_COMMAND, MID_GNE_TAG_SELECTED,        GNEPlanSelector::onCmdSelectTag)
};
...
GNEPlanSelector::GNEPlanSelector(GNEFrame* frameParent, GNETagProperties::TagType type, SumoXMLTag tag, bool onlyDrawables) :
...
GNEAttributeCarrier*
GNEPlanSelector::getTemplateAC(SumoXMLTag ACTag) const {
    // clear myACTemplates and myTagsMatchBox
    for (const auto& ACTemplate : myACTemplates) {
...
void
GNEPlanSelector::setCurrentTagType(GNETagProperties::TagType tagType, const bool onlyDrawables, const bool notifyFrameParent) {
    // check if net has proj
    const bool proj = (GeoConvHelper::getFinal().getProjString() != "!");
    // set new tagType
    myTagType = tagType;
    // change GNEPlanSelector text
    switch (myTagType) {
...
void
GNEPlanSelector::setCurrentTag(SumoXMLTag newTag, const bool notifyFrameParent) {
...
long
GNEPlanSelector::onCmdSelectTag(FXObject*, FXSelector, void*) {
    // iterate over all myTagsMatchBox
    for (int i = 0; i < (int)myACTemplates.size(); i++) {
        if (myACTemplates.at(i)->getAC() && myACTemplates.at(i)->getAC()->getTagProperty().getFieldString() == myTagsMatchBox->getText().text()) {
            ...
            WRITE_DEBUG(("Selected item '" + myTagsMatchBox->getText() + "' in GNEPlanSelector").text());
            return 1;
        }
    }
    // reset templateAC
    ...
    // Write Warning in console if we're in testing mode
    WRITE_DEBUG("Selected invalid item in GNEPlanSelector");
    // call tag selected function
    myFrameParent->tagSelected();
    return 1;
}
...
GNEAttributeCarrier*
GNEPlanSelector::ACTemplate::getAC() const {
    return myAC;
}


GNEPlanSelector::ACTemplate::ACTemplate(GNENet* net, const GNETagProperties tagProperty) :
    myAC(nullptr) {
...
GNEPlanSelector::ACTemplate::~ACTemplate() {
    delete myAC;
}
