    ...
    if (myClickedEdges.empty() || ((myClickedEdges.size() > 0) && (myClickedEdges.back() != edge))) {
        ...
        if (myClickedEdges.size() > 1) {
            ...
            if (myPersonPlanFrameParent->myPersonPlanTagSelector->getCurrentTagProperties().isRide()) {
                myTemporalEdgePath = myPersonPlanFrameParent->myViewNet->getNet()->getPathCalculator()->calculateDijkstraPath(SVC_PASSENGER, myClickedEdges);
            } else {
                myTemporalEdgePath = myPersonPlanFrameParent->myViewNet->getNet()->getPathCalculator()->calculateDijkstraPath(SVC_PEDESTRIAN, myClickedEdges);
            }
        }
        return true;
    } else {
