...
void
GNEDemandElement::writeDemandElement(OutputDevice& device) const {
    // obtain tag properties
    const TagProperties& tagProperties = myTagProperty;
    // first check if minimum number of childs is correct
    if ((tagProperties.hasMinimumNumberOfChilds() || tagProperties.hasMinimumNumberOfChilds()) && !checkDemandElementChildRestriction()) {
        ...
        if (tagProperties.canWriteChildsSeparate() && tagProperties.hasAttribute(SUMO_ATTR_FILE) && !getAttribute(SUMO_ATTR_FILE).empty()) {
            // we assume that rerouter values files is placed in the same folder as the demand element file
            OutputDevice& deviceChilds = OutputDevice::getDevice(FileHelpers::getFilePath(OptionsCont::getOptions().getString("sumo-demand elements-file")) + getAttribute(SUMO_ATTR_FILE));
            deviceChilds.writeXMLHeader("rerouterValue", "demand element_file.xsd");
            // save childs in a different filename
            for (auto i : myDemandElementChilds) {
                // avoid to write two times demand elements that haben two parents (Only write as child of first parent)
                if (i->getSecondDemandElementParent() == nullptr) {
                    i->writeDemandElement(deviceChilds);
        ...
        // save generic parameters (Always after childs to avoid problems with demand elements.xsd)
        writeParams(device);
        // Close tag
        device.closeTag();
    }
