                ...
                if (personPlanSegmentsIT->stops.size() > 0) {
                    ...
                    for (const auto& stop : personPlanSegmentsIT->stops) {
                        ...
                        for (const auto& shapesStopPos : shapesStop.first) {
                            ...
                            if (shapesStopPos == shapesStop.first.back()) {
                                myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge, shapesStopPos, 0, 0, false, true);
                            } else {
                                myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge, shapesStopPos, 0, 0, true, true);
                            }
                        }
                        // check that next person plan segment isn't the last
                        if ((personPlanSegmentsIT + 1) != personPlanSegments.end()) {
                            ...
                            for (const auto& shapesStopPos : shapesStop.second) {
                                myDemandElementSegmentGeometry.insertPartialEdgeSegment((personPlanSegmentsIT + 1)->personPlan, personPlanSegmentsIT->edge, shapesStopPos, 0, 0, true, true);
                            }
                        }
                    }
                    ...
                    for (const auto& busStop : personPlanSegmentsIT->busStops) {
                        ...
                        for (const auto& shapeBusStopPos : shapesBusStop.first) {
                            ...
                            if (shapeBusStopPos == shapesBusStop.first.back()) {
                                myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge, shapeBusStopPos, 0, 0, false, true);
                            } else {
                                myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge, shapeBusStopPos, 0, 0, true, true);
                            }
                        }
                        // check that next person plan segment isn't the last
                        if ((personPlanSegmentsIT + 1) != personPlanSegments.end()) {
                            ...
                            for (const auto& shapeBusStopPos : shapesBusStop.second) {
                                myDemandElementSegmentGeometry.insertPartialEdgeSegment((personPlanSegmentsIT + 1)->personPlan, personPlanSegmentsIT->edge, shapeBusStopPos, 0, 0, true, true);
                            }
                        }
                    }
                    ...
                    for (const auto& shapeArrivalPos : shapeArrival.first) {
                        ...
                        if ((shapeArrivalPos == shapeArrival.first.back()) && (shapeArrival.first.size() > 0) && (shapeArrival.second.size() > 0)) {
                            myDemandElementSegmentGeometry.insertPartialEdgeSegment((personPlanSegmentsIT + 1)->personPlan, personPlanSegmentsIT->edge, shapeArrivalPos, 0, 0, true, true);
                        } else {
                            myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge, shapeArrivalPos, 0, 0, true, true);
                        }
                    }
                    // add second shape
                    ...
                    for (int i = 0; i < (int)lane->getLaneShape().size(); i++) {
                        // insert segment
                        if (i < (int)lane->getLaneShape().size() - 1) {
                            myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge,
                                    lane->getLaneShape()[i],
                                    lane->getShapeLengths()[i],
                                    lane->getShapeRotations()[i],
                                    true, true);
                        } else {
                            myDemandElementSegmentGeometry.insertPartialEdgeSegment(personPlanSegmentsIT->personPlan, personPlanSegmentsIT->edge,
                                    lane->getLaneShape()[i], 0, 0, true, true);
                        }
                    }
                }
...
GNEPerson::calculateSmoothPersonPlanConnection(const GNEDemandElement* personPlanElement, const GNEEdge* edgeFrom, const GNEEdge* edgeTo) {
    ...
    for (const auto& i : smoothShape) {
        myDemandElementSegmentGeometry.insertLane2LaneSegment(personPlanElement, edgeTo->getGNEJunctionSource(), i, 0, 0, true, true);
    }
    */
}
...
GNEPerson::calculatePersonPlanConnectionBusStop(GNELane* previousLane, GNEAdditional* busStop, GNELane* nextLane) {
    ...
    if (previousLane) {
        ...
        double offsetFirstPosition = previousLane->getLaneShape().nearest_offset_to_point2D(firstBusStopShapePosition, false);
        // split laneShape
        auto splittedFirstLaneShape = previousLane->getLaneShape().splitAt(offsetFirstPosition, true);
    ...
    if (nextLane) {
        ...
        double offsetLastPosition = nextLane->getLaneShape().nearest_offset_to_point2D(lastBusStopShapePosition, false);
        // split laneShape
        auto splittedLastLaneShape = nextLane->getLaneShape().splitAt(offsetLastPosition, true);
...
GNEPerson::calculatePersonPlanConnectionStop(GNELane* previousLane, GNEDemandElement* stop, GNELane* nextLane) {
    ...
    if (stop->getTagProperty().getTag() == SUMO_TAG_PERSONSTOP_BUSSTOP) {
        ...
        if (previousLane) {
            ...
            auto splittedFirstLaneShape = previousLane->getLaneShape().splitAt(stop->getAttributeDouble(SUMO_ATTR_STARTPOS), true);
            // fill result position vector
            for (const auto& i : splittedFirstLaneShape.first) {
                result.first.push_back(i);
            }
        ...
        if (nextLane) {
            ...
            auto splittedLastLaneShape = nextLane->getLaneShape().splitAt(stop->getAttributeDouble(SUMO_ATTR_ENDPOS), true);
...
GNEPerson::calculatePersonPlanConnectionArrivalPos(GNELane* previousLane, double arrivalPosPersonPlan, GNELane* nextLane) {
    ...
    if ((previousLane == nextLane) && (previousLane != nullptr)) {
        ...
        return previousLane->getLaneShape().splitAt(arrivalPosPersonPlan, true);
    } else {
        // declare pair of position vectors
        std::pair<PositionVector, PositionVector> solution;
        // split previousLane in arrivalPos
        if (previousLane) {
            solution.first = previousLane->getLaneShape().splitAt(arrivalPosPersonPlan, true).first;
        }
        // split nextLane in arrivalPos
        if (nextLane) {
            solution.second = nextLane ->getLaneShape().splitAt(arrivalPosPersonPlan, true).second;
        }
        // return solution
        return solution;
    }
