...
GNERouteHandler::buildVType(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const SUMOVTypeParameter& vTypeParameter) {
    ...
    if (DEFAULT_VTYPES.count(vTypeParameter.id) > 0) {
        ...
        if (!checkElement(SUMO_TAG_VTYPE, element)) {
            ...
            GNEDemandElement* vType = new GNEVType(myNet, TEMPORAL_FILENAME, vTypeParameter);
...
GNERouteHandler::buildVTypeDistribution(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const int deterministic,
    ...
    if (!checkElement(SUMO_TAG_VTYPE_DISTRIBUTION, element)) {
        ...
        GNEVTypeDistribution* vTypeDistribution = new GNEVTypeDistribution(id, myNet, TEMPORAL_FILENAME, deterministic);
        if (myAllowUndoRedo) {
...
GNERouteHandler::buildRoute(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, SUMOVehicleClass vClass,
    ...
    if (!checkElement(SUMO_TAG_ROUTE, element)) {
        ...
        if (edges.empty()) {
            ...
            // create GNERoute
            GNEDemandElement* route = new GNERoute(id, myNet, TEMPORAL_FILENAME, vClass, edges, color, repeat, cycleTime, probability, routeParameters);
...
GNERouteHandler::buildRouteDistribution(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const std::string& id,
    ...
    if (!checkElement(SUMO_TAG_ROUTE_DISTRIBUTION, element)) {
        ...
        // create distributions
        GNERouteDistribution* routeDistribution = new GNERouteDistribution(id, myNet, TEMPORAL_FILENAME);
        if (myAllowUndoRedo) {
...
GNERouteHandler::buildVehicleOverRoute(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const SUMOVehicleParameter& vehicleParameters) {
    ...
    if (!checkElement(SUMO_TAG_VEHICLE, element)) {
        ...
        if (type == nullptr) {
            ...
            GNEDemandElement* vehicle = new GNEVehicle(SUMO_TAG_VEHICLE, myNet, TEMPORAL_FILENAME, type, route, vehicleParameters);
            if (myAllowUndoRedo) {
    ...
    if (!checkElement(GNE_TAG_VEHICLE_WITHROUTE, element)) {
        ...
        if (edges.empty()) {
            ...
            if (type == nullptr) {
                ...
                // create vehicle using vehicleParameters
                GNEDemandElement* vehicle = new GNEVehicle(GNE_TAG_VEHICLE_WITHROUTE, myNet, TEMPORAL_FILENAME, type, vehicleParameters);
...
GNERouteHandler::buildFlowOverRoute(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const SUMOVehicleParameter& vehicleParameters) {
    ...
    if (!checkElement(GNE_TAG_FLOW_ROUTE, element)) {
        ...
        if (type == nullptr) {
            ...
            GNEDemandElement* flow = new GNEVehicle(GNE_TAG_FLOW_ROUTE, myNet, TEMPORAL_FILENAME, type, route, vehicleParameters);
            if (myAllowUndoRedo) {
    ...
    if (!checkElement(GNE_TAG_FLOW_WITHROUTE, element)) {
        ...
        if (edges.empty()) {
            ...
            if (type == nullptr) {
                ...
                // create vehicle using vehicleParameters
                GNEDemandElement* vehicle = new GNEVehicle(GNE_TAG_FLOW_WITHROUTE, myNet, TEMPORAL_FILENAME, type, vehicleParameters);
                // create embedded route
                GNEDemandElement* route = new GNERoute(myNet, vehicle, edges, color, repeat, cycleTime, probability, routeParameters);
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(vehicle, TL("add ") + vehicle->getTagStr() + " '" + vehicleParameters.id + "'");
    ...
    if (!checkElement(SUMO_TAG_TRIP, element)) {
        ...
        if (!fromEdge || !toEdge) {
            ...
            if (type == nullptr) {
                ...
                GNEDemandElement* trip = new GNEVehicle(SUMO_TAG_TRIP, myNet, TEMPORAL_FILENAME, type, fromEdge, toEdge, vehicleParameters);
                if (myAllowUndoRedo) {
    ...
    if (!checkElement(GNE_TAG_TRIP_JUNCTIONS, element)) {
        ...
        if (!fromJunction || !toJunction) {
            ...
            if (type == nullptr) {
                ...
                GNEDemandElement* flow = new GNEVehicle(GNE_TAG_TRIP_JUNCTIONS, myNet, TEMPORAL_FILENAME, type, fromJunction, toJunction, vehicleParameters);
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(flow, TL("add ") + flow->getTagStr() + " '" + vehicleParameters.id + "'");
                    myNet->getViewNet()->getUndoList()->add(new GNEChange_DemandElement(flow, true), true);
                    myNet->getViewNet()->getUndoList()->end();
    ...
    if (!checkElement(GNE_TAG_TRIP_TAZS, element)) {
        ...
        if (!fromTAZ || !toTAZ) {
            ...
            if (type == nullptr) {
                ...
                // create trip using vehicleParameters
                GNEDemandElement* flow = new GNEVehicle(GNE_TAG_TRIP_TAZS, myNet, TEMPORAL_FILENAME, type, fromTAZ, toTAZ, vehicleParameters);
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(flow, TL("add ") + flow->getTagStr() + " '" + vehicleParameters.id + "'");
                    myNet->getViewNet()->getUndoList()->add(new GNEChange_DemandElement(flow, true), true);
                    myNet->getViewNet()->getUndoList()->end();
    ...
    if (!checkElement(SUMO_TAG_FLOW, element)) {
        ...
        if (!fromEdge || !toEdge) {
            ...
            if (type == nullptr) {
                ...
                // create trip or flow using tripParameters
                GNEDemandElement* flow = new GNEVehicle(SUMO_TAG_FLOW, myNet, TEMPORAL_FILENAME, type, fromEdge, toEdge, vehicleParameters);
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(flow, TL("add ") + flow->getTagStr() + " '" + vehicleParameters.id + "'");
                    myNet->getViewNet()->getUndoList()->add(new GNEChange_DemandElement(flow, true), true);
                    myNet->getViewNet()->getUndoList()->end();
    ...
    if (!checkElement(GNE_TAG_FLOW_JUNCTIONS, element)) {
        ...
        if (!fromJunction || !toJunction) {
            ...
            if (type == nullptr) {
                ...
                GNEDemandElement* flow = new GNEVehicle(GNE_TAG_FLOW_JUNCTIONS, myNet, TEMPORAL_FILENAME, type, fromJunction, toJunction, vehicleParameters);
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(flow, TL("add ") + flow->getTagStr() + " '" + vehicleParameters.id + "'");
                    myNet->getViewNet()->getUndoList()->add(new GNEChange_DemandElement(flow, true), true);
                    myNet->getViewNet()->getUndoList()->end();
    ...
    if (!checkElement(GNE_TAG_FLOW_TAZS, element)) {
        ...
        if (!fromTAZ || !toTAZ) {
            ...
            if (type == nullptr) {
                ...
                // create flow using vehicleParameters
                GNEDemandElement* flow = new GNEVehicle(GNE_TAG_FLOW_TAZS, myNet, TEMPORAL_FILENAME, type, fromTAZ, toTAZ, vehicleParameters);
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(flow, TL("add ") + flow->getTagStr() + " '" + vehicleParameters.id + "'");
                    myNet->getViewNet()->getUndoList()->add(new GNEChange_DemandElement(flow, true), true);
                    myNet->getViewNet()->getUndoList()->end();
...
GNERouteHandler::buildPerson(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const SUMOVehicleParameter& personParameters) {
    ...
    if (!checkElement(SUMO_TAG_PERSON, element)) {
        ...
        if (type == nullptr) {
            ...
            GNEDemandElement* person = new GNEPerson(SUMO_TAG_PERSON, myNet, TEMPORAL_FILENAME, type, personParameters);
            if (myAllowUndoRedo) {
...
GNERouteHandler::buildPersonFlow(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const SUMOVehicleParameter& personFlowParameters) {
    ...
    if (!checkElement(SUMO_TAG_PERSONFLOW, element)) {
        ...
        if (type == nullptr) {
            ...
            GNEDemandElement* personFlow = new GNEPerson(SUMO_TAG_PERSONFLOW, myNet, TEMPORAL_FILENAME, type, personFlowParameters);
            if (myAllowUndoRedo) {
...
GNERouteHandler::buildContainer(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const SUMOVehicleParameter& containerParameters) {
    ...
    if (!checkElement(SUMO_TAG_CONTAINER, element)) {
        ...
        if (type == nullptr) {
            ...
            GNEDemandElement* container = new GNEContainer(SUMO_TAG_CONTAINER, myNet, TEMPORAL_FILENAME, type, containerParameters);
            if (myAllowUndoRedo) {
...
GNERouteHandler::buildContainerFlow(const CommonXMLStructure::SumoBaseObject* /*sumoBaseObject*/, const SUMOVehicleParameter& containerFlowParameters) {
    ...
    if (!checkElement(SUMO_TAG_CONTAINERFLOW, element)) {
        ...
        if (type == nullptr) {
            ...
            GNEDemandElement* containerFlow = new GNEContainer(SUMO_TAG_CONTAINERFLOW, myNet, TEMPORAL_FILENAME, type, containerFlowParameters);
            if (myAllowUndoRedo) {
