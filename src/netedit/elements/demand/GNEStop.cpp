...
GNEStop::isDemandElementValid() const {
    ...
    if (myTagProperty.getTag() != SUMO_TAG_STOP_LANE) {
        ...
        // obtain lane length
        double laneLength = getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength() * getParentLanes().front()->getLengthGeometryFactor();
        ...
        if (!(parametersSet & STOP_START_SET) && !(parametersSet & STOP_END_SET)) {
            ...
            return (endPosCopy <= getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength());
        } else if (!(parametersSet & STOP_END_SET)) {
            return (startPosCopy >= 0);
        } else {
            return ((startPosCopy >= 0) && (endPosCopy <= getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength()) && ((endPosCopy - startPosCopy) >= POSITION_EPS));
        }
    }
}
...
GNEStop::getDemandElementProblem() const {
    ...
    double endPosCopy = endPos;
    // obtain lane length
    double laneLength = getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength();
    // check if position has to be fixed
    if (startPosCopy < 0) {
        startPosCopy += laneLength;
    }
    ...
    } else if (startPosCopy > getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength()) {
        errorStart = (toString(SUMO_ATTR_STARTPOS) + " > lanes's length");
    }
    if (endPosCopy < 0) {
        errorEnd = (toString(SUMO_ATTR_ENDPOS) + " < 0");
    } else if (endPosCopy > getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength()) {
...
GNEStop::moveGeometry(const Position& offset) {
    ...
    if ((getParentLanes().size() > 0) && ((parametersSet & STOP_START_SET) || (parametersSet & STOP_END_SET))) {
        ...
        if ((parametersSet & STOP_START_SET) && (parametersSet & STOP_END_SET)) {
            ...
            double laneLengt = getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength() * getParentLanes().front()->getLengthGeometryFactor();
...
GNEStop::getPositionInView() const {
    if (getParentLanes().size() > 0) {
        ...
        double end = fabs(parametersSet & STOP_END_SET ? endPos : getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength());
    ...
    if (drawPersonPlan) {
        ...
        if (s.drawDetail(s.detailSettings.stopsDetails, exaggeration) && getParentLanes().size() > 0) {
            ...
            GLHelper::drawBoxLines(myDemandElementGeometry.getShape(), myDemandElementGeometry.getShapeRotations(), myDemandElementGeometry.getShapeLengths(), exaggeration * 0.1, 0,
                                   getParentLanes().front()->getParentEdges().front()->getNBEdge()->getLaneWidth(getParentLanes().front()->getIndex()) * 0.5);
            GLHelper::drawBoxLines(myDemandElementGeometry.getShape(), myDemandElementGeometry.getShapeRotations(), myDemandElementGeometry.getShapeLengths(), exaggeration * 0.1, 0,
                                   getParentLanes().front()->getParentEdges().front()->getNBEdge()->getLaneWidth(getParentLanes().front()->getIndex()) * -0.5);
            // pop draw matrix
            glPopMatrix();
    ...
    switch (key) {
        ...
        case SUMO_ATTR_ENDPOS:
            if (parametersSet & STOP_END_SET) {
                ...
                return getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength();
            }
        default:
            throw InvalidArgument(getTagStr() + " doesn't have an attribute of type '" + toString(key) + "'");
    }
    ...
    switch (key) {
        ...
        case SUMO_ATTR_STARTPOS:
            if (value.empty()) {
                ...
                return SUMORouteHandler::isStopPosValid(parse<double>(value), endPos, getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength(), POSITION_EPS, friendlyPos);
            } else {
                return false;
            }
        case SUMO_ATTR_ENDPOS:
            if (value.empty()) {
                ...
                return SUMORouteHandler::isStopPosValid(startPos, parse<double>(value), getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength(), POSITION_EPS, friendlyPos);
            } else {
                return false;
            }
        case SUMO_ATTR_FRIENDLY_POS:
    ...
    switch (key) {
        case SUMO_ATTR_STARTPOS:
            if (parametersSet & STOP_END_SET) {
                ...
                undoList->p_add(new GNEChange_Attribute(this, key, toString(getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength() - MIN_STOP_LENGTH)));
            }
            break;
        case SUMO_ATTR_ENDPOS:
            undoList->p_add(new GNEChange_Attribute(this, key, toString(getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength())));
            break;
        case SUMO_ATTR_DURATION:
...
GNEStop::getStartGeometryPositionOverLane() const {
    ...
    if (parametersSet & STOP_START_SET) {
        ...
        fixedPos = getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength() - MIN_STOP_LENGTH;
    }
...
GNEStop::getEndGeometryPositionOverLane() const {
    ...
    if (parametersSet & STOP_END_SET) {
        ...
        fixedPos = getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength();
    }
    const double len = getParentLanes().front()->getParentEdges().front()->getNBEdge()->getFinalLength();
    if (fixedPos < 0) {
        fixedPos += len;
