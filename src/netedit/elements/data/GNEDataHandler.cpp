...
GNEDataHandler::buildDataSet(const std::string& dataSetID) {
    ...
    if (myNet->getAttributeCarriers()->retrieveDataSet(dataSetID, false) == nullptr) {
        ...
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(dataSet->getTagProperty().getGUIIcon(), TL("add data set"));
...
GNEDataHandler::buildDataInterval(const CommonXMLStructure::SumoBaseObject* /* sumoBaseObject */,
    ...
    if (dataSet == nullptr) {
        ...
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(dataInterval->getTagProperty().getGUIIcon(), TL("add data set and data interval"));
            myNet->getViewNet()->getUndoList()->add(new GNEChange_DataSet(dataSet, true), true);
        ...
        GNEDataInterval* dataInterval = new GNEDataInterval(dataSet, begin, end);
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(dataInterval->getTagProperty().getGUIIcon(), TL("add data interval"));
            myNet->getViewNet()->getUndoList()->add(new GNEChange_DataInterval(dataInterval, true), true);
            myNet->getViewNet()->getUndoList()->end();
        } else {
            // insert dataInterval without allowing undo/redo
    ...
    if (dataSet != nullptr) {
        ...
        if (dataInterval != nullptr) {
            ...
            if (edge) {
                ...
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(edgeData->getTagProperty().getGUIIcon(), TL("add edge rel"));
    ...
    if (dataSet != nullptr) {
        ...
        if (dataInterval != nullptr) {
            ...
            if (fromTAZ == nullptr) {
                ...
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(edgeData->getTagProperty().getGUIIcon(), TL("add TAZ rel"));
                    myNet->getViewNet()->getUndoList()->add(new GNEChange_GenericData(edgeData, true), true);
                    myNet->getViewNet()->getUndoList()->end();
                } else {
                    dataInterval->addGenericDataChild(edgeData);
