...
GNEAdditionalHandler::buildBusStop(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lane == nullptr) {
            ...
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(busStop->getTagProperty().getGUIIcon(), TL("add bus stop '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildTrainStop(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(trainStop->getTagProperty().getGUIIcon(), TL("add train stop '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildAccess(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& laneID,
    ...
    if (lane == nullptr) {
        ...
        // insert depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(access->getTagProperty().getGUIIcon(), TL("add access in '") + busStop->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildContainerStop(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& laneID,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(containerStop->getTagProperty().getGUIIcon(), TL("add container stop '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildChargingStation(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(chargingStation->getTagProperty().getGUIIcon(), TL("add charging station '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildParkingArea(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& laneID,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(parkingArea->getTagProperty().getGUIIcon(), TL("add parking area '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildParkingSpace(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const double x, const double y, const double z,
    ...
    if (!width.empty() && !GNEAttributeCarrier::canParse<double>(width)) {
        ...
        if (parkingArea == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(parkingSpace->getTagProperty().getGUIIcon(), TL("add parking space in '") + parkingArea->getID() + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildE1Detector(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& laneID,
    ...
    if (!SUMOXMLDefinitions::isValidDetectorID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(detectorE1->getTagProperty().getGUIIcon(), TL("add induction loop '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildSingleLaneDetectorE2(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& laneID,
    ...
    if (!SUMOXMLDefinitions::isValidDetectorID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(detectorE2->getTagProperty().getGUIIcon(), TL("add lane area detector '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildDetectorE3(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const Position& pos, const SUMOTime period,
    ...
    if (!SUMOXMLDefinitions::isValidDetectorID(id)) {
        ...
        // insert depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(E3->getTagProperty().getGUIIcon(), TL("add entry-exit detector '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildDetectorEntry(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& laneID, const double pos,
    ...
    if (lane == nullptr) {
        ...
        // insert depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(entry->getTagProperty().getGUIIcon(), TL("add entry detector in '") + E3->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildDetectorExit(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& laneID, const double pos,
    ...
    if (lane == nullptr) {
        ...
        // insert depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(exit->getTagProperty().getGUIIcon(), TL("add exit detector in '") + E3->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildDetectorE1Instant(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& laneID, double pos,
    ...
    if (!SUMOXMLDefinitions::isValidDetectorID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(detectorE1Instant->getTagProperty().getGUIIcon(), TL("add instant induction loop '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildLaneCalibrator(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& laneID, const double pos,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (!checkLanePosition(pos, 0, lane->getParentEdge()->getNBEdge()->getFinalLength(), false)) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(calibrator->getTagProperty().getGUIIcon(), TL("add lane calibrator '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildEdgeCalibrator(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& edgeID, const double pos,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (!checkLanePosition(pos, 0, edge->getLanes().front()->getParentEdge()->getNBEdge()->getFinalLength(), false)) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(calibrator->getTagProperty().getGUIIcon(), TL("add calibrator '") + id + "'");
                overwriteAdditional();
                myNet->getViewNet()->getUndoList()->add(new GNEChange_Additional(calibrator, true), true);
                myNet->getViewNet()->getUndoList()->end();
                // center after creation
...
GNEAdditionalHandler::buildCalibratorFlow(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const SUMOVehicleParameter& vehicleParameter) {
    ...
    if (vType == nullptr) {
        ...
        // insert depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(flow->getTagProperty().getGUIIcon(), TL("add calibrator flow in '") + calibrator->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildRerouter(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const Position& pos,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (edges.size() > 0) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(rerouter->getTagProperty().getGUIIcon(), TL("add rerouter '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildRerouterInterval(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const SUMOTime begin, const SUMOTime end) {
    ...
    if (rerouter == nullptr) {
        ...
        if (checkOverlappingRerouterIntervals(rerouter, begin, end)) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(rerouterInterval->getTagProperty().getGUIIcon(), TL("add rerouter interval in '") + rerouter->getID() + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildClosingLaneReroute(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& closedLaneID, SVCPermissions permissions) {
    ...
    if (lane == nullptr) {
        ...
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(closingLaneReroute->getTagProperty().getGUIIcon(), TL("add closing lane reroute in '") + lane->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildClosingReroute(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& closedEdgeID, SVCPermissions permissions) {
    ...
    if (edge == nullptr) {
        ...
        // add it to interval parent depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(closingLaneReroute->getTagProperty().getGUIIcon(), TL("add closing reroute in '") + edge->getID() + "'");
            overwriteAdditional();
            myNet->getViewNet()->getUndoList()->add(new GNEChange_Additional(closingLaneReroute, true), true);
            myNet->getViewNet()->getUndoList()->end();
        } else {
...
GNEAdditionalHandler::buildDestProbReroute(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& newEdgeDestinationID, const double probability) {
    ...
    if (edge == nullptr) {
        ...
        // add it to interval parent depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(destProbReroute->getTagProperty().getGUIIcon(), TL("add dest prob reroute in '") + edge->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildParkingAreaReroute(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& newParkignAreaID, const double probability, const bool visible) {
    ...
    if (parkingArea == nullptr) {
        ...
        // add it to interval parent depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(parkingAreaReroute->getTagProperty().getGUIIcon(), TL("add parking area reroute in '") + parkingArea->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildRouteProbReroute(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& newRouteID, const double probability) {
    ...
    if (route == nullptr) {
        ...
        // add it to interval parent depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(routeProbReroute->getTagProperty().getGUIIcon(), TL("add route prob reroute in '") + route->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildRouteProbe(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& edgeID, const SUMOTime period,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (edge == nullptr) {
            ...
            // insert depending of allowUndoRedo
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(routeProbe->getTagProperty().getGUIIcon(), TL("add route probe '") + id + "'");
                overwriteAdditional();
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lanes.size() > 0) {
            ...
            if (!vTypes.empty() && !checkListOfVehicleTypes(vTypes)) {
                ...
                // insert depending of allowUndoRedo
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(variableSpeedSign->getTagProperty().getGUIIcon(), TL("add Variable Speed Sign '") + id + "'");
                    overwriteAdditional();
...
GNEAdditionalHandler::buildVariableSpeedSignStep(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const SUMOTime time, const std::string& speed) {
    ...
    if (VSS == nullptr) {
        ...
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(variableSpeedSignStep->getTagProperty().getGUIIcon(), TL("add VSS Step in '") + VSS->getID() + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildVaporizer(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& edgeID, const SUMOTime beginTime,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(edgeID)) {
        ...
        if (edge == nullptr) {
            ...
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(vaporizer->getTagProperty().getGUIIcon(), TL("add vaporizer in '") + edge->getID() + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildTAZ(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const PositionVector& shape,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(TAZ->getTagProperty().getGUIIcon(), TL("add TAZ '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildTAZSource(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& edgeID, const double departWeight) {
    ...
    if (TAZ == nullptr) {
        ...
        if (TAZSink == nullptr) {
            ...
            // add it depending of allow undoRed
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(TAZ->getTagProperty().getGUIIcon(), TL("add TAZ Sink in '") + TAZ->getID() + "'");
                overwriteAdditional();
                myNet->getViewNet()->getUndoList()->add(new GNEChange_Additional(TAZSink, true), true);
                myNet->getViewNet()->getUndoList()->end();
            } else {
        ...
        if (TAZSource == nullptr) {
            ...
            // add it depending of allow undoRed
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(TAZ->getTagProperty().getGUIIcon(), TL("add TAZ Source in '") + TAZ->getID() + "'");
                overwriteAdditional();
                myNet->getViewNet()->getUndoList()->add(new GNEChange_Additional(TAZSource, true), true);
                myNet->getViewNet()->getUndoList()->end();
            } else {
            ...
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(TAZ->getTagProperty().getGUIIcon(), TL("update TAZ Source in '") + TAZ->getID() + "'");
...
GNEAdditionalHandler::buildTAZSink(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& edgeID, const double arrivalWeight) {
    ...
    if (TAZ == nullptr) {
        ...
        if (TAZSink == nullptr) {
            ...
            // update TAZ Attribute depending of allow undoRed
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(TAZ->getTagProperty().getGUIIcon(), TL("update TAZ Sink in '") + TAZ->getID() + "'");
...
GNEAdditionalHandler::buildTractionSubstation(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const Position& pos,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        // insert depending of allowUndoRedo
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(tractionSubstation->getTagProperty().getGUIIcon(), TL("add taction substation '") + id + "'");
            overwriteAdditional();
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lanes.size() > 0) {
            ...
            if (!GNEAdditional::areLaneConsecutives(lanes)) {
                ...
                // insert depending of allowUndoRedo
                if (myAllowUndoRedo) {
                    myNet->getViewNet()->getUndoList()->begin(overheadWire->getTagProperty().getGUIIcon(), TL("add overhead wire '") + id + "'");
                    overwriteAdditional();
...
GNEAdditionalHandler::buildPolygon(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& type,
    ...
    if (type == "jupedsim.walkable_area") {
        ...
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(poly->getTagProperty().getGUIIcon(), TL("add polygon '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildPOI(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& type,
    ...
    if (type == "jupedsim.waypoint") { 
    ...
    } else if (checkDuplicatedID({SUMO_TAG_POI, GNE_TAG_POILANE, GNE_TAG_POIGEO, GNE_TAG_POIWAYPOINT}, id)) {
        // get netedit parameters
        NeteditParameters neteditParameters(sumoBaseObject);
        // create POI
        ...
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(POI->getTagProperty().getGUIIcon(), TL("add POI '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildPOILane(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& type,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        if (lane == nullptr) {
            ...
            // add it depending of allow undoRed
            if (myAllowUndoRedo) {
                myNet->getViewNet()->getUndoList()->begin(POILane->getTagProperty().getGUIIcon(), TL("add POI '") + id + "'");
                overwriteAdditional();
...
GNEAdditionalHandler::buildPOIGeo(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const std::string& type,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(POIGEO->getTagProperty().getGUIIcon(), TL("add POI '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildWalkableArea(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const PositionVector& shape,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        GNEPoly* walkableArea = new GNEPoly(GNE_TAG_WALKABLEAREA, myNet, id, shape, name, parameters);
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(walkableArea->getTagProperty().getGUIIcon(), TL("add walkable area '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildObstacle(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const PositionVector& shape,
    ...
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        ...
        NeteditParameters neteditParameters(sumoBaseObject);
        // create walkable area
        GNEPoly* obstacle = new GNEPoly(GNE_TAG_OBSTACLE, myNet, id, shape, name, parameters);
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(obstacle->getTagProperty().getGUIIcon(), TL("add polygon '") + id + "'");
            overwriteAdditional();
...
GNEAdditionalHandler::buildPOIWaypoint(const CommonXMLStructure::SumoBaseObject* sumoBaseObject, const std::string& id, const double x, const double y,
    ...
    // check conditions
    if (!SUMOXMLDefinitions::isValidAdditionalID(id)) {
        writeInvalidID(GNE_TAG_POIWAYPOINT, id);
    } else if (checkDuplicatedID({SUMO_TAG_POI, GNE_TAG_POILANE, GNE_TAG_POIGEO, GNE_TAG_POIWAYPOINT}, id)) {
        // get netedit parameters
        NeteditParameters neteditParameters(sumoBaseObject);
        // create POI
        ...
        // add it depending of allow undoRed
        if (myAllowUndoRedo) {
            myNet->getViewNet()->getUndoList()->begin(POIWaypoint->getTagProperty().getGUIIcon(), TL("add POI Waypoint '") + id + "'");
            overwriteAdditional();
        ...
        writeErrorDuplicated(GNE_TAG_POIWAYPOINT, id);
    }
}
