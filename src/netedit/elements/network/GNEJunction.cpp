...
GNEJunction::drawDemandElements(const GUIVisualizationSettings& s, const GNEEdge* edge) const {
    ...
    if (s.drawForPositionSelection) {
        ...
        for (const auto& embeddedRoute : edge->getChildDemandElementsByType(GNE_TAG_ROUTE_EMBEDDED)) {
        ...
        if (edge->getChildDemandElementsByType(GNE_TAG_ROUTE_EMBEDDED).size() > 0) {
            const auto& embeddedRoute = edge->getChildDemandElementsByType(GNE_TAG_ROUTE_EMBEDDED).front();
            if (myNet->getViewNet()->getDemandViewOptions().showNonInspectedDemandElements(embeddedRoute)) {
                edge->drawPartialRoute(s, embeddedRoute, this);
            }
        }
    ...
    if (s.drawForPositionSelection) {
        for (const auto& personTripFromTo : edge->getChildDemandElementsByType(GNE_TAG_PERSONTRIP_EDGE_EDGE)) {
            edge->drawPartialPersonPlan(s, personTripFromTo, this);
        }
        for (const auto& personTripBusStop : edge->getChildDemandElementsByType(GNE_TAG_PERSONTRIP_EDGE_BUSSTOP)) {
            edge->drawPartialPersonPlan(s, personTripBusStop, this);
        }
        for (const auto& walkEdges : edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGES)) {
            edge->drawPartialPersonPlan(s, walkEdges, this);
        }
        for (const auto& walkFromTo : edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGE_EDGE)) {
            edge->drawPartialPersonPlan(s, walkFromTo, this);
        }
        for (const auto& walkBusStop : edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGE_BUSSTOP)) {
            edge->drawPartialPersonPlan(s, walkBusStop, this);
        }
        for (const auto& walkRoute : edge->getChildDemandElementsByType(GNE_TAG_WALK_ROUTE)) {
            edge->drawPartialPersonPlan(s, walkRoute, this);
        }
        for (const auto& rideFromTo : edge->getChildDemandElementsByType(GNE_TAG_RIDE_EDGE_EDGE)) {
            edge->drawPartialPersonPlan(s, rideFromTo, this);
        }
        for (const auto& rideBusStop : edge->getChildDemandElementsByType(GNE_TAG_RIDE_EDGE_BUSSTOP)) {
            edge->drawPartialPersonPlan(s, rideBusStop, this);
        }
    } else {
        // if drawForPositionSelection is disabled, only draw the first element
        if (edge->getChildDemandElementsByType(GNE_TAG_PERSONTRIP_EDGE_EDGE).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_PERSONTRIP_EDGE_EDGE).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_PERSONTRIP_EDGE_BUSSTOP).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_PERSONTRIP_EDGE_BUSSTOP).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGES).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGES).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGE_EDGE).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGE_EDGE).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGE_BUSSTOP).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_WALK_EDGE_BUSSTOP).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_WALK_ROUTE).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_WALK_ROUTE).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_RIDE_EDGE_EDGE).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_RIDE_EDGE_EDGE).front(), nullptr);
        }
        if (edge->getChildDemandElementsByType(GNE_TAG_RIDE_EDGE_BUSSTOP).size() > 0) {
            edge->drawPartialPersonPlan(s, edge->getChildDemandElementsByType(GNE_TAG_RIDE_EDGE_BUSSTOP).front(), nullptr);
        }
    }
}
