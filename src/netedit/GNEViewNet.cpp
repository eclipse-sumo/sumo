...
GNEViewNet::onCmdReverseEdge(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("Reverse selected edges"));
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("Reverse edge"));
...
GNEViewNet::onCmdAddReversedEdge(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("Add Reverse edge for selected edges"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("Add reverse edge"));
...
GNEViewNet::onCmdAddReversedEdgeDisconnected(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("Add Reverse disconnected edge for selected edges"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("Add reverse disconnected edge"));
...
GNEViewNet::onCmdResetEdgeEndpoint(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        ...
        if (edge->isAttributeCarrierSelected()) {
            ...
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            // begin operation
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("reset geometry points"));
...
GNEViewNet::onCmdStraightenEdges(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("straighten selected edges"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("straighten edge"));
...
GNEViewNet::onCmdSmoothEdges(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("smooth selected edges"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("smooth edge"));
...
GNEViewNet::onCmdStraightenEdgesElevation(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("straighten elevation of selected edges"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("straighten edge elevation"));
...
GNEViewNet::onCmdSmoothEdgesElevation(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("smooth elevation of selected edges"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
            ...
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("smooth edge elevation"));
...
GNEViewNet::onCmdResetLength(FXObject*, FXSelector, void*) {
    ...
    if (edge != nullptr) {
        if (edge->isAttributeCarrierSelected()) {
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("reset edge lengths"));
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            for (const auto& selectedEdge : selectedEdges) {
...
GNEViewNet::onCmdEgeApplyTemplate(FXObject*, FXSelector, void*) {
    ...
    if ((edge != nullptr) && myViewParent->getInspectorFrame()->getTemplateEditor()->getEdgeTemplate()) {
        // begin copy template
        myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("copy edge template"));
        // copy template
...
GNEViewNet::onCmdSimplifyShape(FXObject*, FXSelector, void*) {
    ...
    if (polygonUnderMouse) {
        ...
        if (polygonUnderMouse->isAttributeCarrierSelected()) {
            // begin undo-list
            myNet->getViewNet()->getUndoList()->begin(polygonUnderMouse->getTagProperty().getGUIIcon(), TL("simplify shapes"));
            // get shapes
...
GNEViewNet::onCmdClosePolygon(FXObject*, FXSelector, void*) {
    ...
    if (polygonUnderMouse) {
        ...
        if (polygonUnderMouse->isAttributeCarrierSelected()) {
            // begin undo-list
            myNet->getViewNet()->getUndoList()->begin(polygonUnderMouse->getTagProperty().getGUIIcon(), TL("close polygon shapes"));
            // get selectedshapes
            const auto selectedShapes = myNet->getAttributeCarriers()->getSelectedShapes();
            // iterate over shapes
            for (const auto& selectedShape : selectedShapes) {
...
GNEViewNet::onCmdOpenPolygon(FXObject*, FXSelector, void*) {
    ...
    if (polygonUnderMouse) {
        ...
        if (polygonUnderMouse->isAttributeCarrierSelected()) {
            // begin undo-list
            myNet->getViewNet()->getUndoList()->begin(polygonUnderMouse->getTagProperty().getGUIIcon(), TL("open polygon shapes"));
            // get shapes
            const auto selectedShapes = myNet->getAttributeCarriers()->getSelectedShapes();
            // iterate over shapes
            for (const auto& selectedShape : selectedShapes) {
    ...
    if (POI) {
        ...
        if (POI->getTagProperty().getTag() == SUMO_TAG_POI) {
            ...
            if (lanes.empty()) {
                ...
                // remove POI
                myUndoList->begin(POI->getTagProperty().getGUIIcon(), TL("attach POI into lane"));
            ...
            // remove POI
            myUndoList->begin(POI->getTagProperty().getGUIIcon(), TL("release POI from lane"));
            myNet->deleteAdditional(POI, myUndoList);
            // add new POI use route handler
            additionalHanlder.parseSumoBaseObject(POIBaseObject);
            myUndoList->end();
...
GNEViewNet::onCmdReverse(FXObject*, FXSelector, void*) {
    ...
    if (demandElement) {
        // begin undo list
        myUndoList->begin(demandElement->getTagProperty().getGUIIcon(), TLF("reverse % '%'", demandElement->getTagStr(), demandElement->getID()));
...
GNEViewNet::onCmdAddReverse(FXObject*, FXSelector, void*) {
    ...
    if (demandElement) {
        // begin undo list
        myUndoList->begin(demandElement->getTagProperty().getGUIIcon(), TLF("add reverse '%'", demandElement->getTagStr()));
...
GNEViewNet::onCmdSetCustomGeometryPoint(FXObject*, FXSelector, void*) {
    ...
    if (lane != nullptr) {
        ...
        if (newPosition != edgeGeometry[index]) {
            ...
            // begin undo list
            myUndoList->begin(lane->getParentEdge()->getTagProperty().getGUIIcon(), TL("change edge Geometry Point position"));
        ...
        if (newPosition != polygonGeometry[index]) {
            ...
            // begin undo list
            myUndoList->begin(poly->getTagProperty().getGUIIcon(), TL("change polygon Geometry Point position"));
            // change shape
        ...
        if (newPosition != TAZGeometry[index]) {
            ...
            // begin undo list
            myUndoList->begin(TAZ->getTagProperty().getGUIIcon(), TL("change TAZ Geometry Point position"));
            // change shape
...
GNEViewNet::onCmdResetEndPoints(FXObject*, FXSelector, void*) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        if (edge->isAttributeCarrierSelected()) {
            ...
            const auto selectedEdges = myNet->getAttributeCarriers()->getSelectedEdges();
            // begin undo list
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("reset end points of selected edges"));
            // iterate over edges
            for (const auto& selectedEdge : selectedEdges) {
                // reset both end points
            ...
            // begin undo list
            myUndoList->begin(edge->getTagProperty().getGUIIcon(), TL("reset end points of edge '") + edge->getID());
            // reset both end points
...
GNEViewNet::onCmdDuplicateLane(FXObject*, FXSelector, void*) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        if (laneAtPopupPosition->isAttributeCarrierSelected()) {
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("duplicate selected lanes"));
            ...
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("duplicate lane"));
...
GNEViewNet::onCmdResetLaneCustomShape(FXObject*, FXSelector, void*) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        // are, otherwise recompute them
        if (laneAtPopupPosition->isAttributeCarrierSelected()) {
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("reset custom lane shapes"));
            const auto selectedLanes = myNet->getAttributeCarriers()->getSelectedLanes();
            for (const auto& lane : selectedLanes) {
            ...
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("reset custom lane shape"));
...
GNEViewNet::onCmdResetOppositeLane(FXObject*, FXSelector, void*) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        // are, otherwise recompute them
        if (laneAtPopupPosition->isAttributeCarrierSelected()) {
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("reset opposite lanes"));
            const auto selectedLanes = myNet->getAttributeCarriers()->getSelectedLanes();
            for (const auto& lane : selectedLanes) {
            ...
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("reset opposite lane"));
...
GNEViewNet::onCmdLaneReachability(FXObject* menu, FXSelector, void*) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("select lane reachability"));
...
GNEViewNet::restrictLane(SUMOVehicleClass vclass) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        if (mapOfEdgesAndLanes.size() > 0) {
            ...
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), "restrict lanes to " + toString(vclass));
            // iterate over selected lanes
            for (const auto& edgeLane : mapOfEdgesAndLanes) {
            ...
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("restrict lane to ") + toString(vclass));
            // Transform lane to Sidewalk
...
GNEViewNet::addRestrictedLane(SUMOVehicleClass vclass, const bool insertAtFront) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        if (laneAtPopupPosition->isAttributeCarrierSelected() || laneAtPopupPosition->getParentEdge()->isAttributeCarrierSelected()) {
            ...
            // begin undo operation
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("add restrictions for ") + toString(vclass));
            // iterate over set of edges
            for (const auto& edge : setOfEdges) {
            ...
            // If only have a single lane, start undo/redo operation
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("add vclass for ") + toString(vclass));
            // Add restricted lane
...
GNEViewNet::removeRestrictedLane(SUMOVehicleClass vclass) {
    ...
    if (laneAtPopupPosition != nullptr) {
        ...
        if (setOfEdges.size() > 0) {
            ...
            // begin undo operation
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), "Remove restrictions for " + toString(vclass));
            // iterate over set of edges
            for (const auto& edge : setOfEdges) {
                // add Sidewalk
            ...
            // If only have a single lane, start undo/redo operation
            myUndoList->begin(laneAtPopupPosition->getTagProperty().getGUIIcon(), TL("Remove vclass for ") + toString(vclass));
            // Remove Sidewalk
...
GNEViewNet::onCmdResetJunctionShape(FXObject*, FXSelector, void*) {
    ...
    if (junction) {
        // are, otherwise recompute them
        if (junction->isAttributeCarrierSelected()) {
            myUndoList->begin(junction->getTagProperty().getGUIIcon(), TL("reset custom junction shapes"));
            const auto selectedJunctions = myNet->getAttributeCarriers()->getSelectedJunctions();
            for (const auto& selectedJunction : selectedJunctions) {
            ...
            myUndoList->begin(junction->getTagProperty().getGUIIcon(), TL("reset custom junction shape"));
