...
#ifdef HAVE_GL_H
#ifndef WIN32
        if (!glXQueryExtension(DISPLAY(application), NULL, NULL)) {
            return FALSE;
        }
        if (!glXQueryVersion(DISPLAY(application), &major, &minor)) {
            return FALSE;
        }
#if GL_VERSION_1_5
        if (minor > 5) {
...
void MFXGLVisual::create() {
    ...
    if (!xid) {
        if (getApp()->isInitialized()) {
            ...
            if (!vi) {
                fxerror("%s::create: unable to obtain any visuals.\n", getClassName());
            }
            ...
            for (i = 0; i < nvi; i++) {
                ...
                dgreen = glgreen - greenSize;
                if (dgreen < 0) {
                    dgreen *= -100;
                }
                dblue  = glblue - blueSize;
                if (dblue < 0) {
                    dblue  *= -100;
                }
                dalpha = glalpha - alphaSize;
                if (dalpha < 0) {
                    dalpha *= -100;
                }

                // Prefer better Z than asked, but colors more important
                ddepth = gldepth - depthSize;
                if (ddepth < 0) {
                    ddepth *= -10;
                }

                // We care about colors and Z depth more than stencil depth
                dstencil = glstencil - stencilSize;
                if (dstencil < 0) {
                    dstencil *= -1;
                }

                // Accumulation buffers
                daccred = glaccred - accumRedSize;
                if (daccred < 0) {
                    daccred   *= -1;
                }
                daccgreen = glaccgreen - accumGreenSize;
                if (daccgreen < 0) {
                    daccgreen *= -1;
                }
                daccblue = glaccblue - accumBlueSize;
                if (daccblue < 0) {
                    daccblue  *= -1;
                }
                daccalpha = glaccalpha - accumAlphaSize;
                if (daccalpha < 0) {
                    daccalpha *= -1;
                }
            ...
            // Shit out of luck
            if (bestvis < 0) {
                fxerror("%s::create: requested OpenGL visual unavailable.\n", getClassName());
            }
...
void MFXGLVisual::create() {
    ...
    if (!xid) {
        if (getApp()->isInitialized()) {
            ...
            if (nvi == 0) {
                fxerror("%s::create: no OpenGL visual available.\n", getClassName());
            }
            ...
            for (i = 1; i <= nvi; i++) {
                ...
                if ((flags & VISUAL_NOACCEL) && !(pfd.dwFlags & PFD_GENERIC_FORMAT)) {
                    continue;
                }
...
#ifndef WIN32
            if (freemap) {
                XFreeColormap(DISPLAY(getApp()), colormap);   // Should we free?
            }
...
FXint MFXGLVisual::getActualRedSize() const {
    if (!info) {
        fxerror("%s::getActualRedSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualGreenSize() const {
    if (!info) {
        fxerror("%s::getActualGreenSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualBlueSize() const {
    if (!info) {
        fxerror("%s::getActualBlueSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualAlphaSize() const {
    if (!info) {
        fxerror("%s::getActualAlphaSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualDepthSize() const {
    if (!info) {
        fxerror("%s::getActualDepthSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualStencilSize() const {
    if (!info) {
        fxerror("%s::getActualStencilSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualAccumRedSize() const {
    if (!info) {
        fxerror("%s::getActualAccumRedSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualAccumGreenSize() const {
    if (!info) {
        fxerror("%s::getActualAccumGreenSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualAccumBlueSize() const {
    if (!info) {
        fxerror("%s::getActualAccumBlueSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXint MFXGLVisual::getActualAccumAlphaSize() const {
    if (!info) {
        fxerror("%s::getActualAccumAlphaSize: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXbool MFXGLVisual::isDoubleBuffer() const {
    if (!info) {
        fxerror("%s::isDoubleBuffer: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXbool MFXGLVisual::isStereo() const {
    if (!info) {
        fxerror("%s::isStereo: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
    FXint s;
#ifndef WIN32
...
FXbool MFXGLVisual::isAccelerated() const {
    if (!info) {
        fxerror("%s::isHardwareAccelerated: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
#ifndef WIN32
    return TRUE;
...
FXbool MFXGLVisual::isBufferSwapCopy() const {
    if (!info) {
        fxerror("%s::isBufferSwapCopy: visual not yet initialized.\n", getClassName());
    }
#ifdef HAVE_GL_H
#ifndef WIN32
    return FALSE;
...
static void glXUseXftFont(XftFont* font, int first, int count, int listBase) {
    ...
    for (i = first; i < count; i++) {
        ...
        if (err) {
            fxwarning("glXUseXftFont: unable to load glyph.\n");
            return;
        }
        ...
        if (err) {
            fxwarning("glXUseXftFont: unable to render glyph.\n");
            return;
        }
...
void glUseFXFont(FXFont* font, int first, int count, int list) {
    if (!font || !font->id()) {
        fxerror("glUseFXFont: invalid font.\n");
    }
