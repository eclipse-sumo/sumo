...
AdditionalHandler::AdditionalHandler(const std::string& file, AdditionalContainer& sc, const GeoConvHelper* geoConvHelper) :
    SUMOSAXHandler(file),
    myAdditionalContainer(sc),
    myPrefix(""),
    ...
    try {
        switch (element) {
            case SUMO_TAG_POLY:
                ...
                myDefaultLayer = Additional::DEFAULT_LAYER;
                addPoly(attrs, false, false);
                break;
            case SUMO_TAG_POI:
                // default layer is different depending if we're parsing a Poly or a POI, therefore it has to be here defined
                myDefaultLayer = Additional::DEFAULT_LAYER_POI;
                ...
                if (myLastParameterised != nullptr) {
                    ...
                    if (ok) {
                        ...
                        if (key.empty()) {
                            ...
                            WRITE_DEBUG("Inserting generic parameter '" + key + "|" + val + "' into additional.");
...
void
AdditionalHandler::addPOI(const SUMOSAXAttributes& attrs, const bool ignorePruning, const bool useProcessing) {
/*
    bool ok = true;
    ...
    const double angle = attrs.getOpt<double>(SUMO_ATTR_ANGLE, id.c_str(), ok, Additional::DEFAULT_ANGLE);
    std::string imgFile = attrs.getOpt<std::string>(SUMO_ATTR_IMGFILE, id.c_str(), ok, Additional::DEFAULT_IMG_FILE);
    bool relativePath = attrs.getOpt<bool>(SUMO_ATTR_RELATIVEPATH, id.c_str(), ok, Additional::DEFAULT_RELATIVEPATH);
    if (imgFile != "" && !FileHelpers::isAbsolute(imgFile)) {
        imgFile = FileHelpers::getConfigurationRelative(getFileName(), imgFile);
    }
    const double width = attrs.getOpt<double>(SUMO_ATTR_WIDTH, id.c_str(), ok, Additional::DEFAULT_IMG_WIDTH);
    const double height = attrs.getOpt<double>(SUMO_ATTR_HEIGHT, id.c_str(), ok, Additional::DEFAULT_IMG_HEIGHT);
    ...
    if (ok) {
        ...
        if (!myAdditionalContainer.addPOI(id, type, color, pos, useGeo, laneID, lanePos, lanePosLat, layer, angle, imgFile, relativePath, width, height, ignorePruning)) {
            WRITE_ERROR("PoI '" + id + "' already exists.");
        }
        myLastParameterised = myAdditionalContainer.getPOIs().get(id);
...
void
AdditionalHandler::addPoly(const SUMOSAXAttributes& attrs, const bool ignorePruning, const bool useProcessing) {
/*
    bool ok = true;
    const std::string id = myPrefix + attrs.get<std::string>(SUMO_ATTR_ID, nullptr, ok);
    // check if ID is valid
    ...
    if (ok) {
        ...
        const double lineWidth = attrs.getOpt<double>(SUMO_ATTR_LINEWIDTH, id.c_str(), ok, Additional::DEFAULT_LINEWIDTH);
        const std::string type = attrs.getOpt<std::string>(SUMO_ATTR_TYPE, id.c_str(), ok, Additional::DEFAULT_TYPE);
        const RGBColor color = attrs.hasAttribute(SUMO_ATTR_COLOR) ? attrs.get<RGBColor>(SUMO_ATTR_COLOR, id.c_str(), ok) : myDefaultColor;
        PositionVector additional = attrs.get<PositionVector>(SUMO_ATTR_SHAPE, id.c_str(), ok);
        ...
        if (geo || useProcessing) {
            ...
            for (int i = 0; i < (int)additional.size(); i++) {
                if (useProcessing) {
                    success &= GeoConvHelper::getProcessing().x2cartesian(additional[i]);
                } else {
                    success &= gch->x2cartesian_const(additional[i]);
                }
            }
            if (!success) {
        ...
        // check that additional's size is valid
        if (additional.size() == 0) {
            WRITE_ERROR("Polygon's additional cannot be empty.");
            return;
        }
        ...
        if (!myAdditionalContainer.addPolygon(id, type, color, layer, angle, imgFile, relativePath, additional, geo, fill, lineWidth, ignorePruning)) {
            WRITE_ERROR("Polygon '" + id + "' already exists.");
        }
        myLastParameterised = myAdditionalContainer.getPolygons().get(id);
    }
*/
}
...
Parameterised*
AdditionalHandler::getLastParameterised() const {
    return myLastParameterised;
}


bool
AdditionalHandler::loadFiles(const std::vector<std::string>& files, AdditionalHandler& sh) {
    for (const auto& fileIt : files) {
        if (!XMLSubSys::runParser(sh, fileIt, false)) {
            WRITE_MESSAGE("Loading of additionals from " + fileIt + " failed.");
            return false;
        }
    }
    return true;
...
void
AdditionalHandler::setDefaults(const std::string& prefix, const RGBColor& color, const double layer, const bool fill) {
    myPrefix = prefix;
...
bool
AdditionalHandler::addLanePosParams() {
    return false;
}
