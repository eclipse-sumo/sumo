...
const std::string OptionHandler::INVALID_INT_STR = toString(INVALID_INT);
const std::string OptionHandler::INVALID_DOUBLE_STR = toString(INVALID_DOUBLE);
...
void
OptionHandler::parseTemplate(OptionsCont& options, const std::string& templateString) {
    // build parser
...
OptionHandler::OptionHandler(OptionsCont& options) :
    myError(false),
    myOptions(options) {
}
...
void
OptionHandler::startElement(const XMLCh* const name, XERCES_CPP_NAMESPACE::AttributeList& attributes) {
    // get current topic
...
bool
OptionHandler::addOption(const std::string value, const std::string& synonymes,
...
void
OptionHandler::endElement(const XMLCh* const /*name*/) {
    myLevel--;
    if (myTopic.length() == 0) {
        return;
    }
...
void
OptionHandler::warning(const XERCES_CPP_NAMESPACE::SAXParseException& exception) {
...
void
OptionHandler::error(const XERCES_CPP_NAMESPACE::SAXParseException& exception) {
...
void
OptionHandler::fatalError(const XERCES_CPP_NAMESPACE::SAXParseException& exception) {
    WRITE_ERROR(StringUtils::transcode(exception.getMessage()));
    WRITE_ERROR(" (At line/column "
                + toString(exception.getLineNumber() + 1) + '/'
                + toString(exception.getColumnNumber()) + ").");
