...
public:
   ...
   * getting in and out of the car or when transporting (heavy) objects in the
   * cargo area.
  ...
  /**
   * \brief Get the maximum torque that the EM can handle during recuperation.
   *
  ...
  /**
   * \brief Get the constant power consumption of all ancillary devices (e.g.
   *        A/C).
   *
   ...
   */
  const CharacteristicMap& get_powerLossMap() const;
   ...
   * can manage or the power loss map is not defined for a given point, the
   * torque and/or power are capped to the motor's limits or the power loss is
   * set to zero.
