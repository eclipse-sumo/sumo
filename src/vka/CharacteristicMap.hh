...
namespace vka
{
   ...
   *        Kennfeld) of arbitrary dimensions and to provide functions in order
   *        to evaluate/interpolate points in it.
   */
  class CharacteristicMap
  {
  private:
     ...
     * \returns A vector containing the image values of the map at the
     *          specified location
     * \throws std::runtime_error
     */
  ...
  public:
     ...
     *
     * \returns A string representation of the characteristic map (cf. example
     ...
     *    outside of the domain). In that case, all entries of the vector are
     *    set to NaN.
     * b) ... an image value of the nearest neighbor is NaN. Then, the result
     *    is also NaN in that image dimension.
     * c) ... evaluation would require interpolating with a support point which
     *    is NaN in an image dimension. The corresponding result entry is set
     *    to NaN in that case.
     * \param[in] ref_p A point
     * \param[in] eps An epsilon value
     * \returns The (interpolated) image values of the map at the specified
     *          point
     * \throws std::runtime_error
     */
