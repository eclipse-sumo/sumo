...
NIFrame::fillOptions(bool forNetedit) {
    ...
    oc.addDescription("sumo-net-file", TL("Input"), TL("Read SUMO-net from FILE"));
    ...
    oc.addDescription("node-files", TL("Input"), TL("Read XML-node defs from FILE"));
    ...
    oc.addDescription("edge-files", TL("Input"), TL("Read XML-edge defs from FILE"));
    ...
    oc.addDescription("connection-files", TL("Input"), TL("Read XML-connection defs from FILE"));

    oc.doRegister("tllogic-files", 'i', new Option_FileName());
    oc.addDescription("tllogic-files", TL("Input"), TL("Read XML-traffic light defs from FILE"));
    ...
    oc.addDescription("type-files", TL("Input"), TL("Read XML-type defs from FILE"));
    ...
    if (!forNetedit) {
        ...
        oc.addDescription("ptstop-files", TL("Input"), TL("Reads public transport stops from FILE"));
        oc.doRegister("ptline-files", new Option_FileName());
        oc.addDescription("ptline-files", TL("Input"), TL("Reads public transport lines from FILE"));
        oc.doRegister("polygon-files", new Option_FileName());
        oc.addDescription("polygon-files", TL("Input"), TL("Reads polygons from FILE for embedding in network where applicable"));
    }
    ...
    oc.addDescription("shapefile-prefix", TL("Input"), TL("Read shapefiles (ArcView, Tiger, ...) from files starting with 'FILE'"));
    ...
    oc.addDescription("dlr-navteq-prefix", TL("Input"), TL("Read converted Navteq GDF data (unsplitted Elmar-network) from path 'FILE'"));

    oc.doRegister("osm-files", new Option_FileName());
    oc.addSynonyme("osm-files", "osm");
    oc.addDescription("osm-files", TL("Input"), TL("Read OSM-network from path 'FILE(s)'"));

    oc.doRegister("opendrive-files", new Option_FileName());
    oc.addSynonyme("opendrive-files", "opendrive");
    oc.addDescription("opendrive-files", TL("Input"), TL("Read OpenDRIVE-network from FILE"));

    oc.doRegister("visum-file", new Option_FileName());
    oc.addSynonyme("visum-file", "visum");
    oc.addDescription("visum-file", TL("Input"), TL("Read VISUM-net from FILE"));

    oc.doRegister("vissim-file", new Option_FileName());
    oc.addSynonyme("vissim-file", "vissim");
    oc.addDescription("vissim-file", TL("Input"), TL("Read VISSIM-net from FILE"));
    ...
    oc.addDescription("robocup-dir", TL("Input"), TL("Read RoboCup-net from DIR"));

    oc.doRegister("matsim-files", new Option_FileName());
    oc.addSynonyme("matsim-files", "matsim");
    oc.addDescription("matsim-files", TL("Input"), TL("Read MATsim-net from FILE"));

    oc.doRegister("itsumo-files", new Option_FileName());
    oc.addSynonyme("itsumo-files", "itsumo");
    oc.addDescription("itsumo-files", TL("Input"), TL("Read ITSUMO-net from FILE"));

    oc.doRegister("heightmap.shapefiles", new Option_FileName());
    oc.addDescription("heightmap.shapefiles", TL("Input"), TL("Read heightmap from ArcGIS shapefile"));

    oc.doRegister("heightmap.geotiff", new Option_FileName());
    oc.addDescription("heightmap.geotiff", TL("Input"), TL("Read heightmap from GeoTIFF"));
    ...
    oc.addDescription("ignore-errors.connections", TL("Report"), TL("Continue on invalid connections"));

    oc.doRegister("show-errors.connections-first-try", new Option_Bool(false));
    oc.addDescription("show-errors.connections-first-try", TL("Report"), TL("Show errors in connections at parsing"));

    oc.doRegister("ignore-errors.edge-type", new Option_Bool(false));
    oc.addDescription("ignore-errors.edge-type", TL("Report"), TL("Continue on unknown edge types"));

    oc.doRegister("speed-in-kmh", new Option_Bool(false));
    oc.addDescription("speed-in-kmh", TL("Processing"), TL("vmax is parsed as given in km/h"));

    oc.doRegister("construction-date", new Option_String());
    oc.addDescription("construction-date", TL("Processing"), TL("Use YYYY-MM-DD date to determine the readiness of features under construction"));

    oc.doRegister("flatten", new Option_Bool(false));
    oc.addDescription("flatten", TL("Processing"), TL("Remove all z-data"));

    oc.doRegister("discard-params", new Option_StringVector());
    oc.addDescription("discard-params", TL("Formats"), TL("Remove the list of keys from all params"));

    oc.doRegister("ignore-change-restrictions", new Option_StringVector(StringVector({"authority"})));
    oc.addDescription("ignore-change-restrictions", TL("Formats"), TL("List vehicle classes that may ignore lane changing restrictions ('all' discards all restrictions)"));
    ...
    oc.addDescription("plain.extend-edge-shape", TL("Processing"), TL("If edge shapes do not end at the node positions, extend them"));

    // register osm options
    oc.doRegister("osm.skip-duplicates-check", new Option_Bool(false));
    oc.addDescription("osm.skip-duplicates-check", TL("Formats"), TL("Skips the check for duplicate nodes and edges"));

    oc.doRegister("osm.elevation", new Option_Bool(false));
    oc.addDescription("osm.elevation", TL("Formats"), TL("Imports elevation data"));

    oc.doRegister("osm.layer-elevation", new Option_Float(0));
    oc.addDescription("osm.layer-elevation", TL("Formats"), TL("Reconstruct (relative) elevation based on layer data. Each layer is raised by FLOAT m"));

    oc.doRegister("osm.layer-elevation.max-grade", new Option_Float(10));
    oc.addDescription("osm.layer-elevation.max-grade", TL("Formats"), TL("Maximum grade threshold in % at 50km/h when reconstrucing elevation based on layer data. The value is scaled according to road speed."));

    oc.doRegister("osm.oneway-spread-right", new Option_Bool(false));
    oc.addDescription("osm.oneway-spread-right", TL("Formats"), TL("Whether one-way roads should be spread to the side instead of centered"));

    oc.doRegister("osm.lane-access", new Option_Bool(false));
    oc.addDescription("osm.lane-access", TL("Formats"), TL("Import lane-specific access restrictions"));

    oc.doRegister("osm.bike-access", new Option_Bool(false));
    oc.addSynonyme("osm.bike-access", "osm.bike-lanes");
    oc.addDescription("osm.bike-access", TL("Formats"), TL("Import bike lanes and fix directions and permissions on bike paths"));

    oc.doRegister("osm.sidewalks", new Option_Bool(false));
    oc.addDescription("osm.sidewalks", TL("Formats"), TL("Import sidewalks"));

    oc.doRegister("osm.crossings", new Option_Bool(false));
    oc.addDescription("osm.crossings", TL("Formats"), TL("Import crossings"));

    oc.doRegister("osm.turn-lanes", new Option_Bool(false));
    oc.addDescription("osm.turn-lanes", TL("Formats"), TL("Import turning arrows from OSM to help with connection building"));

    oc.doRegister("osm.stop-output.length", new Option_Float(25));
    oc.addDescription("osm.stop-output.length", TL("Formats"), TL("The default length of a public transport stop in FLOAT m"));
    oc.doRegister("osm.stop-output.length.bus", new Option_Float(15));
    oc.addDescription("osm.stop-output.length.bus", TL("Formats"), TL("The default length of a bus stop in FLOAT m"));
    oc.doRegister("osm.stop-output.length.tram", new Option_Float(25));
    oc.addDescription("osm.stop-output.length.tram", TL("Formats"), TL("The default length of a tram stop in FLOAT m"));
    oc.doRegister("osm.stop-output.length.train", new Option_Float(200));
    oc.addDescription("osm.stop-output.length.train", TL("Formats"), TL("The default length of a train stop in FLOAT m"));

    oc.doRegister("osm.all-attributes", new Option_Bool(false));
    oc.addDescription("osm.all-attributes", TL("Formats"), TL("Whether additional attributes shall be imported"));

    oc.doRegister("osm.extra-attributes", new Option_StringVector(StringVector({ "bridge", "tunnel", "layer", "postal_code" })));
    oc.addDescription("osm.extra-attributes", TL("Formats"), TL("List of additional attributes that shall be imported from OSM via osm.all-attributes (set 'all' to import all)"));

    oc.doRegister("osm.speedlimit-none", new Option_Float(39.4444));
    oc.addDescription("osm.speedlimit-none", TL("Formats"), TL("The speed limit to be set when there is no actual speed limit in reality"));

    // register matsim options
    oc.doRegister("matsim.keep-length", new Option_Bool(false));
    oc.addDescription("matsim.keep-length", TL("Formats"), TL("The edge lengths given in the MATSIM-file will be kept"));

    oc.doRegister("matsim.lanes-from-capacity", new Option_Bool(false));
    oc.addDescription("matsim.lanes-from-capacity", TL("Formats"), TL("The lane number will be computed from the capacity"));
    ...
    oc.addDescription("shapefile.street-id", TL("Formats"), TL("Read edge ids from column STR"));

    oc.doRegister("shapefile.from-id", new Option_String());
    oc.addSynonyme("shapefile.from-id", "arcview.from-id", true);
    oc.addDescription("shapefile.from-id", TL("Formats"), TL("Read from-node ids from column STR"));

    oc.doRegister("shapefile.to-id", new Option_String());
    oc.addSynonyme("shapefile.to-id", "arcview.to-id", true);
    oc.addDescription("shapefile.to-id", TL("Formats"), TL("Read to-node ids from column STR"));

    oc.doRegister("shapefile.type-id", new Option_String());
    oc.addSynonyme("shapefile.type-id", "arcview.type-id", true);
    oc.addDescription("shapefile.type-id", TL("Formats"), TL("Read type ids from column STR"));

    oc.doRegister("shapefile.laneNumber", new Option_String());
    oc.addDescription("shapefile.laneNumber", TL("Formats"), TL("Read lane number from column STR"));

    oc.doRegister("shapefile.speed", new Option_String());
    oc.addDescription("shapefile.speed", TL("Formats"), TL("Read speed from column STR"));

    oc.doRegister("shapefile.length", new Option_String());
    oc.addDescription("shapefile.length", TL("Formats"), TL("Read custom edge length from column STR"));

    oc.doRegister("shapefile.width", new Option_String());
    oc.addDescription("shapefile.width", TL("Formats"), TL("Read total edge width from column STR"));

    oc.doRegister("shapefile.name", new Option_String());
    oc.addDescription("shapefile.name", TL("Formats"), TL("Read (non-unique) name from column STR"));

    oc.doRegister("shapefile.node-join-dist", new Option_Float(0));
    oc.addDescription("shapefile.node-join-dist", TL("Formats"), TL("Distance threshold for determining whether distinct shapes are connected (used when from-id and to-id are not available)"));

    oc.doRegister("shapefile.add-params", new Option_StringVector());
    oc.addDescription("shapefile.add-params", TL("Formats"), TL("Add the list of field names as edge params"));
    ...
    oc.addDescription("shapefile.use-defaults-on-failure", TL("Formats"), TL("Uses edge type defaults on problems"));
    ...
    oc.addDescription("shapefile.all-bidirectional", TL("Formats"), TL("Insert edges in both directions"));
    ...
    oc.addDescription("shapefile.guess-projection", TL("Formats"), TL("Guess the proper projection"));

    oc.doRegister("shapefile.traditional-axis-mapping", new Option_Bool(false));
    oc.addDescription("shapefile.traditional-axis-mapping", TL("Formats"), TL("Use traditional axis order (lon, lat)"));


    // register dlr-navteq options
    oc.doRegister("dlr-navteq.tolerant-permissions", new Option_Bool(false));
    oc.addDescription("dlr-navteq.tolerant-permissions", TL("Formats"), TL("Allow more vehicle classes by default"));

    oc.doRegister("dlr-navteq.keep-length", new Option_Bool(false));
    oc.addDescription("dlr-navteq.keep-length", TL("Formats"), TL("The edge lengths given in the DLR Navteq-file will be kept"));
    ...
    oc.addDescription("vissim.join-distance", TL("Formats"), TL("Structure join offset"));

    oc.doRegister("vissim.default-speed", new Option_Float(50.0f));
    oc.addDescription("vissim.default-speed", TL("Formats"), TL("Use FLOAT as default speed"));

    oc.doRegister("vissim.speed-norm", new Option_Float(1.0f));
    oc.addDescription("vissim.speed-norm", TL("Formats"), TL("Factor for edge velocity"));

    oc.doRegister("vissim.report-unset-speeds", new Option_Bool(false));
    oc.addDescription("vissim.report-unset-speeds", TL("Formats"), TL("Writes lanes without an explicit speed set"));


    // register visum options
    oc.doRegister("visum.language-file", new Option_FileName());
    oc.addDescription("visum.language-file", TL("Formats"), TL("Load language mappings from FILE"));

    oc.doRegister("visum.use-type-priority", new Option_Bool(false));
    oc.addDescription("visum.use-type-priority", TL("Formats"), TL("Uses priorities from types"));

    oc.doRegister("visum.use-type-laneno", new Option_Bool(false));
    oc.addDescription("visum.use-type-laneno", TL("Formats"), TL("Uses lane numbers from types"));

    oc.doRegister("visum.use-type-speed", new Option_Bool(false));
    oc.addDescription("visum.use-type-speed", TL("Formats"), TL("Uses speeds from types"));

    oc.doRegister("visum.connector-speeds", new Option_Float(100.));
    oc.addDescription("visum.connector-speeds", TL("Formats"), TL("Sets connector speed"));
    ...
    oc.addDescription("visum.connectors-lane-number", TL("Formats"), TL("Sets connector lane number"));

    oc.doRegister("visum.no-connectors", new Option_Bool(true));
    oc.addDescription("visum.no-connectors", TL("Formats"), TL("Excludes connectors"));
    ...
    oc.addDescription("visum.recompute-lane-number", TL("Formats"), TL("Computes the number of lanes from the edges' capacities"));

    oc.doRegister("visum.verbose-warnings", new Option_Bool(false));
    oc.addDescription("visum.verbose-warnings", TL("Formats"), TL("Prints all warnings, some of which are due to VISUM misbehaviour"));
    ...
    oc.addDescription("visum.lanes-from-capacity.norm", TL("Formats"), TL("The factor for flow to no. lanes conversion"));


    // register opendrive options
    oc.doRegister("opendrive.import-all-lanes", new Option_Bool(false));
    oc.addDescription("opendrive.import-all-lanes", TL("Formats"), TL("Imports all lane types"));
    oc.doRegister("opendrive.ignore-widths", new Option_Bool(false));
    oc.addDescription("opendrive.ignore-widths", TL("Formats"), TL("Whether lane widths shall be ignored."));
    oc.doRegister("opendrive.curve-resolution", new Option_Float(2.0));
    oc.addDescription("opendrive.curve-resolution", TL("Formats"), TL("The geometry resolution in m when importing curved geometries as line segments."));
    oc.doRegister("opendrive.advance-stopline", new Option_Float(0.0));
    oc.addDescription("opendrive.advance-stopline", TL("Formats"), TL("Allow stop lines to be built beyond the start of the junction if the geometries allow so"));
    oc.doRegister("opendrive.min-width", new Option_Float(1.8));
    oc.addDescription("opendrive.min-width", TL("Formats"), TL("The minimum lane width for determining start or end of variable-width lanes"));
    oc.doRegister("opendrive.internal-shapes", new Option_Bool(false));
    oc.addDescription("opendrive.internal-shapes", TL("Formats"), TL("Import internal lane shapes"));
    oc.doRegister("opendrive.position-ids", new Option_Bool(false));
    oc.addDescription("opendrive.position-ids", TL("Formats"), TL("Sets edge-id based on road-id and offset in m (legacy)"));
    oc.doRegister("opendrive.lane-shapes", new Option_Bool(false));
    oc.addDescription("opendrive.lane-shapes", TL("Formats"), TL("Use custom lane shapes to compensate discarded lane types"));
