...
Not only route file but trip file can also be used as input. The tool will firstly try to find the start edge and the end edge of each trip or flows and match them to the repsective origin and destination TAZ according the input taz-file. The counts of the TAZ-relations will be calcuated and saved. If only TAZ-information in the given trip or route file is available, this tool will then directly calculate TAZ-based OD relation counts without using the information in the given taz-file, which contains the information of the connection edges in each TAZ. If the option **--edge-relations** is set, edge-based relation counts will then be calculated and saved, only when start/end edge information is available. 
