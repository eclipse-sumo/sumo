...
Routes generated by [randomTrips.py](Trip.md#randomtripspy) (**--route-output**) can be a good input source. The following randomTrips.py options may be helpful:

- **--fringe-factor**: setting a high value will generated lots of through-traffic which is plausible for small networks
- **--junction-taz**: avoid cars that start or arrive in the "wrong" direction (avoiding the need for a turn-around after departure or before arrival)
...
- **--edge-type-file**: Load edge-type / routingType specific probabilities for generating origins and destinations
- **--marouter**: compute routes with macroscopic assignment. These routes consider traffic conditions and therefore give more variety computed to default duarouter outputs (fastest-routes-in-empty-network).
...
The option **--minimize-vehicles <FLOAT>** can be used to configure a weighting term for simultaneously minimizing the total number of used routes (vehicles).
  A higher value, favors solutions with few vehicles that pass multiple counting locations rather than more vehicles that pass fewer locations. Hard constraints on the minimum number of counting locations passed by each route can also be set with option **-min-count**. An alternative approach to reducing the total number of vehicles is by setting option [**--total-count**](#total_count_restrictions).
...
Using option **--optimize-full** may lead to implausible distribution of cars among the candidate routes. This is possible because the fully optimized solution is not restricted by the frequencies or propbabilties of the candidate routes and because edge counts do not constrain the solution space very much. A possible solution is to call routeSampler twice as described below:

0. Identify the routes that are most important for your sceario and hand-draw them in netedit. (If the scenario is small, 10 through-traffic routes might already account for 80% of the traffic)
1. distribute ~80% of traffic on the most important routes (or whatever percentage is approprimate in your case). Any of the options **--minimize-vehicles** or **--total-count** can be used to calibrate the total number of vehicles that are generated in this step.
2. distribute the remaining percentage of traffic on a random selection of routes (i.e. generated by randomTrips.py). The result of **--mismatch-output** from the first call can be used as counting data input by setting **--edgedata-attribute deficit**.
...
- Traffic counts are not stationary within an interval (i.e. a single vehicle passing a long a list of widely spaced counting locations and each interval registers a different location). In this case the route that covers all counting locations will not be sampled. (Longer intervals must be chosen in this case). Even for hourly interval length, there will always be a few vehicles that drive over the begin- or end-time boundary of an internval and can lead to unequal counts along their route. Both kinds of boundary-vehicles can cancel out if there is sufficient traffic but will create greater relative errors in low-traffic conditions.
- Inconsistency in the counting data: It's best to find these with **--mismatch-output** and look for areas of high-mismatch bordered by locations with no or low mismatch.
