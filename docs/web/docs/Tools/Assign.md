...
The first option passes it's arguments only to the [sumo](../sumo.md) process. The other options are passed to [duarouter](../duarouter.md) and ensure that the types are read but are not written to the route output file (since they would than be loaded twice by sumo resulting in an error).
