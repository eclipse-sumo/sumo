...
This tool detects conflicts when merging netDiff-generated diff files from two editing sessions of the same network.
Example use case: Two users edit network A in parallel, thereby creating networks B and C.
To merge changes they may either patch B with the diff (A,C) or patch C with the diff (A,B).
If both users created new edges and junctions in different locations and using the same new ids (which easily happens due to the way netedit assigns new ids), this creates a conflict that can create massive distortions in the final network. This tool reports name clashes in diffs for junctions and edges.
Assume that the diffs are generated by `netdiff.py A B diff` and `netdiff.py A C diff2`.

Usage:

```
python tools/net/diffMerge.py diff diff2
```

It is advisable to modify either Network B or C by renaming junctions and edges so that id clashes in the (renewed) diffs are avoided.
