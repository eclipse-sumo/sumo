...
| x        | m or longitude       |  x |   x  | The absolute X coordinate of the vehicle (center of front bumper). The value depends on the given geographic projection. With the option **--fcd-output.utm**, the value is given in UTM coordinates, taking the network offset into account. With the option **--fcd-output.geo**, the longitude is given (WGS84).|
