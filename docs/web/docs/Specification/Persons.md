...
| period*         | float(s) or "exp(X)" where x is float  | if float is given, insert equally spaced persons at that period. If exp(X) is given, insert persons with exponentially distributed time gaps. This turns insertion into a [Poisson process](https://en.wikipedia.org/wiki/Poisson_point_process) with an expected value of *X* insertions per second. (not together with personsPerHour or probability), see also [Simulation/Randomness](../Simulation/Randomness.md#flows_with_a_random_number_of_vehicles)    |
