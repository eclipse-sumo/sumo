...
def iterateGreenTimes(emissionClass, outputFile, T, MIN_GREEN, log):
    ...
    for i in range(0, 1000):
        print >> fd, '    <vehicle id="v.%s" depart="%s" departspeed="13.89" departpos="0" route="r1" type="t1"/>' % (
            i, i * 5)
    print >> fd, '</routes>'
    fd.close()
...
#    shutil.copy("netstate.xml", "backup/" + str(i) + "_netstate.xml")
        ...
        print >> fdo, "%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s" % (
            T - i, wtv, nv, cov, co2v, hcv, pmv, nov, fv, meanV, msv, 10000. / msv - 10000. / 13.82, mjlv, jlsv, mhdv, shv)
    fdo.close()
