...
sub writeRouteConf($)
{
	...
	if($step==0) {
		...
		print OUTDAT "		<weights>aggregated/due3s_600.xml</weights>\n";
	}
...
sub writeSumoConf($)
{
	...
	print OUTDAT "		<dump-intervals>600</dump-intervals>\n";
...
for(; $step<$end+1; $step++) {
	writeRouteConf($step);
	system("$ARGV[0]/router.exe -c due3s.rou.cfg --gA=0.5 --gBeta=0.9");
