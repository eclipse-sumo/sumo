...
public:
    ...
    {
        if (dev.writeXMLHeader("detector", true,
                           "type=\"E1_Collector\" id=\""+idM+"\"",
                           "<!--meanSpeed [m/s], nVehicles [veh], flow [veh/h], meanOccupancy [%], meanVehLength [m]-->")) {
            dev<<"<location lane=\""<<crossSectionM.laneM->getID()<<"\" pos=\""<<crossSectionM.posM<<"\" />\n";
        }
    }
