...
void
MSE2Collector::checkPositioning(bool posGiven, SUMOReal desiredLength){
...
SUMOReal
MSE2Collector::snap(SUMOReal value, SUMOReal snapPoint, SUMOReal snapDist){
...
std::vector<MSLane*>
MSE2Collector::selectLanes(MSLane* lane, SUMOReal length, std::string dir){
    // direction of detector extension
    assert(dir == "fw" || dir == "bw");
    bool fw = dir == "fw";
    SUMOReal linkLength = 0; // linkLength (used if no internal lanes are present)
    ...
    if (fw) {
        assert(myStartPos != std::numeric_limits<SUMOReal>::max());
        length += myStartPos;
    } else {
        assert(myEndPos != std::numeric_limits<SUMOReal>::max());
...
MSE2Collector::initAuxiliaries(std::vector<MSLane*>& lanes){
    ...
    bool fw = myEndPos == std::numeric_limits<SUMOReal>::max();
    if(fw){
        myDetectorLength -= myStartPos;
    } else {
        myDetectorLength -= myLastLane->getLength() - myEndPos;
...
MSE2Collector::makeMoveNotification(const SUMOVehicle& veh, SUMOReal oldPos, SUMOReal newPos, SUMOReal newSpeed, const VehicleInfo& vehInfo) const {
#ifdef DEBUG_E2_NOTIFY_MOVE
...
MSE2Collector::processJams(std::vector<JamInfo*>& jams, JamInfo* currentJam){
    ...
    for (std::vector<JamInfo*>::const_iterator i = jams.begin(); i != jams.end(); ++i) {
        ...
        const SUMOReal jamLengthInMeters = lastVeh->distToDetectorEnd
...
void
MSE2Collector::calculateTimeLossAndTimeOnDetector(const SUMOVehicle& veh, SUMOReal oldPos, SUMOReal newPos, const VehicleInfo& vi, SUMOReal& timeOnDetector, SUMOReal& timeLoss) const {
