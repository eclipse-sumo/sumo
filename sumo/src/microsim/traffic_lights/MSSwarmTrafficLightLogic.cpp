...
//	Derivative
	unsigned short derivativeHistorySize = (unsigned short) s2f(getParameter("PHERO_DERIVATIVE_HISTORY_SIZE","3"));
	unsigned short meanSpeedHistorySize = (unsigned short) s2f(getParameter("PHERO_MEAN_SPEED_HISTORY_SIZE","3"));
	m_derivativeAlpha = s2f(getParameter("PHERO_DERIVATIVE_ALPHA","1"));
	m_losCounter = 0;
	m_losMaxLimit = (int) s2f(getParameter("LOSS_OF_SIGNAL_LIMIT","10"));
...
//    pheroAdd /= max(derivative, 1.0);
		...
		//		DBG(
				...
				i_str << " op " << oldPhero << " ms " << meanVehiclesSpeed << " p " << pheroAdd * updatePheromone <<
						...
						pheroMap[laneId] << " l " << laneId <<" ID " << getID() <<" c " << getSensors()->countVehicles(laneId) <<" s " << getLaneLightState(laneId) << " ."; if(m_pheroLevelLog[laneId] != i_str.str()) { m_pheroLevelLog[laneId] = i_str.str(); WRITE_MESSAGE(time2string(MSNet::getInstance()->getCurrentTimeStep()) + " MSSwarmTrafficLightLogic::updatePheromoneLevels:: " + logString + i_str.str()); })
...
//			ANALYSIS_DBG(
		DBG(
				std::ostringstream lf; std::ostringstream phero_str; if(getReinforcementMode()==0){ if(policy == currentPolicy) lf <<" ,LearningCox " << getLearningCox() <<" ,LCox*Time " << getLearningCox() * elapsedTime; else lf <<" ,ForgettingCox " << getForgettingCox() <<" ,FCox*Time " << getForgettingCox() * elapsedTime;

				phero_str << " policy " << policy->getName() << " newSensitivity " << newSensitivity << " ,pol.Sensitivity " << policy->getThetaSensitivity() <<" ,elapsedTime " << elapsedTime << lf.str() << " NEWERSensitivity= " << max(min(newSensitivity, getThetaMax()), getThetaMin()) << " ID "<< getID() <<" ."; }else{ if(policy == currentPolicy && eta > 0) lf <<" ,LearningCox " << getLearningCox() <<" ,LCox*Eta " << getLearningCox() * eta; else if(policy == currentPolicy && eta < 0) lf <<" ,ForgettingCox " << getForgettingCox() <<" ,FCox*Eta " << getForgettingCox() * eta; else if(eta > 0) lf <<" ,ForgettingCox " << getForgettingCox() <<" ,FCox*Eta " << getForgettingCox() * eta; else if(eta < 0) lf <<" ,LearningCox " << getLearningCox() <<" ,LCox*Eta " << getLearningCox() * eta; phero_str << " policy " << policy->getName() << " newSensitivity " << newSensitivity << " ,pol.Sensitivity " << policy->getThetaSensitivity() <<" ,eta " << eta <<" ,carsIn " << carsIn << " ,inTarget " << inTarget << " ,notTarget " << notTarget << " ,carsOut " << carsOut << lf.str() << " NEWERSensitivity= " << max(min(newSensitivity, getThetaMax()), getThetaMin()) << " ID "<< getID() <<" ."; } WRITE_MESSAGE(time2string(MSNet::getInstance()->getCurrentTimeStep()) +" MSSwarmTrafficLightLogic::updateSensitivities::"+phero_str.str());)
