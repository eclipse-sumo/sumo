...
MSLane::freeEmit(MSVehicle& veh, SUMOReal mspeed) throw() {
    ...
    if (myVehicles.size()==0) {
        ...
        SUMOReal frontGapNeeded = veh.getSecureGap(speed, leader->getSpeedAfterMaxDecel(leader->getSpeed()));
...
MSLane::isEmissionSuccess(MSVehicle* aVehicle,
    ...
    SUMOReal dist = aVehicle->getCarFollowModel().brakeGap(speed);
    ...
    while (seen<dist&&ri!=bestLaneConts.end()&&nextLane!=0/*&&ce!=r.end()*/) {
        ...
        if (nextLane!=0) {
            ...
            if (leader!=0) {
                ...
                if (nspeed<speed) {
                    if (patchSpeed) {
                        ...
                        dist = aVehicle->getCarFollowModel().brakeGap(speed);
                    } else {
            ...
            if (nspeed<speed) {
                ...
                if (patchSpeed) {
                    speed = MIN2(aVehicle->getCarFollowModel().ffeV(aVehicle, speed, seen, nspeed), speed);
                    dist = aVehicle->getCarFollowModel().brakeGap(speed);
                } else {
            ...
            if ((*link)->hasApproachingFoe()) {
                SUMOReal nspeed = aVehicle->getCarFollowModel().ffeV(aVehicle, speed, seen, 0);
                if (nspeed<speed) {
                    if (patchSpeed) {
                        speed = MIN2(nspeed, speed);
                        dist = aVehicle->getCarFollowModel().brakeGap(speed);
    ...
    if (predIt != myVehicles.end()) {
        ...
        SUMOReal frontGapNeeded = aVehicle->getSecureGap(speed, leader->getSpeedAfterMaxDecel(leader->getSpeed()));
    ...
    if (predIt!=myVehicles.begin()) {
        ...
        SUMOReal backGapNeeded = follower->getSecureGap(follower->getSpeed(), aVehicle->getSpeedAfterMaxDecel(speed));
