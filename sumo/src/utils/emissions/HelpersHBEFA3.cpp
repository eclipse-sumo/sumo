...
HelpersHBEFA3::HelpersHBEFA3() {
    ...
    SumoEmissionClassStrings.insert("HBEFA3/unknown", index++);
    SumoEmissionClassStrings.insert("HBEFA3/zero", index++);
    ...
    for (int i = 0; i < 33; i++) {
        SumoEmissionClassStrings.insert(light[i], index++);
    }
    for (int i = 0; i < 12; i++) {
        SumoEmissionClassStrings.insert(heavy[i], index++ | PollutantsInterface::HEAVY_BIT);
    }
}
