...
PositionVector::extrapolate(SUMOReal val) {
    ...
    Position ne =
        GeomHelper::extrapolate_second(
            (*this)[(int)size() - 2], (*this)[(int)size() - 1], val);
    erase(begin());
    push_front(nb);
    erase(end() - 1);
    push_back(ne);
...
PositionVector::lineAt(size_t pos) const {
    assert(size() > pos + 1);
    return Line((*this)[(int)pos], (*this)[(int)pos + 1]);
}
