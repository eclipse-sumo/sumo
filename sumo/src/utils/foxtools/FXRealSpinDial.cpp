...
long FXRealSpinDialDial::onMotion(FXObject*o,FXSelector s,void*p )
{
  ...
  if (e->root_x >= rootWin->getWidth()-1)  { x-=40; dragpoint-=40; bJump=TRUE; }
  else if (e->root_x <= 10)                { x+=40; dragpoint+=40; bJump=TRUE; }
  if (e->root_y >= rootWin->getHeight()-1) { y-=40; dragpoint-=40; bJump=TRUE; }
  else if (e->root_y <= 10)                { y+=40; dragpoint+=40; bJump=TRUE; }
  if (bJump) {
...
public:
  ...
  FXint getNumberFormatPrecision() const { return precision; }
  FXbool getNumberFormatExponent() const { return exponent; }
  void setFormatString(FXchar *fmt) {
    fmtString = fmt;
    flags |= FLAG_FMTSTRING;
  }
  FXString getNumberFormatString() const { return fmtString; }
...
long FXRealSpinDial::onMouseWheel(FXObject*o,FXSelector s,void*p){
  ...
  if(((FXEvent*)p)->code>0) increment(mode); else decrement(mode);
  if(target) target->handle(this,FXSEL(SEL_COMMAND,message),(void*)&pos);
  return 1;
  }
...
void FXRealSpinDial::setCyclic(FXbool cyclic){
  if(cyclic) options|=SPINDIAL_CYCLIC; else options&=~SPINDIAL_CYCLIC;
  }
