...
NIVissimSingleTypeParser_Verbindungsdefinition::parse(std::istream &from)
{
    ...
    while (tag!="nach") {
        ...
        if (y!="nach") {
            geom.push_back_noDoublePos(
                Position2D(
                    TplConvert<char>::my2SUMOReal(x.c_str()),
                    TplConvert<char>::my2SUMOReal(y.c_str())
                ));
            tag = myRead(from);
            try {
                TplConvert<char>::my2SUMOReal(tag.c_str());
                tag = myRead(from);
    ...
    if (tag=="fahrzeugklassen") {
        ...
        while (tag!="DATAEND"&&tag!="sperrung"&&tag!="auswertung") {
            int classes = TplConvert<char>::my2int(tag.c_str());
