...
ATTRS_NOSAVE = ('plugin','_obj','_manager','get','set','add','del')   
...
class AttrConf:
    ...
    def init_internal(self, man, obj):
...
class ChildConf(AttrConf):
    ...
    def init_external(self):
        self.get_val().init_external()
        
    def init_internal(self, man, obj):
        AttrConf.init_internal(self, man, obj)
        
        # make sure children get initialized
        self.get_val().init_internal(obj)
    ...
    def set_obj(self, obj):
        ...
        child = self.get_default()
        if child == None:
            child = self.get_val() # not necessary
       
        obj.set_child(self.attrname, child) 
    ...
    def save_value(self, state):
        ...
        if self._is_save:
            self._is_modified = False
            print 'ChildConf.save_value',self.attrname,self.get_val().get_ident_abs()
            state[self.attrname]= self.get_val()
            
            # remove default value here, it is just a redundant link
            # to the child instance attached to obj 
            self._default = None
    
        
    def get_name(self):
        return self.get_val().get_name()
    
    def get_info(self):
        return self.get_val().__doc__
...
class LinkConf(AttrConf):
    ...
    def save_value(self, state):
        """
        Save attribute value of managed object.
        Here the absolute ident of the linked object is saved
        """
        if self._is_save:
            self._is_modified = False
            ...
            print 'LinkConf.save_value',self.attrname,self.get_val().get_ident_abs()
            state[self.attrname]= self.get_val().get_ident_abs()
    
    
    def init_external(self):
        self.reset_linkobj()
        
    def reset_linkobj(self):
        """
        Substitute absolute ident with link object.
        Called from init_external of attrsman during load_obj
        """ 
        ident_abs = self.get_val()
...
class ColConf(AttrConf):
    ...
    def __init__(self, attrname, default,  **attrs):
        ...
        self._is_localcolumn = False # arrays store locally set by set_manager
    ...
    def set_manager(self, manager):
        ...
        """
        self._manager = manager
        self._is_localcolumn = manager.is_localcolumn()
    ...
    def __getitem__(self, ids):
        ...
        if hasattr(ids, '__iter__'):
            ...
            return self.get_val()[ids]
    
    def __setitem__(self, ids, values):
        #print '__setitem__',ids,values,type(self.get_val())
        if hasattr(ids, '__iter__'):
            i = 0
            array = self.get_val()
            for _id in ids:
    ...
    def save_value(self, state):
        ...
        Save attribute value of managed object.
        """
        #print 'save_value',self.attrname,self._is_save, self._is_localcolumn,
        # attention tricky:
        # add only value if contained in obj, 
        # otherwise if value is contained in self.column
        # then it will be saved in __getstate__ of attrconfig 
        if not self._is_localcolumn:
            if self._is_save:
                self._is_modified = False
                state[self.attrname]= self.get_val()
...
class Attrsman:
    ...
    def init_internal(self, obj):
        ...
        for attrconfig in  self.get_configs(is_all = True):
            attrconfig.init_internal(self, obj)
...
class Tabman(Attrsman):
    ...
    The column data of attrname is then directly accessible with
    
    obj.attrname
    
    If nothing is spwcified, then column attribute will be stored under
    the respective config instance of this tab man (self).
    The column data of attrname is then directly accessible with
    
    self.attrname.column
    
    """
    def __init__(self, obj=None, **kwargs):
        if obj==None:
            # this means that column data will be stored under column configs
            obj = self 
            self._is_localcolumn = True
        else:
            # this means that column data will be stored under obj
            self._is_localcolumn = False
            
        Attrsman.__init__(self, obj, **kwargs)
        self._colconfigs = [] 
        self._ids = []
        
    def is_localcolumn(self):
        return self._is_localcolumn
    
    #def add(self, attrconf):
    #    """
    #    Add a one new attributes to be managed.
    #    """  
    #    Attrsman.add(self, attrconf) # insert in common attrs database
    #    if attrconf.struct in STRUCTS_COL:
    #        self._colconfigs.append(attrconf)
    #    # returns initial array and also create local array if self._is_localcolumn == True
    #    return  attrconf.get_init(self.get_ids())
...
class BaseObjman:
    ...
    def get_obj_from_ident(self,ident_abs):
        ...
        if len(ident_abs)==1:
            ...
            return self.get_child(ident_abs[1]).get_obj_from_ident(ident_abs[1:])
    
    def get_child(self,attrname):
        """
        Return child instance
        """  
        return self.childs.get(attrname, None)
    
    
    
    def set_child(self,attrname, child):
        """
        Set child instance
        """  
        self.childs[attrname] = child
    ...
    def init_internal(self, parent):
        ...
        self.parent = parent
        self.childs = {}
        self.attrsman.init_internal(self)
        self._is_saved = False
    ...
    def  init_external(self):
        ...
        self.attrsman.init_external()
        
         

class TableObjman(BaseObjman, Tabman):
    """
    Table Object management base methods to be inherited by all object managers.
    """
    def __init__(self,**kwargs):
        self._init_objman(**kwargs)
        
    def _init_objman(self, **kwargs):
        BaseObjman._init_objman(self, managertype = 'table', **kwargs)
