...
class TabIdsConf(ColConf):
    ...
    def __init__(self, tab, is_child = True, **kwargs):
        attrname = tab.get_ident()
        self._tab = tab
        self._is_child = is_child
    ...
    def init_external(self):
        if self._is_child:
            AttrConf.init_external(self)
            self._tab.init_external()
        else:
            # Substitute absolute ident with link object.
            # Called from init_external of attrsman during load_obj
            # 
            ident_abs = self._tab # _tab has bee replaced by its ident
            print 'reset_linkobj',self.attrname,ident_abs
            obj = self.get_obj()
            rootobj = obj.get_root()
            print '  rootobj',rootobj.ident
    ...
    def save_value(self, state):
        ...
        """
        #print 'save_value',self.attrname,self._is_save, self._is_localvalue,
        # this saves the id array , if needed
        AttrConf.save_value(self, state)


        
    def get_name(self):
        return 'Table ID for '+self._tab.get_name()
...
class TableObjman(Tabman, BaseObjman):
    ...
    def get_childobj(self,ident):
        ...
        if hasattr(ident, '__iter__'):
            attrname , _id = ident
            config = self.childs[attrname]
            return config.get_valueobj(_id)
        else:
