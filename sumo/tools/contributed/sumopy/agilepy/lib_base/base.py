...
"""
from numpy import * 
#from numpy.oldnumeric.user_array import numpy.oldnumeric.user_array as UserArray   
from copy import copy,deepcopy

from sys import exit                       
import types,string,pickle
...
class PluginMixin:
    ...
    def init_plugin(self):
        ...
        # dictionary with obj as key and dict with plugid,plugtype,attr as value
        self._pluginobjs={}          
    ...
    def add_plugcodes(self,plugcodes):
        ...
        for plug,plugcode in plugcodes:
            if not self._plugcode.has_key(plug):
                ...
                #print '  added self._plugcode[plug]',plug,self._plugcode[plug]
            else:
...
class ObjConfigurator:
            ...
            if config.has_key('link'):
                ...
                if config['link']:
                    ...
                    if config['struct'] in ('array','dict','marray'):
                        ...
                        for id in self.get_ids():
                            self[attr,id] = self.get_obj_instance(config['ident'][id])
                            
                    else:
                        print '   detected link ',attr
                        setattr(self, attr, self.get_obj_instance(config['ident']) )
    ...
    def print_config(self):
        ...
        print '===================================================================='
        for attr in self._config.keys(): self.print_attr(attr)
    ...
    def config(self,attr,default=None,**args):  
        ...
        self._config[attr]=deepcopy(CONFIG_TEMPLATE) #CONFIG_TEMPLATE.copy()
        ...
        # TODO: would be better to introduce groups as a list with all group names
        ...
        if len(crits_atleastone)>0:
            ...
            for attr in self._config.keys():
                ...
                for criteria in crits_all:
                    #print 'get_attrs:',attr,criteria
                    satisfied &= (self._config[attr][criteria[0]]==criteria[1])
                if satisfied:
                    attrs.append(attr)
    ...
    def get_obj_instance(self,ident):
        ...
        if ident == '':
            ...
            obj=self
            idents=string.split(self.get_ident(is_local=True)+ident,IDENTSEP)
                ...
                # try to recover object with ident
                    if config['link']:
                        if config['struct'] in ('array','dict','marray'):
                            ...
                            config['ident'] = {}
                            for id in self.get_ids():
                                config['ident'][id] = self[attr,id].get_ident() 
                        else:
...
class ArrayObjConfigurator(ObjConfigurator):
    ...
    def get_ids_marray(self,attr,inds_marray=[]):
        ...
        if len(inds_marray)>0:
            ...
            for ind_marray in inds_marray:
                ...
                inds[i]=flatnonzero( (bp[:,0]<=ind_marray)&(bp[:,1]>ind_marray))[0]
                i+=1
    ...
    def suggest_ids(self,n,zeroid=False):
        ...
        Options:
            zeroid=True allows id to be zero.
        """
        #print 'suggest_ids',n,zeroid,self._inds,len(self._inds),self._inds.dtype
        ...
        if not zeroid:   
            if len(self._inds)==0:
                ...
                ids_unused=ids_unused_orig[flatnonzero(greater(ids_unused_orig,0))]
            zid=1
        else:
            if len(self._inds)==0:
                ids_unused=zeros(0,int) 
        ...
        if n_max<zid:
            ...
            if n_unused >= n:
                ...
                ids=concatenate((ids_unused,arange(n_max+1,n_max+1+n-n_unused))) 
    ...
    def create_ids(self,ids,**args):
        ...
        if id_max>id_max_old:
            ...
            self._inds=concatenate((self._inds, -1*ones(id_max-id_max_old,int)))
        ...
        for attr in self._arrayconfig.keys():
            ...
            #print '  ',attr,'self.__dict__[attr]',self.__dict__[attr],'value',value
            if len(self.__dict__[attr])>0:
                ...
                #print '  let NumPy, Numeric or whatever guess the first arrayelement'
            ...
            for id in ids:
                if args.has_key(attr):
                    ...
                    if self._config[attr]['do_init_arrayobj']:
                        ...
                        value.init_arrayobj(id,self,self.make_item_ident(attr,id))
                else:
    ...
    def reset(self,attr):
        ...
        if struct=='array':
            ...
            self._arrayconfig[attr]={}#currently no particular info, just key
            ...
            #print '  created type',self.__dict__[attr].dtype,self.__dict__[attr].shape
            ...
            self._dictconfig[attr]={} # currently no particolar info, just key
            self.__dict__[attr]={}
        ...
        if type(ids) in ARRAYTYPES:
            ...
            if struct=='array':
                ...
                if  self._config[attr]['is_function']:
                    ...
                    #print 'ids=',ids
                    #print 'self.get_inds_marray(attr,[ids]))=',self.get_inds_marray(attr,[ids])
                    return getattr(self,attr)[self.get_inds_marray(attr,[ids])]
                    # Numeric:
                    #return take(self.__dict__[attr],
                    #        self.get_inds_marray(attr,[ids]))
        ...
        if type(key) != types.TupleType: 
            ...
            if struct=='array':
                ...
                if len(shape)==1:
                    ...
                    #print '  self.get_index(ids)=',self.get_index(ids),type(self.get_index(ids)),self.get_index(ids).dtype
...
class CarTest(ArrayObjConfigurator):
    ...
    def __init__(self,**args):
        ...
        self.config('a',0.0, struct='array', group=['state','costs'],unit='m/s^2')
...
class Cars(ArrayObjConfigurator):
    ...
    def __init__(self,**args):
        ...
        self.config('x',0.456685,group=['state'],perm='w',digits_fraction=2)
            
        self.config('color',[0.2,0.8,0.2],metatype='color', struct='array',group=['parameter'],perm='w')
        self.config('load','user',struct='dict',perm='r')
        self.config('n',0,  struct='array', info='number',perm='w',
                            group=['state','parameters'],
                            min=0,max=5)
        self.config('b',False, struct='array', perm='w',info='bool...',group=['state','costs'])
        self.config('a',0.0, struct='array', info='acceleration',group=['state','costs'],digits_fraction=2)
        self.config('v',0.0, struct='array', info='velocity', group=['state'],unit='m/s',digits_fraction=2)
        self.config('d',0.0,unit='m', struct='array', info='this is \nthe distance',digits_fraction=2)
...
class Cabs(ArrayObjConfigurator):
    ...
    def __init__(self,parent=None,**args):
        ...
        self.config('d',[[1,2]],type=[('ArrayType', (1, 2)), 'IntType'],struct='dict')
...
def test_queue():
    ...
    q.insert_items_behind(array([2,],int), array([-1,],int), array([2,],int))
    
    q.print_info()
    
    q.insert_items_behind(array([16,],int), array([2,],int), array([2,],int))
    ...
    q.insert_items_behind(array([18,],int), array([2,],int), array([2,],int))
