    ...
    def get_optionstring(self, is_primed = True):
        ...
        for attr in self._optionattrs:
            ...
            if attr in self._filepathattrs:
                s += ' '+cmlattr+' %s'%filepathlist_to_filepathstring(value.split(','), is_primed = is_primed)
    ...
    def get_options(self):
        ...
        for attrconfig in self.get_attrsman().get_configs(filtergroupnames = [self.optiongroupname]):
            ...
            if is_enabled: # disabeled options are simply not added
                ...
                options.add_option(attrconfig.attrname,attrconfig.get_value(),attrconfig.cml, is_filepath = is_filepath)
...
if __name__ == '__main__':
    ...
    from objpanel import objbrowser

                                
    class TestProcess(Process):
        def __init__(   self,  filepath_config, filepaths_osm = '', time_step = 1.0,):
            Process.__init__(  self, 'testproc', 'sumo', name = 'Test Process')
            
            self.add_inputfile( 'filepath_config', filepath_config,
                                cml = '--configuration-file',
                                name = 'Configuration file',
                                info = 'Sumo configuration file',
                                )
                                
            self.add_outputfile( 'filepaths_osm', filepaths_osm,
                                cml = '-o',
                                name = 'Configuration file',
                                info = 'Sumo configuration file',
                                )
                                
            self.add_option(    'time_step', time_step,
                                cml = '--step-length',
                                name = 'Time step',
                                info = 'Time duration of a single simulation step in seconds',
                                unit = 's')
     
    p= TestProcess('/home/joerg/projects/sumopy/bologna/bologna2.netc.xml', 
                    filepaths_osm ='/home/joerg/projects/sumopy/bologna4/bologna40_16.osm.xml, /home/joerg/projects/sumopy/bologna4/bologna41_16.osm.xml',
                    time_step =2.0)
    p.attrs.print_attrs()  
    p.files_input.print_attrs()  
    print 'p.is_ready()', p.is_ready() 
    print 'p.is_done()', p.is_done() 
    print 'p.get_cml()', p.get_cml()
