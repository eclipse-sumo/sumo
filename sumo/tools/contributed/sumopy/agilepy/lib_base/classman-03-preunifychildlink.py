...
ATTRS_NOSAVE = ('plugin','_obj','_manager','get','set','add','del','childs','parent')   
...
class AttrConf:
    ...
    def set_value(self, value):
        ...
        print 'set_value',self.attrname, self._is_localvalue, value
        if self._is_localvalue:
            self.value = value
        else:
    ...
    def save_value(self, state):
        ...
        # otherwise if value is contained in self.value
        # then it will be saved in __getstate__ of attrconfig 
        if not self._is_localvalue:
            if self._is_save:
                self._is_modified = False
                state[self.attrname]= self.get_value()
...
class ChildConf(AttrConf):
    ...
    def init_external(self):
        AttrConf.init_external(self)
        self.get_value().init_external()
        
    def init_internal(self, man, obj):
        AttrConf.init_internal(self, man, obj)
        
        # make sure children get initialized
        self.get_value().init_internal(obj)
    ...
    def set_obj(self, obj):
        ...
        # but during first save.. default = None
        if child == None:
            child = self.get_value() 
    ...
    # maybe not necessary only because of    self._default = None   
    def save_value(self, state):
        """
        Save attribute value of managed object.
        """
        ...
        AttrConf.save_value(self, state)
    ...
    def save_value(self, state):
        ...
        if not self._is_localvalue:
            if self._is_save:
                ...
                print 'LinkConf.save_value',self.attrname,self.get_value().get_ident_abs()
                state[self.attrname]= self.get_value().get_ident_abs()
...
class ColConf(AttrConf):
    ...
    def __init__(self, attrname, default,  **attrs):
        ...
        self._is_localvalue = False # arrays store locally set by set_manager
...
class TableObjman(Tabman, BaseObjman):
    ...
    def get_child(self,attrname):
        ...
        """ 
        # TODO:
        #if type(attrname)==types.TupleType:
        #    if self.childs[attrname[0]][attrname[1]]
        #else:
        return self.childs[attrname]
    ...
    def init_internal(self, parent):
        ...
        self.parent = parent
        self.childs = {}
        Attrsman.init_internal(self, self)
        self._is_saved = False
