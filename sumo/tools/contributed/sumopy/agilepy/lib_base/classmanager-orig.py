...
class PuginMixin:
    ...
    def _execute_events(self,trigger):
        if self._has_events: 
            #print '**PuginMixin._execute_events',trigger,(EVTGETITEM,EVTGET)
            #if trigger!=EVTGET:
...
class AttrConf(PuginMixin):
    ...
    def set_obj(self, obj):
        """
        Method to set instance of configuration object.
        Used by add method of AttrManager
        """
        self._obj = obj  
    ...
    def  __getstate__(self):
        ...
        for attr in self.__dict__.keys():
            if attr not in ['_events','_plugins']:
...
class ArrayConf(AttrConf):
    ...
    def _execute_events_keys(self,trigger, keys):
        """
    ...
    # use original one
    # def _execute_events(self,trigger, keys=None):
    #    if keys==None:
    #        for function in self._events.get(trigger,[]): function(self)
    #    else:
    #        self._execute_events_keys(trigger, keys)
    ...
    def add(self, ids, keys):
        if self._is_keyindex:
            ...
            self._execute_events_keys(EVTADDITEM,keys)
        else:
            self.set_attr(np.concatenate((self.get_attr(),self.get_defaults(ids))) )
            self._execute_events_keys(EVTADDITEM,ids)
    ...
    def get(self,keys):
        ...
        if self._is_keyindex:
            ...
            if hasattr(keys, '__iter__'):
                ...
                self._execute_events_keys(EVTGETITEM,keys)
                return values
    ...
    def set(self, keys, values, is_add_row = True):
        ...
        if self._is_keyindex:
            ...
            self._execute_events_keys(EVTSETITEM,keys)
        else:
            if is_add_row:
                if hasattr(keys, '__iter__'):
                    ids = keys
    ...
    def delete(self,id=None,key=None):
        ...
        if self._is_keyindex:
            if id == None:# only keys are given
                self._execute_events_keys(EVTDELITEM,[key])
            ...
            self._execute_events_keys(EVTDELITEM,[id])
            del self[id]
...
class ListConf(ArrayConf):
    ...
    #            self._execute_events_keys(EVTDELITEM,keys)
    ...
    #                     
    #    else:
    #        self._execute_events_keys(EVTDELITEM,ids)
    #        del self[ids]
...
class AttrsManager(PuginMixin):
    ...
    def _execute_events(self,trigger, attrconfig):
        if self._has_events: 
            #if trigger!=EVTGET:
            #    print '  call set_modified',self._obj
            #    self._obj.set_modified(True)
            #print '**AttrsManager._execute_events NOMOD',attrconfig.attrname,trigger
    ...
    def add(self, attrconf):
        ...
        self._execute_events(EVTADD, attrconf)
    ...
    def delete(self, attrname):
        ...
        if hasattr(self,attrname): 
            ...
            self._execute_events(EVTDEL, attrconf)
...
class TableManager(AttrsManager,BasicManagerMixin):
    ...
    def _execute_events_keys(self,trigger, attrconf, keys):
        """
        Executes all functions assigned for this trigger.
        Keys are ids if is_keyindex=False otherwise they are index keys (any object)
        """
        if self._has_events:
            #print '**TableManager._execute_events_keys NOMOD',attrconf.attrname,self.ident
    ...
    def get_row(self,key):
        if self._is_keyindex:
            ...
            #attrconf._execute_events_item(EVTADDITEM,keys=[key])
        else:
            id = key
            #attrconf._execute_events_item(EVTADDITEM,ids=[id])
        attrs={}    
        for attrconfig in self._attrconfigs:
    ...
    def delete(self,attr):
        ...
        #attrconf._execute_events(EVTDEL)
        if attrconf in self._attrconfigs:
