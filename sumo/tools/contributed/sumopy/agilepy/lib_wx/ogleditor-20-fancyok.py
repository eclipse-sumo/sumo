...
class Vbo:
    def __init__(self, ident, glelement, n_vpe):
        self._ident = ident
    ...
    def update_vertices(self, vertices,  n_drawobjs):
        ...
        """
        #print 'update_vertices',self.get_ident(),n_drawobjs
    ...
    def update_colors(self, colors):
        #print 'update_colors',self.get_ident(),self
        n_repreat = len(self._vertexvbo)/self._n_drawobjs
        #print '  len(self._vertexvbo),n_repreat',len(self._vertexvbo),n_repreat
        #print '  np.repeat(colors,n_repreat,0)\n',np.repeat(colors,n_repreat,0)
...
class Lines(DrawobjMixin):
    def __init__(self,ident,  parent, name = 'Lines',
        ...
        self.add_vbo(Vbo('line', GL_LINES, 2))# 2 verts for OGL line element
    ...
    def _update_colorvbo(self):
        #n=len(self)
        #n_vert = self.get_n_vert()
        #print '_update_colorvbo of lines'
        self.get_vbo('line').update_colors(self.colors.value + self.are_highlighted.value.reshape(len(self),1)*self.colors_highl.value)
...
class Fancylines(Lines):
    def __init__(self,ident,  parent, name = 'Lines',
            ...
            is_fill = True,
            is_outline = False,
            beginstyle = 'flat',#'bevel','flat','arrow','arrow_left','arrow_right','fancyarrow'
            endstyle = 'triangle',
            arrowstretch = 1.2,
            arrowlength = 0.5,
            c_highl=0.3, 
        ...
        if is_fill:
            self.add_vbo(Vbo('line_fill', GL_QUADS,4))
            if beginstyle !='flat':
                self.add_vbo(Vbo('begin_fill', GL_TRIANGLES,3))
            if  endstyle !='flat':   
                self.add_vbo(Vbo('end_fill', GL_TRIANGLES,3))
        
        
            
        styles = ['bevel','flat','triangle','arrow','arrow_left','arrow_right','fancyarrow']
    ...
    def _update_vertexvbo(self):
        ...
        if self._is_fill.value:
            ...
            if self.beginstyle.value !='flat':
                x1_new = x2 -(length_xy-halfwidth)*np.cos(alpha_xy)
                y1_new = y2 -(length_xy-halfwidth)*np.sin(alpha_xy)
                self._update_vertexvbo_begin_fill(x1_new,y1_new,z1,x2,y2,z2,length_xy, alpha_xy,halfwidth)
            else:
                x1_new = x1
                y1_new = y1
                
            if  self.endstyle.value !='flat':   
                x2_new = x1 +(length_xy-halfwidth)*np.cos(alpha_xy)
                y2_new = y1 +(length_xy-halfwidth)*np.sin(alpha_xy)
                self._update_vertexvbo_end_fill(x1,y1,z1,x2_new,y2_new,z2,length_xy, alpha_xy,halfwidth)
            else:
                x2_new = x2
                y2_new = y2
                
            self._update_vertexvbo_line_fill(x1_new,y1_new,z1,x2_new,y2_new,z2,length_xy, alpha_xy,halfwidth)
        
    def _update_vertexvbo_line_fill(self,x1,y1,z1,x2,y2,z2,length_xy, alpha_xy,halfwidth):
        n=len(self)
        n_vert = self.get_n_vert()
        dphi = np.pi/(n_vert-1)
    ...
    def _update_vertexvbo_begin_fill(self,x1,y1,z1,x2,y2,z2,length_xy, alpha_xy,halfwidth):
        n=len(self)
        if self.beginstyle.value =='bevel':
            n_vert = 6
        if self.beginstyle.value =='triangle':
            n_vert = 3
        else:
            n_vert = 6
        ...
        alphas = alpha_xy+np.arange(np.pi/2,3*np.pi/2+dphi,dphi).reshape(-1,1)
        print '  alpha_xy\n',alpha_xy/np.pi*180
        ...
        x = np.cos(alphas) * halfwidth + x1
        y = np.sin(alphas) * halfwidth + y1
        
        z = np.ones((n_vert,1)) *z2
        ...
        xcf = (np.ones((n_vert,1)) * x1).transpose().flatten()
        ycf = (np.ones((n_vert,1)) * y1).transpose().flatten()
        #print '  xcf\n',xcf
        #print '  ycf\n',ycf
        ...
        n_vpe = self.get_vbo('begin_fill').get_vpe()
        ...
        vertices[:,8]=zf[ind2]
        #print '  vertices=\n',vertices
        self.get_vbo('begin_fill').update_vertices(vertices,n)
        
        #self._vertexvbo_fill = vbo.VBO(vertices.reshape((-1,3)))
        #self._indexvbo_fill = vbo.VBO(np.arange(self._n_vert*self._n_elem_fill, dtype=np.int32), target=GL_ELEMENT_ARRAY_BUFFER)
    
        
    def _update_vertexvbo_end_fill(self,x1,y1,z1,x2,y2,z2,length_xy, alpha_xy,halfwidth):
        n=len(self)
        if self.endstyle.value =='bevel':
            n_vert = 6
        if self.endstyle.value =='triangle':
            n_vert = 3
        else:
            n_vert = 6
        ...
        alphas = alpha_xy+np.arange(-np.pi/2,np.pi/2+dphi,dphi).reshape(-1,1)
        print '  alpha_xy\n',alpha_xy/np.pi*180
        print '  alphas=alpha0+phi\n',alphas/np.pi*180
        ...
        #print '  y2_new=',y2_new,y2_new.dtype
        x = np.cos(alphas) * halfwidth + x2
        y = np.sin(alphas) * halfwidth + y2
        ...
        n_vpe = self.get_vbo('end_fill').get_vpe()
        #print '  n_elem_fill ',self._n_elem_fill
        #print '  n_vpe',n_vpe
        vertices = np.zeros((n_elem_fill, n_vpe * 3),dtype=np.float32)
        #print '  vertices.reshape((-1,3)).shape',vertices.reshape((-1,3)).shape
        ...
        vertices[:,8]=zf[ind2]
        #print '  vertices=\n',vertices
        self.get_vbo('end_fill').update_vertices(vertices,n)
    ...
    def _update_colorvbo(self):
        ...
        if self._is_fill.value:
            colors_fill = self.colors_fill.value + self.are_highlighted.value.reshape(n,1)*self.colors_fill_highl.value

            self.get_vbo('line_fill').update_colors(colors_fill)
            if self.beginstyle.value !='flat':
                self.get_vbo('begin_fill').update_colors(colors_fill)

                
            if  self.endstyle.value !='flat':   
                self.get_vbo('end_fill').update_colors(colors_fill)
...
class Circles(DrawobjMixin):
    ...
    def __init__(self,ident,  parent, name = 'Circles',
        ...
        if is_outline:
            self.add_vbo(Vbo('outline',GL_LINES,2))
        if is_fill:
            self.add_vbo(Vbo('fill',GL_TRIANGLES,3))
    ...
    class OGLeditorMainframe(AgileToolbarFrameMixin, wx.Frame):
        ...
        def on_test(self,event=None):
            ...
            if 1:
                ...
                fancylines.add_drawobjs(vertices_fancy,[0.5, 0.3],colors)
