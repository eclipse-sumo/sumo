    ...
    def recreate_panel( self, objinfo, **kwargs):
        if hasattr(objinfo, '__iter__'):
            ...
            if hasattr(idinfo, '__iter__'):
                self.recreate_panel_table(objinfo, ids = idinfo, **kwargs)
            else:
                self.recreate_panel_scalar(objinfo, id = idinfo, **kwargs)
...
class NaviPanelMixin:
    ...
    def init_navbar(self,obj):
        ...
        self.path=wx.TextCtrl(self, -1, obj.format_ident_abs(), style=wx.TE_RIGHT)#size=(-1, -1))#,size=(300, -1))
