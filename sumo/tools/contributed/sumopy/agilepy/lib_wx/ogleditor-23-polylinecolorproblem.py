...
class PolyLines(Fancylines):
    def __init__(self,ident,  parent, name = 'Polylines', joinstyle = 0,**kwargs):
        Fancylines.__init__(self, ident,  parent, name = 'Polylines',**kwargs)
    ...
    def make_lines(self):
        ...
        self._lineinds = np.array(lineinds, np.int32)
    ...
    def _update_vertexvbo(self):
        ...
        alpha_xy=np.arctan2(dy,dx)
        length_xy = np.sqrt(dx*dx + dy*dy)
        halfwidth = 0.5*self.widths.value[self._lineinds]
        x1_new = x1.copy()
        y1_new = y1.copy()
        x2_new = x2.copy()
        y2_new = y2.copy()
