...
class SelectTool(BaseTool):
    ...
    def pick_all(self, event):
        ...
        for drawobj in  drawing.get_drawobjs():
            ...
            # experiment
            handle = drawobj.pick_handle(p)
            if len(handle)>0:
                print '  handle',drawobj.get_ident(),handle
...
class MoveTool(SelectTool):
    ...
    def aboard(self, event):
        is_draw = False
        if self.is_move:
            self.drawobj_anim.del_drawobj(self.id_anim)
            del self.drawobj_anim
            del self.id_anim
            self.is_move = False
            is_draw = True
...
class DrawobjMixin(am.ArrayObjman):
    ...
    def set_anim(self, _id, drawobjelem_anim):
        ...
        self.vertices[_id] = drawobj_anim.vertices[id_anim]
...
class Lines(DrawobjMixin):
    ...
    def pick_handle(self, coord, detectwidth = 0.01):
        """
        Retuns list [ id, ind_vert] when handle is near coord,
        otherwise []
...
class Fancylines(Lines):
    ...
    def get_anim(self,_id):
        drawobj_anim = self.parent.get_drawobj_by_ident('anim_fancylines')
        
        id_anim = drawobj_anim.add_drawobj( self.vertices[_id], 
                                            1.0*self.widths[_id], 
                                            [0.9,0.9,0.9,0.5],
                                            beginstyle = self.beginstyles[_id], 
                                            endstyle = self.endstyles[_id],
                                            )
...
class Polylines(Fancylines):
    ...
    def get_anim(self,_id):
        drawobj_anim = self.parent.get_drawobj_by_ident('anim_polylines')
...
class Circles(DrawobjMixin):
    ...
    def get_anim(self,_id):
        drawobj_anim = self.parent.get_drawobj_by_ident('anim_circles')
        
        id_anim = drawobj_anim.add_drawobj( self.centers[_id], 
                                            1.0*self.radii[_id], 
                                            [0.9,0.9,0.9,0.5],
                                            )
    ...
    def set_anim(self, _id, drawobjelem_anim):
        ...
        #print '    self.vertices[_id]=',self.vertices[_id]
        #print '    drawobjset_anim.vertices[id_anim]=',drawobjset_anim.vertices[id_anim]
        self.centers[_id] = drawobj_anim.centers[id_anim]
        self.radii[_id] = drawobj_anim.radii[id_anim]
        drawobj_anim.del_drawobj(id_anim)
        self._update_vertexvbo()
        return True
...
class Polygons(DrawobjMixin):
    ...
    def get_anim(self,_id):
        drawobj_anim = self.parent.get_drawobj_by_ident('anim_polygons')
