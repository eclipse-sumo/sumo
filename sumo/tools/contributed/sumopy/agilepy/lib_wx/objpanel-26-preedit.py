...
class ListWidgetContainer(WidgetContainer):
    ...
    def get_valuewidget_write(self):
        ...
        if is_list_flat(value):
            widget =  wx.TextCtrl( self.parent, -1, list_to_str(value),style= wx.ALIGN_RIGHT|wx.TE_MULTILINE)
    ...
    def on_objlink(self,event):
        ...
        if self.parent.func_change_obj:
            if mt == 'obj':
                ...
                linktab = self._attrconf.get_linktab()
                navitimer = wx.FutureCall(1, self.parent.func_change_obj, linktab, None, value)# here value is list with ids
...
class ColorWidgetContainer(AttrBase,TextWidgetContainer):
    ...
    def get_widgetvalue(self):
        ...
        color = np.array(s[1:-1].split(','),float)/256.0
        print 'ColorWidgetContainer.get_widgetvalue',s,color
        return color
...
class TableGrid(AttrBase,gridlib.PyGridTableBase):
    ...
    def GetValue(self, row, col):
        try:
            ...
            mt = attrconf.metatype
            #print 'GetValue',attrconf.get_name(),mt
            if mt =='objs':
    ...
    def SetValue(self, row, col, value):
        ...
        if attrconf != None:
            if attrconf.is_writable():
                ...
                #print '    attrconf',attrconf.get_name(), attrconf.is_writable()
                # attr=self.attrs[col]
                tt = type(attrconf.get_default())
...
class ObjPanelMixin:
    ...
    def recreate_panel( self, obj, id = None, ids = None,  **kwargs):  
        #print '\nrecreate_panel:',obj.ident, id , ids 
        #print '  args =',kwargs
        self.obj = obj
        self.id = id
        self.ids = ids
        if (id == None)&(ids == None):
            if obj.managertype=='table': 
                self.recreate_panel_table(**kwargs)
            else:
                self.recreate_panel_scalar(**kwargs)
        else:
            if ids != None:
    ...
    def recreate_panel_table(self, attrconfigs=None,
        ...
        """
        #
        #print 'recreate_panel_table:',obj.ident, func_change_obj
    ...
    def recreate_panel_scalar( self, attrconfigs=None,
        ...
        """
        #
        #print 'recreate_panel_scalar:',obj.ident, func_change_obj
...
class ObjPanel(ObjPanelMixin,wx.Panel):
    ...
    def __init__(self, parent, obj = None, 
        ...
        #print 'ObjPanel.recreate',obj.ident,id
        self.recreate_panel(obj, id=id, ids=ids,
                            attrconfigs = attrconfigs,
                            groupnames = groupnames, show_groupnames = show_groupnames,
...
class NaviPanelMixin:
    ...
    def change_obj(self,obj,id=None, ids=None,**kwargs):
        #print 'Navicanvas.change_obj',obj.format_ident(),id,ids,kwargs.keys()
...
class NaviPanel(NaviPanelMixin, wx.Panel):
    ...
    on a panel.
    """
    def __init__(self, parent, obj, id = None, mainframe=None, **args):
        if obj==None:
            obj=cm.BaseObjman('empty')
        wx.Panel.__init__(self,parent,-1,wx.DefaultPosition,wx.DefaultSize) 
        #print 'NaviPanel',obj.ident,id
        sizer=wx.BoxSizer(wx.VERTICAL)
        ...
        self.init_objpanel(obj, id=id, mainframe=mainframe,is_modal=False,**args)
