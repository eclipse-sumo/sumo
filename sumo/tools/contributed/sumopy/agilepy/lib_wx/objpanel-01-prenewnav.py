...
from wxmisc import KEYMAP, MetaPopupMenu, MetaToolbarMixin,get_tablecolors
...
class ScalarPanel(wx.Panel):
    ...
    def create_attrs(self, attrconfigs, immediate_apply=False, panelstyle='default'):
        ...
        # works on panel...
        #parent.SetHelpText('The Meta Panel')
        self.SetAutoLayout( True )
        self.SetSizer(sizer)
        sizer.Fit(self)
...
class TabPanel(AttrBase,gridlib.Grid):
    ...
    def on_contextmenu(self, event):
        ...
        menu=MetaPopupMenu(self.parent)
    ...
    def on_edit_cell_miniframe(self, event):
        ...
        #print 'on_edit_cell EventObject=',EventObject
        table = self.GetTable()
        id, attrconf = table.get_id_attrconf(event.GetRow(), event.GetCol())
    ...
    def OnLabelLeftDoubleClick(self, evt):
        ...
        if evt.GetRow()>=0:
            ...
            if self.func_choose_id==None:
                ...
                dlg = ObjPanelDialog(   self,self.tab.get_obj(),table = self.tab, id= id,
                                        #title = '',
                                        size=(350, 200),
                                        style = wx.DEFAULT_DIALOG_STYLE
                                    )
...
class ObjPanelMixin:
    ...
    def recreate_panel( self, obj = None, 
                        attrconfigs=None,
                        tables = None,
    ...
    def add_table(self, parent, tab, ids = None, attrconfigs = None, groupnames = None,show_title=False):
        """
        ...
        if show_title:
            self.add_title(sizer,tab.get_name())
    ...
    def add_title(self,sizer,title=None,id=None, fontsize = 12):
        ...
        if title==None:
            ...
            if id==None:
                ...
                if self.obj.is_keyindex():
                    title=self.obj.get_name().title()+' '+str(self.obj.get_key_from_id(id))
                else:
...
class NaviPanelMixin(MetaToolbarMixin):
    ...
    def change_obj(self,obj,id=None,**kwargs):
        #print 'Navicanvas.change_obj',obj,id
        if id!=None:
            if id <= -1: 
    ...
    def init_navbar(self,obj):
        ...
        self.add_tool(
            'back',self.on_go_back,
            wx.ArtProvider.GetBitmap(wx.ART_GO_BACK, wx.ART_TOOLBAR, tsize),
            'show last panel') 
            
        self.add_tool(
            'forward',self.on_go_forward,
            wx.ArtProvider.GetBitmap(wx.ART_GO_FORWARD, wx.ART_TOOLBAR, tsize),
            'show next panel') 
        ...
        self.path=wx.TextCtrl(self.toolbar, -1, obj.format_ident_abs(), style=wx.TE_RIGHT|wx.EXPAND)#size=(-1, -1))#,size=(300, -1))
