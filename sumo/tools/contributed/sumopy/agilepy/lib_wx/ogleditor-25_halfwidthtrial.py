...
class Fancylines(Lines):
    ...
    def _update_vertexvbo_begin_fill(self, style, inds_style, x1_new, y1_new,z1,x2,y2,z2, length_xy, alpha_xy, halfwidth):
        ...
        if style =='bevel':
            n_vert = 6
            dphi = np.pi/(n_vert-1)
            alpha_delta = (np.arange(-np.pi/2,np.pi/2+dphi,dphi)+np.pi)
            
        if style =='triangle':
            n_vert = 3
            alpha_delta = (np.array([-np.pi/2, 0.0, np.pi/2])+np.pi)
            
        if style =='arrow':
            n_vert = 3
            dphi = np.pi/(n_vert-1)
            alpha_delta = (self.arrowstretch.value*np.arange(-np.pi/2,np.pi/2+dphi,dphi)+np.pi)
            
        if style =='lefttriangle':
            n_vert = 2
            dphi = np.pi/(n_vert-1)
            alpha_delta = (np.arange(-np.pi/2,np.pi/2+dphi,dphi)+np.pi)
            
        if style =='righttriangle':
            n_vert = 2
            dphi = np.pi/(n_vert-1)
            alpha_delta = (alphastretch*np.arange(-np.pi/2,np.pi/2+dphi,dphi)+np.pi)
        ...
        alphas = alpha_xy[inds_style]+alpha_delta.reshape(-1,1)
        #print '  alpha_xy\n',alpha_xy/np.pi*180
...
#-------------------------------------------------------------------------------
            if 1:
                fancylines = Fancylines('fancylines', drawing, is_righthalf = False)
                ...
                fancylines.add_drawobjs(    vertices_fancy,
                                            ...
                                            endstyles = [BEVELHEAD,BEVELHEAD],
                                            )
                drawing.add_drawobj(fancylines)
#-------------------------------------------------------------------------------
            if 0:
                polylines = Polylines('polylines', drawing, joinstyle = BEVELHEAD, is_righthalf = False)
