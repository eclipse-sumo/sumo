...
class Fancylines(Lines):
    def __init__(self,ident,  parent, name = 'Lines',
            ...
            is_fill = True,
            is_outline = False,
            beginstyle = 'bevel',#'bevel','flat','arrow','arrow_left','arrow_right','fancyarrow'
            endstyle = 'arrow',
            arrowstretch = 2.5,
            arrowlength = 0.5,
            c_highl=0.3, 
        ...
        if is_fill:
            ...
            if beginstyle !='flat':
                self.add_vbo(Vbo('begin_fill', GL_TRIANGLES,3,objtype='fill'))
            if  endstyle !='flat':   
                self.add_vbo(Vbo('end_fill', GL_TRIANGLES,3,objtype='fill'))
    ...
    def _update_vertexvbo_begin_fill(self,x1,y1,z1,x2,y2,z2,length_xy, alpha_xy,halfwidth):
        ...
        if self.beginstyle.value =='arrow':
            n_vert = 3
            alphastretch = 1.2
            arrowstretch = self.arrowstretch.value
        ...
        alphas = alpha_xy+(alphastretch*np.arange(-np.pi/2,np.pi/2+dphi,dphi)+np.pi).reshape(-1,1)
        print '  alpha_xy\n',alpha_xy/np.pi*180
        ...
        x = np.cos(alphas) * halfwidth* arrowstretch + x1
        y = np.sin(alphas) * halfwidth* arrowstretch + y1
    ...
    def _update_vertexvbo_end_fill(self,x1,y1,z1,x2,y2,z2,length_xy, alpha_xy,halfwidth):
        ...
        alphas = alpha_xy+alphastretch*np.arange(-np.pi/2,np.pi/2+dphi,dphi).reshape(-1,1)
        print '  alpha_xy\n',alpha_xy/np.pi*180
        print '  alphas=alpha0+phi\n',alphas/np.pi*180
        ...
        #print '  y2_new=',y2_new,y2_new.dtype
        x = np.cos(alphas) * halfwidth* arrowstretch + x2
        y = np.sin(alphas) * halfwidth* arrowstretch + y2
...
class Circles(DrawobjMixin):
    ...
    def __init__(self,ident,  parent, name = 'Circles',
        ...
        if is_fill:
            self.add_vbo(Vbo('fill',GL_TRIANGLES,3,objtype='outline'))
