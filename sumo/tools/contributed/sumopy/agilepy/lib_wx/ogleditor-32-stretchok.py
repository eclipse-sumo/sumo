...
class MoveTool(SelectTool):
    ...
    def move(self,event):
        ...
        if np.any(np.abs(vertex_delta)>0):
            is_draw = self.drawobj_anim.move(self.id_anim, coords, vertex_delta) 
            if is_draw:
...
class StretchTool(HandleTool):
    ...
    def activate(self,canvas = None):
        ...
        activated and can now interact with metacanvas.
        """
        HandleTool.activate(self, canvas)
        self.is_move = False
    ...
    def begin_anim(self, event):
        drawobj, _id, self.ind_vert =  self.get_current_vertexselection()
        self.drawobj_anim, self.id_anim = drawobj.get_anim(_id)
        self.coord_last = self._canvas.unproject(event.GetPosition())
    ...
    def move(self,event):
        ...
        vertex_delta = coords-self.coord_last
        if np.any(np.abs(vertex_delta)>0):
            is_draw = self.drawobj_anim.stretch(self.id_anim, coords, vertex_delta, self.ind_vert)  
            if is_draw:
                self.coord_last = coords
        return is_draw
...
class Circles(DrawobjMixin):
    ...
    def get_anim(self,_id):
        ...
        id_anim = drawobj_anim.add_drawobj( np.array(self.centers[_id]),
                                            1.0*self.radii[_id], 
                                            [0.9,0.9,0.9,0.5],
                                            )
    ...
    def set_anim(self, _id, drawobjelem_anim):
        ...
        self.centers[_id] = 1.0*drawobj_anim.centers[id_anim]
...
class Polygons(DrawobjMixin):
    ...
    def get_anim(self,_id):
        ...
        #print 'get_anim',self.ident,_id,id_anim,self.vertices[_id]
        return drawobj_anim, id_anim
