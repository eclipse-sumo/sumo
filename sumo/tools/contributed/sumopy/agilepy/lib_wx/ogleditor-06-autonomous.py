...
class Lines(am.ArrayObjman):
    def __init__(self,ident,  parent, name = 'Lines',c_highl=0.3,
            n_vert =2, linewidth=3,  
            detectwidth = 0.1,#m
            vertices = None, colors = None,
            **kwargs):
        ...
        self.c_highl = c_highl
        self.detectwidth = detectwidth
        
        self.add_drawobjs(vertices, colors)
        self.set_attrs(vertices, colors)
    
    def add_drawobjs(self, vertices, colors):
        #print 'add_drawobjs'
        #print '  before add ',type(self.vertices.value),self.vertices.value.dtype
        ids = self.add_rows(len(vertices), 
                            vertices=vertices, 
                            colors=colors
                            )
        #print '  after add ',type(self.vertices.value),self.vertices.value.dtype
    ...
    def _update_vertexvbo(self):
        print '_update_vertexvbo', type(self.vertices.value),self.vertices.value.dtype
        self._vertexvbo = vbo.VBO(self.vertices.value.reshape((-1,3)))
        self._indexvbo = vbo.VBO(np.arange(self.n_vert_per_elem*len(self.vertices.value),dtype=np.int32), target=GL_ELEMENT_ARRAY_BUFFER)
    ...
    def _update_colorvbo(self):
        ...
        self._colorvbo = vbo.VBO(np.clip((self.colors.value+self.colors_highl)[np.array(np.arange(0,len(self.colors.value),1.0/self.n_vert_per_elem),int)],0.0,1.0))
    ...
    def draw(self):
        ...
        glDrawElements(GL_LINES, self.n_vert_per_elem*len(self.vertices.value), GL_UNSIGNED_INT, None)
...
class OGLlines(am.ArrayObjman):
    def __init__(self,ident,  parent, name = 'Lines',c_highl=0.3,
        ...
        self.add(cm.AttrConf(  'c_highl', c_highl,
                                        ...
                                        is_plugin = False,
                                        name = 'Highl. const.', 
                                        info = 'Highlight constant takes values betewwn 0 and 1 and is the amount of brightness added for highlighting',
                                        ))
