...
class GlEditor(wx.SplitterWindow):
...
class TestMainframe(wx.Frame):
    ...
    def __init__(self, parent=None,  id=-1, title ='mainframe', pos=wx.DefaultPosition,
        ...
        splitter.SetSashPosition(width/2, True)
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        frame = TestMainframe()
        gleditor = frame.add_view('GL Editor',GlEditor)
        gleditor.canvas.add_element(lines)
        gleditor.canvas.add_element(triangles)
        gleditor.canvas.add_element(rectangles)
    elif 0:
        frame = TestMainframe()
        glcanvas = frame.add_view('GL canvas',WxGLTest2)
