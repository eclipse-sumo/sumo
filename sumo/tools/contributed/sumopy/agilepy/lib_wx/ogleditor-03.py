...
class OGLcanvas(glcanvas.GLCanvas):
    ...
    def OnLeftDown(self, event):
        ##
        event.point = self.unproject(event)[0:2]
    ...
    def OnMotion(self, event, is_draw=False):
        ...
        event.point = self.unproject(event)[0:2]
        #print 'OnMotion (px,py)=',event.point,'(x,y)=',event.GetPosition() 
        if self._drawing:
            for drawobj in  self._drawing.get_drawobjs():
                 inds_pick = drawobj.pick(event.point)
    ...
    def BeginGrap(self,event):
        ...
        self.xStart,self.yStart=event.point
        self.xTrans_start,self.yTrans_start = self.xTrans,self.yTrans
    ...
    def MoveGrap(self,event):
        ...
        self.xTrans = self.xTrans_start + (px-self.xStart)
        self.yTrans = self.yTrans_start + (py-self.yStart)
        
        #self.xTrans += px-self.xStart
        #self.yTrans += py-self.yStart
        
        #self.xTrans += x-self.xStart
        #self.yTrans += y-self.yStart
        print 'MoveGrap',self.xTrans,self.yTrans, px, py
        #self.xStart,self.yStart = x,y
        
        #self.xStart,self.yStart = px, py
    ...
    def polarView(self):
        glTranslatef( self.yTrans, 0.0, 0.0 )
        glTranslatef(  0.0, -self.xTrans, 0.0)
