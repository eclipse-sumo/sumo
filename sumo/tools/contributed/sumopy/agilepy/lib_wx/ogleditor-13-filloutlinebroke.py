...
class DrawobjMixin(am.ArrayObjman):
    ...
    def _update_colorvbo(self):
        ...
        #print '_update_colorvbo',self._n_vpe, n ,n_vert,len(self._vertexvbo)
        #print '   len(colors)',self._n_vpe*n*(n_vert-1)
        
        
        if self._is_outline.value:
            self._colorvbo = vbo.VBO( np.repeat((self.colors.value + self.are_highlighted.value.reshape(n,1)*self.colors_highl.value), self._n_vpe*(n_vert-1),0))
        
        if  self._is_fill.value:
            self._colorvbo_fill = vbo.VBO( np.repeat((self.colors_fill.value + self.are_highlighted.value.reshape(n,1)*self.colors_fill_highl.value),self._n_vpe_fill*(n_vert-1),0))
...
class Circles(DrawobjMixin):
    ...
    def __init__(self,ident,  parent, name = 'Circles',
        ...
        self.init_common(   ident, parent = parent, name = name,
                            ...
                            is_parentobj = is_parentobj, 
                            n_vert = n_vert, 
                            n_vpe = 2, # outline with line sehments
                            n_vpe_fill = 3, # fill with triangl elements
                            is_fill = is_fill,
    ...
    def add_drawobj(self, center, radius, 
        ...
        if color_fill_highl == None:
            color_fill_highl=self._get_colors_highl(np.array([color_fill]))
        ...
        _id = self.add_row( centers=center, 
                            ...
                            colors_fill_highl = colors_fill_highl[0],
                            )
        if is_update:
            self._update_vertexvbo()
            self._update_colorvbo()
    ...
    def _update_vertexvbo_lines(self,x,y,z):
        ...
        # create and compose main vertices array
        self._n_elem = (n_vert-1)*n
        
        
        n_vpe = 2 # vertices per OGL element
            
        vertices = np.zeros((self._n_elem, n_vpe * 3),dtype=np.float32)
        ...
        self._indexvbo = vbo.VBO(np.arange(self._n_vert*self._n_elem,dtype=np.int32), target=GL_ELEMENT_ARRAY_BUFFER)
    ...
    class OGLeditorMainframe(AgileToolbarFrameMixin, wx.Frame):
        ...
        def on_test(self,event=None):
            ...
            if 1:
                ...
                circles.add_drawobjs([[-1.0,0.0,0.0],[1.0,0.5,0.0]],[0.5, 0.8],colors2o, colors2)
