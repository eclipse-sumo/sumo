...
class Vbo:
    def __init__(self, ident, glelement, n_vpe, noncyclic = 1):
        self._ident = ident
        self._glelement = glelement
        self._n_vpe = n_vpe
        self._noncyclic = noncyclic
...
class Fancylines(Lines):
    def __init__(self,ident,  parent, name = 'Lines',
        ...
        #if is_fill:
        #    #if beginstyle !='flat':
        #    #    self.add_vbo( Vbo('begin_fill', GL_TRIANGLES,3, noncyclic = 0))
        #    if beginstyle !='flat':
        self.add_vbo(Vbo('end_fill', GL_TRIANGLES,3, noncyclic = 0))
    ...
    def _update_colorvbo(self):
        ...
        print '  colors =\n',colors
        self.get_vbo('end_fill').update_colors(colors)
        
        #self.get_vbo('end_fill').update_colors(self.colors_fill.value + self.are_highlighted.value.reshape(n,1)*self.colors_fill_highl.value)
        #print '  filllll',self._n_vpe_fill, n ,n_vert,len(self._vertexvbo_fill),self._n_elem_fill
...
class Circles(DrawobjMixin):
    ...
    def __init__(self,ident,  parent, name = 'Circles',
        ...
        if is_outline:
            self.add_vbo(Vbo('outline',GL_LINES,2, noncyclic = 0))
        if is_fill:
            self.add_vbo(Vbo('fill',GL_TRIANGLES,3, noncyclic = 0))
    ...
    class OGLeditorMainframe(AgileToolbarFrameMixin, wx.Frame):
        ...
        def on_test(self,event=None):
            ...
            if 1:
                ...
                vertices_fancy = vertices+[0.0,-1.0,0.0]
