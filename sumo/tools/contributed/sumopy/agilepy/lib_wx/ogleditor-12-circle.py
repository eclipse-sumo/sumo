...
class DrawobjMixin(am.ArrayObjman):
    ...
    def _update_colorvbo(self):
        ...
        # oldself._colorvbo = vbo.VBO((self.colors.value + self.are_highlighted.value.reshape(n,1)*self.colors_highl.value)[np.array(np.arange(0,n,1.0/self._n_vert),int)])
        
        #colors = np.repeat((self.colors.value+ self.are_highlighted.value.reshape(n,1)*self.colors_highl.value),2*(n_vert-1),0)
        #print '  self.colors.value=\n',self.colors.value,self.colors.value.shape,self.colors.value.dtype
        #print '  colors=\n',colors
        self._colorvbo = vbo.VBO( np.repeat((self.colors.value+ self.are_highlighted.value.reshape(n,1)*self.colors_highl.value),2*(n_vert-1),0))
...
class Circles(DrawobjMixin):
    ...
    def __init__(self,ident,  parent, name = 'Circles',
        ...
        # number of vertices of one circle.
        # ...could  be computed dynamically based on resolution
        self._n_vert = n_vert
    ...
    def add_drawobj(self, center, radius, color, color_highl = None, is_update = True):
        if color_highl == None:
            colors_highl=self._get_colors_highl(np.array([color]))
    ...
    def add_drawobjs(self, centers, radii, colors, colors_highl = None, is_update = True):
        if colors_highl == None:
            colors_highl=self._get_colors_highl(colors)
        #print 'add_drawobjs'  
        ...
        #print '  radii',radii
        #print '  colors',colors
        #print '  colors_highl',colors_highl
    ...
    def _update_vertexvbo(self):
        """
        yb =np.sin(np.arange(0,2*np.pi,np.pi/4)).reshape(-1,1)
        yc =yb*[1,100,200]+[1,2,3]
        ycf = yc.transpose().flatten()
        xb =np.cos(np.arange(0,2*np.pi,np.pi/4)).reshape(-1,1)
        xc =yb*[-1,-100,-200]+[-1,-2,-3]
        xcf = xc.transpose().flatten()
        
        vvv = np.zeros(((8-1)*3,6))
        
        ind=np.arange(0,3*8).reshape(3,8)[:,:-1].flatten()
        
        vvv[:,0]=xcf[ind]
        vvv[:,1]=ycf[ind]
        
        ind2=np.arange(0,3*8).reshape(3,8)[:,1:].flatten()
        vvv[:,3]=xcf[ind2]
        vvv[:,4]=ycf[ind2]
        ...
        n=len(self)
        n_vert = self.get_n_vert()
        #print '_update_vertexvbo ',n,n_vert
        dphi = 2*np.pi/(n_vert-1)
        phi = np.arange(0,2*np.pi+dphi,dphi)
        
        xb =np.cos(phi).reshape(-1,1)
        xc =xb*self.radii.value+self.centers.value[:,0]
        xcf = xc.transpose().flatten()
        
        yb =np.sin(phi).reshape(-1,1)
        yc =yb*self.radii.value+self.centers.value[:,1]
        ycf = yc.transpose().flatten()
        
        self._n_elem = (n_vert-1)*n
        vertices = np.zeros((self._n_elem,6),dtype=np.float32)
        ...
        vertices[:,3]=xcf[ind2]
        vertices[:,4]=ycf[ind2]
        #print '  vertices.reshape((-1,3))',vertices.reshape((-1,3)).shape
    ...
    def _update_colorvbo_new(self):
        n=len(self)
        n_vert = self.get_n_vert()
        #print '_update_colorvbo n,n_vert,self._n_elem',n,n_vert,self._n_elem
        #print '   colors=\n',self.colors.value
        #print '   colors_highl=\n',self.colors_highl.value
        #print '   are_highlighted=\n',self.are_highlighted.value.reshape(n,1)
        #print '    =\n',self.are_highlighted.value.reshape(n,1) * self.colors_highl.value
        
        #self._colorvbo = vbo.VBO(np.clip((self.colors.value+self.colors_highl.value)[np.array(np.arange(0,len(self),1.0/self._n_vert),int)],0.0,1.0))
        
        # OK self._colorvbo = vbo.VBO( np.repeat([self.colors.value[0]],self._n_elem,0)[np.array(np.arange(0,self._n_elem,1.0/self._n_vert),int)])
        colors = np.repeat((self.colors.value+ self.are_highlighted.value.reshape(n,1)*self.colors_highl.value),2*(n_vert-1),0)
        #print '  self.colors.value=\n',self.colors.value,self.colors.value.shape,self.colors.value.dtype
        #print '  colors=\n',colors
        self._colorvbo = vbo.VBO( colors)
    ...
    class OGLeditorMainframe(AgileToolbarFrameMixin, wx.Frame):
        ...
        def on_test(self,event=None):
            ...
            if 1:
                ...
                circles.add_drawobjs([[0.0,0.0,0.0],[1.0,0.5,0.0]],[0.5, 0.8],colors)
