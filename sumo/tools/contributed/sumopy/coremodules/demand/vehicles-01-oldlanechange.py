...
class VehicleTypes(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add_col(am.ArrayConf(  'emissionclasses', "",
                                        dtype = 'object',# 
                                        groupnames = ['parameters'], 
        ...
        #self.add_col(am.ArrayConf(  'lanechangemodels', LANECHANGEMODELS[0],
        #                                dtype = 'object',# 
        #                                groupnames = ['parameters'], 
        #                                choices = LANECHANGEMODELS,
        #                                name = 'Lanechange model',
        #                                info = "Lanechange model",
    ...
    def add_vtypes_default(self):
        ...
        self.add_vtype('pedestrian',  
                        ...
                        times_boarding = 0.0,
                        times_loading = 0.0,
                        sublane_alignments_lat = 'nice',
                        sublane_speeds_max_lat = 1.0,
                        sublane_gaps_min_lat = 0.5,
                        )
        ...
        self.add_vtype('passenger1',  
                        ...
                        times_boarding = 0.5,
                        times_loading = 90.0,
                        sublane_alignments_lat = 'center',
                        sublane_speeds_max_lat = 1.0,
                        sublane_gaps_min_lat = 0.12,
                        )
        ...
        self.add_vtype('bicycle',  
                        ...
                        lanechange_coop = 0.0,
                        lanechange_gain = 0.0,
                        lanechange_rightkeeping = 0.0,
                        times_boarding = 2.0,
                        times_loading = 0.0,
                        sublane_alignments_lat = 'right',
                        sublane_speeds_max_lat = 1.0,
                        sublane_gaps_min_lat = 0.12,
                        sublane_alignments_eager = 0.0,
                        )
        ...
        self.add_vtype('bus',  
                        ...
                        times_loading = 90.0,
                        sublane_alignments_lat = 'center',
                        sublane_speeds_max_lat = 0.5,
                        sublane_gaps_min_lat = 0.24,
                        sublane_alignments_eager = 10.0,
                        )
        ...
        self.add_vtype('bus_flexible',  
                        ...
                        sublane_speeds_max_lat = 0.5,
                        sublane_gaps_min_lat = 0.24,
                        sublane_alignments_eager = 100.0,
                        )
        ...
        self.add_vtype('tram1',  
                        ...
                        sublane_speeds_max_lat = 0.5,
                        sublane_gaps_min_lat = 0.24,
                        sublane_alignments_eager = 1000000.0,
                        )
    ...
    def select_by_mode(self,id_mode, is_sumoid = False):
    ...
    def parse_attrs(self, attrs):
        """
        Parse attributes from an XML handler and set vehicle types accordingly
        """
        #print 'parseAttrs'
        id = attrs['id']
        params = {}
        for attrname in attrs.keys():
            attstr=str(attrname)# necessary as problems with unicode as key
            if (attstr!= 'id') & self.has_attrname(attstr):
                if attrname== 'color':
                    params[attstr] = parseColor(attrs[attstr])
                    
                elif attrname== 'accel':
                    params[attstr] = float(attrs[attstr])
                    ...
                    params[attstr] = float(attrs[attstr])    
                elif attrname== 'minGap':
                    params[attstr] = float(attrs[attstr])    
                elif attrname== 'maxSpeed':
                    params[attstr] = float(attrs[attstr])    
                elif attrname== 'width':
                    params[attstr] = float(attrs[attstr])    
                elif attrname== 'height':
                    params[attstr] = float(attrs[attstr]) 
                elif attrname== 'capacity_persons':
                    params[attstr] = int(attrs[attstr])    
                elif attrname== 'impatience':
                    if attrs[attstr]=='off':
                        params[attstr] = -1000
                    else:
                        params[attstr] = float(attrs[attstr])     
                else:
                    params[attstr] = attrs[attstr]
        
        #print '  params'
        #for key in params.keys():
        #    print ' ',key,type(key)
            
        self.setVtype(id, **params)
    ...
    #def getVtypeClassMap(self):
    #    return self.cols.vClass # this is already a dict
    
    def write_vtypes(self, filehandle, vtypeList=None, is_routefile = False):
        #print 'writeTypes',vtypeList
        
        
        if is_routefile:
            attrs_param = ['maxSpeed','guiShape','emissionClass','color','length','width','height']
        else:
            attrs_param = self.get_group_attrs('param').keys()
        
        #print '  attrs_param',attrs_param
        
        for id in self.get_keys():
            is_write = True
            if vtypeList!=None:
                if id not in vtypeList:
                    is_write = False
            if is_write:
                filehandle.write('<vType id="%s" '%id)
                vType = self.get_row(id)
                #print '  id,vType',id,vType
                for attr, value  in vType.iteritems():
                    if attr in attrs_param:
                        if attr == 'color':
                            filehandle.write('%s = "%.d,%d,%d" '%(attr, value[0], value[1], value[2]))
                        elif attr == 'impatience':
                            if value<=-1000.0:
                                pass
                                #filehandle.write('%s = "off" '%(attr))
                            else:
                                filehandle.write('%s = "%s" '%(attr, value))
                        # not yet implemented :(
                        elif attr == 'capacity_persons':
                            pass
