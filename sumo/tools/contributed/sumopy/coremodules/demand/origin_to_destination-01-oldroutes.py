...
class OdTrips(am.ArrayObjman):
    ...
    def generate_trips(self, demand, time_start, time_end, id_mode, 
                        pos_depart_default = dm.OPTIONMAP_POS_DEPARTURE['random_free'],
                        pos_arrival_default = dm.OPTIONMAP_POS_ARRIVAL['max'],
                        speed_depart_default = 0.0,
                        speed_arrival_default = 0.0,
                        ind_lane_depart_default = dm.OPTIONMAP_LANE_DEPART['allowed'], # pedestrians always depart on lane 0
                        ind_lane_arrival_default = dm.OPTIONMAP_LANE_ARRIVAL['current'],# pedestrians always arrive on lane 0
                        ):
        """
        ...
        for id_od in self.get_ids():
            ...
            for i in xrange(len(ids_edges_dest_raw)):
                ...
                # if check accessibility...    
                ind_lane_arrival = edges.get_laneindex_allowed(id_edge, id_mode)
            ...
            if (n_edges_orig>0) & (n_edges_dest>0) & (tripnumber>0):
                ...
                for d in xrange(int(tripnumber+0.5)):
                    ...
                    i_orig = np.argmax(random.rand(n_edges_orig)*prob_edges_orig)
                    id_edge_orig = ids_edges_orig[i_orig]
                    ind_lane_orig = inds_lane_orig[i_orig]
                    
                    i_dest = np.argmax(random.rand(n_edges_dest)*prob_edges_dest)
                    ...
                    id_trip = trips.make_trip(id_vtype=id_vtype,
                                    ...
                                    positions_arrival = pos_arrival_default,
    ...
    def add_od_trips(self, scale, names_orig, names_dest, tripnumbers):
        ...
        for name_orig, name_dest, tripnumber in zip( names_orig, names_dest, tripnumbers):
            print '  add',name_orig, name_dest, tripnumbers,zones.ids_sumo.has_index(name_orig),zones.ids_sumo.has_index(name_dest)
            if (zones.ids_sumo.has_index(name_orig))&(zones.ids_sumo.has_index(name_dest)):
                self.add_row(   ids_orig = zones.ids_sumo.get_id_from_index(name_orig),
                                ids_dest = zones.ids_sumo.get_id_from_index(name_dest),
...
class OdModes(am.ArrayObjman):
    ...
    def add_od_trips(self,id_mode, scale, names_orig, names_dest, tripnumbers):
        ...
        if len(ids_mode)==0:
            ...
            odtrips = OdTrips( (self.odtrips.attrname, id_od_modes), self, self.zones.get_value() )
            ...
            self.odtrips[id_od_modes].add_od_trips( scale, names_orig, names_dest, tripnumbers)
...
class OdIntervals(am.ArrayObjman):
    ...
    def add_od_trips(self, t_start, t_end, id_mode, scale, 
        ...
        if len(ids_inter)==0:
            ...
            odmodes = OdModes( (self.odmodes.attrname, id_inter), parent = self, modes = self.get_net().modes, zones = self.get_zones())
            ...
            odmodes.add_od_trips(id_mode, scale, names_orig,names_dest,tripnumbers)
            ...
            print '  use',id_inter
            self.odmodes[id_inter].add_od_trips(id_mode, scale, names_orig,names_dest,tripnumbers)
