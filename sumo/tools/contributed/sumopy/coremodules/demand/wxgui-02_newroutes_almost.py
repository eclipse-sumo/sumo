...
#-------------------------------------------------------------------------------
        ...
        menubar.append_item( 'demand/trips/export routes to SUMO xml format...',
...
##        
    ...
    def on_route(self, event=None):
        ...
        Generates trips from origin to destination zone from current OD matrices.
        """
        filepath = self._demand.trips.route(is_export_net = True, is_export_trips = True)
    ...
    def on_import_triproutes_xml(self, event = None):
        ...
        wildcards_all = 'All files (*.*)|*.*|XML trip files (*.trip.xml)|*.trip.xml|XML route files (*.rou.xml)|*.rou.xml'
    ...
    def on_import_routes_alternative_xml(self, event = None):
        ...
        defaultFile = os.path.dirname(filepath)
        dirpath = os.path.dirname(filepath)
        wildcards_all = 'All files (*.*)|*.*|XML route files (*.rou.xml)|*.rou.xml'
        dlg = wx.FileDialog(None, message='Import trips and routes from SUMO xml', 
                            defaultDir=dirpath, 
                            defaultFile=defaultFile, 
    ...
    def on_export_sumotrips(self, event = None):
        ...
        defaultFile = os.path.dirname(filepath)
        dirpath = os.path.dirname(filepath)
        wildcards_all = 'All files (*.*)|*.*|XML trip files (*.trip.xml)|*.trip.xml'
