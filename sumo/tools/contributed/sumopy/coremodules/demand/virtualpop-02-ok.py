...
class IndividualVehicles(am.ArrayObjman):
    ...
    def __init__(self,ident, polulation, vtypes, edges, parking, **kwargs):
        ...
        self.add_col(am.IdsArrayConf( 'ids_vtype', vtypes, 
                                        id_default  = vtypes.ids_sumo.get_id_from_index('passenger1'),
                                        groupnames = ['state'], 
                                        name = 'Veh. type', 
                                        info = 'Vehicle type.',
                                        #xmltag = 'type',
    ...
    def route_between_parking(self, id_veh, id_parking_from, id_parking_to):
        ...
        ids_edge, dist, duration_approx = self.parent.get_route_between_parking(id_parking_from, id_parking_to, vtype)
...
class Rides(StageTypeMixin):
    ...
    def append_stage( self, id_plan, time_start=-1.0,
        ...
        id_stage, time_end = StageTypeMixin.append_stage(self, id_plan,time_start)
        ...
        ind = self.get_ind(id_stage)
        #print '  duration,ids_parking_from,ids_parking_to',duration,self.ids_parking_from[id_stage],self.ids_parking_to[id_stage],ids_edge
        
        if duration==None:
            # this will route and compute duration
            duration = self.parent.get_individualvehicles().route_between_parking(self.ids_veh.value[ind],self.ids_parking_from.value[ind], self.ids_parking_to.value[ind])
        else:
            # case wehn route has been already determined
            self.parent.get_individualvehicles().set_route(self.ids_veh.value[ind],ids_edge,self.ids_parking_from.value[ind], self.ids_parking_to.value[ind])
...
class Activities(StageTypeMixin):
    def __init__(self,ident,population, lanes,name='Activities'):
        self.init_stagetype(ident, population, name = name, info = 'Do some activity  at a position of a lane.')
...
class Virtualpolulation(am.ArrayObjman):
    def __init__(self, demand, **kwargs):
        ...
        self.add(cm.ObjConf(IndividualVehicles('individualvehicles',self, demand.vtypes, scenario.net.edges, scenario.landuse.parking))   ) 
    ...
    def get_route_between_parking(self, id_parking_from, id_parking_to, id_veh=-1):
       ...
       costs, tree = routing.edgedijkstra(id_edge_from,scenario.net.nodes, scenario.net.edges, set([id_edge_to,]))
       dist, route = routing.get_mincostroute_edge2edge(id_edge_from,id_edge_to, costs, tree)
       ...
       duration_approx =   np.sum(edges.lengths[route] /edges.speeds_max[route])                                           
    ...
    def get_route(self,  id_edge_from, id_edge_to,pos_from, pos_to, id_mode = -1):
        ...
        edges = scenario.net.edges
        #lanes = scenario.net.lanes
        costs, tree = routing.edgedijkstra(id_edge_from,scenario.net.nodes, edges, set([id_edge_to,]))
        dist, route = routing.get_mincostroute_edge2edge(id_edge_from,id_edge_to, costs, tree)
    ...
    def make_plans_private(self, ids_person = None):
        ...
        if ids_person == None:
            ...
            ids_person = self.select_ids(self.ids_mode_preferred.value == MODES['private'])
        
        n_plans = len(ids_person)
        print 'make_plans_private n_plans=',n_plans
        ids_plan = self.plans.value.add_rows(n=n_plans,ids_person = ids_person)
        ...
        #print '  ids_vtype',self.parent.vtypes.ids_sumo.get_id_from_index('passenger1') 
        ids_veh = self.individualvehicles.value.add_rows(n = n_plans, 
                        ids_person = ids_person, 
                        ids_vtype = self.parent.vtypes.ids_sumo.get_id_from_index('passenger1')*np.ones(n_plans), 
                        )
        ...
        ids_parking_home, inds_parking_home = parking.assign_parking(ids_veh, centroids_home)
        ids_parking_activity, inds_parking_activity = parking.assign_parking(ids_veh, centroids_activity)
        ...
        ids_lane_parking_activity = parking.ids_lane.value[inds_parking_activity]
        ids_edge_parking_activity = lanes.ids_edge[ids_lane_parking_activity]
        poss_edge_parking_activity = parking.positions.value[inds_parking_activity]
        ...
        for id_person, id_plan, time_start, id_veh, id_edge_home, pos_edge_home, id_edge_parking_home, pos_edge_parking_home, id_parking_home, id_parking_activity, id_edge_parking_activity,pos_edge_parking_activity, id_edge_activity, pos_edge_activity\
          in zip(ids_person, ids_plan, times_start, ids_veh, ids_edge_home, poss_edge_home, ids_edge_parking_home, poss_edge_parking_home, ids_parking_home, ids_parking_activity, ids_edge_parking_activity, poss_edge_parking_activity, ids_edge_activity, poss_edge_activity):
            ...
            ids_edge_car, dists_car, duration_approx_car = self.get_route_between_parking(id_parking_home, id_parking_activity, id_veh)
            print 79*'_'
            ...
            if len(ids_edge_car)>0: # simple check for reachibility of destination
                ...
                # preroute walking
                ids_edge_walk, dists_walk, durations_walk = self.get_route(id_edge_parking_activity, id_edge_activity,pos_edge_home, pos_edge_activity,id_mode = MODES['pedestrian'])
                
                # walk from nearest street at home facility to car parking
                ids_edge_walk, dists_walk, durations_walk = self.get_route(id_edge_home, id_edge_parking_home,pos_edge_home, pos_edge_parking_home,id_mode = MODES['pedestrian'])
    ...
    def _export_plans(self, fd, indent = 0):
        ...
        ivehs = self.individualvehicles.value # instance holding all arrays for individual vehicles
        ...
        times_sorted = np.concatenate((persons.times_start.value.reshape(-1,1),  persons.get_inds().reshape(-1,1) ),1).tolist()
        times_sorted.sort()
        #print '  times_sorted',times_sorted
        inds_pers = np.array(times_sorted,int)[:,1]
        #print '  inds_pers',inds_pers
        
        time_first,ind_first = times_sorted[0]# used to start first vehcile appearance
        ...
        for id_veh, id_vtype, ids_edge, ids_parking in zip( ivehs.get_ids(inds_veh), ivehs.ids_vtype.value[inds_veh], ivehs.routes.value[inds_veh], ivehs.ids_parkings.value[inds_veh] ):
                if (ids_edge != None) & (ids_parking!=None):
                    if len(ids_edge)>0:
                        ...
                        fd.write(xm.num('departLane', lanes.indexes[ parking.ids_lane[ids_parking[0] ] ] ) )
                        fd.write(xm.stopit())
                        ...
                        poss_start_parking = poss_end_parking - parking.lengths[inds_parking]
                        for id_edge_parking,laneindex_parking,pos_start_parking, pos_end_parking in zip(ids_edge_parking,laneindexes_parking,poss_start_parking,poss_end_parking):
                            ...
                            fd.write(xm.num('lane', edges.ids_sumo[id_edge_parking]+'_%d'%laneindex_parking ))
