...
class Sinkzones(am.ArrayObjman):
    def __init__(self,ident, parent, edges, **kwargs):
        self._init_objman(  ident = ident, parent=parent, 
                            name = 'Sink zones', 
                            info = 'Sink zones are zones where trips end.',
                            is_plugin = True,
                            **kwargs)
        
        self.add_col(SumoIdsConf('Sink', name = 'Name', perm = 'rw'))     
        ...
        self.add_col(am.IdlistsArrayConf( 'ids_edges', edges,
                                            groupnames = ['state'], 
                                            name = 'IDs edges', 
                                            info = 'List with edge IDs in this zone. Vehicles entering these edges will be removed.',   
                                            ))
    ...
    def get_edges(self):
        return self.ids_edges.get_linktab()
    ...
    def identify_zoneedges(self,id_zone):
        ...
        self.ids_edges[id_zone] = self.get_edges().get_ids(inds_within)    
...
class Turnflows(am.ArrayObjman):
    ...
    def export_flows_and_turns(self, flowfilepath, turnsfilepath, id_mode, indent=0):
        ...
        ids_sinkedge = set(np.sum(self.sinkzones.get_value().ids_edges))
        ...
        print '  determined sink edges',ids_sinkedge
        if len(ids_sinkedge)>0:   
            f.write(xm.start('sink'))
            f.write(xm.arr('edges',ids_sumoedge[ids_sinkedge]))
            f.write(xm.stopit())
    ...
    def turnflows_to_routes(self, routesindex = None, is_clear_trips = True, is_export_network = False):
        ...
        routefilepath = trips.get_routefilepath(routesindex = routesindex)
        ...
        for id_mode in  ids_mode:
            ...
            if time_end>time_start:# means there exist some flows for this mode
                ...
                if os.path.isfile(routefilepath): 
                    trips.import_sumoxml(routefilepath, routesindex = routesindex)
...
class TurnflowImporter(Process):
    def __init__(self, turnflows, ident_scenario = None, rootdirpath = None, tffilepath = None, 
                    logger = None,**kwargs):
        ...
        if ident_scenario == None:
            ident_scenario = self._net.parent.get_ident()
            
        
        if rootdirpath == None:
            if self._net.parent != None:
                rootdirpath = self._net.parent.workdirpath
            else:
                rootdirpath = os.getcwd()
        
        if tffilepath == None:
            tffilepath =os.path.join(rootdirpath,ident_scenario+'.net.xml')
    ...
    def import_pat_csv(self, sep = ","):
        ...
        for line in f.readlines():
            ...
            if len(cols)>=2:
                ...
                print 'WARNING: inconsistent row in line %d, file %s'%(i_line,filepath)
            i_line +=1
        f.close()
