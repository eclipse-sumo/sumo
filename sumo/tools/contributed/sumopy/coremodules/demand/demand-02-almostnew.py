...
class Trips(am.ArrayObjman):
    ...
    def get_time_depart_first(self):
        if len(self)>0:
            return np.min(self.times_depart.get_value())
        else:
            return 0.0
    
    def get_time_depart_last(self):
        if len(self)>0:
            return np.max(self.times_depart.get_value())
        else:
            return 0.0
    ...
    def export_routes_xml(self, filepath=None, method_routechoice = None, encoding = 'UTF-8'):
        ...
        xmltag_routes, xmltag_rou, attrname_id = ("routes","vehicle","ids_sumo")
        xmltag_trip = "trip"
        ...
        #                        is_print_begin_end = False)
        scalarcolconfigs = [self.ids_vtype,
                            self.times_depart,
        ...
        for id_trip in self.times_depart.get_ids_sorted():
            ...
            if ids_mode[self.ids_vtype[id_trip]] == id_pedestrian:
                ...
                if id_route>=0:
                    ...
                    fd.write(xm.start(xmltag_rou,indent+2))
                else:
    ...
    def import_routes_xml(self, filepath,  is_clear_trips = False, 
        ...
        reader = RouteReader(self, counter.n_trip)
        try:
