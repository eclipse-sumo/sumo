...
class Crossings(am.ArrayObjman):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.add_col(am.IdlistsArrayConf( 'ids_edges', parent.edges,
                                            groupnames = ['state'], 
                                            name = 'IDs Edges', 
                                            info = 'List of edge IDs where street crossing is possible.',   
                                            ))
...
class Network(cm.BaseObjman):
        ...
        def import_xml(self, rootname, dirname, is_clean_nodes = True):
...
class SumoConnectionReader(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'crossing':
            ...
            self._net.add_crossing(     id_node = self._ids_node_osm.get_id_from_index(attrs['node']),
                                        ids_edge = self._ids_edge_osm.get_ids_from_indices(attrs['edges'].split(' ')),
