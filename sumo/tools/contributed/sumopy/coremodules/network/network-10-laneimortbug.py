...
class Network(cm.BaseObjman):
        ...
        def call_netedit(self, filepath = None):
        ...
        def import_netxml(self, filepath=None, rootname=None, is_clean_nodes = False, is_remove_xmlfiles= False):
            ...
            self.clear()
            if rootname == None:
                rootname = self.parent.get_ident()
        ...
        def import_xml(self, rootname=None, dirname = None, is_clean_nodes = False, is_remove_xmlfiles= False):
            ...
            tlsfilepath = os.path.join(dirname,rootname+'.tls.xml')
            if os.path.isfile(edgefilepath) & os.path.isfile(nodefilepath)& os.path.isfile(confilepath):
                self.import_sumonodes(nodefilepath, is_remove_xmlfiles)
                self.import_sumoedges(edgefilepath, is_remove_xmlfiles)
                if is_clean_nodes:
...
class SumoNodeReader(handler.ContentHandler):
    ...
    def __init__(self, net, counter):
        ...
        self.ids_tl_prog = np.zeros(counter.n_node,np.int32)
    ...
    def write_to_net(self):
        ...
        #print 'write_to_net'
        ids_node = self._net.add_nodes(
                        ids_sumo = self.ids_sumo,
                        types = self.types,
                        coords = self.coords,
                        types_tl = self.types_tl,
                        #ids_tl_prog = nself.ids_tl_prog,
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'nodes':
            ...
            if attrs['id'][0]!=':':# no internal node
                ...
                self.types[i] = self._nodetypemap[str(attrs.get('type','priority'))]
                self.coords[i] = [ float(attrs['x'])-x0, float(attrs['y'])-y0, float(attrs.get('z',0.0)) ]
                self.types_tl[i] = self._tltypemap[attrs.get('tlType','none')]
                #??self.ids_tl_prog[i] = np.zeros(counter.n_edge,np.int32)
...
class SumoEdgeCounter(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'edge': 
            ...
            self.n_lane += 1
            self._n_edgelane += 1
        elif name == 'roundabout': self.n_roundabout  += 1
    ...
    def endElement(self, name):
        ...
        if name == 'edge':
            ...
            if self._n_edgelane ==0:
                # a lane must be created, even if no lane info are provided
                self.n_lane += 1
...
class SumoEdgeReader(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'edge':
            ...
            self.widths[ind] = float(attrs.get('width',3.5))
            ...
            if len(shape) < 2: # insufficient shape data
                ...
                # ATTENTIOn: we need to copy here, otherwise the reference 
                # to node coordinates will be kept!!
                shape = [ 1.0*self._nodecoords[id_fromnode], 1.0*self._nodecoords[id_tonode] ]
            
            self.shapes[ind] = shape
            
            
            self.types_spread[ind] = self.spread_choices[str(attrs.get('spreadType','right'))]
            ...
            if attrs.has_key('allow'):
                modes_allow = list(self._modenames.get_ids_from_indices(attrs['allow'].split(' ')))
            else:
                edgetype = self.types_edge[self._ind_edge] 
                
                if OSMEDGETYPE_TO_MODES.has_key(edgetype):
                    modes_allow, speed_max_default = OSMEDGETYPE_TO_MODES[edgetype]
                else:
                    modes_allow = []
                
            if attrs.has_key('disallow'):   
                modes_disallow  = list(self._modenames.get_ids_from_indices(attrs['disallow'].split(' ')))
            else:
                 modes_disallow  = []
            ...
            if index == 0:
                width_sidewalk_edge = self.widths_sidewalk[self._ind_edge]# copy from edge
                is_sidewalk_edge = width_sidewalk_edge>0
                is_sidewalk = (MODES['pedestrian'] in modes_allow) # test for pedestrian sidewalk
            ...
            if speed_max<0:
                if (index == 0)&is_sidewalk:
                    ...
                    speed_max = self.speeds_max[self._ind_edge]# copy from edge
                        
            #print ' is_sidewalk_edge ,is_sidewalk',is_sidewalk_edge ,is_sidewalk
            if width < 0:
                width = self.widths_lanes_default[self._ind_edge]# copy from edge
                ...
                if index == 0:
                    if is_sidewalk_edge: # edge wants sidewalks
                        ...
                        width = 0.9# default sidewalk width
                        self.widths_sidewalk[self._ind_edge] = width # update edge attr!!
            ...
            self.ids_nodes_ra[self._ind_ra] = self._ids_node_sumo.get_ids_from_indices(attrs.get('nodes','').split(' '))
    ...
    def endElement(self, name):
        ...
        if name == 'edge':
            ...
            if len(self._ind_lanes_edges) ==0:
                # edge description provided no lane information
                # create one lane and us some properties from current edge
                self._ind_lane +=1
                ind = self._ind_lane
                ...
                self.width_lanes[ind] = self.widths[self._ind_edge] # copy from edge attr
                self.speed_max_lanes[ind] = self.speeds_max[self._ind_edge] # copy from edge attr
                self.offset_end_lanes[ind] = self.offset_end_lanes[self._ind_edge]# copy from edge attr
