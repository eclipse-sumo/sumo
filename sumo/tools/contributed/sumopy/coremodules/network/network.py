...
class Network(cm.BaseObjman):
        ...
        def call_sumogui(self, filepath = None,is_maps = False):
            ...
            if os.path.isfile(polyfilepath):
                option_addfiles = '  --additional-files '+filepathlist_to_filepathstring(polyfilepath)
            else:
                option_addfiles = ''
    ...
    def do(self):
        ...
        if self.status == 'success':
            ...
            if os.path.isfile(self.netfilepath):
                ...
                cml = self._command+ ' --sumo-net-file '+ filepathlist_to_filepathstring(self.netfilepath) +' --plain-output-prefix '+filepathlist_to_filepathstring( os.path.join(self.workdirpath, self.rootname) )
                self.run_cml(cml)
                if self.status == 'success':
