...
class Facilities(am.ArrayObjman):
    ...
    def get_net(self):
        ...
        return self.ids_edge_closest_road.get_linktab().parent
        #if self.parent != None:
...
class Parking(am.ArrayObjman):
    ...
    def get_parkinglane_from_edge_old(self, id_edge, id_mode, length_min=15.0, priority_max=8, n_lanes_min = 2):
        """
        Check if edge can have on-road parking
        """
        edges = self.edges.get_value()
        ...
        # check size
        #laneindex = 
        print 'check prio,l_min,nlanes,nlanesOK,mode',id_edge,edges.priorities[id_edge]<=priority_max,edges.lengths[id_edge]>length_min,len(edges.ids_lanes[id_edge]),len(edges.ids_lanes[id_edge])>=n_lanes_min,lanes.modes_allow[edges.ids_lanes[id_edge][0]]
        if  (edges.priorities[id_edge]<=priority_max)&(edges.lengths[id_edge]>length_min):
            ids_lane = edges.ids_lanes[id_edge]
            n_lanes = len(ids_lane)
            #lanes = self.lanes.get_value()
            
            if  MODES["pedestrian"] in lanes.modes_allow[ids_lane[0]]:
                if len(ids_lane)>n_lanes_min:
                    #print '   check footpath',(id_mode in lanes.modes_allow[ids_lane[1]]),(id_mode not in lanes.modes_disallow[ids_lane[1]])
                    if (id_mode in lanes.modes_allow[ids_lane[1]])|(id_mode not in lanes.modes_disallow[ids_lane[1]]):
                        return ids_lane[1]
                    
            else:
                if len(ids_lane) >= n_lanes_min:
                    # this checks only lane 0 for accessibility
                    #print '   no footpath',(id_mode in lanes.modes_allow[ids_lane[1]]),(id_mode not in lanes.modes_disallow[ids_lane[1]])
                    if (id_mode in lanes.modes_allow[ids_lane[0]]) | (id_mode not in lanes.modes_disallow[ids_lane[0]]):
                        return ids_lane[0]
            ...
            if id_lane >= 0:
                ...
                for i in xrange(n_spaces):
                    ...
                    id_park = self.add_row( ids_lane = id_lane, 
                                            ...
                                            lengths = length_lot,
                                            angles = angle,
                                            vertices = get_vec_on_polyline_from_pos(lanes.shapes[id_lane],pos, length_lot, angle = angle)
                                           )
