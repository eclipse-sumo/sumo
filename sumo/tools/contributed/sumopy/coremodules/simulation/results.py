...
class Tripresults(am.ArrayObjman):
    def __init__(self, parent, trips, edges, is_add_default=True, **kwargs):
        ...
        attrinfos = OrderedDict([\
                ...
                ('depart',   {'name':'Dep. time', 'xmltag':'depart',   'unit':'s',     'default':0, 'info':'Departure time','groupnames':['tripdata']}),
                ('arrival',   {'name':'Arr. time', 'xmltag':'arrival',   'unit':'s',    'default':0, 'info':'Departure time','groupnames':['tripdata']}),
                ('departPos',   {'name':'depart pos', 'xmltag':'departPos',   'unit':'m',    'default':0.0, 'info':'depart position','groupnames':['tripdata']}),
                ('arrivalPos',   {'name':'arrival pos','xmltag':'arrivalPos',    'unit':'m',    'default':0.0, 'info':'arrival position','groupnames':['tripdata']}),
        ...
        if name == self._element:
            ...
            for attrsconfig in self._attrsconfigs_average:
                ...
                if attrs.has_key(xmltag):
                    ...
                    if a.strip()!='':
                        if self._values[attrname].has_key(id):
                            self._values[attrname][id] = ( (n-1)*self._values[attrname][id] + float(a))/n
...
def read_interval2(filepath, element, attrsconfigs):
    ...
    reader = IntervalAvReader2(element, attrsconfigs_cumulative, attrsconfigs_average)
    #parser = make_parser()
