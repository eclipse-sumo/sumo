...
def write_netconfig(filename_netconfig, filename_net, 
    ...
    if (filepath_output_edgedata!= None)\
        ...
        filename_add = string.join(filename_netconfig.split('.')[:-2]+['add.xml'],'.')
...
<input>\n""")
    ...
    filenames_add = set([filename_poly,filename_add,filename_add, filename_ptstops])
...
class Sumo(CmlMixin, Process):
    def __init__(   self, scenario, 
        ...
        if is_export_rou:
            routefilepaths = scenario.demand.trips.export_routes_xml(method_routechoice = method_routechoice)
            if routefilepaths is not False:
                # export OK, network is not an option
                groupnames = ['input','_private']
            else:
                # something went wrong with exporting
                routefilepaths = kwargs.get('routefilepaths',scenario.demand.trips.get_routefilepath())
                groupnames = ['input','options']
