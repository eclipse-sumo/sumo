...
os.system("incrementalAssignment.py -c ../input/districts.xml -m %s -n %s -p ../parameter.txt -u ../CRcurve.txt" % (mtxNamesList, netFile))
clogDir = makeAndChangeDir("../clogit")
os.system("cLogit.py -c ../input/districts.xml -m %s -n %s -p ../clogit_parameter.txt -u ../CRcurve.txt" % (mtxNamesList, netFile))
...
os.system("networkStatistics.py -n %s -c ../input/districts.xml -x vehroutes_dua.xml -o networkStatistics_%s.txt" % (netFile, os.path.basename(duaDir)))
for step in [-1, 3600, 1800, 900, 300, 150, 90, 60, 30, 15]:
    shutil.copy("%s/vehroutes_%s.xml" % (shotDir, step), "vehroutes_oneshot%s.xml" % step)
    os.system("networkStatistics.py -n %s -c ../input/districts.xml -x vehroutes_oneshot%s.xml -o networkStatistics_%s_%s.txt" % (netFile, step, os.path.basename(shotDir), step))
...
os.system("networkStatistics.py -n %s -c ../input/districts.xml -x vehroutes_successive.xml -o networkStatistics_%s.txt" % (netFile, os.path.basename(succDir)))
os.system("sumo --no-step-log -n %s -e 90000 -r %s/routes.txt --vehroute-output vehroutes_clogit.xml %s" % (netFile, clogDir, sumoAdds))
os.system("networkStatistics.py -n %s -c ../input/districts.xml -x vehroutes_clogit.xml -o networkStatistics_%s.txt" % (netFile, os.path.basename(clogDir)))
os.chdir("..")
