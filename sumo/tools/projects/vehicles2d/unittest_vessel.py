...
class testCaseVessel(unittest.TestCase):
    ...
    def test_get_grey_and_black_shade_at_zero_position(self):
        ...
        for ii in range(len(result_grey)):
            result_grey[ii] = (result_grey[ii][0] +10, result_grey[ii][1] +10)

        for ii in range(len(result_black)):
            result_black[ii] = (result_black[ii][0] +10, result_black[ii][1] +10)
    ...
    def test_get_grey_and_black_shade_small_vessel_sector_0(self):
        ...
        sector_to_inspect  = 0 
        vessel_inclination = myFlaeche.get_angle_from_sector_id(sector_to_inspect)
        ...
        result_grey           = sorted(vessel.get_grey_shade(vessel_inclination, (105, 105)))
        ...
        result_black          = sorted(vessel.get_black_shade(vessel_inclination, (105, 105)))
...
#           myFlaeche.vis_add_grey(result_grey)
    ...
    def test_get_predefiend_shadow_shape_negatives(self):
        ...
        self.assertEqual(sorted(left_sector_shade_expected),sorted(left_sector_shade_result))
        ...
        self.assertEqual(sorted(right_sector_shade_expected),sorted(right_sector_shade_result))
        ...
        if visual:
            ...
            myFlaeche.output = self.__dict__['_testMethodName'] + '_003_inspect'
            vessel.transform_hull_points(myFlaeche.get_angle_from_sector_id(0),
                                     (105, 105) )
            myFlaeche.vis_reset()
    ...
    def test_get_radius_to_point_ego(self):
        ...
        * for points the vehicle must turn 180 degrees for
        ...
        result_right_radius , result_right_angle = vessel.get_radius_to_point_ego((xx_r, yy_r))

        expected_result_left_radius              = R
        expected_result_left_angle               = - gamma_majuscle
        result_left_radius , result_left_angle   = vessel.get_radius_to_point_ego((xx_l, yy_l))

        self.assertAlmostEqual(result_right_radius, expected_result_right_radius )
        self.assertAlmostEqual(result_right_angle,  expected_result_right_angle  )
        ...
        yy_r  =  40
        self.assertAlmostEqual(vessel.get_radius_to_point_ego((xx_r, yy_r))[0], R)
        self.assertAlmostEqual(vessel.get_radius_to_point_ego((xx_r, yy_r))[1], AA_r)

        AA_l  = -math.pi
        xx_l  =  0 
        yy_l  = -40
        self.assertAlmostEqual(vessel.get_radius_to_point_ego((xx_l, yy_l))[0], R)
        self.assertAlmostEqual(vessel.get_radius_to_point_ego((xx_l, yy_l))[1], AA_l)
        ...
        yy_l  =  -40
        self.assertAlmostEqual(vessel.get_radius_to_point_ego((xx_l, yy_l))[0], R)
        self.assertAlmostEqual(vessel.get_radius_to_point_ego((xx_l, yy_l))[1], AA)
    ...
    def test_get_reachables_old_style_pp(self):
        ...
        myFlaeche = main.Flaeche(xdim=100,ydim=100,scale=20,output='reachables_pp')
        vessel = main.Vessel(myFlaeche,
                             [(0, 0), (4, 4), (4, 20), (-4, 20), (-4, 4), ( 0, 0)])

        vessel.x = 10; vessel.y = 10; vessel.rotation = .25*math.pi; vessel.r = 400
...
#        vessel.x = 300; vessel.y = 305; vessel.rotation = 0; vessel.r = 20
        ...
        expected_result_pos = [ ( 5.0, 25.0, 0), (10.0, 25.0, 0), (15.0, 25.0, 0),
                                (20.0, 25.0, 0), (25.0, 25.0, 0), (30.0, 25.0, 0),
                                (35.0, 25.0, 0), (40.0, 25.0, 0), (45.0, 25.0, 0),
        ...
        if visual:
            for pp in result_pos:
                myFlaeche.vis_add_single_point((pp[0], pp[1] + 10), 'darkseagreen')
...
#        vessel.x = 300; vessel.y = 305; vessel.rotation = 0; vessel.r = 20
        ...
        expected_result_pos = [(0.0, 0.0, 0.0),
                               (1.0452846326765346, 0.0547810463172671, 0.10471975511965977),
                               (2.079116908177593, 0.2185239926619431, 0.20943951023931953),
                               (3.090169943749474, 0.4894348370484647, 0.3141592653589793),
                               (4.0673664307580015, 0.8645454235739913, 0.41887902047863906),
                               (4.999999999999999, 1.339745962155613, 0.5235987755982988),
                               (5.877852522924732, 1.9098300562505255, 0.6283185307179586),
                               (6.691306063588582, 2.5685517452260576, 0.7330382858376184),
                               (7.431448254773942, 3.3086939364114176, 0.8377580409572782),
                               (8.090169943749475, 4.12214747707527, 0.942477796076938),
                               (8.660254037844387, 5.000000000000001, 1.0471975511965979),
                               (9.135454576426008, 5.9326335692419985, 1.1519173063162575),
                               (9.510565162951535, 6.9098300562505255, 1.2566370614359172),
                               (9.781476007338055, 7.920883091822406, 1.361356816555577),
                               (9.945218953682733, 8.954715367323464, 1.4660765716752366),
        ...
        expected_result_neg = [(0.0, 0.0, 0.0),
                               (1.0452846326765373, -0.0547810463172671, -0.10471975511965977),
                               (2.079116908177593, -0.2185239926619431, -0.20943951023931953),
                               (3.090169943749475, -0.4894348370484647, -0.3141592653589793),
                               (4.067366430758004, -0.8645454235739913, -0.41887902047863906),
                               (4.999999999999999, -1.339745962155613, -0.5235987755982988),
                               (5.877852522924733, -1.9098300562505255, -0.6283185307179586),
                               (6.691306063588583, -2.5685517452260576, -0.7330382858376184),
                               (7.431448254773945, -3.3086939364114176, -0.8377580409572782),
                               (8.090169943749475, -4.12214747707527, -0.942477796076938),
                               (8.660254037844387, -5.000000000000001, -1.0471975511965979),
                               (9.13545457642601, -5.9326335692419985, -1.1519173063162575),
                               (9.510565162951536, -6.9098300562505255, -1.2566370614359172),
                               (9.781476007338057, -7.920883091822406, -1.361356816555577),
                               (9.945218953682733, -8.954715367323464, -1.4660765716752366),
                               (10.0, -9.999999999999996, -1.5707963267948963)]
...
#        vessel.x = 300; vessel.y = 305; vessel.rotation = 0; vessel.r = 20
        ...
        expected_result_pos = [(0.0, 0.0, 0.0),
                               (4.999999999999999, 1.339745962155613, 0.5235987755982988),
                               (8.660254037844386, 4.999999999999999, 1.0471975511965976),
                               (10.0, 9.999999999999998, 1.5707963267948966),
                               (8.660254037844387, 14.999999999999998, 2.0943951023931953),
                               (5.0000000000000036, 18.660254037844382, 2.617993877991494),
                               (5.665538897647979e-15, 20.0, 3.1415926535897927),
                               (-4.999999999999994, 18.66025403784439, 3.6651914291880914),
                               (-8.660254037844384, 15.000000000000004, 4.1887902047863905),
                               (-10.0, 10.000000000000002, 4.71238898038469),
                               (-8.660254037844386, 4.999999999999999, 5.235987755982989),
        ...
        expected_result_neg = [(0.0, 0.0, 0.0),
                               (3.090169943749475, -0.4894348370484647, -0.3141592653589793),
                               (5.877852522924733, -1.9098300562505255, -0.6283185307179586),
                               (8.090169943749475, -4.122147477075268, -0.9424777960769379),
                               (9.510565162951536, -6.9098300562505255, -1.2566370614359172),
                               (10.0, -9.999999999999998, -1.5707963267948966),
                               (9.510565162951535, -13.090169943749475, -1.8849555921538759),
                               (8.090169943749475, -15.87785252292473, -2.199114857512855),
                               (5.877852522924732, -18.090169943749473, -2.5132741228718345),
                               (3.090169943749474, -19.510565162951536, -2.827433388230814),
                               (0.0, -20.0, -3.141592653589793),
                               (-3.090169943749474, -19.510565162951536, -3.4557519189487724),
                               (-5.877852522924732, -18.090169943749473, -3.7699111843077517),
                               (-8.090169943749475, -15.877852522924734, -4.084070449666731),
                               (-9.510565162951535, -13.090169943749475, -4.39822971502571),
                               (-10.0, -10.000000000000002, -4.71238898038469),
                               (-9.510565162951536, -6.909830056250527, -5.026548245743669),
                               (-8.090169943749475, -4.122147477075271, -5.340707511102648),
                               (-5.877852522924733, -1.9098300562505266, -5.654866776461628),
        ...
        if visual:
            for pp in result_pos:
                myFlaeche.vis_add_single_point((pp[0] + 30 , pp[1] + 30), 'darkseagreen')
