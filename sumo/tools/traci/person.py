...
def _getUniversal(varID, personID):
    result = traci._sendReadOneStringCmd(tc.CMD_GET_PERSON_VARIABLE, varID, personID)
...
def subscribe(personID, varIDs=(tc.VAR_ROAD_ID, tc.VAR_LANEPOSITION), begin=0, end=2**31-1):
    ...
    """
    traci._subscribe(tc.CMD_SUBSCRIBE_PERSON_VARIABLE, begin, end, personID, varIDs)
...
def subscribeContext(personID, domain, dist, varIDs=(tc.VAR_ROAD_ID, tc.VAR_LANEPOSITION), begin=0, end=2**31-1):
    traci._subscribeContext(tc.CMD_SUBSCRIBE_PERSON_CONTEXT, begin, end, personID, domain, dist, varIDs)
