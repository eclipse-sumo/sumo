    ...
    for f1 in range(self.getInt("f1from"), self.getInt("f1to"), self.getInt("f1step")):
        for f2 in range(self.getInt("f2from"), self.getInt("f2to"), self.getInt("f2step")):
            ...
            if fileNeedsRebuild(s.demandName, "duarouter"):
              ...
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f1, "2/1_to_1/1", "1/1_to_0/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f1, "0/1_to_1/1", "1/1_to_2/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f2, "1/2_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f2, "1/0_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
...
class ScenarioSet_RiLSA1LoadCurvesSampled(ScenarioSet):
  ...
  def runSingle(self, RWScurves, iWE, iNS, iEW, iSN, uID=None):
    ...
    if True:#fileNeedsRebuild(s.demandName, "duarouter"):
      ...
      desc = {"scenario":"RiLSA1LoadCurvesSampled", "iWE":str(iWE), "iNS":str(iNS), "iEW":str(iEW), "iSN":str(iSN)}
      return s, desc, sID
  def getRunsMatrix(self):
    ret = []
    ranges = [[], []]
...
class ScenarioSet_BasicOutflow(ScenarioSet):
  ...
  def getNumRuns(self):
    ...
    g2num = 1 + (self.getInt("g2to") - self.getInt("g2from")) / self.getInt("g2step")
    return g1num * g2num
  def iterateScenarios(self):
    ...
    for f1 in range(self.getInt("g1from"), self.getInt("g1to"), self.getInt("g1step")):
        for f2 in range(self.getInt("g2from"), self.getInt("g2to"), self.getInt("g2step")):
            ...
            if fileNeedsRebuild(s.demandName, "duarouter"):
              ...
              s.demand._f1Value = f1
              s.demand._f2Value = f2
            desc = {"scenario":"BasicOutflow", "g1":str(g1), "g2":str(g2)}
            yield s, desc, sID
  def getRunsMatrix(self):
    ret = []
    ranges = [[], []]
    for f1 in range(self.getInt("g1from"), self.getInt("g1to"), self.getInt("g1step")):
      ...
      ranges[0].append(g1)
      for f2 in range(self.getInt("g2from"), self.getInt("g2to"), self.getInt("g2step")):
        ret[-1].append({"scenario":"BasicOutflow", "g1":str(g1), "g2":str(g2)})
        ranges[1].append(g2)
    return (ret, ranges)
  def getAverageDuration(self):
    return -1 # !!!
...
class ScenarioSet_RiLSA1Outflow(ScenarioSet_RiLSA1LoadCurvesSampled):
  ...
  def getNumRuns(self):
    ...
    f2num = 1 + (self.getInt("g2to") - self.getInt("g2from")) / self.getInt("g2step")
    return f1num * f2num
  """
  Yields returning a built scenario and its description as key/value pairs
  """
  ...
  def getRunsMatrix(self):
    ...
    for g1 in range(self.getInt("g1from"), self.getInt("g1to"), self.getInt("g1step")):
        ...
        ranges[0].append(g1)
        for g2 in range(self.getInt("g2from"), self.getInt("g2to"), self.getInt("g2step")):
            ret[-1].append({"g1":str(g1), "g2":str(g2), "scenario":"RiLSA1Outflow"})
            ranges[-1].append(g2)
    return (ret, ranges)
  def getAverageDuration(self):
    return -1 # !!!        
...
class ScenarioSet_RiLSA1PedFlow(ScenarioSet_RiLSA1LoadCurvesSampled):
  ...
  def getRunsMatrix(self):
    ...
    for f1 in range(self.getInt("f1from"), self.getInt("f1to"), self.getInt("f1step")):
        ...
        ranges[0].append(f1)
        for f2 in range(self.getInt("f2from"), self.getInt("f2to"), self.getInt("f2step")):
            ret[-1].append({"f1":str(f1), "f2":str(f2), "scenario":"RiLSA1PedFlow"})
            ranges[-1].append(f2)
    return (ret, ranges)
  def getAverageDuration(self):
    return -1 # !!!        
...
class ScenarioSet_RiLSA1PTIteration(ScenarioSet_RiLSA1LoadCurvesSampled):
  ...
  def getNumRuns(self):
    ...
    p2num = 1 + (self.getInt("p2to") - self.getInt("p2from")) / self.getInt("p2step")
    return p1num * p2num
  """
  Yields returning a built scenario and its description as key/value pairs
  """
...
class ScenarioSet_OneSinDemand(ScenarioSet):
  ...
  def __init__(self, params):
    ScenarioSet.__init__(self, "OneSinDemand", merge(
      {"amplFrom":"0", "amplTo":"1201", "amplStep":"200","freqFrom":"0", "freqTo":"11", "freqStep":"1"},
      params))
    self.amplitudes = []
  ...
  def genDemand(self, scenario, simSteps, amplitude, frequency):  
    ...
    for i in range(simSteps):
        ...
        if random.uniform(0,1) < pv1:
            ...
            vehNr += 1
            lastVeh = i	
        if frequency!=0: o1 = o1 + ((math.pi*2)/(180*frequency))
  ...
  def iterateScenarios(self):
    ...
    for amplitude in self.amplitudes:
        for freq in self.frequencies:
            ...
            desc = {"scenario":"OneSinDemand", "amplitude":str(amplitude), "frequency":str(freq)}
            yield s, desc, sID
  def getRunsMatrix(self):
    ret = []
    ranges = [[], []]
    for amplitude in self.amplitudes:
      ...
      for freq in self.frequencies:
        ret[-1].append({"scenario":"OneSinDemand", "amplitude":str(amplitude), "frequency":str(freq)})
        ranges[1].append(freq)
    return (ret, ranges)
  def getAverageDuration(self):
    return -1 # !!!
        ...
        for f2end in range(self.getInt("f2endFrom"), self.getInt("f2endTo"), self.getInt("f2endStep")):
          for f2duration in range(self.getInt("f2durationFrom"), self.getInt("f2durationTo"), self.getInt("f2durationStep")):        
            ...
            if True:#fileNeedsRebuild(s.demandName, "duarouter"):
              hd = f2duration/2
              s.demand = demandGenerator.Demand()
              s.demand.addStream(demandGenerator.Stream(None, 0, 7200, f1, "2/1_to_1/1", "1/1_to_0/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 7200, f1, "0/1_to_1/1", "1/1_to_2/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              # begin              
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600-hd, f2begin, "1/2_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600-hd, f2begin, "1/0_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              # between
              for t in range(0, f2duration, 300):
                fat = (f2end-f2begin) / f2duration * (t+150)
                s.demand.addStream(demandGenerator.Stream(None, 3600-hd+t, 3600-hd+t+300, fat, "1/2_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
                s.demand.addStream(demandGenerator.Stream(None, 3600-hd+t, 3600-hd+t+300, fat, "1/0_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              # end              
              s.demand.addStream(demandGenerator.Stream(None, 3600+hd, 7200, f2end, "1/2_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 3600+hd, 7200, f2end, "1/0_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
    ...
    for f1 in range(self.getInt("f1from"), self.getInt("f1to"), self.getInt("f1step")):
      for f2 in range(self.getInt("f2from"), self.getInt("f2to"), self.getInt("f2step")):
        for d1 in range(self.getInt("d1from"), self.getInt("d1to"), self.getInt("d1step")):
            ...
            if fileNeedsRebuild(s.demandName, "duarouter"):
              s.demand = demandGenerator.Demand()
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f1, "6/1_to_5/1", "1/1_to_0/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f1, "0/1_to_1/1", "5/1_to_6/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              for i in range(1, 6):
                s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f2, "%s/2_to_%s/1" % (i,i), "%s/1_to_%s/0" % (i,i), { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
                s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f2, "%s/0_to_%s/1" % (i,i), "%s/1_to_%s/2" % (i,i), { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.build(0, 3600, s.netName, s.demandName)
              # !!! the following two lines are a hack to pass the numbers instead of recomputing them 
              s.demand._f1Value = f1
              s.demand._f2Value = f2
      ...
      for f2 in range(self.getInt("f2from"), self.getInt("f2to"), self.getInt("f2step")):
        for o in self.offsets:
            ...
            if fileNeedsRebuild(s.demandName, "duarouter"):
              s.demand = demandGenerator.Demand()
              for i in range(1, 6):
                s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f1, "6/%s_to_5/%s" % (i,i), "1/%s_to_0/%s" % (i,i), { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
                s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f1, "0/%s_to_1/%s" % (i,i), "5/%s_to_6/%s" % (i,i), { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              for i in range(1, 6):
                s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f2, "%s/6_to_%s/5" % (i,i), "%s/1_to_%s/0" % (i,i), { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
                s.demand.addStream(demandGenerator.Stream(None, 0, 3600, f2, "%s/0_to_%s/1" % (i,i), "%s/5_to_%s/6" % (i,i), { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.build(0, 3600, s.netName, s.demandName)
              # !!! the following two lines are a hack to pass the numbers instead of recomputing them 
              s.demand._f1Value = f1
              s.demand._f2Value = f2
...
class ScenarioSet_TurnIteration(ScenarioSet):
  ...
  def __init__(self, params):
    ScenarioSet.__init__(self, "TurnIteration", merge(
      {"rightFrom":"0", "rightTo":"51", "rightStep":"10","leftFrom":"0", "leftTo":"51", "leftStep":"10"},
      params))
  def getNumRuns(self):
    f1num = 1 + (self.getInt("rightFrom") - self.getInt("rightTo")) / self.getInt("rightStep")
    f2num = 1 + (self.getInt("leftFrom") - self.getInt("leftTo")) / self.getInt("leftStep")
    return f1num * f2num
    ...
    for r in range(self.getInt("rightFrom"), self.getInt("rightTo"), self.getInt("rightStep")):
        for l in range(self.getInt("leftFrom"), self.getInt("leftTo"), self.getInt("leftStep")):
            ...
            if fileNeedsRebuild(s.demandName, "duarouter"):
              ...
              aL = float(l)/100. 
              aR = float(r)/100. 
              if aR!=0: s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW*aR, "2/1_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              if aL!=0: s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW*aL, "2/1_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              if (1.-aR-aL)!=0: s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW*(1.-aR-aL), "2/1_to_1/1", "1/1_to_0/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW, "0/1_to_1/1", "1/1_to_2/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW, "1/2_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW, "1/0_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              s.demand.build(0, 3600, s.netName, s.demandName)
  ...
  def getRunsMatrix(self):
    ...
    for r in range(self.getInt("rightFrom"), self.getInt("rightTo"), self.getInt("rightStep")):
      ...
      ranges[0].append(r)
      for l in range(self.getInt("leftFrom"), self.getInt("leftTo"), self.getInt("leftStep")):
        ret[-1].append({"scenario":"TurnIteration", "r":str(r), "l":str(l)})
        ranges[1].append(l)
    return (ret, ranges)
  def getAverageDuration(self):
    return -1 # !!!
    ...
    for r in range(self.getInt("rightFrom"), self.getInt("rightTo"), self.getInt("rightStep")):
        for l in range(self.getInt("leftFrom"), self.getInt("leftTo"), self.getInt("leftStep")):
            ...
            if fileNeedsRebuild(s.demandName, "duarouter"):
              ...
              if aR!=0: s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW*aR, "0/1_to_1/1", "1/1_to_1/0", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              if aL!=0: s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW*aL, "0/1_to_1/1", "1/1_to_1/2", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
              if (1.-aR-aL)!=0: s.demand.addStream(demandGenerator.Stream(None, 0, 3600, self.MAIN_FLOW*(1.-aR-aL), "0/1_to_1/1", "1/1_to_2/1", { "passenger":1})) # why isn't it possible to get a network and return all possible routes or whatever - to ease the process
...
class ScenarioSet_RealWorld(ScenarioSet):
  def __init__(self, params):
    ScenarioSet.__init__(self, "RealWorld", merge(
      {"scaleFrom":"50", "scaleTo":"151", "scaleStep":"25", "which":"unknown"},
      params))
    print params
    print self.params
  def getNumRuns(self):
    f1num = 1 + abs(self.getInt("scaleFrom") - self.getInt("scaleTo")) / self.getInt("scaleStep")
    return f1num
  ...
  def iterateScenarios(self):
    ...
    for scale in range(self.getInt("scaleFrom"), self.getInt("scaleTo"), self.getInt("scaleStep")):
        ...
        #s.demandName = s.fullPath("routes_%s.rou.xml" % sID)
        desc = {"scenario":"RealWorld", "which":which, "scale":str(scale)}
        yield s, desc, sID
  def getRunsMatrix(self):
    ret = []
    ranges = [[], []]
    ...
    for s in range(self.getInt("scaleFrom"), self.getInt("scaleTo"), self.getInt("scaleStep")):
        ret[-1].append({"scenario":"RealWorld", "which":self.params["which"], "scale":str(s)})
        ranges[1].append(s)
    return (ret, ranges)
  def getAverageDuration(self):
    return -1 # !!!
  ...
  def getRunsMatrix(self):
    ...
    for iWE,cWE in enumerate(RWScurves):
      for iNS,cNS in enumerate(RWScurves):
        ...
        for iEW,cEW in enumerate(RWScurves):
          for iSN,cSN in enumerate(RWScurves):
            ret[-1].append({"iWE":str(iWE), "iNS":str(iNS), "iEW":str(iEW), "iSN":str(iSN), "scenario":"BasicRiLSANet2x2"})
            ranges[-1].append(j)
            j = j + 1
    return (ret, ranges)
  def getAverageDuration(self):
...
class ScenarioSet_BasicRiLSACorridor3(ScenarioSet):
  ...
  def getSingle(self, RWScurves, iWE, iNS, iEW, iSN, uID=None):
    ...
    s = getScenario("BasicRiLSACorridor3", "BasicRiLSACorridor3", self.params)
    s.demandName = s.fullPath("routes_%s.rou.xml" % sID)
    if True:#fileNeedsRebuild(s.demandName, "duarouter"):
      nStreams = []
      for stream in s.demand.streams:
  ...
  def getRunsMatrix(self):
    ...
    for iWE,cWE in enumerate(RWScurves):
      for iNS,cNS in enumerate(RWScurves):
        ...
        for iEW,cEW in enumerate(RWScurves):
          for iSN,cSN in enumerate(RWScurves):
            ret[-1].append({"iWE":str(iWE), "iNS":str(iNS), "iEW":str(iEW), "iSN":str(iSN), "scenario":"BasicRiLSACorridor3"})
            ranges[-1].append(j)
            j = j + 1
    return (ret, ranges)
  def getAverageDuration(self):
