    ...
    if fileNeedsRebuild(self.fullPath(self.NET_FILE), "netconvert"):
      ...
      for y in range(1, 4):
        for x in range(1, 4):
            ...
            fdow.write('      <phase duration="4"  state="rrrrryygrrrrryygrrrr" type="transient"/>\n')
            fdow.write('      <phase duration="6"  state="rrrrrrrGrrrrrrrGrrrr" minDur="2" maxDur="20" type="decisional" targetLanes="%s_2 %s_2"/>\n' % (eedge, wedge))
            fdow.write('      <phase duration="4"  state="rrrrrrryrrrrrrryrrrr" type="transient"/>\n')
            ...
            fdow.write('      <phase duration="4"  state="ryygrrrrryygrrrrrrrr" type="transient"/>\n')    
            fdow.write('      <phase duration="6"  state="rrrGrrrrrrrGrrrrrrrr" minDur="2" maxDur="20" type="decisional" targetLanes="%s_2 %s_2"/>\n' % (sedge, nedge))
            fdow.write('      <phase duration="4"  state="ryryrrrrryryrrrrrrrr" type="transient"/>\n')
            fdow.write('      <phase duration="4"  state="rrrrrrrrrrrrrrrrrrrr" type="transient;commit"/>\n')
            fdow.write('   </tlLogic>\n\n')
      fdow.write('</additional>\n')
    # demand
        ...
        for oe,rel in enumerate(f[1]):
          ...
          for ie in range(1, 4): # over input
            ...
            if f[0]=="nmp": 
                ...
                for ve in range(4, 0, -1): via.append("%s/%s_to_%s/%s" % (ie, ve, ie, ve-1)) 
            if f[0]=="smp": 
                iedge = "%s/0_to_%s/1" % (ie, ie) 
                for ve in range(0, 4, 1): via.append("%s/%s_to_%s/%s" % (ie, ve, ie, ve+1)) 
            if f[0]=="wmp": 
                iedge = "0/%s_to_1/%s" % (ie, ie)
                for ve in range(0, 4, 1): via.append("%s/%s_to_%s/%s" % (ve, ie, ve+1, ie))
            if f[0]=="emp": 
                iedge = "4/%s_to_3/%s" % (ie, ie) 
                for ve in range(4, 0, -1): via.append("%s/%s_to_%s/%s" % (ve, ie, ve-1, ie))
            if oe==0:
                if rel[0]=="mn": oedge = "%s/3_to_%s/4.-100" % (ie, ie) 
                if rel[0]=="ms": oedge = "%s/1_to_%s/0.-100" % (ie, ie) 
                if rel[0]=="mw": oedge = "1/%s_to_0/%s.-100" % (ie, ie) 
                if rel[0]=="me": oedge = "3/%s_to_4/%s.-100" % (ie, ie) 
