    ...
    if True:
      ...
      for y in range(1, 5):
        for x in range(1, 5):
            ...
            fdow.write('      <phase duration="4"  state="rrryrrrrrrryrrrrrrrr" type="transient"/>\n')
            fdow.write('      <phase duration="4"  state="rrrrrrrrrrrrrrrrrrrr" type="transient;commit"/>\n')
            fdow.write('   </tlLogic>\n\n')
      fdow.write('</additional>\n')
    # demand
        ...
        for oe,rel in enumerate(f[1]):
          ...
          for ie in range(1, 5): # over input
            ...
            if f[0]=="nmp": 
                ...
                for ve in range(5, 0, -1): via.append("%s/%s_to_%s/%s" % (ie, ve, ie, ve-1)) 
            if f[0]=="smp": 
                iedge = "%s/0_to_%s/1" % (ie, ie) 
                for ve in range(0, 5, 1): via.append("%s/%s_to_%s/%s" % (ie, ve, ie, ve+1)) 
            if f[0]=="wmp": 
                iedge = "0/%s_to_1/%s" % (ie, ie)
                for ve in range(0, 5, 1): via.append("%s/%s_to_%s/%s" % (ve, ie, ve+1, ie))
            if f[0]=="emp": 
            ...
            if oe==0:
                ...
                if rel[0]=="me": oedge = "4/%s_to_5/%s.-100" % (ie, ie) 
