...
class PlatoonManager(traci.StepListener):
        ...
        for origType, mappings in cfg.PLATOON_VTYPES.iteritems():
            ...
            for typeID in mappings.values()+[origType]:
                ...
                if origLength != mappedLength:
                    warn("length of mapped vType '%s' (%sm.) does not equal length of original vType '%s' (%sm.)\nThis will probably lead to collisions."%(typeID, mappedLength, origID, origLength))
                if origEmergencyDecel != mappedEmergencyDecel:
                    warn("emergencyDecel of mapped vType '%s' (%sm.) does not equal emergencyDecel of original vType '%s' (%sm.)"%(typeID, mappedEmergencyDecel, origID, origEmergencyDecel))
                ...
                if self._isConnected(veh.state.leaderInfo[0]):
                    ...
                    while dist < self._catchupDist:
                        ...
                        if self._isConnected(vehAheadID):
                            veh.state.connectedVehicleAhead = True
                            if cfg.VERBOSITY >= 4:
                                report("Found connected vehicle '%s' downstream of vehicle '%s' (at distance %s)"%(vehAheadID, veh.getID(), dist + nextLeaderInfo[1]))
                            break
    ...
    def reorderVehicles(vehicles, actualLeaders):
        ...
        if cfg.VERBOSITY >= 4:
            report("reorderVehicles(vehicles, actualLeaders)\nvehicles = %s\nactualLeaders=%s"%(array2String(vehicles), array2String(actualLeaders)),3)
        
        done = False
        iter_count = 0 # this is needed as abort criterion if two have the same leader (This cannot be excluded for sublane at least)
        nVeh = len(vehicles)
        ...
        while(not done and iter_count < nVeh):
            ...
            for (ego, registeredLeader, actualLeader) in reversed(zip(vehicles, registeredLeaders, actualLeaders)):
                if (ego == actualLeader):
                    if cfg.VERBOSITY >= 2:
                        warn("Platoon %s:\nVehicle '%s' was found as its own leader. Platoon order might be corrupted."%(array2String(vehicles), str(ego)))
                    return vehicles
        ...
        if iter_count != 0:
            if cfg.VERBOSITY >= 4:
                report("Ordering within Platoon %s was corrupted.\nNew Order: %s\nLeaders: %s"%(vehicles[0].getPlatoon().getID(), array2String(vehicles), array2String(actualLeaders)), 3)
