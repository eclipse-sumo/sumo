...
def get_options():
    ...
    optParser.add_option("-w", "--width", type="float", default=0.7, help="pedestrian width")
    optParser.add_option("-l", "--length", type="float", default=0.35, help="pedestrian length")
    optParser.add_option("--departPos", type="float", default=0, help="depart position")
    optParser.add_option("--arrivalPos", type="float", default=-1, help="arrival position")
    optParser.add_option("--prob", type="float", default=0.1, help="depart probability per second")
    ...
    optParser.add_option("-e", "--end", type="int", default=600, help="end time")
    optParser.add_option("-i", "--index", type="int", default=0, help="starting index for naming pedestrians")
    optParser.add_option("-n", "--name", default="p", help="base name for pedestrians")
    ...
    with open(options.output, 'w') as f:
        ...
        for depart in range(options.begin, options.end):
            if random.random() < options.prob:
                write_ped(f, index, options, depart, ' '.join(options.route.split(',')))
                index += 1
        f.write('</routes>')
