...
def parseArgs(args=None):
    ...
    op.add_argument("-s", "--osm-file", category="input", required=True, type=op.file, help="read OSM file from FILE (mandatory)", metavar="FILE")
    op.add_argument("-n", "--net-file", category="input", type=op.net_file, help="read SUMO net from FILE", metavar="FILE")
    op.add_argument("-o", "--output-file", category="output", required=True, type=op.file, help="write stopping places to the output FILE", metavar="FILE",
                           default="stopping_places.add.xml")
    op.add_argument("-f", "--fleet-file", category="output", type=op.file, help="write taxi fleet to the output FILE", metavar="FILE")
    ...
    op.add_argument("-l", "--length", category="input", type=float, help="(minimum) length of the stopping place", default=20.)
