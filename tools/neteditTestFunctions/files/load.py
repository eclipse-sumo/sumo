...
def reloadMultipleFile(referencePosition, type: str):
    """
    @brief reload multiplefile
    """
    # move mouse (to avoid problems with file menu)
    moveMouse(referencePosition, positions.reference, 200, 0)
    # open load mean data dialog (because doesn't have shortcut)
    ...
    if (type == "config"):
        ...
        for _ in range(attrs.toolbar.file.aditionalElements.menu):
            typeKey('down')
        typeKey('space')
        for _ in range(attrs.toolbar.file.aditionalElements.reload + 1):
            typeKey('down')
        typeKey('space')
    elif (type == "demand"):
        for _ in range(attrs.toolbar.file.demandElements.menu):
            typeKey('down')
        typeKey('space')
        for _ in range(attrs.toolbar.file.demandElements.reload + 1):
            typeKey('down')
        typeKey('space')
    elif (type == "data"):
        for _ in range(attrs.toolbar.file.dataElements.menu):
            typeKey('down')
        typeKey('space')
        for _ in range(attrs.toolbar.file.dataElements.reload + 1):
            typeKey('down')
        typeKey('space')
    elif (type == "meanData"):
        for _ in range(attrs.toolbar.file.meanDataElements.menu):
            typeKey('down')
        typeKey('space')
        for _ in range(attrs.toolbar.file.meanDataElements.reload + 1):
            typeKey('down')
        typeKey('space')
    time.sleep(2)
