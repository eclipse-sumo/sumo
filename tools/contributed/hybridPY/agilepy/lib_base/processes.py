...
import subprocess,  os, sys, types, pickle


#hybridPYDIR = os.path.join(os.path.dirname(__file__),"..")
#TOOLSDIR = os.path.join(hybridPYDIR,"..")
        ...
        for attrconfig in self.get_attrsman().get_configs(is_all = True):
            if self.optiongroupname in attrconfig.groupnames:
                ...
                if is_enabled: # disabeled options are simply not added
                    ...
                    options.add_option(attrconfig.attrname,attrconfig.get_value(),attrconfig.cml, is_filepath = is_filepath, cmlvaluemap=cmlvaluemap)
