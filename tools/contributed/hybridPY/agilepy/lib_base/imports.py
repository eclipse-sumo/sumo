...
class CsvImporter(Process):
    ...
    def do(self):
        ...
        INTTYPES = (int, np.int32,  np.int32,  np.int64)
        FLOATTYPES = (float,int,complex, np.float32,  np.float32,  np.float64)
        ...
        if len(index_to_type) == 0:
            if logger: logger.w('file %s does not contain valid column names.'%os.path.basename(filepath))
            return False
        ...
        if logger: logger.w('import CSV from %s'%os.path.basename(filepath))
        n_imported = 0
