...
import types, os, pickle, sys, string, math
...
NODATATYPES = (types.FunctionType, types.LambdaType)
...
class AttrConf:
    ...
    def _write_xml_value(self,val,fd):
        #if self.xmltag == 'id':
        #    print ('_write_xml_value',val,type(val),isinstance(val, Iterable),isinstance(val, str))
        ...
        elif isinstance(self._default, bool):# use default and hope it is no a numpy bool!!!
            if val:
                fd.write(xm.num(self.xmltag,1))
            else:
                fd.write(xm.num(self.xmltag,0))
        
        elif isinstance(self._default, (str ,bytes)):
    ...
    def get_choice_key(self, value):
        """
        In case self.choices is a dictionary, it retuens the 
        chosen key from the value.
        """
        # TODO: this method is not very memory efficient nor fast.
        # would be better to create an inverse dictionary beforehand
        # as hash
        return list(self.choices.keys())[list(self.choices.values()).index(value)]
    ...
    def format_value(self, show_unit = False, show_parentesis=False):
        ...
        if not (isinstance(v, Iterable) and not isinstance(v, str)):
...
class ListConf(AttrConf):
    ...
    def get_value_from_string(self,s, sep = None):
        ...
        for val in s.split(self.sep):
            #print 'get_value_from_string val=',val
...
class Indexing:
    ...
    def get_ids_sorted(self):
        ...
        #print '  sorted',sorted(self._index_to_id.items())
...
class ColConf(Indexing, AttrConf):
    ...
    def add(self, ids, values = None):
        if not (isinstance(ids, Iterable) and not isinstance(ids, str)):
            _ids = [ids]
...
class Attrsman:
    ...
    def write_csv(self, fd, sep = ',', 
        ...
        for attrconf in attrconfigs:
            ...
            if (attrconf.is_save()|is_export_not_save)& (attrconf.struct in STRUCTS_SCALAR):
                ...
                if mt == 'id':
                    fd.write( '%s %s%s%s\n'%(attrconf.attrname,attrconf.format_unit(show_parentesis), sep, attrconf.get_linktab().format_ids([attrconf.get_value()])))
                else:
                    fd.write( '%s %s%s%s\n'%(attrconf.attrname,attrconf.format_unit(show_parentesis), sep, attrconf.format_value()))
    ...
    def print_attrs(self, show_unit = True, show_parentesis=False, attrconfigs = None):
        ...
        for attrconf in attrconfigs:
            print ('  ',attrconf.attrname,':',attrconf.format_value(show_unit=True))
    ...
    def delete(self, attrname):
        ...
        #for attrconf in self._attrconfigs:
        #    print ('  managed attr:',attrconf.attrname)
        
        #if hasattr(self,attrname): 
        attrconf = getattr(self,attrname)
        #print ('delete','attrname=',attrname,attrconf in self._attrconfigs)
        #self.print_attrs()
        ...
        if attrconf in self._attrconfigs:
            ...
            #print ('   removed',attrname,'remaining attrs',self._attrconfigs) 
            return True
    ...
    def  __getstate__(self):
        if hasattr(self,'attrname'):
            #print ('\nAttrsman.__getstate__ of',self.attrname,'id',id(self),'self._attrs_nosave',self._attrs_nosave,)
            if hasattr(self,'_obj'):
                pass
                #print('  obj.ident=',self._obj.ident, 'id(obj)',id(self._obj))
            else:
                #print('no _obj attr')
                #print ('  warning dir',dir(self))
                return {}
            
        else:
            #print ('\nWARNING in Attrsman.__getstate__  attrname missing','id',id(self))
            return {}
        
        if not hasattr(self,'_obj'):
            #print('WARNING: unknown obj in attrman',type(self))
            #print ('  warning dir',dir(self))
            return {}
...
class BaseObjman:
    ...
    def _format_ident(self, ident):
        if (isinstance(ident, Iterable) and not isinstance(ident, str)):
...
class TableMixin(BaseObjman):
    ...
    def get_obj_from_ident(self,ident_abs):
        ...
        if len(ident_abs)==1:
            ...
            #if (isinstance(ident_check, Iterable) and not isinstance(ident_check, str)):
    ...
    def _write_xml_body(self, fd,  indent, objconfigs, idcolconfig_include_tab, colconfigs, 
        ...
        #print ('TableMixin._write_xml_body ident',self.ident,'xmltag_id',xmltag_id, 'ids_xml',ids_xml, 'ids',ids)                
        if ids is None:
            ids = self.get_ids()
        ...
        for _id, id_xml  in zip(ids,ids_xml):
            ...
            #print ('   make tag for id',_id)
            if xmltag_id == '':
                #print ('    no id tag will be written')
                pass
            elif (attrconfig_id is None)&(xmltag_id is not None):
                #print ('    use specified id tag and and specified id values')
                fd.write(xm.num(xmltag_id, id_xml))
                
            elif (attrconfig_id is not None):
                #print ('    use id tag and values of attrconfig_id',attrconfig_id,attrconfig_id.attrname,attrconfig_id[_id],'xmltag',attrconfig_id.xmltag)
            ...
            if (len(idcolconfig_include_tab)>0)|(len(objcolconfigs)>0):
                ...
                for attrconfig in idcolconfig_include_tab:
                    #print ('    idcolconfig_include_tab',attrconfig.attrname)
                    attrconfig.write_xml(fd,_id,indent+4)
            
                for attrconfig in objcolconfigs:
                    #print ('    objcolconfigs',attrconfig.attrname)
    ...
    def write_xml(self, fd, indent, xmltag_id = 'id', ids = None, ids_xml = None,
        ...
        #print ('TableMixin.write_xml',self.ident,'xmltag_id',xmltag_id,'ids',ids,'ids_xml',ids_xml,'self.xmltag',self.xmltag)
        if self.xmltag is not None:
            ...
            for attrconfig in self.get_attrsman().get_configs(is_all=True):
                #print '  check',attrconfig.attrname,'xmltag',attrconfig.xmltag,'is_colattr',attrconfig.is_colattr(),attrconfig.metatype
            ...
            #        print '    colconfig',colconfig.attrname,'xml',colconfig.xmltag
            #        
            #    for  colconfig in objcolconfigs:
            #        print '    objcolconfig',colconfig.attrname,'xml',colconfig.xmltag
            #    
            #    for  colconfig in objconfigs:
            #        print '    objconfig',colconfig.attrname,'xml',colconfig.xmltag
            #    
            #print '  idcolconfig_include_tab',idcolconfig_include_tab
            # start writing
            if len(attrconfigs)>0:
                #print '  there are also scalar attributes'
