...
def get_tablecolors():
    cs= [wx.Colour('grey85'),wx.Colour("light blue")]
...
class AgileToolbarMixin:
    ...
    def init_toolbar(self,orientation='horizontal',size=(16,16),separation=5):
        ...
        """
        #print('init_toolbar',orientation,size)
        ...
        if orientation=='horizontal':
            ...
            self.toolbar = self.CreateToolBar( ( wx.TB_VERTICAL|wx.TB_NODIVIDER  \
                                                | wx.NO_BORDER
                                                | wx.TB_FLAT
                                                #| wx.TB_TEXT
                                                #| wx.TB_HORZ_LAYOUT
    ...
    def get_toolbarsize(self):
        return self._toolbarsize
    ...
    #    return self._toolwidgets.get(key, None)
        
    def add_tool(self,key, func=None,bitmap=None,info='',widget=None,**args):
        """
        Add a tool to the tool bar
        """
        #print ('add_tool',key,key not in self._tools)#,func,self)
        #print('  keys',self._tools.keys())
        
        
        if key in self._tools:
            # tool with key already in toolbar
            #    delete it, then recreate
            print('WARNING in add_tool: key already present in toolbar',key)
            #self.del_tool(key)
        
        if key not in self._tools:
            bmp_null = wx.NullBitmap#wx.BitmapBundle(wx.NullBitmap)
            
            _id=wx.NewId()
            
            name = args.get('name', str.capitalize(key.replace("_"," ")))
            ...
            if widget is not None:
                ...
                if func is not None:
                    self.Bind(wx.EVT_TOOL, func, id=_id)
            else:
                # tools is a simple button
                if not bitmap:
                    bitmap=bmp_null
                
                self.toolbar.AddTool(_id, key, bitmap, bmp_null, wx.ITEM_NORMAL, name, info, None)
                self.Bind(wx.EVT_TOOL, func, id=_id)
                if 'func_rclick' in args:
                    self.Bind(wx.EVT_TOOL_RCLICKED,args['func_rclick'], id=_id)
...
class AgileMenuMixin:
    ...
    def get_menuitem(self,path):
        menupath=str.split(path,'/')[:-1]
        key=str.split(path,'/')[-1]
...
class AgileStatusbar(wx.StatusBar):
    def __init__(self, parent,fields=[  ('action',-4),
        ...
        wx.StatusBar.__init__(self, parent, -1, wx.STB_SIZEGRIP)
    ...
    def set_progress(self,percent):
        ...
        """
        print ('set_progress',percent,int(percent))
