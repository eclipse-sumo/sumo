#!/usr/bin/env python
# cd /home/joerg/projects/hybridPY/tools/hybridPY/wxpython
# python objpanel.py
...
from wx.lib import scrolledpanel, colourdb, masked
...
from  agilepy.lib_base.classman import BaseObjman 
...
# hack that is not recommended
# but prevents an error with a newer wx versions
if hasattr(wx.SizerFlags(),'DisableConsistencyChecks'):
...
def is_obj(obj):
    return isinstance(obj, BaseObjman)
...
#-------------------------------------------------------------------------------
    ...
    def set_tooltip(self,widget=None):
        ...
        widget.SetToolTip(wx.ToolTip(infostr))
...
class ChoiceWidgetContainer(WidgetContainer):
    ...
    def define_widgetset(self):
        ...
        if type(self._attrconf.choices) in (OrderedDict, dict):
            ...
            self._choicenames = list(str(x) for x in self._attrconf.choices.keys())
        else:
            self._choicevalues = list(self._attrconf.choices)
            self._choicenames = list(str(x) for x in self._attrconf.choices)
...
class ChecklistWidgetContainer(ChoiceWidgetContainer):
    ...
    def get_valuewidget_write(self):
        ...
        widget = wx.CheckListBox(self.parent, -1, (80, 50), wx.DefaultSize, list(map(str, self._choicenames))) 
...
class ObjWidgetContainer(AttrBase,WidgetContainer):
    ...
    def get_valuewidget_write(self):
        ...
        if mt == 'id':
            #text = self._attrconf.get_linktab().format_id(value)
            choices = linktab.format_ids(linktab.get_ids()).split(',')
            if (len(choices)>0) & (value != -1):
    ...
    def on_objlink(self,event):
        ...
        if self.parent.func_change_obj:
            if mt == 'obj':
                ...
                if is_obj(value): #inspect.isclass(type(value)):
                    #print '  ident?',hasattr(value,'ident')
                    if hasattr(value,'ident'):
                        navitimer = wx.CallLater(1, self.parent.func_change_obj, value)

                   
            elif mt == 'id':
                linktab = self._attrconf.get_linktab()
                navitimer = wx.CallLater(1, self.parent.func_change_obj, linktab, value)# here value is id
            
            elif mt == 'ids':
                linktab = self._attrconf.get_linktab()
                navitimer = wx.CallLater(1, self.parent.func_change_obj, linktab, None, value)# here value is list with ids
                
            elif  mt =='tabid':
                linktab, id = value
                navitimer = wx.CallLater(1, self.parent.func_change_obj, linktab, id)#
            ...
            #navitimer = wx.CallLater(1, self.parent.func_change_obj,self.get_objvalue())
            pass
...
class ColorWidgetContainer(AttrBase,TextWidgetContainer):
    ...
    def get_valuewidget_write(self):
        ...
        widget = coloursel.ColourSelect(self.parent, -1, "", wx.Colour(cint[0],cint[1],cint[2],cint[3]), size = wx.DefaultSize)
        #self.set_textevents(widget)
        #widget.Enable(True)
        if self.immediate_apply:
...
class TableGrid(AttrBase,gridlib.GridTableBase):
    ...
    def GetValue(self, row, col):
        ...
        if 1:
            ...
            if mt =='objs':
                ...
                #print ('  id=',val)
                #print ('  linktab',attrconf.get_linktab(),idtext,'choices',hasattr(attrconf,'choices'))
                if val <= 0:#val == -1:
                    return '-'
                ...
                if hasattr(attrconf,'choices'):
                    ...
                    if isinstance(attrconf.choices, OrderedDict) | isinstance(attrconf.choices, dict):
                        if list(attrconf.choices.values()).count(val)>0:
                            ...
                            return str(list(attrconf.choices.keys())[ind])
                        else:
        ...
        if attrconf  is not None:
            ...
            if attrconf.is_writable():
                ...
                if hasattr(attrconf,'choices'):
                    ...
                    # Definition added for Config
                    if type(value) in STRINGTYPES:
                        attrconf[id] = value
                    else:
                        attrconf[id] = str(value)
                        print('WARNING in SetValue: value to be added was converted to string :',tt) 
                else:
...
class TabPanel(AttrBase,gridlib.Grid):
    def __init__(self, parent, tab, objpanel = None, attrconfigs=None, ids=None, show_ids=True,
        ...
        self.Bind(gridlib.EVT_GRID_CELL_LEFT_DCLICK, self.on_edit_cell) 
        self.Bind(gridlib.EVT_GRID_CELL_LEFT_CLICK, self.on_click_cell) 
        
        self.Bind(gridlib.EVT_GRID_LABEL_LEFT_DCLICK, self.OnLabelLeftDoubleClick)
    ...
    def on_select_rowfunction(self, event):
        ...
        #del table
        newtable = TableGrid(self,self.tab, attrconfigs = self.tab.get_colconfigs(), ids=self.tab.get_ids(), show_ids=table.show_ids)
        del table
        # The second parameter means that the grid is to take ownership of the
        # table and will destroy it when done.  Otherwise you would need to keep
        # a reference to it and call it's Destroy method later.
        ...
        if self.CanEnableCellControl():
            ...
            if mt =='obj':
                if is_obj(value):
                    #print '  ident?',hasattr(value,'ident')
                    if hasattr(value,'ident'):
                        navitimer = wx.CallLater(1, self.func_change_obj, value)
                ...
                if attrconf.is_readonly():
                    ...
                    if value != -1:
                        navitimer = wx.CallLater(1, self.func_change_obj, linktab, value)# here value is id
                    else:
                        event.Skip()
                else:
                    event.Skip()
                ...
                if value is not None:
                    linktab, id = value
                    navitimer = wx.CallLater(1, self.func_change_obj, linktab, id)# here value is id  
                else:
                    event.Skip()    
                ...
                if value is not None:
                    linktab = attrconf.get_linktab()
                    navitimer = wx.CallLater(1, self.func_change_obj, linktab,None,  value)# here value is ids list  
                else:
                    event.Skip()    
            else:
                event.Skip()    
            #elif config.has_key('do_init_arrayobj'):
            #    if config['do_init_arrayobj']:
            #        navitimer = wx.CallLater(1, self.func_change_obj,table.obj[attr,id])
            #    else:
            #        event.Skip()
    ...
    def on_edit_cell(self, event):
        ...
        if self.func_choose_id is not None:
            ...
            #
            wx.CallLater(1,self.func_choose_id,id,attrconf)
            #event.Skip()
            return
        
        elif self.func_choose_attr is not None:
            # call self.func_choose_id
            wx.CallLater(1,self.func_choose_attr,attrconf)
            #event.Skip()
            return
        ...
        if self.CanEnableCellControl():
            ...
            if metatype == 'color': # special config for colors 
                ...
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    attrconf[id]=np.array([c.Red(),c.Green(),c.Blue(),c.Alpha()],np.float32)/255.0#self.ffffff_to_color([wxc.Red(),wxc.Green(),wxc.Blue()] )
                    self.SetCellBackgroundColour(event.GetRow(), event.GetCol(),c)
                    self.ForceRefresh()
                    if self.func_apply is not None:
                        wx.CallLater(1,self.func_apply, table, id, None)
    ...
    def OnLabelLeftDoubleClick(self, evt):
        ...
        if evt.GetRow()>=0:
            ...
            if self.func_choose_id is None:
                ...
                # call self.func_choose_id
                wx.CallLater(1,self.func_choose_id,id,'')
...
class ObjPanelMixin:
    ...
    def make_scalartablepanel(   self,parent, attrconfigs_scalar, table, objpanel =None, attrconfigs=None, groupnames=None, 
        ...
        tablepanel = self.add_table(splitter, table, objpanel = objpanel, ids = self.ids, attrconfigs = attrconfigs, groupnames = groupnames)
        #if  datawidget is not None:
    ...
    def make_tablepanel(   self,parent, tab,objpanel =None, attrconfigs=None, groupnames=None, 
        ...
        datawidget = self.add_table(parent, tab, ids =  self.ids,objpanel =objpanel, attrconfigs = attrconfigs, groupnames = groupnames)
...
##            btnsizer.Realize()
    ...
    def apply(self):
        ...
        if self.func_apply is not None:
            ...
            # 
            #wx.CallLater(1,self.func_apply, self.obj, self.id, self.ids)
...
class ObjPanelDialog(ObjPanelMixin,wx.Dialog):
    def __init__(self, parent, 
        ...
        self.Create(parent, -1,title , pos, size=size, style=style)
...
class NaviPanelMixin:
    ...
    def init_navbar(self,obj, _id=None):
        ...
        #self.init_toolbar()
        #bottons = []
        bitmap = wx.ArtProvider.GetBitmap(wx.ART_GO_BACK,wx.ART_TOOLBAR)
        b = wx.BitmapButton(self, -1, bitmap, bottonsize,
                       (bitmap.GetWidth()+bottonborder, bitmap.GetHeight()+bottonborder))
        b.SetToolTip("Go back in browser history.")
        ...
        bitmap = wx.ArtProvider.GetBitmap(wx.ART_GO_FORWARD,wx.ART_TOOLBAR)
        b = wx.BitmapButton(self, -1, bitmap, bottonsize,
                       (bitmap.GetWidth()+bottonborder, bitmap.GetHeight()+bottonborder))
        b.SetToolTip("Go forward in browser history.")
        self.Bind(wx.EVT_BUTTON, self.on_go_forward, b)
        self.toolbar.Add(b,0, wx.EXPAND, border=toolbarborder)
        #bottons.append(b)
        
        bitmap = wx.ArtProvider.GetBitmap(wx.ART_GO_UP,wx.ART_TOOLBAR)
        b = wx.BitmapButton(self, -1, bitmap, bottonsize,
                       (bitmap.GetWidth()+bottonborder, bitmap.GetHeight()+bottonborder))
        b.SetToolTip("Go up to parent object.")
        ...
        bitmap = wx.ArtProvider.GetBitmap(wx.ART_GO_HOME,wx.ART_TOOLBAR)
        b = wx.BitmapButton(self, -1, bitmap, bottonsize,
                       (bitmap.GetWidth()+bottonborder, bitmap.GetHeight()+bottonborder))
        b.SetToolTip("Go to main object, the mother of all objects.")
        ...
        self.path=wx.TextCtrl(self, -1, self.get_ident_abs(obj, _id), style=wx.TE_RIGHT)#size=(-1, -1))#,size=(300, -1))
        ...
        b = wx.BitmapButton(self, -1, bitmap, bottonsize,
                 (bitmap.GetWidth()+bottonborder, bitmap.GetHeight()+bottonborder))
        b.SetToolTip("Object browser menu. Click for actions on this object")
...
class ObjBrowserMainframe(wx.Frame):
    ...
    def OnSave(self,event):
        dlg = wx.FileDialog(\
            ...
            #defaultFile="", 
            wildcard=wildcards_all, style=wx.FD_SAVE
            )
...
class ObjBrowserApp(wx.App):
    ...
    def OnInit(self):
    	...
    	frame = ObjBrowserMainframe(None,-1, 'Object browser',wx.DefaultPosition, size=wx.DefaultSize,obj=self._obj, table = None)  
