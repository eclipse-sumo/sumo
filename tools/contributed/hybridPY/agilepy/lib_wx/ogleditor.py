...
class Vbo:
    ...
    def update_colors(self, colors):
        ...
        if self._vertexvbo is None: return
...
class SelectTool(SelectToolMixin):
    ...
    def set_cursor(self):
        # http://www.wxpython.org/docs/api/wx.Cursor-class.html
        if self._canvas  is not None:
            self._canvas.SetCursor(wx.Cursor(wx.CURSOR_QUESTION_ARROW))
...
class DrawobjMixin(am.ArrayObjman):
    def init_common(self,ident,parent=None, name = None,
        ...
        self.add(cm.AttrConf(  '_is_visible', _is_visible,
                                        groupnames = ['options'],
                                        perm='wr',
                                        is_save = True,
                                        is_plugin = False,
                                        name = 'Visible?',
                                        info = 'If True, object are visible, otherwise it is not drawn.',
                                        ))
...
class Lines(DrawobjMixin):
    ...
    def pick_handle(self, coord, detectwidth = 0.1):
        ...
        handle1 = np.ones((len(ids),2), np.int32)
        handle1[:,0] = ids
        handle1[:,1] = 0
        #print '  ',d,handle1
        ...
        ids = self._ids[dx*dx+dy*dy<dw]
        handle2 = np.ones((len(ids),2), np.int32)
        handle2[:,0] = ids
        handle2[:,1] = 1
        #print '  ',d,handle2
...
class Triangles(Lines):
    ...
    def pick_handle(self, coord, detectwidth = 0.1):
        ...
        ids = self._ids[dx*dx+dy*dy<dw]
        handle3 = np.ones((len(ids),2), np.int32)
        handle3[:,0] = ids
        handle3[:,1] = 2
...
class Circles(DrawobjMixin):
    ...
    def pick_handle(self, coord, detectwidth = 0.1):
        ...
        #handles =  np.concatenate(( ids.reshape((len(ids),1)), np.zeros((len(ids),1),np.int32)),1)

        #print '  ids',ids
        #print '  handles',handles
        return  np.concatenate(( ids.reshape((len(ids),1)), np.zeros((len(ids),1),np.int32)),1)
...
class OGLnavcanvas(wx.Panel):
    ...
    def make_navbar(self):
        ...
        b = wx.BitmapButton(self, -1, bitmap, bottonsize,
            (bitmap.GetWidth()+bottonborder, bitmap.GetHeight()+bottonborder))
        b.SetToolTip("Select drawing components")
        ...
        self.coordsdisplay.Bind(wx.EVT_RIGHT_DOWN,self.ShowPopupCopy)
        
        # no:self.coordsdisplay= wx.TextCtrl(self, -1, 'x,y',  style=wx.TE_RIGHT)
    ...
    def ShowPopupCopy(self,event):
        popmenu = wx.Menu()
        popmenu.Append(1, "Copy coordinates to clipboard")
        popmenu.Bind(wx.EVT_MENU, self.CopyText)
        self.PopupMenu(popmenu)
        popmenu.Destroy()

    def CopyText(self,event):
        clipdata = wx.TextDataObject()
        text = self.coordsdisplay.GetLabelText()
    ...
    def on_restart(self,event):
        self._canvas.get_mainframe().restart_simulation()
        
    def on_continue(self,event):
        self._canvas.get_mainframe().continue_simulation()
    
    def on_stop(self,event):
        self._canvas.get_mainframe().stop_simulation()
        
    def on_step(self,event):
        self._canvas.get_mainframe().on_step(event)
...
class OGLcanvas(glcanvas.GLCanvas):
    ...
    def OnDraw(self, *args, drawobjs = None, is_anim = True, **kwargs):
        """Draw the window."""
        # print ('OGLCanvas.draw id(self._drawing)',id(self._drawing),self._drawing)
        ...
        #if drawobjs is None:
        drawobjs = self._drawing.get_drawobjs(is_anim = is_anim)
    ...
    def zoom_tofit(self, event = None, is_draw = True):
        ...
        #Anpassung für die Handhabung von sehr großen MATSim Netzwerken
        # old: zoomfactor = 0.8*np.min(window/dp[:2])
        zoomfactor = np.min(window/dp[:2])

        #print '  zoomfactor,zoom',zoomfactor,self.zoom

        self.zoom /= zoomfactor
        self.zoom = min(self.zoom, 160)
