...
class StopresultsPlotter(Process):
    ...
    def plot_flow_stop(self, fig):
        ...
        if self.is_title:
            ax.set_title('Vehicle and person in-flows over time at PRT stop ID=%d'%id_stop, fontsize=self.size_titlefont)
    ...
    def plot_flows_compare(self, fig):
        ...
        for id_stop  in stopresults.ids_stop.get_value():
            ...
            flowmatrix[np.array(time_update_flows*stopresults.inflows_veh_sched[id_stop], dtype = np.int32), np.array(time_update_flows*stopresults.inflows_veh[id_stop], dtype = np.int32)] +=1
