...
LEADVEHICLESTATES = [VEHICLESTATES['boarding'],VEHICLESTATES['waiting'],VEHICLESTATES['emptytrip'],VEHICLESTATES['occupiedtrip']]
...
class Compressors(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.ids_comp_to_ids_detectedge_sumo = np.zeros(n_id_max,dtype = np.object_)
        ...
        self.releasetimes_queues = np.zeros(n_id_max,dtype = np.object_)
        ...
        self.ids_vehs_detect_sumo = np.zeros(n_id_max,dtype = np.object_) 
    ...
    def process_step(self, process):
        ...
        for id_comp,\
            ...
            ids_veh_entered_sumo, ids_veh_left_sumo,  ids_veh_new_sumo= vehicles.get_entered_left(id_detectedge_sumo, ids_veh_detect_sumo)
            ...
            if len(ids_veh_entered_sumo)>0:
                ...
                for id_veh_entered,id_veh_entered_sumo, id_leader,length_plat\
                    ...
                    if (length_plat < 0.001)&(id_leader == -1):
                        ...
                        inds_queue = np.flatnonzero( (np.array(ids_edge_sumo_target, object)==id_targetedge_sumo) & np.array(are_queue_avail, dtype = np.bool_))
...
class Decompressors(Compressors):
    ...
    def prepare_sim(self, process):
        ...
        self.ids_targetedgess_sumo = np.zeros(n_id_max,dtype = np.object_)
    ...
    def process_step(self, process):
        ...
        for id_comp,\
            ...
            if 1:#(simtime - self.releasetimes[id_comp]> self.time_release_min.get_value()): # do this later ...simtime - self.releasetimes[id_comp]> 10:                 
                # now check if any of the vehicle in any of the queues has arrived
                # and if a queue is complete, create a platoon, send it off and
                # reset the queue
                #print '  Deal with queues...'
...
class Mergenodes(am.ArrayObjman):
                ...
                if (ids_edge_from is None):
                    ...
                    if self.is_prt_only(ids_lane1, lanes) & self.is_prt_only(ids_lane2, lanes):
                        ...
                        #                        )
                            ...
                            ids_node_out = self.search_downstream_merges(ids_edge_to[0], edges, lanes, id_prtmode, ids_sinkedge = ids_shuntedges)
                            id_merge = self.add_row(   ids_node = id_node,
                    ...
                    if self.is_platform(ids_lane_in, lanes):
                        ...
                        if self.is_prt_only(ids_lane2, lanes): 
                            ...
                            #id_node_up, dist = self.search_upstream_merge(id_edge_from, edges, lanes, id_prtmode)
                            ids_node_out = self.search_downstream_merges(id_edge2, edges, lanes, id_prtmode, ids_sinkedge = ids_shuntedges)
                            id_merge = self.add_row(    ids_node = id_node,
                                                        #ids_node_in1 = id_node_up,
                                                        #distances_node_in1 = dist,
                                                        ids_nodes_out = ids_node_out,
                            ...
                            #id_node_up, dist = self.search_upstream_merge(id_edge_from, edges, lanes, id_prtmode)
                            ids_node_out = self.search_downstream_merges(id_edge1, edges, lanes, id_prtmode, ids_sinkedge = ids_shuntedges)
                            id_merge = self.add_row(   ids_node = id_node,
                                            #ids_node_in1 = id_node_up,
                                            #distances_node_in1 = dist,
                                            ids_nodes_out = ids_node_out,
    ...
    def prepare_sim(self, process):
        ...
        #self.ids_merge_to_ids_node_sumo = np.zeros(n_id_max,dtype = np.object_)
        #self.ids_merge_to_ids_node_sumo[ids] = nodes.ids_sumo[self.ids_node[ids]]
        
        self.ids_merge_to_ids_edge_out_sumo = np.zeros(n_id_max,dtype = np.object_)
        #self.ids_node_to_ids_edge_out_sumo = np.zeros(n_id_max,dtype = np.object_)
        ...
        # it is used to identify whether a new vehicle has left the merge 
        self.ids_vehs_out_sumo = np.zeros(n_id_max,dtype = np.object_) 
        
        # this queue contains all vehicles on line 1 between 
        # merge and id_node_in1
        self.ids_vehs_in1 = np.zeros(n_id_max,dtype = np.object_) 
        self.ids_vehs_in1_sumo = np.zeros(n_id_max,dtype = np.object_) 
        
        # this queue contains all vehicles on line 2 between 
        # merge and id_node_in2
        self.ids_vehs_in2 = np.zeros(n_id_max,dtype = np.object_) 
        self.ids_vehs_in2_sumo = np.zeros(n_id_max,dtype = np.object_) 
        
        # this queue contains the merged vehicles for the controlled range
        self.ids_vehs_merged = np.zeros(n_id_max,dtype = np.object_) 
        self.ids_vehs_merged_sumo = np.zeros(n_id_max,dtype = np.object_) 
        
        # this queuse contains the line index (1 or 2, 0= not assigned) 
        self.lineinds_vehs_merged = np.zeros(n_id_max, dtype = np.object_) 
                            ...
                            if len(ids_merge_out)>0:
                                ...
                                if routeinds[mergeind]<INFINT:
                                    ...
                                    if self.are_entry[ids_merge_out[mergeind]]:
                                        ...
                                        self.enter_veh_entry(id_veh, id_veh_sumo, id_merge, ids_merge_out[mergeind], ids_edge_mergeout_sumo[mergeind], vehicles, debug = debug)
                                    else:
                                        if debug>0:
                                            print('      call enter_veh')
                                        self.enter_veh(id_veh, id_veh_sumo, id_merge, ids_merge_out[mergeind], ids_edge_mergeout_sumo[mergeind], vehicles, debug = debug)
                                else:
        ...
        if id_veh in self.ids_vehs_merged[id_merge_from]:
            ...
            if len(self.ids_vehs_merged[id_merge_from])>ind_pos+1:
                ...
                if ind_pos>0:
                    ...
                    dist_to_merge_behind = get_traci_distance(vehicles.get_id_sumo(id_veh_behind), id_edge_mergeout_sumo, 3.0)
                    dist_to_merge_infront = get_traci_distance(vehicles.get_id_sumo(id_veh_infront), id_edge_mergeout_sumo, 3.0)
    ...
    def process_step_entry(self, id_merge, vehicles, debug):
        ...
        #for id_veh, state in self.vehicles_entries.items():
            ...
            if state['status'] == 'wait':
                ...
                if  traci.vehicle.getSpeed(id_veh_sumo) > 0.9*vehicles.speed_max:
                    ...
                    if 'id_veh_infront' in state:
                        ...
                        vehicles.add_ghost(id_veh, id_veh_infront_tail, dist_tomerge, dist_tomerge_tail,is_substitute = True)
                    ...
                    if 'id_veh_behind' in state:
                        ...
                        vehicles.add_ghost(id_veh_behind, id_veh_tail, dist_tomerge_behind, dist_tomerge_tail,is_substitute = True)
...
class PrtStops(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        ## station management
        self.ids_stop_to_ids_edge_sumo = np.zeros(np.max(ids)+1,dtype = np.object_)
        ...
        self.ids_stop_to_ids_acceledge_sumo = np.zeros(np.max(ids)+1,dtype = np.object_)
        ...
        self.ids_vehs_alight_allocated = np.zeros(np.max(ids)+1,dtype = np.object_)
        self.ids_vehs_board_allocated = np.zeros(np.max(ids)+1,dtype = np.object_)
        ...
        self.ids_vehs_sumo_prev = np.zeros(np.max(ids)+1,dtype = np.object_) 
        self.ids_vehs = np.zeros(np.max(ids)+1,dtype = np.object_)
        self.ids_vehs_toallocate = np.zeros(np.max(ids)+1,dtype = np.object_)
        #
        ...
        self.ids_vehs_prog = np.zeros(np.max(ids)+1,dtype = np.object_)
        
        ## person management
        self.ids_persons_sumo_prev = np.zeros(np.max(ids)+1,dtype = np.object_)
        #self.ids_persons_sumo_boarded = np.zeros(np.max(ids)+1,dtype = np.object_)
        self.waittimes_persons = np.zeros(np.max(ids)+1,dtype = np.object_)
    ...
    def process_step(self, process):
        ...
        for id_stop, id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
                    ...
                    if id_person_sumo in self.id_person_to_origs_dests:
                        ...
                        if len(id_edge_sumo_dests)>0:
                            ...
                            if  id_edge_sumo_dests[0][2] == id_edge_sumo:
                                ...
                                self.parent.vehicleman.note_person_entered(id_stop,id_person_sumo, id_edge_sumo_dests[0][1])
            ...
            self.waittimes_tot[id_stop] += self.numbers_person_wait[id_stop]*self.time_update.get_value()
                
            timeconst_flow = self.timeconst_flow.get_value()
            self.flows_person[id_stop] = timeconst_flow*self.flows_person[id_stop] + (1.0-timeconst_flow)*float(n_enter)/self.time_update.get_value()
        ...
        if id_veh == self.ids_veh_lead[id_stop]:
            ...
            if self.types[id_stop] == STOPTYPES['group']:
                ...
                if is_stop:
                    ...
                    self.parent.prtvehicles.set_stop(id_veh, self.ids_stop_to_ids_edge_sumo[id_stop], self.stoplines[id_stop])
...
class PrtVehicles(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.are_update = np.zeros(n_id_max, dtype = np.bool_)
    ...
    def process_step(self, process):
        ...
        dists_min = np.min(self.dists0[ids_contr] + deltas_ghosts - deltas_vehs -self.lengths_plat[ids_ghosts], 1) - self.length
        ...
        dists_comf = self.tau*velocities +0.5/self.decel*( velocities*velocities-velocities_ghost_min*velocities_ghost_min)
...
class VehicleMan(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        # from veh arrays
        #self.are_emptytrips = np.zeros(n_veharray, dtype = np.bool_)
    ...
    def update_flows(self, process):
        ...
        print('  self.inflows_sched.shape',self.inflows_sched.shape)
        ...
        if process.simtime > process.time_warmup:
            ...
            prtstopresults.record(  timestep, ids_stop, 
                                    ...
                                    inflows_veh_sched = np.array(const_time *self.inflows_sched[ids_stop,0],dtype=np.float32),
    ...
    def push_occupied_leadvehs(self, ids_veh_lead, process):
        ...
        if len(inds_valid)==0: return False
        ...
        inds_time_min = n_timeslot_offset+np.array(np.clip(np.array(1.0*durations_est/self.time_update_flows.get_value(),dtype=np.int32),0,self.n_est_max-n_searchint-n_timeslot_offset),dtype = np.int32)
    ...
    def push_empty_leadvehs(self, ids_veh_lead, process):
        ...
        # stops.flows_person is a real flow and needs to be scaled to number of persons
        # in time_update_flows in order to be compatible with the vehicle flow recording
        flow_person_est = (stops.flows_person[ids_stop_target]*time_update_flows).reshape(n_stop_target,1)
        
        n_searchint = int(self.time_search_empty.get_value()/time_update_flows+0.5)
        inds_search_base = n_timeslot_offset+np.arange(n_searchint, dtype = np.int32) * np.ones((n_stop_target,n_searchint), dtype = np.int32)

        # prevent that stops with too high flows are selected
        numbers_veh_sent = np.sum(self.inflows_sched[self.ids_stop, :],1).reshape(n_stop_target,1)#/ (time_update_flows*n_searchint)
        #print '  size(numbers_veh_sent)',numbers_veh_sent.shape
        ...
        for id_veh_lead, id_stop_current, time_order, \
            ...
            inds_time_min = np.array(np.clip(np.array(1.0*durations_est/self.time_update_flows.get_value(),dtype=np.int32),0,self.n_est_max-n_searchint),dtype = np.int32)
            ...
            costs = (   self.weight_demand.get_value() * (  stops.numbers_person_wait[ids_stop_target]-numbers_veh_sent)\
                        + self.weight_flow.get_value() * (flow_person_est-self.inflows_sched[ids_stop_target, inds_search])\
                     )* timeweight
            ...
            # prevent that stop of origin is selected
            ...
            if 0:
                #print '  costs',costs
                for ind, id_stop,number_veh_sent  in zip(list(range(n_stop_target)),ids_stop_target, numbers_veh_sent):
                    print(40*'.')
                    ...
                    print('     number_veh_sent', number_veh_sent)
                    print('     flow_person_est',flow_person_est[ind])
                    
                    print('     inflows_sched',self.inflows_sched[id_stop, inds_search[ind]]/time_update_flows)
            ...
            # get index of maximum costs over all stops and time range
            ind_target = np.argmax(costs)
            
            # from running index 
            ind_stop_target = int(ind_target/n_searchint)
            #ind_time_arrive = ind_target%n_searchint+inds_time_min[ind_stop_target]
            ind_time_delta = int(ind_target%n_searchint)
            
            # if max cost is not greater zero there is no demand and 
            # therefore distribute randomly
            #print '  ind_stop_target',ind_stop_target
            #print '  costs[ind_stop_target,ind_time_delta]',costs[ind_stop_target,ind_time_delta],costs[ind_stop_target,ind_time_delta] < 0.01
            if costs[ind_stop_target,ind_time_delta] < 0.1:
                ind_stop_target = np.random.randint(n_stop_target)
                while ind_stop_target == ind_stop_current:
                    ind_stop_target = np.random.randint(n_stop_target)
                    
                #print '     random stop ind_stop_target:',ind_stop_target
                
            ind_time_arrive = inds_search[ind_stop_target,ind_time_delta]
            id_stop_target = ids_stop_target[ind_stop_target][0]
            time_depart = process.simtime + ind_time_arrive*self.time_update_flows.get_value()-durations_est[ind_stop_target]
            if 0:
    ...
    def pull_empty_leadvehs(self, ids_veh_lead, process):
        ...
        inds_time_min = n_timeslot_offset+np.array(np.clip(np.array(1.0*durations_est/self.time_update_flows.get_value(),dtype=np.int32),0,self.n_est_max-1-n_timeslot_offset),dtype = np.int32)
...
class PrtService(SimobjMixin, DemandobjMixin, cm.BaseObjman):
        ...
        def _init_attributes(self):
            ...
            prtstrategy = virtualpop.get_strategies().add_strategy('prt',PrtStrategy, np.array([208,50,156,230], np.float32)/255 )
            #self.prttransits =  attrsman.add(\
            #                        cm.ObjConf(prttransits,is_child = False ),
            #                        is_overwrite = False,)
        ...
        def get_writexmlinfo(self, is_route=False, is_plain = False, **kwargs):
            ...
            writefuncs = np.zeros(n_veh, dtype = np.object_)
                ...
                if costs is not None:
                    ...
                    for id_edge_target in ids_edge_target:
                        ...
                        if id_edge_target in costs:
                            stop_to_stop[ids_edge_to_ids_prtstop[id_edge],ids_edge_to_ids_prtstop[id_edge_target]] = costs[id_edge_target]
                        else:
...
class Stopresults(am.ArrayObjman):
    def __init__(self, ident, results, prtstops,
        ...
        attrinfos = [\
                ...
                ('numbers_person_wait', {'name':'waiting person',         'dtype':np.int32 , 'info':'Number of waiting persons at stop over time.'}),
                ('waittimes_tot', {'name':'total wait time',         'dtype':np.float32 , 'info':'Wait times of all waiting persons at a stop over time.'}),
                ]
