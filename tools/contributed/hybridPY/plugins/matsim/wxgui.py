...
class MatsimEdgeDrawings(Polylines):
    ...
    def update(self, is_update = True):
        ...
        for edgeclass, cdata in self.edgeclasses.items():
            ...
            self.colors_fill[self._edges.select_ids(self._edges.types.value ==edgetype)] = getattr(self,'color_'+edgeclass).value
...
class WxGui(    ModuleGui):
    ...
    def on_run_sim(self, event = None):
        ...
        networkexporter = matsim_network.MATSimNetExport(self._matsim, self.get_scenario().workdirpath  + '\\docker-matsim-input\\', logger = self._mainframe.get_logger())
        
        networkexporter.do()

        if os.path.exists(self._matsim.config.population_config):
            shutil.copy(self._matsim.config.population_config, self.get_scenario().workdirpath  + '\\docker-matsim-input\\')
    ...
    def on_matchmaps(self, event = None):
        ...
        result = matsim_mapmatch.match_maps(self._matsim.mapmatch, self.get_scenario().workdirpath, self.get_scenario().net,self._matsim.net,tolerance = 10)
