    ...
    def rework_connections(self,nodes,edges,connections_to_rework):
        ...
        for node in connections_to_rework:
            ...
            for incoming in list_incoming: 
                ...
                new_cord = self._net.edges.get_coord_from_pos(incoming,self._net.edges.get_row(incoming)['lengths']-self._net.nodes.get_row(node)['radii'])
            ...
            for connection in set(connections):
                ...
                edges["I_"+str(node_id_sumo)+"-"+str(start_edge_id_sumo)+"-"+str(end_edge_id_sumo)] = [from_node, to_node,capacity,modes,dist,num_lanes,speed]
    ...
    def network_output(self):
        ...
        for id in self._net.edges.get_ids():
            ...
            if self._net.edges.get_accesslevel(id,4)>= 0:
                ...
                numlanes_with_car_access = sum(1 for x in self._net.lanes.get_accesslevel_lanes(self._net.edges.ids_lanes[id], 4) if x >= 0)
...
### Hier haben wir nun das SUMO Netzwerk als MATSim
        ...
        matsim_set_id =  self.filter_matsim(self._matsim.net.nodes,self._matsim.net.edges,self._net.get_boundaries(is_netboundaries = True),x_border = 3000,y_border = 3000)
...
#############################################################################################################################################################################
        ...
        for edge in edges:
            ...
            types[index] = modes
            shapes_edge[index] = [self._matsim.net.nodes.coords[ids_fromnode_matsim[0]],self._matsim.net.nodes.coords[ids_tonode_matsim[0]]]                
        ...
        if True:
            ...
            fstar, check_ids = self.prepare_routing_matsim(self._matsim.net.nodes,self._matsim.net.edges,self._net.get_boundaries(is_netboundaries = True),x_border = 10000, y_border = 10000)
            ...
            matsim_set_start,matsim_set_end,matsim_set_id,matsim_set_prev, matsim_set_after =  self.np_graph_matsim(self._matsim.net.nodes,self._matsim.net.edges,self._net.get_boundaries(is_netboundaries = True),x_border = 1000,y_border = 1000)
        ...
        for id in edges:
            ...
            simfile.write('\t\t<link id=\"'+ id +'\" from=\"' + from_node +'\" to=\"' + to_node +'\" length=\"' + str(length) +'\" freespeed=\"' + str(speed)+'\" capacity=\"' + str(capacity) +'\" permlanes=\"' + str(num_lanes)+'\" oneway=\"1\" modes=\"' + modes + '\" type=\"secondary\"/>\n')
    ...
    def find_suitable_matsim_node(self,sumo_node, mode = None):
        ...
        if mode == 'incoming_sumo':
            ...
            if distance < dist_node_assignment:
                ...
                out_going_matsim = self._matsim.net.edges.select_ids(self._matsim.net.edges.ids_fromnode.value == closest_matsim_node)
        ...
        if mode == 'outgoing_sumo':
            ...
            if distance < dist_node_assignment:
                ...
                incoming_matsim = self._matsim.net.edges.select_ids(self._matsim.net.edges.ids_tonode.value == closest_matsim_node)
