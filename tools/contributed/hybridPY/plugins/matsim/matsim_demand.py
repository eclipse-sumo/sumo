    ...
    def endElement(self, name):
        ...
        if name == 'plan':
            ...
            if self._is_plan_valid:
                if self._chars != "":
                    self._routes.append(self._ids_edge_matsim.get_ids_from_indices_save(self._chars.split(self._routesep)))
                    #print ('    inserted route',len(self._routes),self._routes[-1])    
                # make sure that there is a route
                else:
                    #insert empty one if necessary
                    self._routes.append([-1]) # here we must put something, otherwise trouble with array conversion below
            ...
            if (n_activity >= 2) & (len(self._routes) == n_activity-1):
                    ...
                    if n_activity_valid > 0: # any valid activity
                        ...
                        self.population[self.id_person] = ( np.array(self._activitytypes, dtype = np.int32)[inds_valid_act], 
                                                            ...
                                                            np.array(self._traveldistances, dtype = np.float32)[inds_valid_leg],
...
class TripImporter(ParserMixin,MatsimProcessMixin,Process):
    def __init__(self,  matsim, planfilepath = '', ids_mode = None,
        ...
        if  (ids_mode is None):
            # no info given, ...
            ids_mode_default  = [modechoices['passenger'], modechoices['bicycle'], modechoices['pedestrian'], modechoices['motorcycle'], modechoices['taxi']]
    ...
    def do(self):
        ...
        for id_mode in ids_mode_used:
            if id_mode != -1: # unknown mode
                self.make_fringenodes(priority_min = self.fringepriority_min, numlanes_min = self.fringenumlanes_min, id_mode = id_mode)
                ...
                if id_mode in self.ids_mode:
                    ...
                    if is_inside1 != is_inside2:
                        ...
                        if is_inside1:
                            ...
                            ids_edge_from, dists = edges.get_closest_edge(  coord_from,  n_best = 10, accesslevels = edges.get_accesslevels(id_mode))
                            ...
                            ids_edge_to, dists = edges.get_closest_edge(coord_to,  n_best = 10,accesslevels = edges.get_accesslevels(id_mode))
                    ...
                    for i in range(1, int(self.scaling_factor)): 
                        if i == 1:
                            ...
                            ids_user.append(id_person+'00'+str(ind_trip)+'00'+str(i))
                            ids_vtype.append(_vtypes.get_vtype_for_mode(id_mode))   
                            times_depart.append(max(0,times_end1+(-20*60+np.random.randint(40*60)))) # time end of previous activity     
                            ids_edge_depart.append(id_edge_from)
                            ids_edge_arrival.append(id_edge_to)
    ...
    def do(self):
        ...
        for id_person, (activitytypes, coords_matsim, times_begin,  times_end, are_inside, ids_mode, traveldistances, traveltimes, routes) in population.items():
            ...
            for ind_act, coord1, is_inside1,  id_activitytype, time_begin, time_end in zip(list(range(len(activitytypes))),coords, are_inside, activitytypes,times_begin,times_end):
                ...
                print ('act at',coord1,is_inside1,'time_begin',time_begin,'time_end', time_end,'id_activitytype',id_activitytype)
                ...
                durations.append(float(time_end - time_end_previous)/3600) # not quite correct because excludes time for transport
            ...
            print ('  id_person',id_person,'id_mode_preferred',id_mode_preferred,'hours_begin',hours_begin,'durations',durations)
            i += 1
...
class TripImporterbasedonMapMatching(ParserMixin,MatsimProcessMixin,Process):
    ...
    def do(self):
        ...
        weights_walk = self._net.edges.get_times_cached(  id_mode = id_mode,
                                                            speed_max = self._scenario.demand.vtypes.speeds_max[myvtypes.get_vtype_for_mode(matsim_sumo_modemap['walk'])],
                                                            is_check_lanes = True,
        ...
        weights_bike = self._net.edges.get_times_cached(  id_mode = id_mode,
                                                            speed_max = self._scenario.demand.vtypes.speeds_max[myvtypes.get_vtype_for_mode(matsim_sumo_modemap['bike'])],
                                                            is_check_lanes = True,
                                                            modeconst_excl = -10.0, modeconst_mix = -5.0,
                                                            )
        ...
        weights_car = self._net.edges.get_times_cached(  id_mode = id_mode,
                                                            speed_max = self._scenario.demand.vtypes.speeds_max[myvtypes.get_vtype_for_mode(matsim_sumo_modemap['car'])],
                                                            is_check_lanes = True,
                                                            modeconst_excl = -10.0, modeconst_mix = -5.0,
                                                            )
        ...
        for id_user_matsim in externaltrips:
            ...
            modenames, triptimes, travtimes, ids_edge_sumo_start, ids_edge_sumo_end, routes,start_x_list, start_y_list, end_x_list,end_y_list,reached_dest = externaltrips[id_user_matsim]
                ...
                if True: #(id_edge_sumo_start in all_sumo_ids) & (id_edge_sumo_end in all_sumo_ids):
                    ...
                    if (h >= hour_begin_ext) & (h < hour_end_ext):
                        ...
                        if modename == 'walk':
                            ...
                            ids_edge_from, dists = self._net.edges.get_closest_edge( [start_x, start_y],  n_best = 10, accesslevels = self._net.edges.get_accesslevels(MATSIM_SUMO_MODEMAP[modename]))
                            # id_edge_from = ids_edge_from[0]
                            # # hier werden teilweise mehrere edges zurück gegeben!
                            
                            ids_edge_to, dists = self._net.edges.get_closest_edge( [end_x,end_y],  n_best = 10, accesslevels = self._net.edges.get_accesslevels(MATSIM_SUMO_MODEMAP[modename]))
                        ...
                        if modename == "car" or modename == "bike":
                            if len(route_sumo) >1:
                                ...
                                A = self._scenario.net.edges.select_ids((self._scenario.net.edges.ids_tonode.value == self._scenario.net.edges.get_row(edge_end)['ids_fromnode']))
                                B = self._scenario.net.edges.select_ids((self._scenario.net.edges.ids_fromnode.value == self._scenario.net.edges.get_row(edge_end)['ids_tonode']))
                        ...
                        if (len(route_sumo) >1 and u_turn == False) and  (modename == "bike" or modename == "car"):
                            ...
                            for i in range(1, int(self.scaling_factor)+1): 
                                if i == 1:
                                    ...
                                    ids_user.append(str(id_user_matsim)+'00'+str(n_trips)+'00'+str(i))
                                    ids_vtype.append(myvtypes.get_vtype_for_mode(matsim_sumo_modemap[modename]))
                                    times_depart.append(max(0,arival_time_in_sumo +(-20*60+np.random.randint(40*60)))) # time end of previous activity     
                                    ids_edge_depart.append(id_edge_sumo_start)
                                    ids_edge_arrival.append(id_edge_sumo_end)
                                    arrival_speeds.append(arrival_speed)
                        ...
                        if modename == "walk":
                            ...
                            for i in range(1, int(self.scaling_factor)+1): 
                                if i == 1:
                                    ...
                                    walk_ids_user.append(str(id_user_matsim)+'00'+str(n_trips)+'00'+str(i))
                                    walk_ids_vtype.append(myvtypes.get_vtype_for_mode(matsim_sumo_modemap[modename]))
                                    walk_times_depart.append(max(0,arival_time_in_sumo +(-20*60+np.random.randint(40*60)))) # time end of previous activity     
                                    walk_ids_edge_depart.append(id_edge_sumo_start)
                                    walk_ids_edge_arrival.append(id_edge_sumo_end)
        ...
        self._scenario.demand.trips.make_trips( ids_vtype , is_generate_ids = False,
                                                ...
                                                ids_edge_depart = ids_edge_depart,
                                                ids_edge_arrival = ids_edge_arrival,
                                                inds_lane_depart = np.ones(n_trips_abs, dtype = np.int32)*db.OPTIONMAP_LANE_DEPART["best"],
                                                positions_depart = np.ones(n_trips_abs, dtype = np.int32)*db.OPTIONMAP_POS_DEPARTURE["base"],
                                                speeds_depart = np.ones(n_trips_abs, dtype = np.int32)*db.OPTIONMAP_SPEED_DEPARTURE["max"],
                                                inds_lane_arrival = np.ones(n_trips_abs, dtype = np.int32)*db.OPTIONMAP_LANE_ARRIVAL["current"],
                                                positions_arrival = np.ones(n_trips_abs, dtype = np.int32)*db.OPTIONMAP_POS_ARRIVAL["random"],
        ...
        self._scenario.demand.trips.make_trips( walk_ids_vtype , is_generate_ids = False,
                                                ...
                                                inds_lane_depart = np.ones(len(walk_ids_user), dtype = np.int32)*db.OPTIONMAP_LANE_DEPART["best"],
                                                positions_depart = np.ones(len(walk_ids_user), dtype = np.int32)*db.OPTIONMAP_POS_DEPARTURE["base"],
                                                speeds_depart = np.ones(len(walk_ids_user), dtype = np.int32)*db.OPTIONMAP_SPEED_DEPARTURE["max"],
                                                inds_lane_arrival = np.ones(len(walk_ids_user), dtype = np.int32)*db.OPTIONMAP_LANE_ARRIVAL["current"],
                                                positions_arrival = np.ones(len(walk_ids_user), dtype = np.int32)*db.OPTIONMAP_POS_ARRIVAL["random"],
                                                speeds_arrival = np.ones(len(walk_ids_user), dtype = np.int32)*db.OPTIONMAP_SPEED_ARRIVAL["current"],
                                                 )                                  
        return True
    ...
    def plot_bike_routing_problem(self,line_start,line_end):
        ...
        # Plot the rectangle
        rectangle = patches.Rectangle([x_min,y_min], rect_width, rect_height, linewidth=2, edgecolor='blue', facecolor='none')
            ...
            #handle unrouted    
                ...
                if self.multi_modal_import:
                    ...
                    if (self._leg_mode == 'walk' or self._leg_mode == 'bike')  and self._new_plan ==False: 
                        ...
                        traveled_distance = np.linalg.norm(np.array([float(self.end_x),float(self.end_y)])-np.array([float(self.start_x),float(self.start_y)])) *2
                        ...
                        if relevant_pre_test_1 and relevant_pre_test_2:
                            ...
                            is_relevant, start_coord, end_coord = self.line_rect_intersection(np.array([float(self.start_x),float(self.start_y)]),np.array([float(self.end_x),float(self.end_y)]),np.array([self._boundaries[0],self._boundaries[1]]),np.array([self._boundaries[2],self._boundaries[3]]))
    ...
    def endElement(self, name):
        if name == 'person':
            ...
            if self.id_person is not None:
                self.population[self.id_person] = (self.modes, self.dep_times,self.trav_times, self.entrance_points, self.exit_points, self.route,self.start_x_list, self.start_y_list, self.end_x_list,self.end_y_list,self.reached_dest)
