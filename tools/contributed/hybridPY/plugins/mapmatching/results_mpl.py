    ...
    def show(self):
        ...
        for  id_trip, ids_point, i  in zip(trips.get_ids(),ids_points, list(range(len(ids_trip)))): 
            ...
            if self.select_points == 1:
                points.are_selected[ids_point] = is_point_in_polygon(points.coords[ids_initial_point[i]], zone_shape_origin)
            ...
            if self.select_points == 3:
                points.are_selected[ids_point] = is_point_in_polygon(points.coords[ids_initial_point[i]], zone_shape_origin)*is_point_in_polygon(points.coords[ids_final_point[i]], zone_shape_dest)
...
##                    times_point[id_point] = time
        ...
        if   self.plot_type == 2:
            ...
            if self.titletext != '':
                ...
                if self.select_points == 3:
                    ax.set_title('Density heatmap of trip points from the origin zone to the destination zone', fontsize=self.size_titlefont)
                if self.select_points == 4:
                    ax.set_title('Density heatmap of all trip points', fontsize=self.size_titlefont)

            plt.imshow(list(zip(*reversed(list(zip(*reversed(list(zip(*reversed(H))))))))),vmin = self.vmin,cmap = 'Reds', extent= extent, interpolation='bicubic', alpha = 0.9)
            plt.colorbar()
            if self.is_save:    
                if self.select_points == 1:
...
##                                        ))
    ...
    def plot_times_wait(self, ax):
        ...
        ids_noderes = nodesresults.select_ids(nodesresults.numbers_matched_for_speed_analysis.get_value()>self.n_min_matched)
...
##        #colors = np.array(COLORS,dtype = np.object_)
...
class SpeedprofilePlotter(PlotoptionsMixin, Process):
    ...
    def plot_speed_over_time(self,ax,id_trip, id_route, edges, i_min = None,i_max=None, 
        ...
        if is_scipy & (not (self.method_interp == 'linear')):
            ...
            ax.plot(x, y, 'o-',markersize = self.size_point,color = self.color_line, lw = self.width_line, markerfacecolor = self.color_point ,alpha=alpha  )
        ...
        if self.id_trip >= 0:
            ...
            if self.is_plot_similar_trips:
                ...
                for id_trip_speed, id_route_speed, route_speed, ids_pointedge_speed in zip(\
                    ...
                    if is_sublist(route_speed,route):#is_sublist(route,route_speed):# |  is_sublist(route_speed,route):
                        ...
                        while (ids_pointedge_speed[j] == id_pointedge_last)&(j<n_pointedge-1):
                            j += 1
                        self.plot_speed_over_way(ax, id_trip_speed, id_route_speed, edges, i,j, is_pointlabel=False, alpha = 0.5)
                        self.plot_speed_over_time(ax2, id_trip_speed, id_route_speed, edges, i,j, is_pointlabel=False, alpha = 0.5)
...
class EdgeresultPlotter(PlotoptionsMixin, Process):
    ...
    def plot_speeds_average(self, ax):
        ...
        #ids_result = edgesresults.get_ids()
        ids_result = edgesresults.select_ids(edgesresults.numbers_matched_for_speed_analysis.get_value()>self.n_min_matched)
...
class ConnectionresultPlotter(PlotoptionsMixin, Process):
    ...
    def plot_times_wait(self, ax):
        ...
        ids_result = connectionsresults.select_ids(connectionsresults.numbers_matched_for_speed_analysis.get_value()>self.n_min_matched)
        #resultattrconf = getattr(self.parent.edgesresults, self.edgeattrname)
...
##        dep_arr_times = np.sort(dep_arr_times.sort, axis = 0)   
    ...
    def plot_tldensity(self):
        ...
        ax = fig.add_subplot(111)
        ids_valid = routesresults_matched.select_ids(np.logical_and(routesresults_shortest.distances.get_value()>0, routesresults_matched.distances.get_value()>0))
        ...
        bins = np.linspace(x_min,x_max,self.n_bins)
        bincenters = self.plot_hist(ax, matched, bins = bins, color = self.color_matched, label = 'matched:'+'$\mu = %.2f/km$, $\sigma=%.2f/km$'%(np.mean(matched),np.std(matched)))
        bincenters = self.plot_hist(ax, shortest, bins = bins, color = self.color_shortest, label = 'shortest:'+'$\mu = %.2f/km$, $\sigma=%.2f/km$'%(np.mean(shortest),np.std(shortest)))
    ...
    def plot_lowpriorityshare(self):
        ...
        bins = np.linspace(x_min,x_max,self.n_bins)
        bincenters = self.plot_hist(ax, matched, bins = bins, color = self.color_matched, label = 'matched:'+'$\mu = %.2f$%%, $\sigma=%.2f$%%'%(np.mean(matched),np.std(matched)))
        bincenters = self.plot_hist(ax, shortest, bins = bins, color = self.color_shortest, label = 'shortest:'+'$\mu = %.2f$%%, $\sigma=%.2f$%%'%(np.mean(shortest),np.std(shortest)))
    ...
    def plot_mixshare(self):
        ...
        ax.grid(self.is_grid)
        if self.is_title:
            ax.set_title('Share of mixed reserved access roads of matched and shortest route', fontsize=self.size_titlefont)
    ...
    def plot_lengthdistrib(self):
        ...
        x_max = max(np.max(dists_match),np.max(dists_shortest))
        bins = np.linspace(x_min,x_max,self.n_bins)
        bincenters = self.plot_hist(ax, dists_match, bins = bins, color = self.color_matched, label = 'matched:'+'$\mu = %dm$, $\sigma=%dm$'%(np.mean(dists_match),np.std(dists_match)))
        bincenters = self.plot_hist(ax, dists_shortest, bins = bins, color = self.color_shortest, label = 'shortest:'+'$\mu = %dm$, $\sigma=%dm$'%(np.mean(dists_shortest),np.std(dists_shortest)))
    ...
    def plot_timedistrib(self):
        ...
        bins = np.linspace(x_min,x_max,self.n_bins)
        bincenters = self.plot_hist(ax, times_match, bins = bins, color = self.color_matched, label = 'matched:'+'$\mu = %dm$, $\sigma=%dm$'%(np.mean(times_match),np.std(times_match)))
        bincenters = self.plot_hist(ax, times_fastest, bins = bins, color = self.color_fastest, label = 'fastest:'+'$\mu = %dm$, $\sigma=%dm$'%(np.mean(times_fastest),np.std(times_fastest)))
    ...
    def plot_lengthprob(self):
        ...
        bincenters = self.plot_hist(ax, dists_shortest, bins = bins, 
                                    ...
                                    label = 'shortest:'+'$\mu = %dm$, $\sigma=%dm$'%(np.mean(dists_shortest),np.std(dists_shortest)),
                                    is_rel_frequ = True,
                                    is_percent = True,
                                    )
    ...
    def plot_lengthoverlap_fastest(self):
        ...
        ids_valid = routesresults_matched.select_ids(np.logical_and(routesresults_fastest.distances.get_value()>0, routesresults_matched.distances.get_value()>0))
...
#-------------------------------------------------------------------------------
    ...
    def plot_lengthratio_nonoverlap(self):
        ...
        values = routesresults_shortest_nonoverlap.distances[ids_valid]/routesresults_matched_nonoverlap.distances[ids_valid]
        bincenters = self.plot_hist(    ax, values,
                                        bins = bins, histtype = 'bar', 
                                        label = r'$\mu = %.2f$, $\sigma=%.2f$'%(np.mean(values),np.std(values))
                                        )
    ...
    def plot_tldensity_nonoverlap(self):
        ...
        ax = fig.add_subplot(111)
        ids_valid = routesresults_matched.select_ids(np.logical_and(routesresults_shortest_nonoverlap.distances.get_value()>0, routesresults_matched_nonoverlap.distances.get_value()>0))
    ...
    def plot_prioritychangedensity_nonoverlap(self):
        ...
        ax.grid(self.is_grid)
        if self.is_title:
            ax.set_title('Priority change dens. of non-overlapping matched and shortest route', fontsize=self.size_titlefont)
    ...
    def plot_lowpriorityshare_nonoverlap(self):
        ...
        ax.grid(self.is_grid)
        if self.is_title:
            ax.set_title('Share of low priority roads of non-overlapping matched and shortest route', fontsize=self.size_titlefont)
    ...
    def plot_exclusiveshare_nonoverlap(self):
        ...
        ax.grid(self.is_grid)
        if self.is_title:
            ax.set_title('Share of exclusive access roads of non-overlapping matched and shortest route', fontsize=self.size_titlefont)
    ...
    def plot_mixshare_nonoverlap(self):
        ...
        ax.grid(self.is_grid)
        if self.is_title:
            ax.set_title('Share of mixed reserved access roads of non-overlapping matched and shortest route', fontsize=self.size_titlefont)
...
class PtFlowdigramPlotter(PlotoptionsMixin, Process):
    ...
    def show(self):
        ...
        ids_stoptuple_line, tripnumbers_line = ptlinesresults.get_flowdiagramm(self.id_line, is_add_similar = self.is_add_similar)
        ...
        if len(self.csvfilepath)>0:
            # export to csv
            f = open(self.csvfilepath, 'w', encoding="utf-8")
            ...
            for ids_stoptuple, number in zip(ids_stoptuple_line,tripnumbers_line):
                f.write(ptstops.stopnames_human[ids_stoptuple[0]]+sep+ptstops.stopnames_human[ids_stoptuple[1]]+sep+str(number)+'\n')
