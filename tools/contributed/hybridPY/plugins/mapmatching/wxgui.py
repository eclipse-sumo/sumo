...
class GpsPointsDrawings(Circles):
    ...
    def set_netelement(self, gpspoints):
        print('GpsPointsDrawings.set_netelement')
...
class GpsRoutesDrawings(Polylines):
    ...
    def set_netelement(self, routes):
        print('GpsRoutesDrawings.set_netelement')
        self._routes = routes
...
class WxGui(    ModuleGui):
    ...
    def refresh_widgets(self):
        ...
        if self._scenario != scenario:
            ...
            self._mapmatching =  scenario.demand.add_demandobject(ident = 'mapmatching', DemandClass = mapmatching.Mapmatching)
        ...
        if is_refresh | self._is_needs_refresh:
            ...
            id_pointsdraws = drawing.set_element('gpspointsdraws', GpsPointsDrawings, 
                                    self._mapmatching.points, layer = 150)
            drawing.get_drawobj_by_ident('gpspointsdraws').set_visible(False)
            
            id_routesdraws = drawing.set_element('gpsroutesdraws', GpsRoutesDrawings, 
                                    self._mapmatching.trips.get_routes(), layer = 149)
            drawing.get_drawobj_by_ident('gpsroutesdraws').set_visible(False)
...
#-------------------------------------------------------------------------------
        ...
        menubar.append_item( 'plugins/mapmatching/delete/delete GTFS',
                self.on_clear_gtfs, 
                bitmap = wx.ArtProvider.GetBitmap(wx.ART_DELETE,wx.ART_MENU),
                ) 
...
#-------------------------------------------------------------------------------
        ...
        menubar.append_item( 'plugins/mapmatching/GTFS/delete GTFS',
                self.on_clear_gtfs, 
                bitmap = wx.ArtProvider.GetBitmap(wx.ART_DELETE,wx.ART_MENU),
                )         
#-------------------------------------------------------------------------------
    ...
    def on_clear_gtfs(self, event = None):
        """
        Clear GTFS related data, these are lookup tables.
        """
        self._mapmatching.clear_gtfs()
        self._mainframe.browse_obj(self._mapmatching)
