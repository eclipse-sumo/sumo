...
BERTHSTATES = {'free':0,'waiting':1,'boarding':2,'alighting':3,'allocated':4,'waiting_occupied':5, 'waiting_routed':6}

# 
VEHICLESTATES = {'init':0,'waiting':1,'boarding':2,'alighting':3,'emptytrip':4,'occupiedtrip':5,'forewarding':6,'await_forwarding':7,'forewarding_enter':8,'boarding_completed':9,'forewarding_exit':10,'forewarding_passthrough':11,'forewarding_exit_kickout':12,'kickout_berth':13}
                ...
                if (ids_edge_from is None)|(ids_edge_to is None):
                    ...
                    if self.is_prt_only(ids_lane1, lanes) & self.is_prt_only(ids_lane2, lanes):
                        ...
                        #                        )
                            ...
                            print('      branch 1:')
                            id_node_up1, dist1 = self.search_upstream_merge(id_edge1, edges,lanes, id_prtmode, distances = distances)
                            print('        id_node_up1',id_node_up1,'dist1',dist1)
                            
                            print('\n      branch 2:')
                            id_node_up2, dist2 = self.search_upstream_merge(id_edge2, edges,lanes, id_prtmode, distances = distances)
        ...
        if indpos == -1:
            ...
            if (ind_insert == 0) | self.are_station[id_merge_to]:
                ...
                for id_veh_merged, id_veh_merged_sumo, odo, lineind_merge in zip(ids_vehs_merged[::-1],ids_vehs_merged_sumo[::-1],odos_veh_merged[::-1],lineinds_vehs_merged[::-1]):
                    ...
                    if lineind_merge == 1:
                        dist_tomerge_head = self.distances_node_in1[id_merge_to] - (get_traci_odo(id_veh_merged_sumo)-odo) 
                    else:
                        dist_tomerge_head = self.distances_node_in2[id_merge_to] - (get_traci_odo(id_veh_merged_sumo)-odo) 
...
class PrtPlatoonStops(am.ArrayObjman):
        ...
        for id_stop,id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
            ...
            if self.debug:
                ...
                for id_veh, state, id_berth, route in zip(ids_veh, vehicles.states[ids_veh], vehicles.ids_berth[ids_veh],vehicles.routes[ids_veh] ):
                    print('    id_veh',id_veh,'state',state,'id_berth',id_berth,'route',route)#,'len(route)',len(route)
                        ...
                        for id_veh, id_berth in zip(ids_veh_to_allocate, ids_berths):
                            ...
                            if id_berth > -1:
                                ...
                                vehicles.control_berth_position_enter(id_veh, id_berth,\
                                            ...
                                            position = berths.stoppositions[id_berth]-0.3,# needs to stop a bit before to move in
                                            laneindex = 2,
                                            )
    ...
    def get_vehicles_ready_for_departure(self, id_stop):
        ...
        while  is_cont & (i < n_veh):
            ...
            if  id_stopedge_target == -1:# target not yet defined
                ...
                if self.debug:
                    print('   %d Leader prt.%d'%(i,id_veh),'id_stopedge_target',id_stopedge_target)#,'is_stopped',is_stopped
    ...
    def try_start_vehicles(self, id_stop, simtime, n_plat_min = -1):
        ...
        n_plat_max = self.capas_inqueue[id_stop] - (len(self.ids_vehs_out[id_stop])+len(self.ids_vehs_ready_for_departure[id_stop])) 
        if self.debug:
        ...
        if n_veh_pass_wait>0 :
            ...
            self._kick_out_of_berth(id_stop, ids_veh[inds_start], np.array(ids_berth_veh_wait_exit, dtype = np.int32)[inds_start], vehicles, berths)
        ...
        if n_veh_all > 0:
            ...
            if n_veh_pass > 0:
                ...
                if (n_veh_plat >= n_plat_min) | (np.max(inds_veh_plat) >= n_veh):
                    ...
                    self._kick_out_of_berth(id_stop, ids_veh_ready[inds_veh_plat_nodeviate[iinds_plat_nodeviate_berth]], ids_berth_ready[inds_veh_plat_nodeviate[iinds_plat_nodeviate_berth]], vehicles, berths)
    ...
    def enter(self, id_stop, ids_veh, fase, simtime):
        ...
        if n_veh_completed > 0:
            ...
            if self.debug:
                print('    fase',fase)#,(len(self.ids_vehs_inqueue[id_stop]) == 0),((simtime - self.times_phase_in[id_stop]) > self.time_fase_in_max.get_value())
        ...
        for id_veh, state, id_berth, is_completed in zip(ids_veh,vehicles.states[ids_veh], ids_berth, are_completed):    
            ...
            if not is_completed:
                ...
                route, duration = self.parent.get_route(  id_stopedge, self.ids_stop_to_ids_edge[vehicles.ids_stop_target[id_veh]])
...
class PrtVehicles(am.ArrayObjman):
    ...
    def add_ghost(self, id_veh, id_ghost, dist_to_merge_veh, dist_to_merge_ghost, 
        ...
        #else:
        # add new ghost
        print('add_ghost veh prt.%d ghost prt.%d dtm = %.1f dtmg = %.1f d0=%.1f'%(id_veh, id_ghost,dist_to_merge_veh,dist_to_merge_ghost, dist_to_merge_veh - dist_to_merge_ghost))#,self.ids_ghosts.shape
        ...
        if 1:
            ...
            for id_veh,id_sumo, velocity, velocitiy_ghost_min,\
                ...
                if id_veh in ids_debug:
                    ...
                    dist_min_check = dist0 + delta_ghost - delta_veh-self.lengths_plat[self.ids_ghosts[id_veh][0]] - self.length
...
class HcPrtStrategy(StrategyMixin):
        ...
        def plan(self, ids_person, logger = None):
            ...
            while len(ids_person_act)>0:
                ...
                ids_stop_from, ids_stopedge_from, ids_stop_to, ids_stopedge_to = prtstops.get_closest_for_trip(centroids_from,centroids_to, ids_person = ids_person_act)
...
class VehicleMan(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        for id_stop in self.ids_stop:
            self.flows_stop_max[id_stop] = self.get_stops().get_capacity(id_stop) /(3600.0/self.time_update_flows.get_value()) 
    ...
    def process_step(self, process):
        ...
        is_cont = True
        while is_cont:
            surpluses_veh, surpluses_veh_urgent,  demands_veh, balances_veh,n_move_eff_total  = self.equalize_empty_vehicles(process)
    ...
    def fit_vehicles(self,id_stop, n_veh, ind_time_arrival):
        ...
        is_fit = False
        if self.debug > 1:
            print('fit_vehicles n_veh',n_veh,'for n_slots',n_slots,'capa*slots %.2f'%(capa * n_slots),'at slot ind',ind_time_arrival,'n_slots search inerv.',n_slots_search,'out of range',(ind_time_arrival + ind_delay + n_slots < self.n_est_max))
    ...
    def push_out_vehicles(self, process):
        ...
        for id_stop, id_stop_target, duration_est, n_veh\
            ...
            print('.'*79)               
            print('    >>>check id_stop',id_stop, 'id_stop_target',id_stop_target,'n_veh',n_veh,'dur %.2f'%duration_est,'t_dep',time_depart,'simtime',simtime)
            ...
            if time_depart != -1:
                ...
                if ind_time_arrival < self.n_est_max:
                    ...
                    if n_alloc > 0:
                        ...
                        self.inflows_sched[id_stop_target, ind_time_arrival_sched:ind_time_arrival_sched+n_alloc] += flows_alloc
                        ...
                        if ind_time_arrival_sched == ind_time_arrival:
                            ...
                            self.times_departure_sched[id_stop] = simtime + ind_time_arrival_sched * time_update_flows - duration_est
        ...
        if (np.max(balances_veh)>0)&(max(surpluses_veh)>0):
            ...
            for id_stop_from, ind_from  in zip(ids_stop[inds_sorted[::-1]],inds_sorted[::-1]):
                ...
                if (surplus_from >= 1) & (balance_from >= 1):
                    if self.debug > 5:
                        print('\n    potential emitter stop: id_stop_from',id_stop_from,'surplus_from',surplus_from,'balance_from',balance_from)#,'ind_from',ind_from,'continue?',(surplus_from >= 1) & (balance_from >= 1)
                    ...
                    if (n_move>0) & (utility_max>0):
                        ...
                        n_move_eff = stops.route_empties(id_stop_from, ids_stop[inds_sorted][ind_max], numbers_move[ind_max],simtime)
                        if self.debug > 2:
...
#class HcPrtService(DemandobjMixin, cm.BaseObjman):
        ...
        def _init_attributes(self):
            ...
            prtstrategy = virtualpop.get_strategies().add_strategy('hcprt',HcPrtStrategy, np.array([208,50,156,230], np.float32)/255 )
            #self.prttransits =  attrsman.add(\
            #                        cm.ObjConf(prttransits,is_child = False ),
            #                        is_overwrite = False,)
        ...
        def update_edgetimes(self):
            ...
            for id_edge, id_edge_sumo in zip(ids_edge, self.get_scenario().net.edges.ids_sumo[ids_edge]):
                ...
                print('  id_edge_sumo %s t_before=%d, t_now=%d t_after=%d'%(id_edge_sumo,self.edgetimes[id_edge],t_current,self.constant_edgetime * self.edgetimes[id_edge] + (1. - self.constant_edgetime) * t_current))
        
                self.edgetimes[id_edge] = self.constant_edgetime * self.edgetimes[id_edge] + (1. - self.constant_edgetime) * t_current
