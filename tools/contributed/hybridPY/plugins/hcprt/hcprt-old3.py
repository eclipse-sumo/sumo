            ...
            if len(ids_veh_entered_sumo) > 0:
                ...
                for id_veh_entered, id_veh_entered_sumo, id_leader, length_plat\
                    ...
                    if (length_plat < 0.001) & (id_leader == -1):
                        ...
                        print('    dispatch id_veh_entered_sumo %s' %
                              id_veh_entered_sumo, id_targetedge_sumo, ids_edge_sumo_target)
            ...
            if 1:
                ...
                for ind_queue, queue, queue_alloc, \
                    ...
                    if 0:
                        ...
                        print('      simtime-releasetime_queue', simtime-releasetime_queue,
                              (simtime - releasetime_queue > 15), (simtime - self.releasetimes[id_comp] > 10))
                        print('      queue', queue, id(queue))
                        print('      ids_targetedge_sumo', ids_targetedge_sumo)
                        print('      queue_alloc', queue_alloc)
                        print('      releasetime_queue', releasetime_queue, simtime - releasetime_queue,
                              simtime - releasetime_queue > self.time_accumulation_max.get_value())
...
class Mergenodes(am.ArrayObjman):
    ...
    def make_from_net(self):
        ...
        for id_node, ids_edge_from, ids_edge_to, id_type in zip(
            ...
            if True:
                ...
                if (ids_edge_from is None) | (ids_edge_to is None):
                    ...
                    print('    one in, 2 out => diverge node, access is_prt_in', is_prt_in,
                          'is_platform_in', self.is_platform(ids_lane_in, lanes))
                    ...
                    if self.is_platform(ids_lane_in, lanes):
                        ...
                        print('    out id_edge1', id_edge1, 'ids_lane1', ids_lane1,
                              'is_prt_only', self.is_prt_only(ids_lane1, lanes))
                        print('    out id_edge2', id_edge2, 'ids_lane2', ids_lane2,
                              'is_prt_only', self.is_prt_only(ids_lane2, lanes))
                        # if self.is_prt_only(ids_lane1, lanes) & self.is_prt_only(ids_lane2, lanes):
        ...
        while i < len(ids):
            ...
            print(79*'-')
            print('  check entry of id_merge', id_merge, 'id_node',
                  self.ids_node[id_merge], ' no decomp', self.ids_node[id_merge] not in ids_node_decompressorout)
            if (id_node_in1 > -1) & (id_node_in2 > -1)\
                    & (self.ids_node[id_merge] not in ids_node_decompressorout):

                print('    id_node_in1', self.ids_node_in1[id_merge], nodes.ids_sumo[self.ids_node_in1[id_merge]], self.ids_node.has_index(
                    self.ids_node_in1[id_merge]))
                print('    id_node_in2', self.ids_node_in2[id_merge], nodes.ids_sumo[self.ids_node_in2[id_merge]], self.ids_node.has_index(
                    self.ids_node_in2[id_merge]))
                if 1:
                    ...
                    print('    station check id_merge', id_merge, 'id_merge_in1', id_merge_in1, 'station',
                          self.are_station[id_merge_in1], 'id_merge_in2', id_merge_in2, 'station', self.are_station[id_merge_in2])
    ...
    def prepare_sim(self, process):
        ...
        self.odos_vehs_merged = np.zeros(n_id_max, dtype=np.object_)
    ...
    def process_step(self, process):
        ...
        for id_merge, id_node, id_edge_out, ids_node_out, ids_veh_out_sumo, is_entry in\
            ...
            if ids_veh_out_sumo != ids_veh_sumo:
                ...
                if id_veh_first > -1:
                    if debug > 0:
                        print('    platoon waiting for exit: id_veh_first prt.%d, id_veh_last prt.%d' %
                              (id_veh_first, id_veh_last))
                ...
                if len(inds_entered) > 0:
                    for ind_entered, id_veh in zip(inds_entered, ids_veh_entered[inds_entered]):
                        ...
                        if debug > 0:
                            print('    >>try exiting vehicle', id_veh_sumo, 'is_leader',
                                  vehicles.ids_leader[id_veh] == -1, 'ids_node_out', ids_node_out, 'ids_merge_out', ids_merge_out)
                        ...
                        if ids_node_out is not None:
                            ...
                            if (id_veh_last != id_veh):
                                if id_veh_last not in ids_veh_entered:
                                    ...
                                    if debug:
                                        print('    last plat vehicle', id_veh_last,
                                              'after merge node, exit', id_veh_sumo)
                            ...
                            if len(ids_merge_out) > 0:
                                ...
                                if debug:  # debug>0:
                                    print('      route_sumo', route_sumo)
                                    print('      routeinds', routeinds,
                                          'downstream merge', routeinds[mergeind] < INFINT)
                                    # print '      ids_edge_mergeout_sumo',ids_edge_mergeout_sumo
    ...
    def exit_veh(self, id_veh, id_veh_sumo, id_merge_from, vehicles, is_remove_from_control=False, debug=0):
        print('exit_veh id_veh %s, id_merge_from %d ' % (id_veh_sumo, id_merge_from), 'entry',
              self.are_entry[id_merge_from], 'is_remove_from_control', is_remove_from_control)
        ...
        if id_veh in self.ids_vehs_merged[id_merge_from]:
            ...
            if len(self.ids_vehs_merged[id_merge_from]) > ind_pos+1:
                ...
                if ind_pos > 0:
                    ...
                    if debug:
                        print('   now let the vehicle behind %d, d=%.2f' % (id_veh_behind, dist_to_merge_behind),
                              'see the vehicle in front %d, d=%.2f' % (id_veh_infront, dist_to_merge_infront))
                ...
                if debug > 0:
                    print('    check ghosts of prt.%d (no ghosts from this merge?):' %
                          id_veh, vehicles.ids_ghosts[id_veh])
            ...
            if lineind == 1:
                ...
                if self.are_entry[id_merge_from]:
                    if debug:
                        print('    vehicle is involved in entry merge processes?',
                              id_veh in self.vehicles_mains[id_merge_from])
                    ...
                    if id_veh in self.vehicles_mains[id_merge_from]:
                        if id_veh in self.vehicles_entries[id_merge_from]:
                            ...
                            if 'id_veh_infront' in state:
                                ...
                                print('      should have this veh in front:', id_veh_infront_sumo,
                                      'leader', id_veh_infront_sumo_eff, 'd=%.1fm' % dist_eff)
    ...
    def enter_veh(self, id_veh, id_veh_sumo, id_merge_from, id_merge_to, id_edge_merge_sumo, vehicles, debug=0):
        ...
        if indpos == -1:
            ...
            print('  merge prt.%d' % id_veh, 'lineind', lineind, 'dtm %.1fm' % dist_tomerge_head_new,
                  'n_plat', vehicles.get_platoonsize(id_veh), vehicles.lengths_plat[id_veh]/vehicles.length)
            ...
            if (ind_insert == 0) | self.are_station[id_merge_to]:
                ...
                if debug > 0:
                    ...
                    print('   compare dist_tomerge_head_new', dist_tomerge_head_new,
                          'dist_tomerge_head_new2', dist_tomerge_head_new2)
                ...
                if debug > 0:
                    print('    new prt.%d arriving from in %d at dist head %.2f /__| tail prt.%d %.2fm' %
                          (id_veh, lineind, dist_tomerge_head_new, id_veh_tail_new, dist_tomerge_tail_new))
                ...
                for id_veh_merged, id_veh_merged_sumo, odo, lineind_merge in zip(ids_vehs_merged[::-1], ids_vehs_merged_sumo[::-1], odos_veh_merged[::-1], lineinds_vehs_merged[::-1]):
                    ...
                    if debug > 0:
                        dist_tomerge_head2 = get_traci_distance(id_veh_merged_sumo, id_edge_merge_sumo, 3.0)
                        print('      compare', id_veh_merged_sumo, ' dist_tomerge_head %.1f' %
                              dist_tomerge_head, 'dist_tomerge_head2 %.1f' % dist_tomerge_head2)
                    ...
                    if debug > 0:
                        print('      M:odo_enter=%.1f odo=%.1f delta=%.f dist_node=%.1f line%d platl=%d' % (odo, get_traci_odo(
                            id_veh_merged_sumo), get_traci_odo(id_veh_merged_sumo)-odo, dist_node, lineind_merge, vehicles.lengths_plat[id_veh_merged]))
                ...
                if is_insert:
                    ...
                    if ind_insert == len(ids_vehs_merged):
                        if debug > 0:
                            print('    appended vehicle after veh', id_veh_tail_sumo, 'with leader',
                                  ids_vehs_merged_sumo[ind_insert-1], 'dtm=%.2fm' % dist_tomerge_tail)
                        # V
                        # |
                        # G ind_insert-1
                        ...
                        if debug > 0:
                            print('    vehicle will be inserted in front of', ids_vehs_merged_sumo[ind_insert], 'and in behind',
                                  id_veh_tail_sumo, 'with leader', ids_vehs_merged_sumo[ind_insert-1], 'dtm=%.2fm' % dist_tomerge_tail)
                    ...
                    if debug > 0:
                        print('    prepend prt.%d in front of prt.%d, first index %d' %
            ...
            if id_veh2 == -1:
                ...
                if state2['status'] == 'sync':
                    ...
                    if 'id_veh_behind' in state2:
                        ...
                        id_veh_tail_sumo = vehicles.ids_sumo[id_veh_tail]
                        dist_tomerge_tail = get_traci_distance(id_veh_tail_sumo, id_edge_out_sumo, 3.0)
                        print('  let look the new vehicle prt.%d look at the tail of the entering vehicle prt.%d' %
                              (id_veh, id_veh_tail))
                        vehicles.add_ghost(id_veh, id_veh_tail, dist_tomerge, dist_tomerge_tail)
            ...
            if state['status'] == 'wait':
                        ...
                        while (i < n1) & (not is_found):
                            ...
                            if True:  # id_veh1 not in vehicles_main:
                                ...
                                p_from, p_to = self.get_pos_crit_entry(pos2,
                                                                       ...
                                                                       vehicles)
                                if debug:
                                    print('    potential veh %s (tail %s)' % (id_veh1_sumo, id_veh1_tail_sumo),
                                          'at pos1_tail=%.1f>p_to=%.1f' % (pos1_tail, p_to), pos1_tail > p_to)
                                ...
                                if pos1_tail > p_to:
                                    ...
                                    if i == n1-1:  # last vehicle on main
                                        if debug:
                                            print('      insert id_veh', id_veh_sumo, 'behind id_veh1',
                                                  id_veh1_sumo, 'the only veh on main')
                                        ...
                                        if debug:
                                            print('      vehicle behind', id_veh_behind_sumo,
                                                  'pos=%.1f, p_from=%.1f' % (pos1, p_from), 'ok', pos1 < p_from)
                                        if pos1 < p_from:
                                            state['id_veh_infront'] = id_veh1
                                ...
                                # has not yet reached the critical point p_from
                                    ...
                                    if i == 0:  # first vehicle on main
                                        if debug:
                                            print('      insert id_veh', id_veh_sumo, 'in front of id_veh1',
                                                  id_veh1_sumo, 'the only veh on main')
                ...
                if traci.vehicle.getSpeed(id_veh_sumo) > 0.9*vehicles.speed_max.get_value():
                    ...
                    if 'id_veh_infront' in state:
                        ...
                        if debug > 0:
                            print('      add ghost to entering veh', id_veh_sumo, ' behind',
                                  id_veh_infront_tail_sumo, 'with leader', id_veh_in_front)
                    ...
                    if 'id_veh_behind' in state:
                        ...
                        if debug > 0:
                            print('      add ghost to mainline veh', id_veh_behind_sumo,
                                  ' behind', id_veh_tail_sumo, 'with leader', id_veh_sumo)
                    ...
                    if 0:
                        ...
                        print('    sync of veh', id_veh_sumo, ',v=%.1f, not yet reached: %.2f' %
                              (speed, speed/vehicles.speed_max.get_value()))
...
class PrtPlatoonStops(am.ArrayObjman):
    ...
    def get_closest_for_trip(self, coords_from, coords_to, walkspeeds=None, ids_person=None):
        ...
        for coord_from, coord_to, walkspeed in zip(coords_from, coords_to, walkspeeds):
            ...
            ind_from, ind_to = divmod(np.argmin(times_door_to_door), n_stops)
    ...
    def prepare_sim(self, process):
        ...
        self.are_back_to_move_in = np.zeros(np.max(ids)+1, dtype=np.bool_)
        ...
        for id_stop, ids_board, length_stopedge in zip(ids, self.ids_berths[ids], net.edges.lengths[ids_stopedge]):
            ...
            if (length_stopedge-lastberthstoppos) > stoplinegap+1:
                ...
                print('  Limit:id_stop', id_stop, 'length_stopedge',
                      length_stopedge, 'stopline', self.stoplines[id_stop])
        ...
        self.ids_vehs_routed = np.zeros(np.max(ids)+1, dtype=np.object_)
        ...
        self.ids_berths_veh_wait_exit = np.zeros(np.max(ids)+1, dtype=np.object_)

        # pass through vehicles in input queue waiting for exit
        self.ids_vehs_pass_wait_exit = np.zeros(np.max(ids)+1, dtype=np.object_)

        # vehicles which move sidewards out of the berth
        self.ids_vehs_berth_exit = np.zeros(np.max(ids)+1, dtype=np.object_)

        # vehicles in the output queue ready for departure
        self.ids_vehs_out = np.zeros(np.max(ids)+1, dtype=np.object_)

        # should positions of vehicles in output queue be updated
        self.are_reposition_out = np.zeros(np.max(ids)+1, dtype=np.bool_)

        # vehicles in output queue already concatenated and ready for departure
        # and waiting for vehicle management to be lauched
        self.ids_vehs_ready_for_departure = np.zeros(np.max(ids)+1, dtype=np.object_)
        ...
        self.ids_vehs_inqueue = np.zeros(np.max(ids)+1, dtype=np.object_)
        #
    ...
    def process_step(self, process):
        ...
        for id_stop, id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
            ...
            if id_stop in self._ids_stop_debug:
                self.debug = 1
            else:
                self.debug = 0
            ...
            if self.debug:
                ...
                print('    ids_berths_to_allocate', type(
                    self.ids_berths_to_allocate[id_stop]), self.ids_berths_to_allocate[id_stop])
                ...
                for id_veh, state, id_berth, route in zip(ids_veh, vehicles.states[ids_veh], vehicles.ids_berth[ids_veh], vehicles.routes[ids_veh]):
                    print('    id_veh', id_veh, 'state', state, 'id_berth',
                          id_berth, 'route', route)  # ,'len(route)',len(route)
                    ...
                    if id_person_sumo in self.id_person_to_origs_dests:
                        ...
                        if len(id_edge_sumo_dests) > 0:  # valid dest
                            ...
                            if id_edge_sumo_dests[0][2] == id_edge_sumo:
                                if self.debug:
                                    print('  add to waittimes_persons', id_person_sumo, 'id_stop_dest',
                                          id_edge_sumo_dests[0][1], 'id_toedge_sumo', id_edge_sumo_dests[0][3])
            ...
            if fase == FASE_MOVE_IN:
                ...
                if self.debug:
                    print('  >>timeout fase 0: dt=%.2f timeout=%.2f' % ((simtime - self.times_phase_in[id_stop]), self.time_fase_in_max.get_value()),
                          'TOUT', ((simtime - self.times_phase_in[id_stop]) > self.time_fase_in_max.get_value()), '|ALLOC', (len(self.ids_vehs_inqueue[id_stop]) > 0))
                    # print   '    ap',(len(self.ids_vehs_alight_aproach[id_stop]) == 0)
                    print('    no forward, all vehicles moved into berth',
                ...
                if self.debug:
                    print('  >>timeout fase 1: dt=%.2f timeout=%.2f' % (
                        (simtime - self.times_phase_out[id_stop]), self.time_fase_out_max.get_value()), 'move out of berth', len(self.ids_vehs_berth_exit[id_stop]))
                    print('                    n_berth_exit', len(self.ids_vehs_berth_exit[id_stop]), 'n_veh_pass', len(
                        ids_veh_to_pass), 'ids_veh_to_pass', ids_veh_to_pass)
                ...
                if (((simtime-self.times_phase_out[id_stop]) > self.time_fase_out_max.get_value()) | (len(self.ids_vehs_inqueue[id_stop]) >= self.capas_inqueue[id_stop]) | self.are_back_to_move_in[id_stop])\
                    ...
                    if self.debug:
                        print('   Switch from fase %d to fase %d' % (
                            self.fases[id_stop], FASE_MOVE_IN), 'ind_berth_allocated', self.inds_berth_allocated[id_stop])
                    ...
                    if (len(ids_veh_to_allocate) > 0) & (len(ids_veh_to_pass) == 0) & (self.inds_berth_allocated[id_stop] > -1):
                        ...
                        for id_veh, id_berth in zip(ids_veh_to_allocate, ids_berths):
                            ...
                            if id_berth > -1:
                                if self.debug:
                                    print('     foreward queued vehicle id_veh prt.%d to id_berth %d at pos %.2fm' %
                                          (id_veh, id_berth, berths.stoppositions[id_berth]))
    ...
    def route_trip_occupied(self, id_stop, id_berth, id_veh, id_veh_sumo, simtime):
        ...
        if self.debug:
            print('route_trip_occupied id_stop', id_stop, 'id_berth', id_berth,
                  'veh=%s' % id_veh_sumo, 'simtime', simtime, 'n_pax', n_pax)
        ...
        if len(ids_person_sumo_inveh) > 0:
            ...
            if self.debug:
                print('    found person %s' % id_person_sumo_inveh, 'from', id_stop_orig,
                      id_edge_sumo_from, ' to', id_edge_sumo_to, id_stop_dest)
    ...
    def route_empties(self, id_stop, id_stop_target, n_move, simtime):
        ...
        for id_berth, id_veh in zip(ids_berth[:n_move], ids_veh_empty[:n_move]):
            if self.debug:
                print('    route empty veh prt.%d' % id_veh, 'at berth',
                      id_berth, 'from stop', id_stop, ' to', id_stop_target)
        ...
        if 0:  # not necessary, process step will take care of it
            for id_berth, id_veh in zip(ids_berth_occup, ids_veh_waiting_occup):
                if self.debug:
                    print('    route occupied veh prt.%d' % id_veh, 'at berth',
                          id_berth, 'from stop', id_stop, ' to', id_stop_target)
    ...
    def foreward_vehicles_exit(self, id_stop):
        ...
        are_stopped = np.zeros(n_veh, dtype=np.bool_)
        for i, id_veh, state in zip(range(n_veh), ids_veh_berth_exit, vehicles.states[ids_veh_berth_exit]):
            ...
            if self.debug:
                print('    %s' % id_veh_sumo, 'are_stopped', are_stopped[i], 'stop', traci.vehicle.isStopped(
                    id_veh_sumo), 'state', state, 'laneindex', traci.vehicle.getLaneIndex(id_veh_sumo))
    ...
    def get_vehicles_ready_for_departure(self, id_stop):
        ...
        print('get_vehicles_ready_for_departure check id_stop', id_stop,
              ' len(ids_vehs_out)', n_veh, 'capa', self.capas_outqueue[id_stop])
        if is_veh_ready:
            if self.debug:
        ...
        vehicles = self.parent.prtvehicles
        if self.debug:
            print('  ids_berths_veh_wait_exit', type(
                self.ids_berths_veh_wait_exit[id_stop]), self.ids_berths_veh_wait_exit[id_stop], 'ids_veh_berth_wait', berths.ids_veh[self.ids_berths_veh_wait_exit[id_stop]])
        ...
        while is_cont & (i < n_veh):
            ...
            if id_stopedge_target == -1:  # target not yet defined
                ...
                if self.debug:
                    print('   %d Leader prt.%d' % (i, id_veh), 'id_stopedge_target',
                          id_stopedge_target)  # ,'is_stopped',is_stopped

            else:
                if self.debug:
                    print('   %d Follower prt.%d' % (i, id_veh), 'is_cont next',
                          vehicles.routes[id_veh][-1] == id_stopedge_target)
        ...
        if (i == n_veh):
            if ((len(self.ids_berths_veh_wait_exit[id_stop]) > 0) | (len(self.ids_vehs_pass_wait_exit[id_stop]) > 0))\
                ...
                if self.debug:
                    ...
                    print('  len(ids_berths_veh_wait_exit)', len(self.ids_berths_veh_wait_exit[id_stop]), 'len(ids_vehs_pass_wait_exit)', len(
                        self.ids_vehs_pass_wait_exit[id_stop]), "i", i)
                return []
            else:
            ...
            if is_stopped:
                ...
                if 0:  # be done for all in _reposition_outqueue
                    if not is_stopped:
                        ...
                        are_stopped = False
                        if self.debug:
                            print('      update stoppos of prt.%s:' %
                                  id_veh_sumo, 'from curpos', pos_veh, 'stoppos', stoppos)
    ...
    def prepare_vehs_for_departure(self, id_stop, ids_veh):
        ...
        if self.debug:
            ...
            for id_veh, route in zip(ids_veh, vehicles.routes[ids_veh]):
                id_stopedge = route[-1]
                print('     prep. prt.%d' % id_veh, 'with id_stopedge',
                      id_stopedge, 'id_stop', self.id_edge_to_id_stop[id_stopedge])
        ...
        if len(ids_veh_out) > 0:
            ...
            for ind in range(len(ids_veh_out)):
                ...
                if self.debug:
                    ...
                    print('    update stoppos of prt.%s:' % id_veh_sumo, 'from curpos',
                          traci.vehicle.getLanePosition(id_veh_sumo), 'to stoppos', stoppos)
    ...
    def launch_vehs(self, id_stop, id_stop_target):
        ...
        This is the only method that tells the vehicles to depart from the stop.
        """
    ...
    def is_outputqueue_compact(self, id_stop):
        ...
        print('is_outputqueue_compact n_outqueue', len(
            self.ids_vehs_out[id_stop]), 'pos_lastberth %.2f' % pos_lastberth)
    ...
    def try_start_vehicles(self, id_stop, simtime, n_plat_min=-1):
        ...
        print('try_start_vehicles id_stop', id_stop, 'veh exiting', len(self.ids_vehs_berth_exit[id_stop]),
              'wait from berth', len(self.ids_berths_veh_wait_exit[id_stop]), 'wait to pass', len(self.ids_vehs_pass_wait_exit[id_stop]))
        ...
        if n_veh_pass_wait > 0:
            if self.debug:
                print('  there are passthrough vehicle in the input queue waiting to get started',
                      self.ids_vehs_pass_wait_exit[id_stop])
        ...
        if n_veh_all > 0:
            ...
            if n_veh_pass > 0:
                ...
                if (n_veh_plat >= n_plat_min) | (np.max(inds_veh_plat) >= n_veh):
                    if self.debug:
                        print('  platoon formation long enough n_veh_plat', n_veh_plat,
                              'or passthrough', (np.max(inds_veh_plat) >= n_veh))

                        print('    deviate vehicles berth',
                              ids_veh_ready_all[inds_veh_plat_deviate[iinds_plat_deviate_berth]])
                    ...
                    if self.debug:
                        print('    deviate vehicles passthrough',
                              ids_veh_ready_all[inds_veh_plat_deviate[np.logical_not(iinds_plat_deviate_berth)]])
                    ...
                    if self.debug:
                        print('    kick out ids_veh',
                              ids_veh_ready[inds_veh_plat_nodeviate[iinds_plat_nodeviate_berth]])
                        print('    from berths     ',
                              ids_berth_ready[inds_veh_plat_nodeviate[iinds_plat_nodeviate_berth]])
    ...
    def enter(self, id_stop, ids_veh, fase, simtime):
        ...
        are_completed = np.zeros(n_veh, dtype=np.bool_)
        for id_veh, i in zip(ids_veh, range(n_veh)):
            ...
            if self.debug:
                print('  enter prt.%d' % id_veh, 'with id_stop_target',
                      vehicles.ids_stop_target[id_veh], 'route', route)
            if len(route) == 0:
        ...
        if self.debug:
            print('  n_veh_completed', n_veh_completed, 'no Timeout', ((
                simtime - self.times_phase_in[id_stop]) < self.time_fase_in_max.get_value()), 'veh input', self.ids_vehs_inqueue[id_stop])
        # if fase == FASE_MOVE_IN:
        #    print '  timeout fase 0: dt=%.2f timeout=%.2f'%((simtime - self.times_phase_in[id_stop]), self.time_fase_in_max.get_value())
        ...
        for id_veh, state, id_berth, is_completed in zip(ids_veh, vehicles.states[ids_veh], ids_berth, are_completed):
            ...
            if not is_completed:
                ...
                is_stop_moving_in = True
                if self.debug:
                    print('    stop before first berths and wait for allocation at %.1fm' %
                          (self.stoplines_in[id_stop]))
                self.ids_vehs_inqueue[id_stop].append(id_veh)
                ...
                if self.debug:
                    print('     forewrard entering vehicle id_veh %d to id_berth_alight %d at pos %.2fm' %
    ...
    def allocate_alight(self, id_stop, n_alloc):
        ...
        print('allocate_alight n_alloc', n_alloc, 'n_berth_to_allocate',
              n_berth_to_allocate, 'ind_berth_to_allocate', ind_berth_to_allocate)
        if self.debug:
        ...
        if ind_berth_to_allocate == -1:
            ...
            for i, queue, id_berth in zip(range(n_berth_to_allocate), queues[::-1], ids_berth_to_allocate[::-1]):
                ...
                if queue == 0:
                    ...
                    if self.debug:
                        ...
                        print('      rem avail', n_berth_to_allocate-(i+1), 'rem to alloc',
                              n_alloc-(ind_alloc_eff + 1), 'is_allzero', is_allzero)
            ...
            if self.debug:
                print('    finished allocating i', i, 'ind_berth_to_allocate',
                      ind_berth_to_allocate, 'rem', n_berth_to_allocate-(i+1))
    ...
    def get_berthqueues(self, id_stop, ids_berth):
        ...
        for id_person_sumo in list(self.waittimes_persons[id_stop].keys()):
            ...
            if self.debug:
                print('  check id_person_sumo', id_person_sumo, 'Stagetype',
                      stage.type, 'pos', traci.person.getLanePosition(id_person_sumo))
...
class PrtVehicles(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        # no it is a constant!!!
        # self.add(cm.AttrConf(  'length', 4.0,
        #                                groupnames = ['parameters'],
        #                                name = 'length',
        #                                info = "Vehicle length.",
        #                                unit = 'm',
    ...
    def prepare_sim(self, process):
        ...
        # current route
        self.routes = np.zeros(n_id_max, dtype=np.object_)
        for id_veh in ids:
            self.routes[id_veh] = []
        ...
        # persons
        self.ids_persons_sumo = np.zeros(n_id_max, dtype=np.object_)
        for id_veh in ids:
    ...
    def add_ghost(self, id_veh, id_ghost, dist_to_merge_veh, dist_to_merge_ghost,
        ...
        # else:
        # add new ghost
        print('add_ghost veh prt.%d ghost prt.%d dtm = %.1f dtmg = %.1f d0=%.1f' % (id_veh, id_ghost,
              dist_to_merge_veh, dist_to_merge_ghost, dist_to_merge_veh - dist_to_merge_ghost))  # ,self.ids_ghosts.shape
    ...
    def process_step(self, process):
        ...
        if 0:
            ...
            for id_veh, id_veh_sumo, id_follower, id_leader in zip(ids_debug, self.ids_sumo[ids_debug], self.ids_follower[ids_debug], self.ids_leader[ids_debug]):
                print('  *id_veh prt.%d' % id_veh, 'id_follower', id_follower,
                      'id_leader', id_leader, 'ids_ghost', self.ids_ghosts[id_veh])
                print('      id_stop_target', self.ids_stop_target[id_veh], 'lp=%.1fm' % self.lengths_plat[id_veh])
                if self.states[id_veh] > 0:
                    print('     pax', traci.vehicle.getPersonIDList(id_veh_sumo), 'isStopped',
                          traci.vehicle.isStopped(id_veh_sumo), 'speed', traci.vehicle.getSpeed(id_veh_sumo))
        ...
        if 1:
            ...
            for id_veh, id_sumo, velocity, velocitiy_ghost_min, \
                ...
                if id_veh in ids_debug:
                    ...
                    print('    %7s: v=%3.1f vg=%3.1f d0=%4.1f dh=%4.1f th=%3.1fs ds=%4.1f dc=%4.1f odo=%4.1f %s' %
                          (id_sumo, velocity, velocitiy_ghost_min, dist0, dist_min, th, dist_safe, dist_comf, odo, a))
                    print('       dist_min', dist_min, 'dist_min_check',
                          dist_min_check, 'id_ghost', self.ids_ghosts[id_veh][0])
    ...
    def set_route_target(self, id_veh, route, id_stop_target=None, is_disengage_from_berth=False, is_emptytrip=False):
        ...
        """
        print('set_route_target prt.%d' % id_veh, 'id_stop_target',
              id_stop_target, 'is_disengage_from_berth', is_disengage_from_berth)
        print('  route', route)
    ...
    def _update_concatenate(self, id_veh, length_plat):
        ...
        print('_update_concatenate prt.%s, length_plat=%.1f, length=%.1f,' %
              (id_veh, length_plat, self.length), 'id_leader', self.ids_leader[id_veh])
    ...
    def control_stopline_enter(self, id_veh, id_edge_sumo, stopline, laneindex=1, flags=0):
        ...
        id_veh_sumo = self.get_id_sumo(id_veh)
        print('control_stopline_enter', id_veh_sumo, 'lane %d, pos=%.2f->targetpos %.2f' %
              (laneindex, traci.vehicle.getLanePosition(id_veh_sumo), stopline))
        traci.vehicle.setStop(id_veh_sumo,
                              id_edge_sumo,
                              pos=stopline,
                              startPos=stopline,
    ...
    def control_stop_board(self, id_veh, id_stop, id_berth,
        ...
        # NO LONGER USED
        id_veh_sumo = self.get_id_sumo(id_veh)
        print('control_stop_board', id_veh_sumo, id_stop, id_berth, id_edge_sumo, 'pos=%.2f,target %.2f' %
              (traci.vehicle.getLanePosition(id_veh_sumo), position), end=' ')
    ...
    def launch_from_stop(self, id_veh):
        ...
        print('  after launch is stopped', traci.vehicle.isStopped(id_veh_sumo),
              'getMaxSpeed', traci.vehicle.getMaxSpeed(id_veh_sumo))
    ...
    def init_passthrough(self, id_veh, id_edge_sumo, stopline, laneindex=1, flags=0):
        id_veh_sumo = self.get_id_sumo(id_veh)
        print('init_passthrough', id_veh_sumo, 'lane %d, pos=%.2f->targetpos %.2f' %
              (laneindex, traci.vehicle.getLanePosition(id_veh_sumo), stopline))
    ...
    def is_completed_alighting(self, id_veh):
        print('is_completed_alighting', self.get_id_sumo(id_veh), self.states[id_veh], 'alighting', self.states[id_veh] == VEHICLESTATES['alighting'], 'pers on board', traci.vehicle.getPersonNumber(
        ...
        while len(ids_person_act) > 0:
            ...
            for id_person, id_plan, time_from, id_act_from, id_act_to, name_acttype_to, duration_act_to, id_edge_from, pos_edge_from, id_edge_to, pos_edge_to, id_stop_from, id_stopedge_from, pos_stop_from, id_stop_to, id_stopedge_to, pos_stop_to, dist_from_to, dist_walk_max\
                ...
                if (dist_from_to < dist_walk_max) | (id_edge_from == -1) | (id_edge_to == -1) | (id_stop_from == id_stop_to):
                    print('    go by foot because distance is too short ', dist_from_to,
                          'edges', id_edge_from, id_edge_to, 'stops', id_stop_from, id_stop_to)
...
class VehicleMan(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add(cm.AttrConf('flow_max_default', 700.0,
                             ...
                             info="[DEPRICATED, see berth capacity of stops]Maximum vehicle flow allowed for a stop. This is a default value an may be replaced by capacity capculations that depend on the geometry of the stop.",
                             unit='1/h',
                             ))
    ...
    def prepare_sim(self, process):
        ...
        # now look at the stop to stop time whether this is enough
        self.n_est_max = max(int(2.5*np.max(self.parent.times_stop_to_stop)), self.n_est_max)
    ...
    def update_flows(self, process):
        ...
        # update times with actual travel times
        self.parent.update_edgetimes()
        self.parent.make_times_stop_to_stop()
    ...
    def process_step(self, process):
        ...
        while is_cont:
            ...
            if self.debug > 4:
                print(79*'-')
                print('  check equalize iteration', np.any(surpluses_veh > 0), np.any(
                    balances_veh > 0), np.any(surpluses_veh_urgent > 0), 'is_cont', is_cont)
    ...
    def fit_vehicles(self, id_stop, n_veh, ind_time_arrival):
        ...
        is_fit = False
        if self.debug > 1:
            print('fit_vehicles n_veh', n_veh, 'for n_slots', n_slots, 'capa*slots %.2f' % (capa * n_slots), 'at slot ind', ind_time_arrival,
                  'n_slots search inerv.', n_slots_search, 'out of range', (ind_time_arrival + ind_delay + n_slots < self.n_est_max))
        ...
        while (ind_delay < n_slots_search) & (not is_fit) & (ind_time_arrival + ind_delay + n_slots < self.n_est_max):
            ...
            print('    ind_delay', ind_delay, 'is_fit', is_fit, 'from', ind_time_arrival_sched, 'to',
                  ind_time_arrival_sched + n_slots, 'flows_sched', flows_sched, end=' ')  # 'max',self.n_est_max

            ind_delay += 1

        # check if sufficient time-slots in the future are available
        # at estimated arrival time
        if (self.debug > 2) & is_fit:
            # ATTENTION: if while loop is skipped then thses variables are not assigned
        ...
        if is_fit:
            ...
            while (i_slot < n_slots) & (n_alloc_cum < n_veh):
                ...
                if self.debug > 5:
                    print('   i_slot', i_slot, 'capa_slot %.2f' % capa_slot, 'capa_avail %.2f' %
                          capa_avail, 'n_alloc', n_alloc, 'n_alloc_cum', n_alloc_cum)
                if n_alloc > 0:
    ...
    def get_timeslot_occupation(self, id_stop, n_veh):
        ...
        print('get_timeslot_occupation n_veh', n_veh, 'capa_int', capa_int,
              'n_slots', n_slots, 'rem', n_veh - n_slots*capa_int)
        if n_slots == 0:
    ...
    def push_out_vehicles(self, process):
        ...
        for id_stop, id_stop_target, duration_est, n_veh\
            ...
            print('.'*79)
            print('    >>>check id_stop', id_stop, 'id_stop_target', id_stop_target, 'n_veh',
                  n_veh, 'dur %.2f' % duration_est, 't_dep', time_depart, 'simtime', simtime)
            ...
            if time_depart != -1:
                ...
                print('      time_arr_est', time_arr_est, 'ind_time_arrival',
                      ind_time_arrival, 'in range', ind_time_arrival < self.n_est_max)
                ...
                if ind_time_arrival < self.n_est_max:
                    ...
                    if n_alloc > 0:
                        ...
                        print('    schedule id_stop_target', id_stop_target, 'inflows_sched',
                              self.inflows_sched[id_stop_target, ind_time_arrival_sched:ind_time_arrival_sched+n_alloc])
                        ...
                        print('WARNING: no capacity available at id_stop_target',
                              id_stop_target, ' at this arrival time.')
                        is_launch = False
                else:
                    #
            ...
            if is_launch:
                # here stop is called to launch the vehicles
    ...
    def equalize_empty_vehicles(self, process):
        """
        from /home/joerg/projects/hybridPY/hybridPY-201022/plugins/hcprt/hcprt.py
        ...
        for id_stop, n_pax_wait, inflow_pax\
            ...
            if self.debug > 2:
                print('    surplus_veh', surplus_veh, 'surplus_future',
                      surplus_veh_future, 'surplus_veh_urgent', surplus_veh_urgent)
            ...
            for id_stop_from, ind_from in zip(ids_stop[inds_sorted[::-1]], inds_sorted[::-1]):
                ...
                if (surplus_from >= 1) & (balance_from >= 1):
                    if self.debug > 5:
                        ...
                        print('\n    potential emitter stop: id_stop_from', id_stop_from,
                              'surplus_from', surplus_from, 'balance_from', balance_from)
                    ...
                    for id_stop_to, ind_to in zip(ids_stop[inds_sorted], inds_sorted):
                        if id_stop_to != id_stop_from:
                            ...
                            if self.debug > 5:
                                ...
                                print('        balance_from', balance_from, 'balance_to',
                                      balance_to, 'min', -balance_to+0.5, surplus_from)
                            ...
                            if (surplus_from >= 1) & (balance_from >= 1) & (demand_to > 0):
                                ...
                                if self.debug > 5:
                                    print('        potential n_move', n_move, 'vehs from stop',
                                          id_stop_from, 'to stop', id_stop_to)
                                ...
                                if self.debug > 5:
                                    print('         =>U=%.2f' % (utilities[j]),
                                          'n_move', n_move, 'time_to %ds' % (time_to))
                                ...
                                if self.debug > 5:
                                    print('        forced potential n_move', n_move,
                                          'vehs from stop', id_stop_from, 'to stop', id_stop_to)
                                # utilities[j] =  n_move+ self.constant_timeweight*time_to
                                # utilities[j] =  float(n_move)/float(time_to)
                    ...
                    if (n_move > 0) & (utility_max > 0):
                        ...
                        if self.debug > 2:
                            print('    moved', n_move_eff, 'of', numbers_move[ind_max], 'empty vehs from',
                                  id_stop_from, 'to', ids_stop[inds_sorted][ind_max], 'utility_max', utility_max)
    ...
    def init_trip(self, id_veh, id_stop, id_stop_to, time_order):
        """
        STILL USED or init_trip_occupied???
        Called from the stop when a vehicle in the berth initiate a trip with
...
class HcPrtService(SimobjMixin, DemandobjMixin, cm.BaseObjman):
    # class HcPrtService(DemandobjMixin, cm.BaseObjman):
    ...
    def _init_attributes(self):
        ...
        self.constant_edgetime = attrsman.add(am.AttrConf('constant_edgetime', 0.9,
                                                          groupnames=['options'],
                                                          name='Edgetime update constant',
                                                          info='Edgetime update constant.',
                                                          ))
    ...
    def prepare_sim(self, process):
        print('hcprt.prepare_sim', self.ident, len(self.prtstops))
        if len(self.prtstops) == 0:
            print('  no stops no service')
            return []
    ...
    def update_edgetimes(self):
        print('update_edgetimes')
        if self.fstar is None:
            self.make_fstar()
        ids_edge = list(self.fstar.keys())

        for id_edge, id_edge_sumo in zip(ids_edge, self.get_scenario().net.edges.ids_sumo[ids_edge]):
            t_current = min(traci.edge.getTraveltime(id_edge_sumo), 2*self.edgetimes[id_edge])
            print('  id_edge_sumo %s t_before=%d, t_now=%d t_after=%d' % (
                id_edge_sumo, self.edgetimes[id_edge], t_current, self.constant_edgetime * self.edgetimes[id_edge] + (1. - self.constant_edgetime) * t_current))

            self.edgetimes[id_edge] = self.constant_edgetime * \
                self.edgetimes[id_edge] + (1. - self.constant_edgetime) * t_current
    ...
    def get_times(self, fstar):
        ...
        return self.get_scenario().net.edges.get_times(id_mode=self.id_prtmode, is_check_lanes=False,
                                                       speed_max=None,
                                                       modeconst_excl=0.0,
                                                       modeconst_mix=0.0,
                                                       is_precise=True,
...
class HcPrtStopResults(am.ArrayObjman):
    ...
    def init_recording(self, n_timesteps, time_step):
        print('HC init_recording n_timesteps, time_step', n_timesteps,
              time_step, len(self.ids_stop.get_linktab().get_ids()))
        print('  stops instance', self.prtstops.get_value(), self.ids_stop.get_linktab(),
              id(self.prtstops.get_value()), id(self.ids_stop.get_linktab()))
        self.clear()
    ...
    def record(self, timestep, ids, **kwargs):
        ...
        for attrname, values in kwargs.items():
            print('  record', attrname, 'dtype', values.dtype, values.shape, 'array', getattr(
                self, attrname).get_value().dtype, 'shape', getattr(self, attrname).get_value().shape)
