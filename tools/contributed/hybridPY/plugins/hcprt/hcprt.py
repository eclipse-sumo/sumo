...
from coremodules.misc.petrinet import PetrinetMixin
...
INF = 10.0**10 
INFINT =  10**10 
INVALID_DOUBLE_VALUE = traci.constants.INVALID_DOUBLE_VALUE
            ...
            if len(ids_veh_entered_sumo)>0:
                ...
                for id_veh_entered,id_veh_entered_sumo, id_leader,length_plat\
                    ...
                    if (length_plat < 0.001)&(id_leader == -1):
                        ...
                        inds_queue = np.flatnonzero( (np.array(ids_edge_sumo_target, object)==id_targetedge_sumo) & np.array(are_queue_avail, dtype = np.bool))
...
class PrtBerths(PetrinetMixin,am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add(cm.AttrConf(  'time_update', 0.5,
                                        groupnames = ['parameters'], 
                                        name = 'Update time',
                                        info = "Update time for berth. Must be a faction of 1.",
                                        unit = 's',
                                        ))
                                        
        self.add(cm.AttrConf(  'time_kickout', 5,
                                        groupnames = ['parameters'], 
                                        name = 'Kickout time',
                                        info = "Time after which a loaded, waiting vehicle will be kicked out of the berth. A longer kick out time may increase the chance to platoon with another vehicle with same destination.",
                                        unit = 's',
                                        ))
        ...
        self.add(cm.AttrConf('speed_berth', 4.0, 
                                    groupnames = ['parameters','options'],
                                    name = 'Berth speed', 
                                    info = 'Maximum speed inside berth.',
                                    unit = 'm/s',
                                    ))                            
        
        
        self.add(cm.AttrConf('speed_crawl', 0.1, 
                                    groupnames = ['parameters','options'],
                                    name = 'Crawl speed', 
                                    info = 'MaxCrawl speed is used as speed to approach stop lines, before coming completely to a halt, should be close to zero.',
                                    unit = 'm/s',
                                    )) 
        ...
        self.add_col(am.IdsArrayConf('ids_berth_1_before', self, 
                                    name = 'ID of berth 1 before', 
                                    info = 'ID of berth 1 before',
                                    ))
                                    
        self.add_col(am.IdsArrayConf('ids_berth_2_before', self, 
                                    name = 'ID of berth 2 before', 
                                    info = 'ID of berth 2 before',
                                    ))
        
        self.add_col(am.IdsArrayConf('ids_berth_3_before', self, 
                                    name = 'ID of berth 3 before', 
                                    info = 'ID of berth 3 before',
                                    ))
                                    
        self.add_col(am.IdsArrayConf('ids_berth_1_ahead', self, 
                                    name = 'ID of berth 1 ahead', 
                                    info = 'ID of berth 1 ahead',
                                    )) 
        
        self.add_col(am.IdsArrayConf('ids_berth_2_ahead', self, 
                                    name = 'ID of berth 2 ahead', 
                                    info = 'ID of berth 2 ahead',
                                    ))
    ...
    def _init_constants(self):
        ...
        self.do_not_save_attrs([\
                'states', 'ids_veh','times_routes',
                'enterpositions','exitpositions',
                'ids_berth_internal','ids_berth_first','ids_berth_last','ids_berth_all_but_last','ids_berth_all_but_first',
                #'ids_veh_accelerating','ids_veh_decelerating','ids_veh_stopped','ids_veh_locked',
                #'transitions','places','placenames'
                ])
                
    
    def prepare_basics(self):
        print('PRTBerths.prepare_basics!')
        ids = self.get_ids()
        if len(ids) == 0:  return []
        
        n_bertharray = np.max(ids)+1
        
        self.states = BERTHSTATES['free']*np.ones(n_bertharray,dtype = np.int32)
        
        self.enterpositions = np.zeros(n_bertharray,dtype = np.float32)
        self.enterpositions[ids] = self.stoppositions[ids] - 0.95*self.length_default.get_value()
        self.exitpositions = np.zeros(n_bertharray,dtype = np.float32)
        self.exitpositions[ids] = self.stoppositions[ids] + 0.05*self.length_default.get_value()
        
        self.ids_berth_internal = ids[np.logical_and((self.ids_berth_1_ahead[ids] != -1) , (self.ids_berth_1_before[ids] != -1))]
        self.ids_berth_first = ids[self.ids_berth_1_before[ids] == -1]
        self.ids_berth_last = ids[self.ids_berth_1_ahead[ids] == -1]
        self.ids_berth_all_but_last = ids[self.ids_berth_1_ahead[ids] != -1]
        self.ids_berth_all_but_first = ids[self.ids_berth_1_before[ids] != -1]   
        
        self.ids_dest = EMPTY*np.ones(n_bertharray,dtype = np.int32)
    ...
    def prepare_sim(self, process):
        ...
        self.ids_sumo_passengers = np.zeros(n_bertharray,dtype = np.object)
        self.ids_sumo_passengers[:] = None
        
        
        # PETRINET DEFS
        self.init_petrinet()
        
        self.add_transitions(  ['accelerate', 
                                'stopping', 
                                'request_vehicle',
                                'enter', 
                                'exit', 
                                ...
                                'move_out',
                                'prepare_departure',
                                'accelerate_processed',
                                ])
                                
        self.add_places([   (   'ids_veh_accelerating',     EMPTY*np.ones(n_bertharray,dtype = np.int32),'acc'),
                            (   'ids_veh_decelerating',     EMPTY*np.ones(n_bertharray,dtype = np.int32),'dec'),
                            (   'ids_veh_stopped',          EMPTY*np.ones(n_bertharray,dtype = np.int32),'stop'),
                            (   'ids_veh_locked',           EMPTY*np.ones(n_bertharray,dtype = np.int32),'lock'),
                            (   'requests_continue',        EMPTY*np.ones(n_bertharray,dtype = np.int32),'cont'),
                            (   'ids_veh_processing',       EMPTY*np.ones(n_bertharray,dtype = np.int32),'proc'),
                            (   'ids_veh_moving_in',        EMPTY*np.ones(n_bertharray,dtype = np.int32),'in'),
                            (   'ids_veh_parked',           EMPTY*np.ones(n_bertharray,dtype = np.int32),'park'),
                            (   'ids_veh_loaded',           EMPTY*np.ones(n_bertharray,dtype = np.int32),'load'),
                            (   'timer_kickout',            EMPTY*np.ones(n_bertharray,dtype = np.float32),'time'),
                            (   'ids_veh_moving_out',       EMPTY*np.ones(n_bertharray,dtype = np.int32),'out'),
                            (   'ids_veh_stopped_processed',EMPTY*np.ones(n_bertharray,dtype = np.int32),'stpr'),
                            ])
        ...
        self.times_routes = 0.0 * np.ones(n_bertharray, dtype = np.float32)
        return [(self.time_update.get_value(), self.process_step),  ]# berth has no update function
    
    
    
    def insert_veh(self, id_berth, id_veh):
        """ Insert vehicle in berth control structure. 
            This must be the first berth of a station and the vehicle must be near zero speed.
            Return True, if insertion has been successful, otherwise false
        """
        print('insert_veh id_berth',id_berth,'id_veh',id_veh,self.ids_veh_locked[id_berth] == EMPTY ,(self.ids_veh_locked[self.ids_berth_1_ahead[id_berth]] == EMPTY) & (self.requests_continue[id_berth] == TOKEN))
        
        if ( self.ids_veh_locked[id_berth] == EMPTY ):
            # this berth is clear
            if (self.ids_veh_locked[self.ids_berth_1_ahead[id_berth]] == EMPTY) & (self.requests_continue[id_berth] == TOKEN):
                # this and 1 berth ahead is clear
                print('   we can accelerate vehicle')
                self.ids_veh_accelerating[id_berth] = id_veh
                vehicles = self.get_prtvehicles()
                vehicles.control_speedup(id_veh, speed = self.speed_berth.get_value(),laneindex = 2)
                
            else:
                # this berth is clear but 1 berth ahead is occupied or cannot continue because needs processing
                print('   we must brake vehicle')
                self.ids_veh_decelerating[id_berth] = id_veh
                vehicles = self.get_prtvehicles()
                #edges = self.get_scenario().net.edges
                #vehicles.control_stopline(id_veh,\
                #            id_edge_sumo = edges.ids_sumo[self.parent.get_edges(self.ids_prtstop[id_berth])],
                #            position = self.stoppositions[id_berth],
                #            laneindex = 2,speed = self.speed_berth.get_value(),
                #            )
                
                # slow down at maximum comfort decel
                vehicles.control_slow_down( id_veh, speed = 0.5, time_slowdown = None)
                
            self.ids_veh_locked[id_berth] = id_veh
            
            # debug
            id_veh_sumo = vehicles.ids_sumo[id_veh]
            print('  check speed',traci.vehicle.getSpeed(id_veh_sumo),'is_stopped',traci.vehicle.isStopped(id_veh_sumo),'accel',self.ids_veh_decelerating[id_berth],'decel',self.ids_veh_decelerating[id_berth])
    ...
    def enter_lastberth(self):
        """
        Vehicles on the accelerating position of the previous berth
        have crossed the enter line with their head.
        This is doing transitions for the last berth, 
        checking vehicles on the out queue of the stop.
        """
        
        # check if a locking vehicle of each berth is entering the
        # next berth by crossing its entry line
        # take IDs of berth with accelerating vehicles in the berth before that have crossed with the head the line to enter the berth
        #outberths = self.ids_outberth.get_linktab()
        ids_berth_veh = self.ids_berth_last[self.ids_veh_accelerating[self.ids_berth_1_before[self.ids_berth_last]] != EMPTY]
        
        # get vehicles of accelerating place of previous berth
        #ids_veh_locked = self.ids_veh_locked[self.ids_berth_1_before[ids_berth_veh]]
        ids_veh_accel = self.ids_veh_accelerating[self.ids_berth_1_before[ids_berth_veh]]
        
        
        print('enter_lastberth',len(ids_berth_veh),'veh')
        print('  ids_berth_veh',ids_berth_veh)
        print('  ids_veh_accel',ids_veh_accel)
        
        
        # debug
        #id_berth_debug = 238
        #id_berth_check = self.ids_berth_all_but_first[self.ids_berth_1_before[self.ids_berth_all_but_first] == id_berth_debug] 
        #id_veh_locked_debug = self.ids_veh_locked[id_berth_check]
        #id_veh_accel_debug = self.ids_veh_accelerating[id_berth_check]
        #print  '  debug: id_berth',id_berth_debug,'locked',id_veh_locked_debug,'accel',id_veh_accel_debug,'id_berth_before_check',id_berth_check
        
        if len(ids_veh_accel):
            prtstops = self.parent
            vehicles = self.get_prtvehicles()
            edges = self.get_scenario().net.edges
            
            n_veh = len(ids_veh_accel)
            inds_crossed = np.zeros(n_veh, dtype = np.bool)
            for i,id_berth, id_veh_sumo,  enterpos  in zip(range(n_veh),ids_berth_veh,vehicles.ids_sumo[ids_veh_accel],self.enterpositions[ids_berth_veh]):
                print('    check veh',id_veh_sumo,'berth',id_berth,'pos',traci.vehicle.getLanePosition(id_veh_sumo),'enterpos', enterpos,traci.vehicle.getLanePosition(id_veh_sumo) > enterpos)
                inds_crossed[i] = traci.vehicle.getLanePosition(id_veh_sumo) > enterpos
                    
            # get only berth where locked vehicles crossed enter line
            ids_berth_veh_entered = ids_berth_veh[inds_crossed]
            ids_veh_entered = ids_veh_accel[inds_crossed]
            
            # debug
            for id_veh_sumo, enterpos in zip(vehicles.ids_sumo[ids_veh_entered],self.enterpositions[ids_berth_veh_entered]):
                print('  id_veh_sumo',id_veh_sumo,'pos',traci.vehicle.getLanePosition(id_veh_sumo),enterpos)
            print('      ids_veh_entered',ids_veh_entered)
            print('ids_berth_veh_entered',ids_berth_veh_entered,np.any(inds_crossed))
                
            # check if this berth is not locked
            # this should always be the case if the interlock had worked properly
            # debug:
            ind_occupied = self.ids_veh_locked[ids_berth_veh_entered] != EMPTY
            if np.any(ind_occupied):
                
                print('WARING in enter transition: at stops',self.ids_prtstop[ids_berth_veh_entered[ind_occupied]],'the following berths are not cleared:',ids_berth_veh_entered[ind_occupied])
                print('       with ids_veh',self.ids_veh_locked[ids_berth_veh_entered[ind_occupied]],'entering vehicle:',ids_veh_entered[ind_occupied])
            ...
            # identify those which can enter and accelerating (or keep speed)
            # conditions:
            #   * this berth is clear
            #   * 1 berth ahead is clear
            #   * the 1 berth ahead does exist # always because internal berth
            #   * there is a proceed request
            
            inds_berth_veh_accel = np.logical_and(self.ids_veh_locked[ids_berth_veh_entered] == EMPTY,  self.requests_continue[ids_berth_veh_entered] == TOKEN) 
            #inds_berth_veh_accel = np.logical_and(inds_berth_veh_accel, self.ids_berth_1_ahead[ids_berth_veh_entered] != EMPTY) # next berth exists
            #inds_berth_veh_accel = np.logical_and(inds_berth_veh_accel, )
            print('          berth clear',self.ids_veh_locked[ids_berth_veh_entered] == EMPTY)
            #print '    berth ahead clear',outberths.ids_veh_locked[self.ids_outberth[ids_berth_veh_entered]] == EMPTY
            #print '    berth ahead exist',self.ids_berth_1_ahead[ids_berth_veh_entered] != EMPTY
            print('      proceed request',self.requests_continue[ids_berth_veh_entered] == TOKEN)
            
            #inds_berth_veh_accel = np.flatnonzero(inds_berth_veh_accel)
            #inds_outqueue_clear = np.zeros(len(inds_berth_veh_accel), dtype = np.bool)
            #for id_stop in self.ids_stop[ids_berth_veh_entered[]]
            for ind in  np.flatnonzero(inds_berth_veh_accel):
                # requires 1 berth space in the output queue from the end of the last berth
                inds_berth_veh_accel[ind] = prtstops.get_lastpos_outqueue(self.ids_stop[ids_berth_veh_entered[ind]])\
                                            - self.enterpositions[ids_berth_veh_entered[ind]]\
                                            > 2 * self.length_default.get_value()
            
            
            # transition accel
            ids_berth_veh_accel = 1*ids_berth_veh_entered[inds_berth_veh_accel]
            ids_veh_accel = 1*self.ids_veh_accelerating[self.ids_berth_1_before[ids_berth_veh_accel]] # copy!
            self.enter_acceltransition(ids_berth_veh_accel, ids_veh_accel, edges, prtstops, vehicles)
            
            
                
            # transition decel
            # the condition for decelerating are the inverse conditions of vehicles accelerating
            ids_berth_veh_decel = ids_berth_veh_entered[np.logical_not(inds_berth_veh_accel)]
            ids_veh_decel = 1*self.ids_veh_accelerating[self.ids_berth_1_before[ids_berth_veh_decel]] # copy!
            self.enter_deceltransition(ids_berth_veh_decel, ids_veh_decel, edges, prtstops, vehicles)
            
            
                
            
            # transtition lock
            self.ids_veh_locked[ids_berth_veh_entered] = ids_veh_entered
                                
            # remove from entering vehicles from acceleration place of previous berth
            self.ids_veh_accelerating[self.ids_berth_1_before[ids_berth_veh_entered]] = EMPTY
    ...
    def enter(self):
        ...
        have crossed the enter line with their head.
        """
        # treat all last berth
        self.enter_lastberth()
            
        # check if a locking vehicle of each berth is entering the
        # next berth by crossing its entry line
        # take IDs of berth with accelerating vehicles in the berth before that have crossed with the head the line to enter the berth
        ids_berth_veh = self.ids_berth_internal[self.ids_veh_accelerating[self.ids_berth_1_before[self.ids_berth_internal]] != -1]
        ...
        print('enter',len(ids_berth_veh),'veh')
        print('  ids_berth_veh',ids_berth_veh)
        print('  ids_veh_accel',ids_veh_accel)
        ...
        if len(ids_veh_accel):
            ...
            inds_berth_veh_accel = np.logical_and(self.ids_veh_locked[ids_berth_veh_entered] == EMPTY, self.ids_veh_locked[self.ids_berth_1_ahead[ids_berth_veh_entered]] == EMPTY) 
            #inds_berth_veh_accel = np.logical_and(inds_berth_veh_accel, self.ids_berth_1_ahead[ids_berth_veh_entered] != EMPTY) # next berth exists
            inds_berth_veh_accel = np.logical_and(inds_berth_veh_accel, self.requests_continue[ids_berth_veh_entered] == TOKEN )
            
            print('          berth clear',self.ids_veh_locked[ids_berth_veh_entered] == EMPTY)
            print('    berth ahead clear',self.ids_veh_locked[self.ids_berth_1_ahead[ids_berth_veh_entered]] == EMPTY)
            #print '    berth ahead exist',self.ids_berth_1_ahead[ids_berth_veh_entered] != EMPTY
            print('      proceed request',self.requests_continue[ids_berth_veh_entered] == TOKEN)
    ...
    def enter_acceltransition(self, ids_berth_veh_accel, ids_veh_accel, edges, prtstops, vehicles ):
            
            print('  ids_berth_veh_accel',ids_berth_veh_accel)
            print('  ids_veh_accel      ',ids_veh_accel)
            
            
            self.ids_veh_accelerating[ids_berth_veh_accel] = ids_veh_accel
            
            for id_veh in ids_veh_accel:
                vehicles.control_speedup(id_veh, speed = self.speed_berth.get_value(), laneindex = 2)
            
    def enter_deceltransition(self, ids_berth_veh_decel, ids_veh_decel, edges, prtstops, vehicles):
            print('  ids_berth_veh_decel',ids_berth_veh_decel)
            print('  ids_veh_decel      ',ids_veh_decel)
            #,'ids_stop',self.ids_prtstop[ids_berth_veh_decel],'id_edge_sumo',edges.ids_sumo[prtstops.get_edges(self.ids_prtstop[ids_berth_veh_decel])]
            
            self.ids_veh_decelerating[ids_berth_veh_decel] = ids_veh_decel    
            # 
            for id_veh, id_edge_sumo, pos in zip(ids_veh_decel, edges.ids_sumo[prtstops.get_edges(self.ids_prtstop[ids_berth_veh_decel])], self.stoppositions[ids_berth_veh_decel]):
                # slow down at maximum comfort decel
                #vehicles.control_slow_down( id_veh, speed = 0.1, time_slowdown = None)
                ...
                #traci.vehicle.setSpeedMode(id_veh_sumo, 3)
                #traci.vehicle.slowDown(id_veh_sumo, 0.05, 2.0)
                
                vehicles.control_slow_down(id_veh_sumo = id_veh_sumo,\
                                speed = self.speed_crawl.get_value(),
                                time_slowdown = None,
                                )      
                                     
    def accelerate(self):
        """
        Starts accelerating  vehicles in berths that are stopped at the stopline, if feasible and requested.
        The last berth in a station is not considered, in this case a station transition
        must take the vehicles out.
        """
        print('accelerate')
        #ids_berth = self.get_ids()
        vehicles = self.get_prtvehicles()
        # only get those berth with vehicles on the stopping position
        ids_berth_veh = self.ids_berth_all_but_last[self.ids_veh_stopped[self.ids_berth_all_but_last] != EMPTY]
        
        # requires 1 berthahead to be cleared 
        ids_berth_veh_accel =  ids_berth_veh[np.logical_and(np.logical_and(self.ids_veh_locked[self.ids_berth_1_ahead[ids_berth_veh]] == EMPTY,
                                                                                                self.requests_continue[ids_berth_veh] == TOKEN),
                                                            vehicles.ids_stop_target[self.ids_veh_stopped[ids_berth_veh]] != EMPTY )
                                                            ]
        if len(ids_berth_veh_accel) > 0:
            ids_veh_accel = self.ids_veh_stopped[ids_berth_veh_accel]
            
            # transitions
            self.ids_veh_stopped[ids_berth_veh_accel] = EMPTY
            self.ids_veh_accelerating[ids_berth_veh_accel] = ids_veh_accel
            print('  >>> set ids_veh_accelerating',ids_berth_veh_accel)
            print('                       ids_veh',ids_veh_accel)
            
            # control vehicles
            
            
            for id_veh, id_veh_sumo in zip(ids_veh_accel, vehicles.ids_sumo[ids_veh_accel]):
                vehicles.control_speedup(id_veh, speed = self.speed_berth.get_value(), laneindex = 2)
        
            # now take care of last berth of each station
            #ids_berth_veh = self.ids_berth_last[self.ids_veh_stopped[self.ids_berth_last] != EMPTY]
            #ids_stop = self.ids_prtstop[ids_berth_veh]
            #ids_veh_accel = ids_berth_veh[self.are_free[self.ids_berth_1_ahead[ids_berth_veh]]]
            
            
            #self.ids_veh_stopped[ids_berth_veh_accel]
            
    def accelerate_processed(self):
        """
        Starts accelerating  vehicles in berths that are stopped at the stopline, if feasible and requested.
        The last berth in a station is not considered, in this case a station transition
        must take the vehicles out.
        """
        print('accelerate_processed')
        #ids_berth = self.get_ids()
        
        # only get those berth with vehicles on the stopping position
        ids_berth_veh = self.ids_berth_all_but_last[self.ids_veh_stopped_processed[self.ids_berth_all_but_last] != EMPTY]
        
        # requires 1 berthahead to be cleared 
        ids_berth_veh_accel =  ids_berth_veh[self.ids_veh_locked[self.ids_berth_1_ahead[ids_berth_veh]] == EMPTY]
        
        if len(ids_berth_veh_accel) > 0:
            ids_veh_accel = self.ids_veh_stopped_processed[ids_berth_veh_accel]
            
            # transitions
            self.ids_veh_stopped_processed[ids_berth_veh_accel] = EMPTY
            print('  >>> set ids_veh_accelerating',ids_berth_veh_accel)
            print('                       ids_veh',ids_veh_accel)
            self.ids_veh_accelerating[ids_berth_veh_accel] = ids_veh_accel
    ...
    def stopping(self):
        """
        Stop decelerating vehicles in berths if they stopped at the foreseen stopline.
        The last berth in a station is not considered, a station transition
        must take the vehicles out.
        """
        print('stopping')
        ids_berth = self.get_ids()
        ...
        # only get those berth with vehicles on the deceleration position
        ids_berth_veh = ids_berth[self.ids_veh_decelerating[ids_berth] != -1]
        ids_veh = self.ids_veh_decelerating[ids_berth_veh]
        if len(ids_veh) > 0:
            vehicles = self.get_prtvehicles()
            n_veh = len(ids_veh)
            inds_stopped = np.zeros(n_veh, dtype = np.bool)
            for i, id_veh, id_veh_sumo, pos in zip(range(n_veh), ids_veh, vehicles.ids_sumo[ids_veh], self.stoppositions[ids_berth_veh]):
                # ? traci.vehicle.getLanePosition(id_veh_sumo) >= pos
                #inds_stopped[i] = traci.vehicle.isStopped(id_veh_sumo)
                inds_stopped[i] = (traci.vehicle.getLanePosition(id_veh_sumo) >= pos)# (traci.vehicle.getSpeed(id_veh_sumo) < 0.15) 
                print('   check stop id_veh_sumo',id_veh_sumo,traci.vehicle.getLanePosition(id_veh_sumo) ,pos, inds_stopped[i])
                if inds_stopped[i]:
                    print('   id_veh_sumo',id_veh_sumo,'slow down further down to almost zero')
                    #traci.vehicle.setSpeedMode(id_veh_sumo, 3)
                    #traci.vehicle.setSpeed(id_veh_sumo, 0.01)
                    vehicles.control_halt(id_veh_sumo,\
                                laneindex = 2,
                                speed = 0.005,
                                time_slowdown = 0.0,
                                )
            ...
            ids_berth_veh_stop = ids_berth_veh[inds_stopped]
            ids_veh_stop = 1*self.ids_veh_decelerating[ids_berth_veh_stop] # copy
            
            # transitions
            print('  >>> set stopped',ids_berth_veh_stop)
            print('          ids_veh',ids_veh_stop)
            self.ids_veh_decelerating[ids_berth_veh_stop] = -1
            self.ids_veh_stopped[ids_berth_veh_stop] = ids_veh_stop
            
            # reset detector
            #self.are_stopped[ids_berth_veh] = False
            
            
            # control vehicles
            # idea, we may put vehicles into a stop mode here
            # assuing that their speed is almost zero
    def move_in(self):
        """
        Vehicles in stop place do move in, towards parking position
         if no proceed to next stop is requested is requested.
        """
        print('move_in')
        ids_berth = self.get_ids()
        
        # only get those berth with vehicles on stopped place
        ids_berth_veh = ids_berth[self.ids_veh_stopped[ids_berth] != EMPTY]
        ids_veh = self.ids_veh_stopped[ids_berth_veh]
        
        #  vehicles can start moving in park position if
        #  * no vehicle is in parked position
        #  * proceed place is empty => equates a request to move in to the stop
        #    for example because there is a person
        ids_berth_veh_in = ids_berth_veh[ self.requests_continue[ids_berth_veh] == EMPTY ]
        
        # transitions
        if len(ids_berth_veh_in) > 0:
            ids_veh_in = self.ids_veh_stopped[ids_berth_veh_in]
            vehicles = self.get_prtvehicles()
            for id_veh_sumo in vehicles.ids_sumo[ids_veh_in]:
                traci.vehicle.changeLane(id_veh_sumo, 1, 600.0)
            
            self.ids_veh_moving_in[ids_berth_veh_in] = 1*ids_veh_in
            self.ids_veh_stopped[ids_berth_veh_in] = EMPTY
            
            # this would allow arriving vehicles to proceed
            # still the berth remains locked until parking position is concluded
            print('  >>> set requests_continue',ids_berth_veh_in)
            self.requests_continue[ids_berth_veh_in] = TOKEN
            self.ids_veh_processing[ids_berth_veh_in] = 1*ids_veh_in
    
            
        
    def park(self):
        """
        Vehicles in moving_in place are put in parking position
         when lane change process is concluded.
        """
        ids_berth = self.get_ids()
        print('park')
        # only get those berth with vehicles on moving in place
        ids_berth_veh = ids_berth[self.ids_veh_moving_in[ids_berth] != EMPTY]
        ids_veh = self.ids_veh_moving_in[ids_berth_veh]
        ...
        if n_veh > 0:
            vehicles = self.get_prtvehicles()
            ind_arrived = np.zeros(n_veh, dtype = np.bool)
            #for i, id_veh_sumo in zip(xrange(n_veh), vehicles.ids_sumo[ids_veh]):
            #    # arrived at lane index 1?
            #    ind_arrived[i] = traci.vehicle.getLaneIndex(id_veh_sumo) == 1
                
            for i, id_veh, id_veh_sumo, pos in zip(range(n_veh), ids_veh, vehicles.ids_sumo[ids_veh], self.stoppositions[ids_berth_veh]):
                # arrived at lane index 1?
                ind_arrived[i] = (traci.vehicle.getLaneIndex(id_veh_sumo) == 1) & (np.abs(traci.vehicle.getLateralSpeed(id_veh_sumo)) < 0.01)
                #print '   check park id_veh_sumo',id_veh_sumo,'pos=%.1f, pos_lat=%.2f, v_lat=%.2f, park=%s'%(pos,traci.vehicle.getLateralLanePosition(id_veh_sumo),traci.vehicle.getLateralSpeed(id_veh_sumo),ind_arrived[i])
                ...
                if ind_arrived[i]:
                    vehicles.control_stop_boad( id_veh_sumo = id_veh_sumo,\
                                                id_edge_sumo = None,
                                                position = pos,
                                                laneindex = 1,
                                                speed = 4.0,# speed set until it stops completely
                                                )
                                                
                                                
            ids_berth_veh_parked = ids_berth_veh[ind_arrived]
            print('  >>> park vehicles',ids_veh[ind_arrived])
            self.ids_veh_parked[ids_berth_veh_parked] =  1*ids_veh[ind_arrived]
            self.ids_veh_moving_in[ids_berth_veh_parked] = EMPTY
            
            # now unlock the berth so that other vehicles can bypass
            print('  >>> unlock for',ids_berth_veh_parked)
            self.ids_veh_locked[ids_berth_veh_parked] = EMPTY
            
            # TEST: face passengers entering
            # set timer for berth where passengers just entered        
            #self.timer_kickout[ids_berth_veh_parked] = self.time_kickout.get_value()
            # now time launched in load_vehicle
    
    def request_vehicle(self):
        """
        If a person shows up at the platform near the berth,
        the a vehicle request is launched if no vehicle is already 
        being processed.
        """
        
        ids_stop = self.parent.get_ids()
        ids_berths = self.parent.ids_berths[ids_stop]
        ids_berth = self.get_ids()
        stoppositions = self.stoppositions
        print('request_vehicle')
        
        
        
        # detect if new persons have arrived at any berth 
        #are_pers_new = np.zeros(max(ids_berth)+1,dtype = np.bool)
        #ind = 0
        for id_stop, ids_berth  in zip(ids_stop, ids_berths):
            # TODO: here e could get only new persons an put them in a place for each berth
            # this would also be useful to distribute persons when entering platform
            # changeing the final position of the walking stage to where
            # the shortest queue is 
            # attention: entering the platform happens prior to queuing
            ids_sumo_person = self.parent.get_ids_sumo_person(id_stop).copy()
            print('\n  check id_stop',id_stop)
            for id_sumo_person in ids_sumo_person:
                #
                print('    id_sumo_person',id_sumo_person,'stage',traci.person.getStage(id_sumo_person),'speed', traci.person.getSpeed(id_sumo_person),'pos',traci.person.getLanePosition(id_sumo_person))
                if traci.person.getStage(id_sumo_person).type == 3:#3 waiting for driving
                    pos = traci.person.getLanePosition(id_sumo_person)
                    #print '         ids_berth',ids_berth#,'ids_sumo_person',ids_sumo_person
                    #print '     stoppositions',stoppositions[ids_berth]
                    #print '           compare',pos < stoppositions[ids_berth]
                    #print '           compare',pos >= stoppositions[ids_berth[-1]],np.flatnonzero(pos < stoppositions[ids_berth])
                   
                    if pos >= stoppositions[ids_berth[-1]]:  # this should actually no longer happen if target pos of persons on platform is correct
                        id_berth_pers = ids_berth[-1]
                    else:    
                        id_berth_pers = ids_berth[np.flatnonzero(pos < stoppositions[ids_berth]+0.4)[0]]# +0.4 fixes allignment problem
                        # TODO: fix allignment when positioning walk stage
                    
                        
                    print('    found id_berth_pers',id_berth_pers,(self.ids_veh_processing[id_berth_pers]  == EMPTY),(self.requests_continue[id_berth_pers] == TOKEN))
                    #if (self.ids_veh_processing[id_berth_pers]  == EMPTY) &  (self.requests_continue[id_berth_pers] == TOKEN):
                    #are_pers_new[id_berth_pers] = True
                    
                    # insert person in berth queue in any case, no matter whether there is already a vehicle available
                    if self.ids_sumo_passengers[id_berth_pers] is None:
                        self.ids_sumo_passengers[id_berth_pers] = [id_sumo_person]
                    else:
                        if id_sumo_person not in self.ids_sumo_passengers[id_berth_pers]:
                            self.ids_sumo_passengers[id_berth_pers].append(id_sumo_person)
                                
                    print('      queu of id_berth',id_berth_pers,'ids_sumo_passengers',  self.ids_sumo_passengers[id_berth_pers])      
                    #ind += 1
                    # remove persons from platform, as they are now controlled by berth
                    self.parent.wait_person(id_stop, id_sumo_person) 
                    
                    # debug
                    #id_stop = self.ids_prtstop[id_berth]
                    #print '    check removed id_berth',id_berth,'id_stop',id_stop,self.parent.ids_persons_sumo_prev[id_stop]
                
                # debug
                #for id_berth in ids_berth:
                #    print '    check berth',id_berth,self.ids_sumo_passengers[id_berth]
                     
        # check all berth with queue if qualify for requesting vehicle
        ids_berth = self.get_ids()# yes repeat, or rename in loop above
        ids_berth = ids_berth[self.requests_continue[ids_berth] == TOKEN]
        ids_berth_pers  = ids_berth[np.logical_and(self.ids_sumo_passengers[ids_berth] != None, self.ids_veh_processing[ids_berth]  == EMPTY)]
        
        # transition:
        print('  >>> unset requests_continue',ids_berth_pers)
        self.requests_continue[ids_berth_pers] = EMPTY
        
        
    
    def load_vehicle(self):
        """
        If persons show up inside the vehicle then
        the vehicle is considered loaded and a time-out is triggered.
        """
        
        ids_berth = self.get_ids()
        
        # only get those berth with vehicles parked inside the berth
        ids_berth_veh = ids_berth[self.ids_veh_parked[ids_berth] != EMPTY]
        ids_veh = self.ids_veh_parked[ids_berth_veh]
        ...
        if n_veh > 0:
            ...
            vehicles = self.get_prtvehicles()
            print('load_vehicle ids_veh_sumo',len(vehicles.ids_sumo[ids_veh]))
            ind_loaded = np.zeros(n_veh, dtype = np.bool)
            #for i, id_veh_sumo in zip(xrange(n_veh), vehicles.ids_sumo[ids_veh]):
            #    # arrived at lane index 1?
            #    ind_arrived[i] = traci.vehicle.getLaneIndex(id_veh_sumo) == 1
                
            for i, id_veh, id_veh_sumo, id_berth, id_stop  in zip(range(n_veh), ids_veh, vehicles.ids_sumo[ids_veh], ids_berth_veh, self.ids_prtstop[ids_berth_veh]):
                # detect person inside
                #print '  id_veh_sumo',id_veh_sumo,traci.vehicle.getPersonNumber(id_veh_sumo),traci.vehicle.getPersonIDList(id_veh_sumo)
                ids_sumo_person = traci.vehicle.getPersonIDList(id_veh_sumo)
                if len(ids_sumo_person) > 0:
                    ind_loaded[i] = True
                    
                    print('   found in ',id_veh_sumo,'pax',ids_sumo_person,'at id_berth',id_berth, self.ids_sumo_passengers[id_berth])
                    for id_sumo_person in ids_sumo_person:
                        self.ids_sumo_passengers[id_berth].remove(id_sumo_person)
                    
                    # this call is getting destination from person and informs vehicle managements
                    # and update station satistics
                    # TODO: this is a bit messy
                    id_stop_dest,route, duration\
                          = self.parent.route_trip_occupied(id_stop, id_berth, id_veh, ids_sumo_person, self.simtime)
                    
                    # this is setting route and destination to vehicle database
                    # no conttrol actions are taken at this moment
                    vehicles.set_route_target(id_veh, route, id_stop_target = id_stop_dest)
            ...
            ids_berth_veh_loaded = ids_berth_veh[ind_loaded]
            
            print('  >>> set ids_veh_loaded',ids_berth_veh_loaded)
        
            self.ids_veh_loaded[ids_berth_veh_loaded] =  1*ids_veh[ind_loaded]
            self.ids_veh_parked[ids_berth_veh_loaded] = EMPTY
            
            
            # set timer for berth where passengers just boarded        
            self.timer_kickout[ids_berth_veh_loaded] = self.time_kickout.get_value()
        
                
    def init_pax_UNUSED(self):
        """
        Detect if passengers have boarded, if yes, identify passengers and set a counter.
        """        
        
        ids_berth = self.get_ids()
        
        # update kickout timers here
        ind_timers_active = self.timer_kickout[ids_berth] > 0
        self.timer_kickout[ids_berth[ind_timers_active]] -= self.time_update.get_value()
        
        # only get those berth with vehicles on parked place with no kick out timer started
        ids_berth_veh = ids_berth[np.logical_and((self.ids_veh_parked[ids_berth] != EMPTY) , self.timer_kickout[ids_berth] == -1)] 
        ids_veh = self.ids_veh_parked[ids_berth_veh]
        ...
        vehicles = self.get_prtvehicles()
        n_veh = len(ids_veh)
        ind_has_pax = np.zeros(n_veh, dtype = np.bool)
        for i, id_veh, id_veh_sumo, id_berth in zip(range(n_veh), ids_veh, vehicles.ids_sumo[ids_veh],ids_berth_veh):
            # getPersonIDList(self, vehID) Returns the list of persons who are riding in this vehicle.
            # getPersonNumber(self, vehID)
            if traci.vehicle.getPersonNumber(id_veh_sumo) >0:
                ind_has_pax[i] = True
                ids_pax_sumo =  traci.vehicle.getPersonIDList(id_veh_sumo)
                stage = traci.person.getStage(ids_pax_sumo[0])
                print('  found in',id_veh_sumo,'pax',ids_pax_sumo,'stage',stage)
                
                #self.ids_passenger[id_berth] = vehicles.getPersonIDList(id_veh_sumo)
        
        # set timer for berth where passengers just entered        
        self.timer_kickout[ids_berth_veh[ind_has_pax]] = self.time_kickout.get_value()
        
        
    def move_out(self):
        """
        Vehicles in parked place do move out, towards stopping position
        """
        # TODO: deal with first berths separately
        ids_berth = self.ids_berth_all_but_first#self.get_ids()
        
        # only get those berth with vehicles on parked place
        # and who got timed out( or kick out)
        ids_berth_veh = ids_berth[np.logical_and((self.ids_veh_loaded[ids_berth] != EMPTY) ,  (self.timer_kickout[ids_berth] <= 0) )] 
        
        print('move_out',len(ids_berth_veh))
        #print '  check ids_berth_veh',ids_berth_veh
        
        #  vehicles can start moving out if:
        #  * berth is unlocked
        #  * berth before is unlocked
        
         
        inds_berth_veh_out = np.logical_and(self.ids_veh_locked[ids_berth_veh] == EMPTY, self.ids_veh_locked[self.ids_berth_1_before[ids_berth_veh]] == EMPTY) 
        ids_berth_veh_out = ids_berth_veh[inds_berth_veh_out]
        
        # transitions
        if len(ids_berth_veh_out) > 0:
            
            ids_veh_out = self.ids_veh_loaded[ids_berth_veh_out]
        
            print('    ids_berth_veh_out',ids_berth_veh_out,'ids_veh_out',ids_veh_out)
         
            vehicles = self.get_prtvehicles()
            for id_veh_sumo in vehicles.ids_sumo[ids_veh_out]:
                vehicles.control_move_out(id_veh_sumo)
                #traci.vehicle.changeLane(id_veh_sumo, 2, 600.0)
            
            print('  >>> set ids_veh_moving_out',ids_berth_veh_out)
            print('                 ids_veh_out',ids_veh_out)
            self.ids_veh_moving_out[ids_berth_veh_out] = 1*ids_veh_out
            self.ids_veh_locked[ids_berth_veh_out] = 1*ids_veh_out
            
            self.ids_veh_loaded[ids_berth_veh_out] = EMPTY
            
            
    
    def prepare_departure(self):
        """
        Vehicles go from move_out place in stopping position if 
        if vehicles in move_out place have reached the second lane.
        """
        ids_berth = self.get_ids()
        
        # only get those berth with vehicles on move out and processing
        ids_berth_veh = ids_berth[np.logical_and(self.ids_veh_moving_out[ids_berth] != EMPTY, self.ids_veh_processing[ids_berth] != EMPTY)]
        ids_veh = self.ids_veh_moving_out[ids_berth_veh]
        
        print('prepare_departure',len(ids_veh))
        ...
        if len(ids_veh) > 0:
            ...
            for i, id_veh, id_veh_sumo, pos in zip(range(n_veh), ids_veh, vehicles.ids_sumo[ids_veh], self.stoppositions[ids_berth_veh]):
                # arrived at lane index 1?
                ind_arrived[i] = (traci.vehicle.getLaneIndex(id_veh_sumo) == 2) & (np.abs(traci.vehicle.getLateralSpeed(id_veh_sumo)) < 0.01)
                print('   check park id_veh_sumo',id_veh_sumo,'pos=%.1f, pos_lat=%.2f, v_lat=%.2f, park=%s'%(pos,traci.vehicle.getLateralLanePosition(id_veh_sumo),traci.vehicle.getLateralSpeed(id_veh_sumo),ind_arrived[i]))
            ...
            ids_berth_veh_ready = ids_berth_veh[ind_arrived]
            self.ids_veh_stopped_processed[ids_berth_veh_ready] = 1*ids_veh[ind_arrived]
            self.ids_veh_moving_out[ids_berth_veh_ready] = EMPTY
            self.ids_veh_processing[ids_berth_veh_ready] = EMPTY
            
            print('  >>> set ids_veh_stopped_processed',ids_berth_veh_ready)
            print('                            ids_veh',ids_veh[ind_arrived])
            
    
    def exit_lastberth(self, vehicles, length_veh):
        """
        Vehicles leaves completely the berth in the sense that the tail crosses the exit line.
        If this is the case the respective berth in unlocked.
        This method treats the last berths of each stop.
        Leaving vehicles are transferred to the outqueue.
        """
        # take IDs of berth with locking vehicles
        ids_berth_veh = self.ids_berth_last[self.ids_veh_locked[self.ids_berth_last] != EMPTY]
        ids_veh_locked = self.ids_veh_locked[ids_berth_veh]
        if len(ids_veh_locked):
            
            n_veh = len(ids_veh_locked)
            inds_crossed = np.zeros(n_veh, dtype = np.bool)
            for i, id_veh_sumo,  exitpos  in zip(range(n_veh),vehicles.ids_sumo[ids_veh_locked],self.exitpositions[ids_berth_veh]):
                inds_crossed[i] = (traci.vehicle.getLanePosition(id_veh_sumo) - length_veh) > exitpos
                    
            # get only berth where locked vehicles crossed enter line
            ids_berth_veh_exit = ids_berth_veh[inds_crossed]
            ids_veh_exit = ids_veh_locked[inds_crossed]
            print('exit last:ids_veh_locked',ids_veh_locked,'ids_veh_exit',ids_veh_exit)
            #for id_veh_sumo, exitpos in zip(vehicles.ids_sumo[ids_veh_exit],self.exitpositions[ids_berth_veh_exit]):
            #    print '  id_veh_sumo',id_veh_sumo,'pos',traci.vehicle.getLanePosition(id_veh_sumo)- length_veh,exitpos
            
            # transtition lock
            self.ids_veh_locked[ids_berth_veh_exit] = EMPTY
            self.ids_veh_accelerating[ids_berth_veh_exit] = EMPTY
            
            for id_veh, id_stop in zip(ids_berth_veh_exit, self.ids_stop[ids_berth_veh_exit]):
                self.parent.append_to_outqueue( id_stop, id_veh)
    ...
    def exit(self):
        ...
        """
        vehicles = self.get_prtvehicles()
        length_veh = vehicles.get_length()
        
        self.exit_lastberth(vehicles, length_veh)
        
        # take IDs of berth with locking vehicles
        ids_berth_veh = self.ids_berth_all_but_last[self.ids_veh_locked[self.ids_berth_all_but_last] != EMPTY]
        ids_veh_locked = self.ids_veh_locked[ids_berth_veh]
        if len(ids_veh_locked):
            ...
            ids_berth_veh_exit = ids_berth_veh[inds_crossed]
            ids_veh_exit = ids_veh_locked[inds_crossed]
            print('exit:ids_veh_locked',ids_veh_locked,'ids_veh_exit',ids_veh_exit)
            #for id_veh_sumo, exitpos in zip(vehicles.ids_sumo[ids_veh_exit],self.exitpositions[ids_berth_veh_exit]):
            #    print '  id_veh_sumo',id_veh_sumo,'pos',traci.vehicle.getLanePosition(id_veh_sumo)- length_veh,exitpos
            ...
            self.ids_veh_locked[ids_berth_veh_exit] = EMPTY
            self.ids_veh_accelerating[ids_berth_veh_exit] = EMPTY
            # here we could put a token in an empty place
            # but for practical reasons the empty locked means free berth
            
    
    def process_step(self, process):
        simtime = process.simtime
        print(79*'_')
        print('PrtBerths.process_step at',simtime)
        self.simtime = simtime
        # count down timers until negative
        ids = self.get_ids()
        self.timer_kickout[ids[self.timer_kickout[ids] >= 0]] -=  self.time_update.get_value()
        
        # execute transitions
        #for transition in self.transitions:
        #    transition()
        self.iterate_transitions()
    ...
    def make(self, id_stop, position_from=None, position_to=None, 
        ...
        for i, id_berth in zip(range(n_berth),ids_berth):
        
            if i >= 1:
                self.ids_berth_1_before[id_berth] = ids_berth[i-1]
                
            if i >= 2:
                self.ids_berth_2_before[id_berth] = ids_berth[i-2]
                
            if i >= 3:
                self.ids_berth_3_before[id_berth] = ids_berth[i-3]
                
            if i < n_berth-1:
                self.ids_berth_1_ahead[id_berth] = ids_berth[i+1]
                
            if i < n_berth-2:
                self.ids_berth_2_ahead[id_berth] = ids_berth[i+2]    
                
        return ids_berth
        
            
    def set_id_outberth(self, id_berth, id_outberth):
        self.ids_outberth[id_berth] = id_outberth
    
    def set_outberths(self, outberths):
        """
        Defines attributes which are linked with outberths
        """
        self.add_col(am.IdsArrayConf('ids_outberth', outberths, 
                                        is_save = True,
                                        name = 'Out Berth ID', 
                                        info = 'ID of first berth that represents the output queue.',
                                        ))    
    
    def set_id_inberth(self, id_berth, id_inberth):
        self.ids_inberth[id_berth] = id_inberth
    
    def set_inberths(self, inberths):
        """
        Defines attributes which are linked with inberths
        """
        self.add_col(am.IdsArrayConf('ids_inberth', inberths, 
                                        is_save = True,
                                        name = 'In Berth ID', 
                                        info = 'ID of last berth that represents the input queue.',
                                        ))    
...
class PRTOutBerths(PrtBerths):
    """PRT Output Buffers store vehicles at PRT stops"""
    def __init__(self,ident, prtstops, **kwargs):
        
        self._init_objman(  ident = ident, 
                            parent = prtstops, 
                            name = 'PRT Out Berths', 
                            info = 'PRT Output Buffers store vehicles at PRT stops.',
                            **kwargs)
    ...
    def prepare_sim(self, process):
        print('PRTOutBerths.prepare_sim!')
        ids = self.get_ids()
        if len(ids) == 0:  return []
        ...
        self.add_places([   (   'ids_veh_accelerating',     EMPTY*np.ones(n_bertharray,dtype = np.int32),'acc'),
                            ...
                            (   'ids_veh_stopped',          EMPTY*np.ones(n_bertharray,dtype = np.int32),'stop'),
                            (   'ids_veh_locked',           EMPTY*np.ones(n_bertharray,dtype = np.int32),'lock'),
                            (   'requests_continue',        TOKEN*np.ones(n_bertharray,dtype = np.int32),'cont'),
                            (   'ids_veh_processing',       EMPTY*np.ones(n_bertharray,dtype = np.int32),'proc'),
                            (   'ids_veh_moving_in',        EMPTY*np.ones(n_bertharray,dtype = np.int32),'in'),
                            (   'ids_veh_parked',           EMPTY*np.ones(n_bertharray,dtype = np.int32),'park'),
                            #(   'ids_veh_loaded',           EMPTY*np.ones(n_bertharray,dtype = np.int32),'load'),
    ...
    def enter(self):
        ...
        ids_berth_veh = self.ids_berth_all_but_first[self.ids_veh_accelerating[self.ids_berth_1_before[self.ids_berth_all_but_first]] != EMPTY]
        ...
        if len(ids_veh_accel):
            ...
            # remove from entering vehicles from acceleration place of previous berth
            self.ids_veh_accelerating[self.ids_berth_1_before[ids_berth_veh_entered]] = -1
...
class PrtPlatoonStops(am.ArrayObjman):
    ...
    def _init_attributes(self):
        self.add(cm.ObjConf(PrtBerths('berths',self))   ) 
        self.add(cm.ObjConf(PRTOutBerths('outberths',self))   ) 
        berths = self.get_berths()
        berths.set_outberths(self.outberths.get_value())
        self.outberths.get_value().set_inberths(self.berths.get_value())
    ...
    def get_waitpositions(self, ids, offset = -1.0):
        """
    ...
    def prepare_sim(self, process):
        ...
        self.ids_persons_sumo_walking = np.zeros(np.max(ids)+1,dtype = np.object) # walking on platform, not yet waiting
        ...
        for id_stop, id_edge_sumo in zip(ids, self.ids_stop_to_ids_edge_sumo[ids]): 
            ...
            self.ids_persons_sumo_walking[id_stop] = set([]) # walking on platform, not yet waiting
        ...
        #updatedata_outberth = self.outberths.get_value().prepare_sim(process)
        return [    (self.time_update.get_value(),self.process_step),
                    ]+updatedata_berth#+updatedata_outberth
    ...
    def process_step(self, process):
        ...
        for id_stop,id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
            ...
            if ids_veh_sumo_prev != ids_veh_sumo:
                ...
                # TODO: pass all ids_veh_left to exit
            ...
            ##
            ## check whether first vehicle in input queue has reached first stopline
            ##
            if len(self.ids_vehs_inqueue[id_stop]) > 0:
                # TODO: compact to speed up
                id_veh = self.ids_vehs_inqueue[id_stop][0] # take first
                id_veh_sumo = vehicles.get_id_sumo(id_veh)
                pos = traci.vehicle.getLanePosition(id_veh_sumo)
                print('    before: self.ids_vehs_inqueue[id_stop]',self.ids_vehs_inqueue[id_stop],traci.vehicle.isStopped(id_veh_sumo))
                if (pos >= self.stoplines_in[id_stop]-0.5)&traci.vehicle.isStopped(id_veh_sumo): # check a bit before -0.3
                    #if self.debug:
                    print('    Arrived stopline id_veh_sumo',id_veh_sumo,'at pos',pos)
                     
                    
                    
                    # deliver to berth control
                    id_firstberth = self.ids_berths[id_stop][0]
                    
                    ans = berths.insert_veh(id_firstberth, id_veh)
                    if ans:
                        # insertion successful
                        
                        # remove from input queue
                        self.ids_vehs_inqueue[id_stop].pop(0)
                        
                    print('    after: self.ids_vehs_inqueue[id_stop]',ans,self.ids_vehs_inqueue[id_stop])
            ...
            if ids_person_sumo_prev != ids_person_sumo:
                ...
                # new persons walk on platform
                self.ids_persons_sumo_walking[id_stop].update(ids_person_sumo_entered)
    ...
    def get_ids_sumo_person (self, id_stop):
        return  self.ids_persons_sumo_walking[id_stop]
    
    def wait_person(self, id_stop, id_sumo_person):
        """
        Persons are no longer walking on the platform, but 
        started to wait at a berth
        """
        print('wait_person remove',id_sumo_person)
        # TODO: do statistics? 
        #self.ids_persons_sumo_walking[id_stop].difference_update(ids_person_sumo)
        self.ids_persons_sumo_walking[id_stop].discard(id_sumo_person)
        print('  remaining ids_persons_sumo_prev',self.ids_persons_sumo_walking[id_stop])
        
             
    def route_trip_occupied(self, id_stop, id_berth, id_veh,  ids_person_sumo_inveh, simtime):
        
        #berths = self.get_berths()
        #vehicles = self.parent.prtvehicles
        #id_veh_sumo = self.parent.prtvehicles.get_id_sumo(id_veh)
        n_pax = len(ids_person_sumo_inveh)#traci.vehicle.getPersonNumber(id_veh_sumo)
        if self.debug:
            print('route_trip_occupied id_stop', id_stop,'id_berth', id_berth, 'veh=%s'%id_veh,'simtime',simtime,'n_pax',n_pax)
        ...
        if self.debug:
            print('  found person %s in veh prt.%s'%(id_person_sumo_inveh,id_veh)) 
        ...
        return id_stop_dest,route, duration#, id_edge_sumo_from, id_edge_sumo_to 
    ...
    def get_lastpos_outqueue(self, id_stop):
        """
        Returns tail position of last vehicle in out queue
        """
        vehicles = self.parent.prtvehicles
        ids_veh_out = self.ids_vehs_out[id_stop]
        if len(ids_veh_out) == 0:
            return  self.stoplines[id_stop]
        else:
            id_veh_pre, stoppos_pre = ids_veh_out[-1]
            return  stoppos_pre - vehicles.length - 1.5*vehicles.dist_min
    ...
    def try_start_vehicles(self, id_stop, simtime, n_plat_min = -1):
        ...
        if len(self.ids_berths_veh_wait_exit[id_stop])>0:
    ...
    def enter(self, id_stop, ids_veh, fase, simtime):
        ...
        ###########################
        # test: 
        # vehicles complete trip but get no berth assigned (all ids_berth = -1) and 
        # this make them stop
        # before the first berth
        # make always vehicle move in
        ...
        for id_stop, ids_lane, id_lane, position_from, position_to, edgelength  in  zip(\
            ...
            if len(ids_lane) == 3:
                ...
                if (len(ids_mode_allow[1]) == 1)&(len(ids_mode_allow[2]) == 1):
                    ...
                    print('WARNING in make_from_net:',id_stop,'edge has incorrect access:',ids_mode_allow)
            else:
                print('WARNING in make_from_net:',id_stop,'edge has not 3 lanes but',len(ids_lane)) 
    ...
    def make(self, id_ptstop, position_from , position_to):
        ...
        id_edge = self.get_edges([id_stop])
        edgeslength = self.get_scenario().net.edges.lengths[id_edge]
        ids_outberths = self.outberths.get_value().make( id_stop, 
                                        position_from=position_to, 
                                        position_to = edgeslength - self.stoplinegap.get_value()
                                        )
                                        
        # link last boarding berth with first output berth
        self.get_berths().set_id_outberth(ids_berth[-1], ids_outberths[0]) 
        self.outberths.get_value().set_id_inberth(ids_outberths[0],ids_berth[-1] )                                  
        return  id_stop
...
class PrtVehicles(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.ids_stop_target = EMPTY*np.ones(n_id_max, dtype = np.int32)
    ...
    def set_route_target(self, id_veh, route, id_stop_target = None, is_emptytrip = False):
        """
        This is the vehicle route to the next target, not necessary the final one.
        """
        print('set_route_target prt.%d'%id_veh,'id_stop_target',id_stop_target)
        #print '  route',route
    ...
    def control_speedup(self, id_veh, speed = None, laneindex = 2):
        
        id_veh_sumo = self.get_id_sumo(id_veh)
        traci.vehicle.setSpeedMode(id_veh_sumo, 7)
        if speed is None:
            speed = self.speed_max.get_value()
        print('control_speedup',id_veh_sumo,'isStopped',traci.vehicle.isStopped(id_veh_sumo),'to speed',speed)
        
        if traci.vehicle.isStopped(id_veh_sumo):
            traci.vehicle.resume(id_veh_sumo)
        
        traci.vehicle.setMaxSpeed(id_veh_sumo, speed)
        traci.vehicle.changeLane(id_veh_sumo, laneindex, 600.0)
        self.control_slow_down(id_veh, speed)
        
                                                                
    def control_slow_down(self, id_veh = -1, speed = 1.0, time_slowdown = None, id_veh_sumo = None):
        if id_veh_sumo is None:
            id_veh_sumo = self.get_id_sumo(id_veh)
        print('control_slow_down',id_veh_sumo,speed,time_slowdown)
        ...
        traci.vehicle.setSpeedMode(id_veh_sumo, 6)
        traci.vehicle.setMaxSpeed(id_veh_sumo, speed) 
        #self.speed_max = vtypes.speeds_max[id_vtype]
        #self.accel = vtypes.accels[id_vtype]
        #self.decel = vtypes.decels[id_vtype]
        
    def control_halt(self, id_veh_sumo,\
                                laneindex = 2,
                                speed = 0.05,
                                time_slowdown = None,
                                ):
        print('control_halt',id_veh_sumo,speed,time_slowdown)
        # bit0: Regard safe speed 
        # bit1: Regard maximum acceleration 
        # bit2: Regard maximum deceleration - 
        # bit3: Regard right of way at intersections 
        ...
        if time_slowdown == 0.0:
            #print '    set speed to 0'
            traci.vehicle.setSpeedMode(id_veh_sumo, 0)
            traci.vehicle.setSpeed(id_veh_sumo, 0.0) 
        else:
            #print '    set target speed to ',speed,'time_slowdown', time_slowdown
            traci.vehicle.setSpeedMode(id_veh_sumo, 3)
            traci.vehicle.slowDown(id_veh_sumo, speed, time_slowdown)
            
    def control_stopline(self, id_veh = None,\
                            id_edge_sumo = None,
                            position = 0.0,
                            laneindex = 2,
                            speed = 4.0,
        ...
        """Makes vehicle accelerate (if necessary) and stop at stopline with given position.
        """
        # state forewarding_exit_kickout was unknown!!!
        if id_veh_sumo is None:
            id_veh_sumo = self.get_id_sumo(id_veh)
            
        if id_edge_sumo is None:
            id_edge_sumo = traci.vehicle.getRoadID(id_veh_sumo)
            
        p=traci.vehicle.getLanePosition(id_veh_sumo)
        
        print('control_stopline',id_veh_sumo,p,'->',position,'index',laneindex,'edge',id_edge_sumo)
        if p > position:
            
            print('WARNING in control_stopline: veh',id_veh_sumo,'over stopline',position,'by',p-position,'doing Emergency break')
            position = p+0.05
    ...
    def control_stop_boad(self, id_veh = None,\
                            id_edge_sumo = None,
                            position = 0.0,
                            laneindex = 1,
                            speed = 4.0, # set max allowed speed
                            id_veh_sumo = None,
                            ):
        """Makes vehicle accelerate (if necessary) and stop at stopline with given position.
        Vehicle is then in boarding mode, can take passengers and will depart as soon as 
        ...
        traci.vehicle.setSpeed(id_veh_sumo, 0.0)
        traci.vehicle.setSpeedMode(id_veh_sumo, 6)
        print('  set stop for boarding at pos',position)
        traci.vehicle.setStop(  id_veh_sumo,
                                id_edge_sumo, 
        ...
        # try to force vehicle angle to align with station
        #traci.vehicle.changeLane(id_veh_sumo, laneindex, 600.0)  
        #x,y = traci.vehicle.getPosition(id_veh_sumo)  # INVALID_DOUBLE_VALUE                       
        #traci.vehicle.moveToXY(id_veh_sumo, id_edge_sumo, laneindex, x, y, angle= 0.0, keepRoute=1, matchThreshold=100)
    ...
    def control_move_out(self, id_veh_sumo):
        print('control_move_out',id_veh_sumo)
        if traci.vehicle.isStopped(id_veh_sumo):
            #print '  resume stop'
            traci.vehicle.resume(id_veh_sumo) 
        traci.vehicle.changeLane(id_veh_sumo, 2, 600.0)
    ...
    def control_forewarding_exit(self, id_veh,\
        ...
        id_veh_sumo = self.get_id_sumo(id_veh)
        p=traci.vehicle.getLanePosition(id_veh_sumo)
        print('control_forewarding_exit',id_veh_sumo,p,'->',position,'laneindex',laneindex,'edge',id_edge_sumo)
        if traci.vehicle.isStopped(id_veh_sumo):
            print('  resume stop')
            traci.vehicle.resume(id_veh_sumo) 
    ...
    def control_stop_board_old(self, id_veh, id_stop, id_berth,\
                            id_edge_sumo = None,
                            position = None,
                            ):
...
class HcPrtStrategy(StrategyMixin):
        ...
        def plan(self, ids_person, logger = None):
            ...
            while len(ids_person_act)>0:
                ...
                poss_stop_from = prtstops.get_waitpositions(ids_stop_from,  offset = -1.5)
                poss_stop_to = prtstops.get_waitpositions(ids_stop_to, offset = -1.5)
