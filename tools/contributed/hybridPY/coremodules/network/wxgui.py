...
from . import network, routing, netgenerate, netconvert, networktools
...
class WxGui(PtWxGuiMixin, ModuleGui):
    ...
    def make_menu(self):
        ...
        menubar.append_item( 'network/export/Unreal Engine...',
            self.on_net_to_ue, 
            bitmap = self.get_icon('Files-Osm-icon_24.png'),#
            ) 
        ...
        # edit
        menubar.append_menu( 'network/edit',
            bitmap = self.get_icon('netedit.png'),
            )
        
        
        menubar.append_item( 'network/edit/Network...',
            self.on_netedit, 
            info="Edit roads and rail with SUMO's netedit.",
            bitmap = self.get_icon('netedit.png'),
            )
        
        menubar.append_item( 'network/edit/Network with facilities...',
            self.on_netedit_with_poly, 
        ...
        menubar.append_item( 'network/edit/Network on map...',
            self.on_netedit_on_map, 
            info="Edit network with SUMO's netedit. In addition to the network, backround maps are shown. You need to download the maps before with landuse/import maps.",
            bitmap = self.get_icon('netedit.png'),
            )
            
        menubar.append_item( 'network/show network on map...',
            self.on_sumogui, 
        ...
        menubar.append_item( 'network/tools/TL signals optimize...',
            self.on_optimize_tlsignals, 
            #bitmap = wx.ArtProvider.GetBitmap(wx.ART_NORMAL_FILE,wx.ART_MENU),
            )
        ...
        #menubar.append_item( 'network/tools/set all Tls to static',
        #    self.on_set_all_tls_to_static, 
        #    #bitmap = wx.ArtProvider.GetBitmap(wx.ART_NORMAL_FILE,wx.ART_MENU),
        #    )    
        
        menubar.append_item( 'network/tools/Remove all Tls',
            self.on_switch_off_all_tls, 
            #bitmap = wx.ArtProvider.GetBitmap(wx.ART_NORMAL_FILE,wx.ART_MENU),
            )
    ...
    def on_netedit(self,event = None):
        if self._net.call_netedit(is_maps = False, is_poly = False):
            self._mainframe.refresh_moduleguis()
            
    def on_netedit_with_poly(self,event = None):
    ...
    def on_sumogui(self,event = None):
        self._net.call_sumogui(is_maps = True, is_poly = False)
    ...
    def on_optimize_tlsignals(self, event = None):
        """Optimizes phases of traffic lights using demand information."""
        obj =  networktools.TlsOptimizer(self._net.parent, logger = self._mainframe.get_logger())
    ...
    def on_switch_off_all_tls(self,event = None):
        """Remove all Traffic lights from the network"""
        ids_tls = self._net.tlss.get_ids()
        remove_tls = self._net.nodes.remove_tls
        for id_tls in ids_tls:
            remove_tls(id_tls=id_tls, is_remove_id_tls = True)
         
        self._mainframe.browse_obj(self._net.nodes)
        
    #def on_set_all_tls_to_static(self,event = None):
    ...
    def on_net_to_ue(self, event = None):
        """Exports network to various files used for pavement in Unreal Engine Scenario."""
        
        proc = ue.UeNetExporter(self.get_scenario(), logger = self._mainframe.get_logger())
        dlg = ProcessDialog(self._mainframe, proc)
