...
class EdgeDrawings(Polylines):
    def __init__(self,ident, edges, parent,   **kwargs):
        ...
        self.edgeclasses = {
            ...
            'private':('highway.residential',[0.4,0.2,0.6,0.9]),
            'rail':('railway.rail',[0.4,0.2,0.1,0.99]),
            'delivery':('highway.delivery',[0.4,0.4,0.8,0.9]),
            }
    ...
    def update(self, is_update = True):
        ...
        #self.widths.value[:] = self._edges.widths_lanes.value[inds_edges]#np.ones(n, np.float64)*3.2
...
class CrossingDrawings(Fancylines):
    ...
    def update(self, is_update = True):
        ...
        inds_tonode = np.flatnonzero(edges.ids_tonode[ids_edge] == self._crossings.ids_node[ids])    #.value[self._inds_map])
        inds_fromnode = np.flatnonzero(edges.ids_fromnode[ids_edge] == self._crossings.ids_node[ids]) #.value[self._inds_map])
        ...
        vertices_cross[inds_tonode,0,0] = vertices[inds_tonode,1,0] -  0.5*np.cos(angles[inds_tonode] )*widths_edge[inds_tonode]
        vertices_cross[inds_tonode,0,1] = vertices[inds_tonode,1,1] - 0.5*np.sin(angles[inds_tonode] )*widths_edge[inds_tonode]
        vertices_cross[inds_tonode,1,0] = vertices[inds_tonode,1,0] +  0.5*np.cos(angles[inds_tonode] )*widths_edge[inds_tonode]
        vertices_cross[inds_tonode,1,1] = vertices[inds_tonode,1,1]+ 0.5*np.sin(angles[inds_tonode] )*widths_edge[inds_tonode]
        ...
        vertices_cross[inds_fromnode,0,0] = vertices[inds_fromnode,0,0]- 0.5*np.cos(angles[inds_fromnode] )*widths_edge[inds_fromnode]
        vertices_cross[inds_fromnode,0,1] = vertices[inds_fromnode,0,1]- 0.5*np.sin(angles[inds_fromnode] )*widths_edge[inds_fromnode]
        vertices_cross[inds_fromnode,1,0] = vertices[inds_fromnode,0,0]+ 0.5*np.cos(angles[inds_fromnode] )*widths_edge[inds_fromnode]
        vertices_cross[inds_fromnode,1,1] = vertices[inds_fromnode,0,1]+ 0.5*np.sin(angles[inds_fromnode] )*widths_edge[inds_fromnode]
...
class Netdrawing(OGLdrawing):
    ...
    def set_element(self, ident_drawobj, DrawClass, element, layer = 50, is_visible = True):
        ...
        if drawobj is not None:
                ...
                drawobj = DrawClass(ident_drawobj, element, self, _is_visible = is_visible)
...
class NeteditorMainframe(AgileToolbarMixin, wx.Frame):
    """
...
class NeteditorApp(wx.App):
    ...
    def OnInit(self):
        ...
        #hybridPYicon = wx.Icon(os.path.join(IMAGEDIR,'icon_hybridPY.png'),wx.BITMAP_TYPE_PNG, 16,16)
        #wx.Frame.SetIcon(self.mainframe,hybridPYicon)
        #self.mainframe.SetIcon(hybridPYicon)
        

        #if True: #len(sys.argv)>=2:
        #    from  lib.net import readNet
        #    filepath = '/home/joerg/projects/hybridPY/bologna/bologna4.net.xml'#sys.argv[1]
