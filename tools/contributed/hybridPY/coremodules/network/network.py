...
class TrafficLightSystems(am.ArrayObjman):
    ...
    def __init__(self, net, **kwargs):
        ...
        self._init_objman(      ident = 'tlss', parent = net,
                                ...
                                xmltag = ('tlSystems','tlSystem','ids_sumo'),# actually tlls table is exported, but ids_sumo is required for ID
                                **kwargs)
...
class Lanes(am.ArrayObjman):
    ...
    def get_accesslevel_lane(self, id_lane, id_mode):
        """
        Returns accesslevel for id_mode on lane id_lane:
            -1 = No access
            0 = all modes can access
            1 = mode can access with a restricted number of other modes
            2 = exclusive access for id_mode
        ...
        ids_modes_allow, ids_modes_disallow = (self.ids_modes_allow[id_lane], self.ids_modes_disallow[id_lane])
        is_mode_only = False
        is_mode_mixed = False
        is_modes_all = False
...
##                                            groupnames = ['state'],
    ...
    def get_fstar(self, is_return_lists = False, is_return_arrays = False,
        ...
        if is_ignor_connections:
            ...
            if 1: #id_mode is None:
                ...
                # has_access = np.zeros(np.max(ids_lane)+1, dtype = np.bool_)
    ...
    def get_max_connected(self, ids_edge=None, id_mode = None, is_bidir = True,
        ...
        if id_mode == MODES["pedestrian"]:
            # for pedestrians, just return accessible edges
            # connectivity of foodpath cannot be verified with this method
            return ids_edge[self.get_accesslevels(id_mode)[ids_edge] >= 0]
        ...
        #print ('  accessible n_edge',n_edge)
        ...
        print ('    final result: ids_edges_reachable_max', ids_edges_reachable_max) 
        if is_bidir:
        ...
        #print '  final result bidir: ids_edges_reachable_max', ids_edges_reachable_max       
    ...
    def get_times(self, id_mode = 0, is_check_lanes = False, speed_max = None,
                        modeconst_excl = 0.0,modeconst_mix = 0.0, is_precise = True,
                        ids_edge = None ):
        """
        ...
        if speed_max is not None:
            ...
            # limit allowed speeds with max speeds of mode
            speeds = np.clip(speeds, 0.0, self.parent.modes.speeds_max[id_mode])
        #print '  len(speeds)',len(speeds),len(self.get_distances(id_mode = id_mode, is_check_lanes = is_check_lanes, is_precise = is_precise))
        if is_precise:
            times[ids_edge] = self.get_distances(id_mode = id_mode, is_check_lanes = is_check_lanes, is_precise = is_precise)[ids_edge]/speeds
        else:
    ...
    def get_times_cached(self, id_mode, **kwargs):
        """
        Returns edge times for id_mode   from cash
        """
        #print 'get_weighs_and_fstar'
        
        
        if id_mode not in self._timesmap:
            self._timesmap[id_mode] = self.get_times(  id_mode = id_mode, **kwargs)   
  
        return self._timesmap[id_mode]
        
    
        
    def get_fstar_cached(self, id_mode, **kwargs):
        """
        Returns edge  fstar for id_mode  from cash
        """
        #print 'get_fstar_cached'
        
        
        if id_mode not in self._fstarmap:   
            self._fstarmap[id_mode] = self.get_fstar(  id_mode = id_mode, **kwargs)
    ...
    def select_accessible_mode(self, id_mode):
        ...
        are_allowed = np.zeros(len(ids_edges), dtype = np.bool_)
    ...
    def make_segment_edge_map(self, ids = None, is_laneshapes = True):
        ...
        x1 = self._segvertices[:,0,0]
        y1 = self._segvertices[:,0,1]
        z1 = self._segvertices[:,0,2]

        x2 = self._segvertices[:,1,0]
        y2 = self._segvertices[:,1,1]
        z2 = self._segvertices[:,1,2]
        dx = x2-x1
        dy = y2-y1
        alpha_xy=np.arctan2(dy,dx)
        length_xy = np.sqrt(dx*dx + dy*dy)
        
        ind_edge = self._edgeinds[ind_seg]
        inds_seg  = self._seginds[ind_id_edge]
            ...
            while is_cont:
                ...
                if np.isinf(dist2_min):
                    ...
                    if accesslevels is not None:
                        ...
                        is_valid = accesslevels[id_edge] >- 1
                    
                    if priority_max > -1:
                        is_valid = (self.priorities[id_edge] >= priority_min) & (self.priorities[id_edge] <= priority_max)
    ...
    def update(self, ids = None, is_update_lanes = False):
        ...
        lanes = self.get_lanes()
        for id_edge in ids:
            self.speeds_max[id_edge] = np.max(lanes.speeds_max[self.ids_lanes[id_edge]])
...
class Nodes(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        if self.get_version() < 0.2:
                self.delete('n_u_turns')
                self.delete('n_crossings')
                self.delete('n_right_turns')
                self.delete('n_connections')
                
                
                self.types_tl.set_perm('r')
    ...
    def configure_tls(self, id_node, id_tls, typecode = None, tlstype = None, nodetype = "traffic_light"):
        ...
        # types_tl node attribute is depricated 
        if typecode is None:
        ...
        self.parent.tlss.ptypes[id_tls] = self.types_tl[id_node]

    def remove_tls(self, id_node = None, id_tls=None, nodetype = "priority", is_remove_id_tls = True):
        if id_tls is not None:
            ids_node = self.select_ids(self.ids_tls.get_value()==id_tls)
            ids_tls = list(set(self.ids_tls[ids_node]))
            self.types_tl[ids_node] = self.types_tl.choices["none"]
            self.types[ids_node] = self.types.choices[nodetype]
            if is_remove_id_tls:
                self.parent.tlss.del_rows(ids_tls)
                self.ids_tls[ids_node] = -1
        else:
            id_tls = self.ids_tls[id_node]
            
            self.types_tl[id_node] = self.types_tl.choices["none"]
            self.types[id_node] = self.types.choices[nodetype]
            
            if is_remove_id_tls:
                self.parent.tlss.del_row[id_tls]
    ...
    def write_xml(self, fd, indent):
        ...
        for _id,  is_costum_shape in zip(ids, self.are_costum_shape[ids]):
            ...
            for attrconfig in colconfigs:
                ...
                elif attrconfig.attrname == 'types_tl':
                    # no longer part of nod.xmk file
                    pass
...
class Network(cm.BaseObjman):
        ...
        def call_netedit(self, filepath = None, is_maps = False, is_poly = None, command = 'netedit'):
            print('call_netedit')
            ...
            filepath_edges,filepath_nodes,filepath_connections,filepath_tlss = self.export_netxml(filepath = filepath, is_export_tlss = True, is_return_plainfilepath = True)
            # with plain text files netedit does no longer write to  --output-file
            #filepath_edges,filepath_nodes,filepath_connections,filepath_tlss = self.export_painxml(filepath = filepath)
            
            if os.path.isfile(filepath):
                time_mod_filepath = os.path.getmtime(filepath)
                ...
                if is_poly is None: # not specified, let's find out
                    is_poly = (len(self.parent.landuse.facilities) > 0) | (len(self.parent.landuse.pois) >0)
                
                is_ptstops = len(self.ptstops) > 0
                
                addfilepath = self.export_addxml(is_ptstops = is_ptstops, is_poly = is_poly)
                ...
                #print ('  now wait for pid = ',proc.pid)
                
                proc.wait()
                #print ('  Waiting')
                # remove net file if there have been no changes
                # netfile will be actually reimported
                if time_mod_filepath == os.path.getmtime(filepath):#if os.path.isfile(filepath):
                    os.remove(filepath)
        ...
        def write_guiconfig(self, rootname= None, dirname = None, is_maps=False, is_SAV_high_vis=False, delta = np.zeros(3,dtype=np.float32)):
            
            # is_SAV_high_vis is depricated!
            ...
            #if is_SAV_high_vis:
            #    fd_template = open(os.path.join(templatedirpath,'netedit_config_SAV_high_vis.xml'),'r', encoding="utf-8")
            #else:
            fd_template = open(os.path.join(templatedirpath,'netedit_config_hybridpy.xml'),'r', encoding="utf-8")
            ...
            fd_config = open(configfilepath,'w', encoding="utf-8")
        ...
        def export_netxml(self, filepath = None, is_export_tlss = True, is_netconvert = True, is_return_delta = False, is_return_plainfilepath = False, is_no_internal_links = False):
            ...
            filepath_edges,filepath_nodes,filepath_connections,filepath_tlss = self.export_painxml(filepath = filepath, is_export_tlss = is_export_tlss)
        ...
        def import_sumoconnections(self, filename,is_remove_xmlfiles=False, logger = None, **others):
            ...
            # timeit
            exectime_end = time.perf_counter()
...
class SumoConnectionReader(handler.ContentHandler):
    ...
    def __init__(self, net, counter):
        ...
        self.are_passes = np.zeros(counter.n_con,np.bool_)
        self.are_keep_clear = np.zeros(counter.n_con,np.bool_)
        self.positions_cont = np.zeros(counter.n_con,np.float32)
        self.are_uncontrolled = np.zeros(counter.n_con,np.bool_)


        # crossings
        self._ind_cross = -1
        self.ids_node = np.zeros(counter.n_cross,np.int32)
        self.ids_edges = np.zeros(counter.n_cross,np.object_)
        self.widths = np.zeros(counter.n_cross,np.float32)
        self.are_priority = np.zeros(counter.n_cross,np.bool_)
        self.are_discard = np.zeros(counter.n_cross,np.bool_)
...
class SumoNodeReader(handler.ContentHandler):
    ...
    def __init__(self, net, counter):
        ...
        ### node attrs
        self.ids_sumo = np.zeros(counter.n_node,np.object_)
        ...
        self.ids_sumo_tls = np.zeros(counter.n_node,np.object_)
        self.turnradii = np.zeros(counter.n_node,np.float32)
        self.are_costum_shape = np.zeros(counter.n_node,np.bool_)
        self.shapes = np.zeros(counter.n_node,np.object_)
        self.are_keep_clear = np.zeros(counter.n_node,np.bool_)
        self._ind_node = -1
        self.ids_sumo_controlled = np.zeros(counter.n_node,np.object_)
    ...
    def write_to_net_post(self):
        ...
        tlss = self._net.tlss
        tlls = tlss.ids_tlls.get_linktab()
        self._net.nodes.ids_tls[self.ids_node] = tlss.ids_sumo.get_ids_from_indices_save(self.ids_sumo_tls)

        # depricated:
        # set node attribute types_tl which is useless and redundant
        # and will disappear in the future
        for id_node, id_tls in zip(self.ids_node,  self._net.nodes.ids_tls[self.ids_node]):
                #id_tll = tlss.ids_tlls[id_node][0] # there should be at least 1 program
                if id_tls > -1:
                    self._net.nodes.types_tl[id_node] = tlls.ptypes[tlss.ids_tlls[id_tls][0]]
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'nodes':
            ...
            if attrs['id'][0]!=':':# no internal node
                ...
                # set in post...and later removed entirely
...
class SumoEdgeReader(handler.ContentHandler):
    ...
    def __init__(self, net, counter, offset_delta = np.array([0.0,0.0]) ):
        ...
        self.ids_edge_sumo = np.zeros(counter.n_edge,np.object_)#net.edges.ids_sumo
        ...
        self.types_edge = np.zeros(counter.n_edge,np.object_)
        ...
        self.inds_lanes_edges = np.zeros(counter.n_edge,np.object_)
        ...
        self.ids_modes_allow = np.zeros(counter.n_lane,np.object_)
        self.ids_modes_disallow = np.zeros(counter.n_lane,np.object_)
        self.ids_mode_lanes = np.zeros(counter.n_lane,np.int32)
        self.inds_edge_lanes = np.zeros(counter.n_lane,np.int32)
        #self.shapes_lanes = np.zeros(counter.n_lane,np.object_)


        ### roundabout attrs
        #print '  n_roundabout',counter.n_roundabout
        self._ind_ra = -1
        self.ids_sumoedges_ra = np.zeros(counter.n_roundabout,np.object_)
        self.ids_nodes_ra = np.zeros(counter.n_roundabout,np.object_)
        ...
        if name == 'edge':
            ...
            while index < n_lane:
                ...
                if self._allow_egdeattr is not None:
                    ...
                    ids_modes_disallow  = []
                elif self._disallow_egdeattr is None:
                    if edgetype in OSMEDGETYPE_TO_MODES:
                        ids_modes_allow, speed_max_default = OSMEDGETYPE_TO_MODES[edgetype]
                    else:
                        ids_modes_allow = []
                ...
                elif self._disallow_egdeattr == 'all':
                    ids_modes_disallow = list(self._modes.get_ids())
                    ids_modes_allow = []
                else:
                    ids_modes_disallow  = list(self._modenames.get_ids_from_indices_save(self._disallow_egdeattr.split(' ')))
                    ids_modes_allow = []
...
class SumonetMerger(CmlMixin,Process):
    ...
    def do(self):
        ...
        self.run_cml(cml)
        if self.status == 'success':
             self._net.import_netxml(filepath=self.filepath_out, rootname=None, is_clean_nodes = False, is_remove_xmlfiles= False)
