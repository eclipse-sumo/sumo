...
class Road:
    ...
    def make_bikewayattrs(self):
        ...
        if osmattrs.get('oneway','no')=='no':
            ...
            if cycleway in LANEATTRS:
                ...
                self.make_bikelane(is_opp = True, is_rightside = True, ids_modes_allow = self.ids_modes_pt_shared,width = width_right)
                #if osmattrs.get('bicycle','no') == 'yes':
                #    self.make_bikelane(is_opp = False, is_rightside = True,width = width_right)
                return
        ...
        if busway is not "":
            ...
            if osmattrs.get('oneway','no')=='no':
                ...
                if busway == 'lane':
                    ...
                    self.make_buslane(is_opp = True,  is_rightside = False,ids_modes_allow = self.ids_modes_pt_shared,width = width_left)
    ...
    def configure_edge(self, id_edge, net, is_opp = False, is_remove_sidewalk = 0):
        ...
        if self.highway not in ['residential','living_street','cycleway']:
            ...
            for laneattrs in lanes:
                if (ind==0) & laneattrs['is_sidewalk'] & (is_remove_sidewalk==2)& (ind_lastlane>0):# &lanes[-1]['is_sidewalk']:
                    print('  sidewalk removed on right side')
                    pass
                elif (ind==ind_lastlane) & laneattrs['is_sidewalk'] & (is_remove_sidewalk==1)&(ind_lastlane>0):#&lanes[0]['is_sidewalk']:
...
class OxImporter(Process):
    ...
    def configure_footpath(self):
        ...
        footpath_nomerge = ('footway','path','pedestrian','stairs','steps','platform','primary_link','secondary_link')
            ...
            if road.highway in footpath_condig:
                ...
                if 0:#(road.footway == 'sidewalk'):
                    ...
                    for i in range(0,len(shape)):
                        point = shape[i]
                        is_foundmatch = False
                        inds_edge_main, dists,angles = self.get_closest_edge_dist(point, ind, n_best = 20, d_max = dist_min_detect)
                        #matchdata = {}
