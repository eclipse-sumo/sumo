...
class TlsGenerator(netconvert.NetConvertMixin):
    ...
    def make_majortls(self, id_node_major, inds_route):
        ...
        for id_node, nodeattrs in nodes_tls.items():
            ...
            for id_edge in nodes.ids_incoming[id_node]:
                ...
                print('       is external',edges.ids_fromnode[id_edge] not in ids_nodes_tls,'is_cycle_edge',is_cycle_edge,'is_major',id_edge in edges_major,'is_noped')#,not is_ped_edge,'not from TLS',edges.ids_fromnode[id_edge] not in ids_nodes_tls
        ...
        for id_incoming_tls in ids_incoming_major_tls:
            ...
            for id_outgoing_tls in ids_outgoing_major_tls:
                ...
                if (id_outgoing_tls in route_major) & (ind_from<(len(route_major)-1)):
                    ...
                    routes, routecosts = self.correct_tls_route(route_major[ind_from:ind_to+1], nodes_tls, is_check_angle = False)
                ...
                if len(route) == 0:
                    ...
                    if len(route_raw)>0:
                        ...
                        routes,routecosts = self.correct_tls_route(route_raw, nodes_tls, is_check_angle = True, angle_max = 95.0/180*np.pi)
                    else:
                        routes = []
                        routecosts = 0.0
                    ...
                    if (id_incoming_tls in D) & (id_outgoing_tls in D):
                        ...
                        if edges.ids_tonode[id_outgoing_tls] != edges.ids_fromnode[id_incoming_tls]:
                            ...
                            if len(route_raw)>0:
                                ...
                                routes, routecosts = self.correct_tls_route(route_raw, nodes_tls, is_check_angle = True, angle_max = 60.0/180*np.pi)
                                if len(routes)>0:
                ...
                if not ((id_incoming_tls in ids_outgoing_major_tls)&(id_outgoing_tls in ids_outgoing_major_tls)):
                    ...
                    if edges.ids_tonode[id_outgoing_tls] != edges.ids_fromnode[id_incoming_tls]:
                        ...
                        if len(route_raw)>0:
                            ...
                            routes, routecosts = self.correct_tls_route(route_raw, nodes_tls, is_check_angle = True, angle_max = 90.0/180*np.pi)
                            if len(routes)>0:
                                # there are one or several valid subroutes
                                for route, routecost in zip(routes, routecosts):
                                    routes_tls.append(route)
        ...
        for ind_route in np.argsort(routes_tls_cost)[::-1]:
            ...
            for id_edge_from,   id_edge_to, id_node in zip(ids_edges[:-1], ids_edges[1:],  edges.ids_tonode[ids_edges[:-1]]):
                ...
                if  id_node in nodes_tls:
                    if not nodes_tls[id_node]['is_cycleped']:
                        ...
                        inds_con_route |= (lanes.ids_edge[ids_fromlane_tls] == id_edge_from) & (lanes.ids_edge[ids_tolane_tls] == id_edge_to)
            ...
            if  prio != -1:
                ...
                routeconnectordata.append((ind_route, inds_con_route, prio, inds_cons_conflict_route, inds_cons_merge_route))
        ...
        while i_route<n_routes:
            ...
            if phasegroups[i_route] == -1:
                ...
                ind_tlsroute, inds_con_route, prio, inds_cons_conflict_route, inds_cons_merge_route = routeconnectordata[i_route]
                ...
                while j_route<n_routes:
                    if phasegroups[j_route] == -1:
                        ...
                        ind_tlsroute2, inds_con_route2, prio2, inds_cons_conflict_route2, inds_cons_merge_route2 = routeconnectordata[j_route]
            ...
            if nodeattrs['is_crossing']:
                ...
                for id_edge_from in nodeattrs['ids_cycleped_incoming']:#nodes.ids_incoming[id_node]:
                    for id_edge_to in nodeattrs['ids_cycleped_outgoing']:
                        ...
                        inds_con_edge = inds_con_node & (lanes.ids_edge[ids_fromlane_tls] == id_edge_from)& (lanes.ids_edge[ids_tolane_tls] == id_edge_to)
                        ...
                        if np.any(inds_con_edge):
                            ...
                            for id_con, ind_con in zip(ids_con_edge, np.flatnonzero(inds_con_edge)):
                                ...
                                inds_con_check |= inds_cons_conflict[id_con] & are_enabeled & (lanes.ids_edge[ids_tolane_tls] != id_edge_to)
        ...
        if 1:
            ...
            for i in np.arange(n_phase):
                ...
                for ind_confl, slot_raw  in zip(inds, slots_raw[inds]):
                    self.block_connection(ind_confl, slot_raw , slots, ids_fromlane_tls, ids_tolane_tls, ids_con_tls,are_enabeled)
    ...
    def init_slots(self,  inds_con_route, ids_con_tls,
        ...
        for id_con, ind_con in zip(ids_con_tls_route, np.flatnonzero(inds_con_route)):
            ...
            slots_raw = 2*(inds_cons_conflict[id_con]&np.logical_not(inds_cons_merge[id_con])) + 1* inds_cons_merge[id_con]
            ...
            if edges.lengths[lanes.ids_edge[id_fromlane]] < self.length_min_notls:
                ...
                if (len(inds_con)>0) & np.all(slots[inds_con] >=0):
                    ...
                    for ind_con in inds_con:
                        self.block_connection(ind_con,2, slots, ids_fromlane_tls, ids_tolane_tls,ids_con_tls,are_enabeled)
                    print('.')
...
class BikenetworkCompleter(Process):
    def __init__(self,  net, logger = None, **kwargs):
        ...
        self.ids_modes_tocomplete = set([modes.get_id_mode("pedestrian"), modes.get_id_mode("delivery"),modes.get_id_mode("bus")])
