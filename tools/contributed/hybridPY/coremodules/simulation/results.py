...
from agilepy.lib_base.misc import format_seconds
...
#        ids_routeres = ids_routeres[700:1000]
        ...
        ids_routeres_od_auto = ids_routeres_od[(ids_od_modes == self._edges.parent.modes.names.get_id_from_index('passenger'))]
        ids_routeres_od_taxi = ids_routeres_od[(ids_od_modes == self._edges.parent.modes.names.get_id_from_index('taxi'))]
        ids_routeres_od_car = list(ids_routeres_od_auto)+list(ids_routeres_od_taxi)
        ids_routeres_od_bike = ids_routeres_od[(ids_od_modes == self._edges.parent.modes.names.get_id_from_index('bicycle'))]
        ids_routeres_od_moto = ids_routeres_od[(ids_od_modes == self._edges.parent.modes.names.get_id_from_index('motorcycle'))]
    ...
    def __init__(self, ident, parent, trips, edges, datapathkey = 'routesdatapath',
        ...
        attrinfos = OrderedDict([\
                ...
                ('type',   {'name':'Vehicle type', 'xmltag':'type',   'unit':None,\
                    'default':np.object_, 'info':'Vehicle type','groupnames':['routedata']}),
        ...
        for attrconfig in attrconfigs: 
            ...
            if attrname != 'ids_sumo':
                ...
                if type(default) in (bool,):
                    conversion = 'b'#str
                    values_attr = np.zeros(n,dtype = np.bool_)
                else:
                    conversion = 's'#str
                    values_attr = np.zeros(n,dtype = np.object_)
...
class Edgeresults(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        attrinfos = OrderedDict([\
                ...
                ('detectorflows',  {'name':'Detector flows',      'unit':None,    'default':0., 'info':'Flows measured by detectors','xmltag':'detectorflows','groupnames':['edgedata']}),                           
                ...
                ('total_flows',  {'name':'Total flows',      'unit':None,    'default':0, 'info':'Total flow in the edge','xmltag':'entered','groupnames':['total_flows']}),
                ('total_od_flows',  {'name':'Total OD flows',      'unit':None,    'default':0, 'info':'Total OD flow in the edge','xmltag':'entered','groupnames':['total_od_flows']}),
                ('total_vp_flows',  {'name':'Total VP flows',      'unit':None,    'default':0, 'info':'Total VP flow in the edge','xmltag':'entered','groupnames':['total_vp_flows']}),
                ('total_ptline_flows',  {'name':'Total pt flows',      'unit':None,    'default':0, 'info':'Total pt flow in the edge','xmltag':'entered','groupnames':['total_ptline_flows']}),
                ('total_car_flows',  {'name':'Total car flows',      'unit':None,    'default':0, 'info':'Total car flow in the edge','xmltag':'entered','groupnames':['total_car_flows']}),
                ('total_bike_flows',  {'name':'Total bike flows',      'unit':None,    'default':0, 'info':'Total bike flow in the edge','xmltag':'entered','groupnames':['total_bike_flows']}),
                ('total_moto_flows',  {'name':'Total moto flows',      'unit':None,    'default':0, 'info':'Total moto flow in the edge','xmltag':'entered','groupnames':['total_moto_flows']}),
                ('total_iauto_flows',  {'name':'Total iauto flows',      'unit':None,    'default':0, 'info':'Total iauto flow in the edge','xmltag':'entered','groupnames':['total_iauto_flows']}),
                ('total_ibike_flows',  {'name':'Total ibike flows',      'unit':None,    'default':0, 'info':'Total ibike flow in the edge','xmltag':'entered','groupnames':['total_ibike_flows']}),
                ('total_imoto_flows',  {'name':'Total imoto flows',      'unit':None,    'default':0, 'info':'Total imoto flow in the edge.','xmltag':'entered','groupnames':['total_imoto_flows']}),
    ...
    def _init_attributes_marouter(self):
    ...
    def add_edgelength(self):
        ...
        self.add_resultattr(  'lengths',default = 0.0,
                                    perm = 'r',
                                    unit = 'm', 
                                    name = 'Edge length',
                                    info = 'Edge length.')
    ...
    def add_edgetimes_est(self):
        edges = self.parent.get_scenario().net.edges
        self.add_resultattr(  'times_est',
                                    default = 0.0,
                                    perm = 'r',
                                    unit = 's', 
                                    name = 'Est. edge times ',
                                    info = 'Estimated edge travel times.')
        ids = self.get_ids()
        
        self.times_est[self.ids_edge[ids]] = edges.get_times()[self.ids_edge[ids]]
        
    def add_entered_est(self, ids_edge, entered_vec, attrname = 'entered_est'):
        #print 'add_entered_est',attrname,len(ids_edge)
        edgeattrconf = getattr(self, attrname)
        edgeattrconf.reset()
        ids_results = np.zeros(len(ids_edge), dtype=np.int32)
        i = 0
        for id_edge, entered in zip(ids_edge, entered_vec):
            #print '  id_edge',id_edge,self.ids_edge.has_index(id_edge)
            if self.ids_edge.has_index(id_edge):
                id_res = self.ids_edge.get_id_from_index(id_edge)
            else:
                id_res =  self.add_row(ids_edge = id_edge)
            edgeattrconf[id_res] = entered
    ...
    def filter_zoneedges(self, ids_zone, is_invert = False):
        ...
        if len(ids_zoneedge)>0:
            ...
            inds = np.zeros(len(ids_res), dtype = np.bool_)
...
#        ids_routeres = ids_routeres[700:1000]
    ...
    def import_sumoxml(self,filepath,sumo, attrconfigs):    
        ...
        #id_type = 'edge',
        #reader = 'interval',
        ids_sumo, results, interval = read_interval2( filepath,
                                            sumo.simtime_start + sumo.time_warmup, 
                                            sumo.simtime_end, element, attrconfigs)
        #print '  ids_sumo',ids_sumo
        #print '  results.keys()',results.keys()
        #print '  results',results
        # create ids for all colums
    ...
    def import_marouterxml(self, filepath, marouter):
            ...
            self._init_attributes_marouter()    
            self.import_sumoxml(filepath, marouter, self.get_group('marouter'))
...
class EdgeresultsEvolution(Edgeresults):
    """
    Evolution of edgeresults.
    Here each result attribute of a specific edge is a vector
    where each entry refers to a different time interval. 
    """
    def __init__(self, parent, edges, is_add_default=True, **kwargs):
        
        self._init_objman(  ident='edgeresultsevolution', 
                            parent = parent, # main results object
                            name = 'Edge results evolution', 
                            info = 'Table with temporal evolution of simulation results for each network edge.', 
                            #xmltag = ('vtypes','vtype','ids_sumo'),
                            **kwargs)
        
        self.add(cm.AttrConf( 'n_interval', 24,
                                                groupnames = ['options',],
                                                name = 'Number of intervals',
                                                perm = 'r',
                                                info = 'Number of reguler time intervals. This will be determined by the simulation time.',
                                                ))
        
        self.add(cm.AttrConf( 'time_start_recording', 0,
                                groupnames = ['options',],
                                name = 'Time start recording',
                                perm = 'r',
                                unit = 's',
                                info = 'Time when recording starts.',
                                ))
        
        self.add(cm.AttrConf( 'time_interval', 3600,
                                groupnames = ['options',],
                                name = 'Time of interval',
                                perm = 'r',
                                unit = 's',
                                info = 'Time of one interval.',
                                ))
    ...
    def format_interval(self, ind_int):
        """
        Returns a formatted string with interval time information in hours:mins.
        """
        time_start_recording, time_end_recording = self.get_timeinterval(ind_int)
        return format_seconds(time_start_recording)+'-'+format_seconds(time_end_recording)
        
    def adapt_intervals(self, time_start_recording, n_interval, time_interval, ids_edge = None):
        """Adapt intervals in order to fit given time range"""
        print('adapt_intervals',time_start_recording, 'n_interval', n_interval,'time_interval', time_interval,'len',len(self),len(self.get_ids()))
        
        if self.time_interval.get_value() != time_interval:
            print('  WARNING: time_interval',time_interval,'incompatible with existing',self.time_interval.get_value(),'=>reset.')
            self.reset_intervals(time_start_recording, n_interval, time_interval, is_fill = True, ids_edge = ids_edge)
            return 0, n_interval 
            
        
        
        if (len(self) == 0):
            print('  Results empty, fill results with zeros.')
            self.reset_intervals(time_start_recording, n_interval, time_interval, is_fill = True, ids_edge = ids_edge)
            return 0, n_interval
            
        ids_edgeres = self.get_ids()
        if ids_edge is not None:
            ids_edge_missing = set(ids_edge).difference(self.ids_edge[ids_edgeres])
            print('  Complete rows with missing edges',len(ids_edge_missing))
            if len(ids_edge_missing) > 0:
                self.add_rows(n = len(ids_edge_missing), ids_edge = list(ids_edge_missing), )
            ids_edgeres = self.get_ids()
        else:
            ids_edge_missing = set()
        
        n_edges = len(ids_edgeres)
        
        #resultvec = self.get_group_attrs('results').values[0][ids_edge[0]]
        
        inds = self.get_inds(ids_edgeres)
        
        #  time intervals must coincide
        
        n_interval_cur = self.n_interval.get_value()        
        
        
            
        i_start = self.get_ind_interval(time_start_recording)
        
        if i_start < 0:
            n_interval_new = max(n_interval_cur -i_start, n_interval)
            ind_offset = -i_start
            i_start_data = 0
            time_start_recording_new = time_start_recording
        if i_start >= 0:
            n_interval_new = max(n_interval_cur , i_start + n_interval)
            ind_offset = 0
            i_start_data = i_start
            time_start_recording_new = self.time_start_recording.get_value()
        #print '    n_interval_cur',n_interval_cur,'time_start_recording_cur',self.time_start_recording.get_value(),'len(inds)',len(inds),'n_edges',n_edges
        #print '    n_interval_new',n_interval_new,'time_start_recording_new',time_start_recording_new,
        #print '    i_start',i_start,'ind_offset',ind_offset,'i_start_data',i_start_data,'i_end_data',i_start_data + n_interval
        
        
        if (ind_offset == 0) &(time_start_recording == self.time_start_recording.get_value()) & (n_interval_cur == n_interval_new) & (len(ids_edge_missing) == 0):
            #print '  All parameters OK, nothing to adapt.'
            return i_start_data, i_start_data + n_interval
           
        
        for attrname, attrconf in self.get_group_attrs('results').items():
            values = attrconf.get_value()[inds]
            attrconf.set_value( np.zeros((n_edges, n_interval_new), dtype = values.dtype) ) 
            #print '  adapt attrconf',attrname,values.shape, attrconf.get_value()[inds,ind_offset:].shape
            attrconf.get_value()[inds,ind_offset:ind_offset+n_interval_cur] = values
        # set  n_interval, time_interval
        self.time_start_recording.set_value(time_start_recording_new)
        self.n_interval.set_value(n_interval_new)
        # must be unchanged self.time_interval.set_value(time_interval)
        
        #print '  return indexes for placing requested data i_start', i_start_data,'i_end', i_start_data + n_interval
        return i_start_data, i_start_data + n_interval
        
        
    def reset_intervals(self, time_start_recording, n_interval, time_interval, is_fill = False, ids_edge = None):
        print('reset_intervals',time_start_recording, n_interval, time_interval,'is_fill',is_fill)
        # if these parameters are already satisfied, do not reset
        if (self.time_start_recording.get_value() != time_start_recording) | (self.n_interval.get_value() != n_interval) | (self.time_interval.get_value() != time_interval):
            #print '  delete result group',self.get_group_attrs('results').keys()
            attrnames = list(self.get_group_attrs('results').keys())
            is_marouter = 'entered_mar' in attrnames
            for attrname in  attrnames:
                self.delete(attrname)
            
            # set  n_interval, time_interval
            self.time_start_recording.set_value(time_start_recording)
            self.n_interval.set_value(n_interval)
            self.time_interval.set_value(time_interval)
            
            self._init_attributes()
            if is_marouter:
                self._init_attributes_marouter()
            #print '  finish reset time_start_recording',self.time_start_recording.get_value()
            
            
            if is_fill | (ids_edge is  None):
                if ids_edge is  None:
                    edges = self.ids_edge.get_linktab()
                    ids_edge = edges.get_ids()
                print('  add_rows',len(ids_edge))
                self.add_rows(n = len(ids_edge), ids_edge = ids_edge)
    ...
    def add_resultattr(self, attrname, **kwargs):
        ...
        self.add_col(am.ArrayConf(  attrname, default*np.ones(self.n_interval.get_value(), dtype = type(default)),**kwargs))
    
    def import_edgedata(self,sumo,  filepath):
        print('EdgeresultsEvolution.import_edgedata',filepath)
        self.import_sumoxml(filepath, self.get_group('edgedata'))
    
    def get_ind_interval(self, t):
        """Returns time interval index for time t"""
        #print 'get_ind_interval',t,self.time_start_recording.get_value(),self.time_interval.get_value()
        return int((1.0*t - self.time_start_recording.get_value())/self.time_interval.get_value()+0.5)
    
    def add_edgevalue(self, id_edge, attrname, t, attrval):
        """Set or add, if existant, attribute value at a given time t to result attribute attrname"""
        #print 'add_edgevalue id_edge',id_edge, attrname
        
        if self.ids_edge.has_index(id_edge):
            id_edgeres = self.ids_edge.get_id_from_index(id_edge)
        else:
            id_edgeres = self.add_row(ids_edge = id_edge, )
        
        #print '  id_edgeres',id_edgeres,'ind',self.get_ind_interval(t),'attrval',attrval
        getattr(self, attrname)[id_edgeres][self.get_ind_interval(t)] = attrval
        
    
    def get_timeinterval(self, ind_int):
        """
        Returns begin time and end time in seconds of interval with index  ind_int
        """
        time_start_recording = self.time_start_recording.get_value() + ind_int*self.time_interval.get_value()
        time_end_recording = time_start_recording + self.time_interval.get_value()
        return time_start_recording,time_end_recording
    
    def get_time_end_recording(self):
        return self.time_start_recording.get_value() + (self.n_interval.get_value() +1 ) * self.time_interval.get_value()
    
    def import_marouterxml(self, filepath, marouter = None):
        self._init_attributes_marouter()    
        self.import_sumoxml(filepath, attrconfigs = self.get_group('marouter'))
            
    def import_sumoxml(self, filepath,  attrconfigs = None, time_start_recording = 0,  time_end_recording = np.inf):    
        element = 'edge'
        print('EdgeresultsEvolution.import_sumoxml',element)
        if attrconfigs is None:
            attrconfigs = self.get_group('edgedata')
        ...
        if len(self) != n_edges:
            self.clear()
            print ('Add',n_edges,'rows in results')
            self.add_rows(n=n_edges, ids_edge = ids_edge)
            
        #time_start_recording, time_end_recording = self.get_timeinterval(ind_int)
        reader = read_edgeevolution(filepath, edges.ids_sumo[ids_edge], time_start_recording, time_end_recording,  element, attrconfigs)
                                      
        
        time_start_recording,  time_interval, n_interval = reader.get_interval()
        
        if n_interval == 0: 
            print('WARNING: no intervals found.')
            return False
        
        print('  n_edges',n_edges,'time_start_recording',time_start_recording,'time_interval',time_interval,'n_inter',n_interval)
        print('  len(ids_edge)',len(ids_edge),self.ids_edge.get_linktab(),'len(res)=',len(self))
        
        
        i_start, i_end = self.adapt_intervals(time_start_recording, n_interval, time_interval, ids_edge = ids_edge)
                            
                    
        inds_res = self.get_inds(ids_edge)
        
        print('  i_start',i_start,'i_end',i_end,'n_interval_new',self.n_interval.get_value(),'n_res',len(inds_res))
            
        results = reader.get_data()

        for attrconfig in attrconfigs:
            # here we need to get the np array to broadcast rows and cols
            print('    copy attrconfig', attrconfig.attrname,np.max(results[attrconfig.attrname]),np.max(attrconfig[ids_edge]),'shape',results[attrconfig.attrname].shape)
            
            attrconfig.get_value()[inds_res,i_start:i_end] = results[attrconfig.attrname]# already in correct datatype
            
        del reader
        return True
                    
    def import_sumoxml_slow(self, filepath,   attrconfigs):    
        element = 'edge'
        print('EdgeresultsEvolution.import_sumoxml',element)
        
        for ind_int in range(self.n_interval.get_value()):
            #print '='*70
            time_start_recording, time_end_recording = self.get_timeinterval(ind_int)
            #print '  start with interval ind_int',ind_int,'time_start_recording',time_start_recording,'time_end_recording',time_end_recording
            ids_sumo, results, interval = read_interval2( filepath,
                                                time_start_recording, 
                                                time_end_recording, 
                                                element, attrconfigs)
            #print '  ids_sumo',ids_sumo
            #print '  results.keys()',results.keys()
            ...
            for i in ind_range:
                id_edge = ids_sumoedge.get_id_from_index(ids_sumo[i])
                if self.ids_edge.has_index(id_edge):
                    # edge does already exist in results
                    # get result id from edge id
                    ids[i] =  self.ids_edge.get_id_from_index(id_edge)
                else:
                    # new eddge found
                    # assign new result id
                    ids[i] =  self.add_row(ids_edge = id_edge)
            ...
            for attrconfig in attrconfigs: 
                ...
                # pick first in default array to identify conversion
                default = attrconfig.get_default()[0]
                #print '   default',attrname,'scalar def',default#,'vec def',attrconfig.get_default()
                if type(default) in (np.int32, np.int64,int, int):
                    conversion = 'i'#int
                    #values_attr = np.zeros(n,dtype = np.int32)
                elif type(default) in (np.float32, np.float64, float,complex):
                    conversion = 'f'#float
                    #values_attr = np.zeros(n,dtype = np.float32)
                else:
                    conversion = 's'#str
                ...
                if hasattr(attrconfig,'is_average'):
                    is_average = attrconfig.is_average
                #print '   copy',attrname,'is_average',is_average,default,type(default)
                # this is a tricky way to read the data stored in
                # dictionarie into array tructures as used in results
                # problem is that not all dictionaries have all ids    
                for i, _id in zip(ind_range, ids):
                    if is_average:
                        valcum, num = results[attrname].get(ids_sumo[i],(default,1))
                        val = valcum/float(num) # average over measurements!
                    else:
                    ...
                    #else:
                    #    values_attr[i]=val
                    #print '      attrname',attrname,conversion,'val',val,is_average,type(val)
                    ...
                    attrconfig[_id] = vec
                    #print '        attrconfig[_id]',attrconfig[_id]
...
class Simresults(cm.BaseObjman):
        def __init__(self, ident= 'simresults', 
                        parent = None, # typically scenario.simulation
        ...
        def _init_attributes(self):
            ...
            self.edgeresultsevolution = attrsman.add(cm.ObjConf( EdgeresultsEvolution(self, scenario.net.edges),
                                                        groupnames = ['Edge results'],
                                                        ))
            ...
            for simobj in self.parent.get_simobjects():
                #print((type(simobj)))
                if str(type(simobj)) != "<type 'NoneType'>" and simobj.ident != "lost_object":
                    simobj.config_simresults(self)
                else:
                    i+=1
            if i>0:
                print(("WARNING: " + str(i) +  " of " + str(len(self.parent.get_simobjects())) + " simobjects were of type 'NoneType'. These could not be loaded."))
        ...
        def clear_results(self):
            ...
            self.edgeresultsevolution.clear_rows()
        ...
        def add_resultobj(self, resultobj, **kwargs):
            ...
            if  hasattr(self, resultobj.get_ident()):
                # Funktion wird nur während des Ladens von Objekten verwendet -> Wenn das Objekt bereits vorhanden ist, 
                # dann wurde es bisher gelöscht => Besser wäre hier ein Abgleich der Attribute? 
                # getattr(self,resultobj.get_ident()).clear()
                print(resultobj.get_ident())
        ...
        def import_xml(self, sumo, **datapaths):
            """
            Central method to read all results from different XML files after the simulation.
            sumo: Sumo process instance
            
            """
            print('Simresults.import_xml',self.get_ident_abs(),'is_result_evolution',sumo.is_result_evolution)
##          # print '  datapaths',datapaths
            ...
            if 'edgedatapath' in datapaths:
                ...
                if sumo.is_result_evolution & (sumo.time_interval_evolution > 0):
                    print('  import evolution results!')
                    n_interval = int((sumo.simtime_end - (sumo.simtime_start + sumo.time_warmup))/sumo.time_interval_evolution + 0.5)
                    
                    self.edgeresultsevolution.reset_intervals(sumo.simtime_start + sumo.time_warmup, n_interval, sumo.time_interval_evolution)
                    self.edgeresultsevolution.import_edgedata(sumo,datapaths['edgedatapath'])
            ...
            if 'edgeemissionspath' in datapaths:
                ...
                if sumo.is_result_evolution:
                    print('  import_edgeemissions??????????')
                #    self.edgeresultsevolution.import_edgeemissions(sumo,datapaths['edgeemissionspath'])
...
class IntervalCounter(handler.ContentHandler):
    """Parses a SUMO edgeresult XML file and counts intervals."""
    # TODO: here we could actually create a list with sumo edge IDs
    # this could be useful also for the edge result reader
    def __init__(self):
        self.n_edge = 0
        self.n_interval = 0

    def startElement(self, name, attrs):
        #print 'IntervalCounter.startElement',name,self.n_interval
        if name == 'interval':
                self.n_interval += 1


class IntervalReader(handler.ContentHandler):
    """
    Reads info from intervals.
    """

        
    def __init__(self, element, counter, ids_sumo, time_start_recording, time_end_recording,  attrsconfigs ):
        """
        element is "lane" or "edge" or "tripinfo"
        attrnames is a list of attribute names to read.
        """
        print('IntervalReader n_interval',counter.n_interval,element,'len(ids_sumo)',len(ids_sumo),'len(attrsconfigs)',len(attrsconfigs))
        print('  time_start_recording',time_start_recording,'time_end_recording',time_end_recording)
        ...
        self._time_start_recording = time_start_recording
        self._time_end_recording = time_end_recording
        self._attrsconfigs = attrsconfigs
        self._time_begin = None
        self._time_end = None
        self._time_interval = None
        self._values= {}
        self._ids_sumo = ids_sumo
        n_edges = len(ids_sumo)
        #self._map_ids_sumo_to_ind = {}
        #for id_sumo, ind in zip(ids_sumo, xrange(n_edges)):
        #    self._map_ids_sumo_to_ind[id_sumo] = ind
        ...
        self._attrtypes = []
        for  attrconfig in attrsconfigs:
            attrtype = type(attrconfig.get_default()[0])
            
            if attrtype in am.TYPES_INT:
                self._values[attrconfig.attrname]= np.zeros((n_edges, counter.n_interval), dtype = np.int32)
                self._attrtypes.append('i')
                
            elif attrtype in am.TYPES_FLOAT:
                self._values[attrconfig.attrname]= np.zeros((n_edges, counter.n_interval), dtype = np.float32)
                self._attrtypes.append('f')
                
            else:
                self._values[attrconfig.attrname]= np.zeros((n_edges, counter.n_interval), dtype = np.object_)
                self._attrtypes.append('o')
    ...
    def startElement(self, name, attrs):
        ...
        if name == 'interval':
            ...
            time_inter_end = float(attrs['end'])
            print(79*'-')
            print('startElement  interval',self.is_inter_valid,' time_inter_begin',time_inter_begin,'is_valid',(time_inter_begin >= self._time_start_recording) & (time_inter_end < self._time_end_recording))
            if (time_inter_begin >= self._time_start_recording) & (time_inter_end < self._time_end_recording):
                self.is_inter_valid = True
                self.ind_inter += 1
                if self._time_begin is None:
                    self._time_begin = int(time_inter_begin)
                    self._time_interval =  int(time_inter_end - time_inter_begin)
                self._time_end = time_inter_end
        ...
        if name == self._element:
            ...
            id_sumo  = self.get_id_elem(attrs)
            inds_val = np.where(self._ids_sumo == id_sumo)[0]
            if len(inds_val) == 0: return # edge not in network?!
  
            for attrconfig, attrtype  in zip(self._attrsconfigs, self._attrtypes):
                ...
                if xmltag in attrs:
                    ...
                    a = attrs[xmltag].strip()
                    #print '  attrname',attrname,'val=',a,'inds_val',inds_val,'ind_inter',self.ind_inter,'shape',self._values[attrname].shape
                    
                    if a != '':
                        if attrtype == 'i':
                            #conversion = 'i'#int
                            self._values[attrname][inds_val, self.ind_inter] += int(float(a))
                            
                            
                        elif attrtype == 'f':
                            #conversion = 'f'#float
                            self._values[attrname][inds_val, self.ind_inter] += float(a)
                            
                        else:
                            #conversion = 's'#str
                            self._values[attrname][inds_val, self.ind_inter] = a
    ...
    def get_interval(self):
        # returns time of first, the time interval and number of intervals
        return self._time_begin, self._time_interval, self.ind_inter+1
        
    def get_ids(self):
        return self._ids_sumo
...
class IntervalAvReader2(handler.ContentHandler):
    ...
    def __init__(self, element, time_start_recording, time_end_recording,  attrsconfigs_cumulative, attrsconfigs_average):
        """
        element is "lane" or "edge" or "tripinfo"
        attrnames is a list of attribute names to read.
        """
...
class TripresultReader(IntervalAvReader2):
    ...
    def startElement(self, name, attrs):
        #if attrs.has_key('id'):
        #    print '  parse',name,self._element,name in self._element,'id=',attrs.get('id','-')
        ...
        if name in self._element:
            ...
            id_elem  = self.get_id_elem(attrs)
            #print '  ---id_elem',id_elem,len(self._attrsconfigs_cumulative),len(self._attrsconfigs_average)
            if id_elem == -1: 
            ...
            if name == 'personinfo':
                self._id_element_current = id_elem
                #for key in attrs.keys():
                #    print '      attrname',key,'=',attrs[key]
            
            # no arrival data availlable if trip has not been finished!!
            for attrsconfig in self._attrsconfigs_cumulative:
                xmltag = attrsconfig.xmltag
                attrname = attrsconfig.attrname
                #print '    cum attrname',attrname,'xmltag',xmltag,attrs.has_key(attrname),attrs.has_key(xmltag)
                if xmltag in attrs:
                    #print '  attrname',attrname,attrs.has_key(attrname)
                    #if 1:#attrs.has_key(attrname):
                    
                    #print '   val *'+attrs[xmltag]+'*'
                    
                    #TODO
                    if attrs[xmltag] == "end" or attrs[xmltag] == "collision":
                        a = '0'
                    else:
                        a = attrs[xmltag]
                    if a.strip()!='':
                        if xmltag == 'vaporized':
                            self._values[attrname][id_elem] =  a not in ('','false')
                        else:
                            if id_elem in self._values[attrname]:
                                self._values[attrname][id_elem] += float(a)
                            else:
            ...
            for attrsconfig in self._attrsconfigs_average:
                xmltag = attrsconfig.xmltag
                attrname = attrsconfig.attrname
                #print '    av attrname',attrname,'xmltag',xmltag,attrs.has_key(attrname),attrs.has_key(xmltag)
                if xmltag in attrs:
                    #n=float(self.n_inter)
                    a = attrs[xmltag]
                    if a.strip()!='':
            ...
            if self._id_element_current is not None:
                id_elem = self._id_element_current
                for attrsconfig in self._attrsconfigs_cumulative:
                    xmltag = attrsconfig.xmltag
                    attrname = attrsconfig.attrname
                    #print '    cum attrname',attrname,'xmltag',xmltag,attrs.has_key(attrname),attrs.has_key(xmltag)
...
class ElectricalEnergyReader(IntervalAvReader2):
    ...
    def __init__(self, element,sumo,  attrsconfigs_cumulative, attrsconfigs_average):
        ...
        #(self, sumo.simtime_start + sumo.time_warmup, 
        #                                    sumo.simtime_end, element, [],[])
        self._sumo = sumo # could be passed to IntervalAvReader2.__init__
        IntervalAvReader2.__init__( self,  element, sumo.simtime_start + sumo.time_warmup,
                                     sumo.simtime_end, attrsconfigs_cumulative, attrsconfigs_average)
...
class TrajectoryReader(IntervalAvReader2):
    ...
    def __init__(self, element, sumo):
        ...
        IntervalAvReader2.__init__(self,  element,sumo.simtime_start + sumo.time_warmup, 
                                            sumo.simtime_end,  [],[])
        #element, time_start_recording, time_end_recording,  attrsconfigs_cumulative, attrsconfigs_average)
        #self.time_lastint = -1.0
        self.n_inter = 0
        self.times = []
        self.trajectories = {}
    ...
    def startElement(self, name, attrs):
        #print 'startElement',name,name == self._element,'n_inter',self.n_inter
        ...
        if name == 'timestep':
            ...
            self.times.append(time_inter_end)
            #print 79*'-'
            #print '   time_inter_end',time_inter_end
            #self.energies.append(self._energy_step)
            #self._energy_step = 0.0
        ...
        if name == self._element:
            ...
            #print '  ---id_veh_sumo',id_elem,'append veh?',id_elem not in self._ids
            if id_elem == -1: return # id did not fit requested trip type
...
def read_trajectories(filepath, sumo,  element):
    ...
    n_veh = len(ids_sumo)
    trajectories = np.ones(n_veh, dtype = np.object_)
    angles = np.ones(n_veh, dtype = np.object_)
    speeds = np.ones(n_veh, dtype = np.object_)
...
def read_interval2(filepath, time_start_recording, time_end_recording,  element, attrsconfigs):
    ...
    reader = IntervalAvReader2(element, time_start_recording, time_end_recording,  attrsconfigs_cumulative, attrsconfigs_average)
    #parser = make_parser()
...
def read_edgeevolution(filepath, ids_sumo, time_start_recording, time_end_recording,  element, attrsconfigs):
    print('read_edgeevolution',element,filepath)
    
    
    counter = IntervalCounter()
    parse(filepath, counter)
    reader = IntervalReader(element, counter, ids_sumo , time_start_recording, time_end_recording,  attrsconfigs)
    #element, counter, ids_sumo, time_start_recording, time_end_recording,  attrsconfigs 
    #parser = make_parser()
    #parser.setContentHandler(reader)
    #fn = '"'+filepath+'"'
    #print 'read_interval >'+fn+'<'
