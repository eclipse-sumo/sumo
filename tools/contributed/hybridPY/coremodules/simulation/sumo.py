...
class Sumo(CmlMixin, Process):
    def __init__(   self, scenario,
        ...
        self.guimode = attrsman.add(cm.AttrConf( 'guimode', guimode,
                                                ...
                                                choices = ['hybridPY','hybridPY+map','native','nogui','SAV_high_vis'],# 'openscene'???
                                                name = 'GUI mode',
                                                perm = 'rw',
        ...
        self.electricenergypath = attrsman.add(cm.AttrConf('electricenergypath',os.path.join(rootdirpath,rootname+'.'+outfile_prefix+'.electricenergy.xml'),
                                                ...
                                                metatype = 'filepath',
                                                info = """SUMO xml file with edge emission data.""",
                                                attrnames_data = ['Consum','energyCharged','energyChargedInTransit','energyChargedStopped'],
    ...
    def do(self):
        ...
        if not self.is_import_results_only:
            ...
            if self.is_export_net:
                ...
                netfilepath, delta = scenario.net.export_netxml(self.netfilepath, is_return_delta = True, is_no_internal_links = self.is_no_internal_links)
            else:
            ...
            if self.guimode in ['hybridPY','hybridPY+map','SAV_high_vis']:
                filepath_gui = os.path.basename(self.parent.net.write_guiconfig(is_maps= self.guimode in ['hybridPY+map'], is_SAV_high_vis= self.guimode in ['SAV_high_vis'], delta = delta))
            else:
                filepath_gui = None
        ...
        if self.is_recording_runtime:
            ...
            if len(self.ids_edge_record_sumo) > 0:
                ...
                for id_edge_sumo in self.ids_edge_record_sumo:
                  self.ids_veh_record_sumo.update(set(traci.edge.getLastStepVehicleIDs(id_edge_sumo)) - self.ids_veh_del_record_sumo)
                  self.ids_pers_record_sumo.update(set(traci.edge.getLastStepPersonIDs(id_edge_sumo)) - self.ids_pers_del_record_sumo)
                
                print ('  recording %d vehicles, %d persons'%(len(self.ids_veh_record_sumo),len(self.ids_pers_record_sumo)))
                for id_veh_sumo in self.ids_veh_record_sumo:
    ...
    def finish_sim(self):
        ...
        if self.is_recording_runtime:
                ...
                if len(trajectory) > 3:
                    ...
                    if vtype_sumo in vtypes_unreal:
                            ...
                            if trajectory[0][0]- self.simtime_start > t_min:
                                ...
                                times_app, shape_interp,angles, speeds_app, accels_app = (trajectory_arr[:,0],trajectory_arr[:,[1,2]],np.pi/180*(-trajectory_arr[:,4]+90.0),trajectory_arr[:,5],trajectory_arr[:,6])
                                if debug:
                                ...
                                if np.any(speeds_app < 0.0):
                                    ...
                                    print ('  export veh: (id_veh, vtype)=(%s,%s),%s'%(id_veh_sumo, vtype_sumo, vtype_sumo in vtypes_unreal))
                                    ...
                                    for t, (x,y), speed, accel, lon, lat in zip(times_app[1:], shape_interp[1:,:], speeds_app[1:], accels_app[1:], lons[1:], lats[1:]):
                                        z = 0.0
                                        if self.is_lonlat:
                                            csvfile.write(f'{t:0.2f} {lon:0.6f} {lat:0.6f} {z:0.1f} {speed:0.3f} {accel:0.3f},')
                                        else:
                                            if id_node_ref != -1:
                                                east_scaled, north_scaled, up_scaled = ue.lonlat_to_enu_scaled(lon, lat, z, x_ueref, y_ueref, z_ueref, lon_ref_rad, lat_ref_rad, scale = ue.uescale)
                                                csvfile.write(f'{t:0.2f} {east_scaled:0.6f} {north_scaled:0.6f} {up_scaled:0.6f}  {speed:0.3f} {accel:0.3f},')
                                            else:
                                                csvfile.write(f'{t:0.2f} {x:0.6f} {y:0.6f} {z:0.1f} {speed:0.3f} {accel:0.3f},')
            ...
            for (id_pers_sumo, ptype_sumo), trajectory  in self.persontrajectories.items():
                print ('  export pers: (id_pers, ptype)=(%s,%s),%s, len=%d'%(id_pers_sumo, ptype_sumo, ptype_sumo in ptypes_unreal,len(trajectory)))
                if len(trajectory) > 3:
                    if ptype_sumo in ptypes_unreal:
                            ...
                            if trajectory[0][0] - self.simtime_start > t_min:
                                ...
                                times_app, shape_interp, speeds_app = (trajectory_arr[:,0],trajectory_arr[:,[1,2]],trajectory_arr[:,5])
                                
                                if np.any(speeds_app < 0.0):
                                    print ('  WARNING: veh',id_veh_sumo,'negative speeds.')
                                
                                else:
                                    print ('  export pers: (id_pers_sumo, ptype)=(%s,%s),%s'%(id_pers_sumo, ptype_sumo, ptype_sumo in ptypes_unreal))
                                    ...
                                    for t,(x,y), speed, lon,lat in zip(times_app[1:], shape_interp[1:,:], speeds_app[1:], lons[1:], lats[1:]):
                                        ...
                                        if self.is_lonlat:
                                            ...
                                            if id_node_ref != -1:
                                                east_scaled, north_scaled, up_scaled = ue.lonlat_to_enu_scaled(lon, lat, z, x_ueref, y_ueref, z_ueref, lon_ref_rad, lat_ref_rad, scale = ue.uescale)
                                                csvfile.write(f'{t:0.2f} {east_scaled:0.6f} {north_scaled:0.6f} {up_scaled:0.6f}  {speed:0.3f},')
                                            else:
...
def interpolate_trajectories(trajectory_arr, time_step, is_accel = True):
    ...
    for i, t_app in zip(range(n_steps), times_app):
        ...
        print('    i=%d|%d,t=%.1f|%.1f*%s*p=%.1f|%.1f,v=%.1f|%.1f'%(i_traj,i,t1,t_app,is_interp,positions[i_traj],positions_app[i-1],trajectory_arr[i_traj, 5],speeds_app[i-1]))
        ...
        for attrconfig in self.get_attrsman().get_configs(is_all = True):
            if ('sumo' in attrconfig.groupnames) & hasattr(attrconfig,'cml'):
                ...
                if is_enabled: # disabeled options are simply not added
                    ...
                    sumooptions.add_option(attrconfig.attrname,attrconfig.get_value(),attrconfig.cml, is_filepath = is_filepath, cmlvaluemap=cmlvaluemap)
