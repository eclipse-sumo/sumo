...
from . import sumo, results, antijammer
...
if 1:#try:
    ...
    import coremodules.demand.turnflows_mpl as turnflows_mpl
...
class WxGui(ModuleGui):
    ...
    def refresh_widgets(self):
        ...
        if self._simulation != scenario.simulation:
            ...
            #del self._demand
            #Korrektur von Fabian -> Hier kommt es zum Fehler!
        ...
        if True:#self._simulation.results.is_modified():
            #
    ...
    def make_menu(self):
        ...
        #    self.on_sumo_lowmem, 
        #    info='Define simulation parameters and simulate with SUMO micro-simulator. Low memory is used because the hybridPY scenario will be deleted prior to run the simulation.',
        #    bitmap = self.get_icon('icon_sumo.png'),#,
        #    )    
        ...
        menubar.append_item( 'simulation/results/add edge est. times to edgeresults',
            self.on_add_edgetimes_est, 
            )
        ...
        menubar.append_item( 'simulation/results/export edge results to shape file...',
            self.on_edgeresults_to_shapefile, 
        ...
        if is_mpl:
            ...
            menubar.append_item( 'simulation/results/plot simulated vs. measured counts...',
                self.on_plot_counts, 
                bitmap = self.get_icon('icon_mpl.png'),#,
                )
             
             
            menubar.append_item( 'simulation/results/plot sum of edgeflows...',
                self.on_plot_sum_of_edgeflows_evolution, 
                bitmap = self.get_icon('icon_mpl.png'),#,
                )    
    ...
    def on_edgeresults_to_shapefile(self, event = None):
        """
        Export Network edge results to shape file.
        """
        
        edgeresults = self._simulation.results.edgeresults
        myscenario = edgeresults.parent.get_scenario()
        print ('on_edgeresults_to_shapefile',edgeresults,edgeresults.parent,edgeresults.parent.get_scenario())
        
        dirpath = myscenario.get_workdirpath()
        defaultFile = myscenario.get_rootfilename()+'.edgeres.shp'
        ...
        shapeformat.edgesresults_to_shapefile( edgeresults,
                                                filepath,
    ...
    def on_plot_counts(self, event = None):
        """Plot simulated vs. measured counts using matplotlib. Requires turnflow tables and estimated entered from turnflows."""
        if self._simulation.results is None:
            self._simulation.results = results.Simresults(ident= 'simresults', simulation=self._simulation)
        
        if is_mpl:
            resultplotter = turnflows_mpl.TurnflowComparisonPlotter(  self._simulation.results, 
                                                        logger = self._mainframe.get_logger())
            dlg = ResultDialog(self._mainframe, resultplotter)
    ...
    def on_plot_sum_of_edgeflows_evolution(self, event = None):
        """Plot the evolution of the sum of edgeflows using the Matplotlib plotting envitonment."""
        if self._simulation.results is None:
            self._simulation.results = results.Simresults(ident= 'simresults', simulation=self._simulation)
        
        if is_mpl:
            resultplotter = results_mpl.EdgeFlowSumEvolutionPlotter(  self._simulation.results, 
                                                        logger = self._mainframe.get_logger())
            dlg = ResultDialog(self._mainframe, resultplotter)
    ...
    #TODO:
    def on_sumo_traci_SAV(self, event = None):
        #self.prepare_results()
        self.simulator = sumo.SumoTraciSAV( \
                                    scenario = self.get_scenario(), 
                                    results = self._simulation.results,
                                    logger = self._mainframe.get_logger(),
                                    is_gui = True,
        ...
        self.open_sumodialog_interactive_SAV()  
    ...
    #TODO:
    def open_sumodialog_interactive_SAV(self, title = 'SUMO-Traci-SAV Dialog'):
        dlg = ProcessDialogInteractive( self._mainframe, 
                                        self.simulator,
                                        title = title,
                                        func_close = self.close_sumodialog_interactive,
        ...
        # this does not return until the dialog is closed.
        #val = dlg.ShowModal()
        print('open_sumodialog_interactive_SAV')
        dlg.Show()
        dlg.MakeModal(True) 
        #print '  val,val == wx.ID_OK',val,wx.ID_OK,wx.ID_CANCEL,val == wx.ID_CANCEL
        #print '  status =',dlg.get_status()
    ...
    def on_estimate_entered_demand(self, event = None):
        print('on_estimate_entered_demand')
        results = self._simulation.results
        trips = self.get_scenario().demand.trips
        results.edgeresults.add_entered_est(*trips.estimate_entered() , attrname = 'entered')# , attrname = 'entered'
        #ids_edge, entered_vec = trips.estimate_entered()
        #print '  len(ids_edge)',len(ids_edge),len(entered_vec)
        #results.edgeresults.add_entered_est(ids_edge, entered_vec)
    ...
    def on_add_edgetimes_est(self, event = None):
        """
        Add estimated edgetimes to edgeresults.
        """
        results = self._simulation.results
        results.edgeresults.add_edgetimes_est()
        self._mainframe.browse_obj(results.edgeresults)
        self.refresh_widgets()
