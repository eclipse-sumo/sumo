    ...
    def show(self):
        ...
        if self.is_plot_measured_flows:
            ...
            if self.is_plot_simulated_flows:
                ...
                if (len(edgeresultsevolution) > 0)&(edgeresultsevolution.time_interval.get_value() == interval_time):
                    ...
                    step_hours_new = []
                    dep_times_sim = np.arange(edgeresultsevolution.n_interval.get_value()+1, dtype = np.int32)*edgeresultsevolution.time_interval.get_value()+edgeresultsevolution.time_start_recording.get_value()
            ...
            if len(edgeresultsevolution) > 0:
                ...
                dep_times_sim = np.arange(edgeresultsevolution.n_interval.get_value()+1, dtype = np.int32)*interval_time+edgeresultsevolution.time_start_recording.get_value()
        ...
        if self.is_plot_simulated_flows:
            ...
            if len(edgeresultsevolution) > 0:
                    ...
                    simflows = np.array(edgeresultsevolution.entered[edgeresultsevolution.ids_edge.get_ids_from_indices_save(ids_detectoredge)])
