...
class OdFlowTable(am.ArrayObjman):
    def __init__(self, parent, modes, zones, activitytypes=None, **kwargs):
            ...
            self.add_col(am.ArrayConf('tripnumbers_failed', 0,
                                      groupnames=['state'],
                                      perm='r',
                                      is_save = False,
                                      name='Failed generated trips',
                                      info='Number of trips that have not been generated from zone with ID Orig to zone with ID Dest.',
                                      ))
    ...
    def add_flows(self,  time_start,
                  ...
                  ids_dest,
                  tripnumbers,
                  tripnumbers_failed = None,
                  id_activitytype_orig=1,
                  id_activitytype_dest=1,
                  ):
        n = len(tripnumbers)
        if tripnumbers_failed is None:
            tripnumbers_failed = np.zeros(n, dtype = np.int32)
        ...
        self.add_rows(n=n,
                      ...
                      tripnumbers_failed = tripnumbers_failed,
...
class OdTrips(am.ArrayObjman):
    ...
    def _init_attributes(self, zones=None):
            ...
            self.add_col(am.ArrayConf('tripnumbers_failed', 0,
                                      groupnames=['state'],
                                      perm='r',
                                      #is_save = False,# this meas only that values are not saved, but is creating just chaos during save
                                      name='Failed trips',
                                      info='Number of trips that have not been generated from zone with ID Orig to zone with ID Dest.',
                                      ))
    ...
    def get_zone_edgeinfo(  self, id_mode, id_zone, 
        ...
        """
        Returns two arrays, edge IDs and the respective edge weights for zone id_zone and mode id_mode 
        """
        #print 'get_zone_edgeinfo id_zone',id_zone,'id_mode',id_mode
        ...
        if id_zone not in self._mode_to_edgeinfo[id_mode]:
            ...
            if n_edges > 0:
                ...
                self._mode_to_edgeinfo[id_mode][id_zone] = (ids_edge, 
                                                            self._get_edgeweights(ids_edge, n_edges_min_length, n_edges_max_length, priority_max, speed_max),
                                                            )
                #edgeweights = self._get_edgeweights(ids_edge, n_edges_min_length, n_edges_max_length, priority_max, speed_max)
    ...
    def _get_edgeweights(self, ids_edge, n_edges_min_length, n_edges_max_length, priority_max, speed_max):
        ...
        n_edges = len(ids_edge)
        #print 'get_edgeweights n_edges_min_length',n_edges_min_length,'n_edges_max_length',n_edges_max_length,'priority_max',priority_max,'length proportional',(n_edges > n_edges_min_length)&(n_edges < n_edges_max_length)
        
        if (n_edges > n_edges_min_length)&(n_edges < n_edges_max_length):
            #print '  >n_edges_min_length'
            #print '    lengths',edges.lengths[ids_edge]
            #print '    priorities',edges.priorities[ids_edge]
            #print '    speeds_max',edges.speeds_max[ids_edge]
            #print '    result',edges.lengths[ids_edge]*((edges.priorities[ids_edge]<priority_max) & (edges.speeds_max[ids_edge] < speed_max))
            return edges.lengths[ids_edge]*((edges.priorities[ids_edge]<priority_max) & (edges.speeds_max[ids_edge] < speed_max))
        else:
            #print '  <n_edges_min_length'
            #print '    priorities',edges.priorities[ids_edge]
            #print '    speeds_max',edges.speeds_max[ids_edge]
            #print '    result',np.ones(n_edges,dtype = np.float32)*((edges.priorities[ids_edge]<priority_max) & (edges.speeds_max[ids_edge] < speed_max))
    ...
    def generate_odflows(self, odflowtab,  time_start, time_end, id_mode, **kwargs):
        ...
        odflowtab.add_flows(time_start,
                            ...
                            self.tripnumbers_failed.get_value(),
                            **kwargs
                            )
    ...
    def generate_trips(self, demand, time_start, time_end, id_mode_primary,
        ...
        Not generated trips are recorded in the tripnumbers_failed attribute
        """
        print('OdTrips.generate_trips', time_start, time_end, 'id_mode_primary',id_mode_primary,'id_mode_fallback',id_mode_fallback)
        id_mode_ped = MODES['pedestrian']
        zones = self.get_zones()
        edges = self.get_edges()
        # TODO: a bid crazy to pass demand as an argument here
        trips = demand.trips
        self.tripnumbers_failed.reset()
        ...
        for id_od, id_orig, id_dest,  tripnumber in \
            ...
            print('  generate',tripnumber,' trips from id_orig',id_orig,'to id_dest',id_dest, 'id_mode_primary',id_mode_primary)
            
            ids_edge_orig, weights_orig = self.get_zone_edgeinfo(id_mode_primary, id_orig, n_edges_min_length, n_edges_max_length, priority_max, speed_max)
            ids_edge_dest, weights_dest = self.get_zone_edgeinfo(id_mode_primary, id_dest, n_edges_min_length, n_edges_max_length, priority_max, speed_max)
            ...
            if 1:
                print('-----')
                for id_edge_orig, weight_orig in zip(ids_edge_orig, weights_orig):
                    print('      id_edge_orig',id_edge_orig,'weight_orig', weight_orig)
                
                print('-----')
                for id_edge_dest, weight_dest in zip(ids_edge_dest, weights_dest):
                    print('      id_edge_dest',id_edge_dest,'weight_orig', weight_dest)
                
            ##    
                
            
            if (n_edges_orig > 0) & (n_edges_dest > 0):
                for i_trip in range(tripnumber):
                    ...
                    id_trip = trips.make_trip(id_vtype = id_vtype_primary,
                                              ...
                                              id_edge_depart = ids_edge_orig[np.argmax(random.rand(n_edges_orig)*weights_orig)],
                                              id_edge_arrival = ids_edge_dest[np.argmax(random.rand(n_edges_dest)*weights_dest)],
                ...
                # no suitable edges found for this pair 
                # record failed trips 
                self.tripnumbers_failed[id_od] += self.tripnumbers[id_od]
    ...
    def __init__(self, odintervals, logger = None,**kwargs):
        ...
        self.id_mode_fallback = attrsman.add(am.AttrConf('id_mode_fallback',  modechoices['No fallback'], 
                                        ...
                                        info = """Transport mode to be used instead of "passenger" mode
                                         ...
                                         Choose for example "taxi" to get access to traffic restricted Zones.
                                         """,
                                        ))
