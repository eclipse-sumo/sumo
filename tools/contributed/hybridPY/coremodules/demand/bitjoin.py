    ...
    def __init__(self, ident, virtualpop,  logger = None, **kwargs):
        ...
        if hasattr(zones, 'population'):
            self.method_number_pop = attrsman.add(cm.AttrConf( 'method_number_pop',kwargs.get('method_number_pop',3),
                                                        choices = {'Use average living density data': 1, 'Use aggregated population data': 2, 'Use disaggregated population data': 3},
                                                        groupnames = ['options'],
            ...
            self.method_number_pop = attrsman.add(cm.AttrConf( 'method_number_pop',kwargs.get('method_number_pop',2),
                                                        choices = {'Use average living density data': 1, 'Use aggregated population data': 2},
                                                        groupnames = ['options'],
                                                        name = 'Input data type',
        ...
        self.method_household_sizes = attrsman.add(cm.AttrConf( 'method_household_sizes',kwargs.get('method_household_sizes',2),
                                                        choices = {'Use average and stdev of household sizes': 1, 'Use share of household sizes': 2},
                                                        groupnames = ['options'],
        ...
        self.method_household_income = attrsman.add(cm.AttrConf( 'method_household_income',kwargs.get('method_household_income',2),
                                                        choices = {'Use average and stdev values of household incomes': 1, 'Use average incomes for each size and stdev': 2},
                                                        groupnames = ['options'],
...
#######> STEP 1: IDENTIFYING ELIGIBLE FACILIITES - RESIDENTIAL WITH MIN AND MAX AREA  
        ...
        ids_fac_valid = ids_fac[(facilities.areas[ids_fac]>self.min_area_fac)&(facilities.areas[ids_fac]<self.max_area_fac)]
        print(f"➔ Number of buildings with area between {self.min_area_fac} and {self.max_area_fac}: {len(ids_fac_valid)}")
        ids_fac_valid_global = ids_fac_valid[(facilities.ids_landusetype[ids_fac_valid] == landuse.landusetypes.get_id_from_formatted('residential'))]
...
#######> STEP 2: FILTERING FACILITIES WITHIN THE STUDY AREA
        ...
        #Use single zone
            ...
            ids_fac_valid = ids_fac_valid_global[(facilities.ids_zone[ids_fac_valid_global] == zones.ids_sumo.get_id_from_index(self.select_zone))]
            ...
            for id_fac in tqdm(np.random.permutation(ids_fac_valid), desc="Assigning HH by landuse densities"):
                ...
                while assigned_household_size < max_household_size:
                    ...
                    if remaining_area < min_required_area:
                        print('WARNING: The remaining area of facility ID {id_fac} is insufficient to accommodate more households; moving to the next residential building')
                        break
...
#######> STEP 1: GENERATING VIRTUAL POPULATION GENDER
                    ...
                    id_gender = np.random.choice([GENDERS['male'],GENDERS['female']], p=[self.share_male,1-self.share_male])
...
#######> STEP 3: GENERATING VIRTUAL POPULATION AGE
                    ...
                    print(f"Demographic information of Household {id_hsh}: Person {person+1}, Name={name}, Gender={gender_str}, Age={age}")
...
#######> STEP 3: ADDING VEHICLES TO THE VIRTUAL POPULATION
            ...
            if ids_pers != []:
                ...
                if n_motos > 0 and len (ids_pers_moto) >0: # Prioritize unassigned people, if motors still remain, assign as second vehicles to people who was already assigned 
                    ...
                    if len(elig_ids_pers) >= n_motos:
                        ids_person_moto = np.random.choice(elig_ids_pers,size = min(n_motos, len(elig_ids_pers)), replace = False)
                        
                    elif len(elig_ids_pers) == 0:
                        ids_person_moto = np.random.choice(ids_pers_moto, size=min(n_motos, len(ids_pers_moto)), replace=False)
                    
                    else:
                        ids_person_moto = elig_ids_pers
                        remain_motos = n_motos - len(elig_ids_pers)
                        if remain_motos > 0 and len(ids_pers_veh) > 0:
                            remain_ids_person_moto = np.random.choice(list(ids_pers_veh), size=min(remain_motos, len(ids_pers_veh)), replace=False)
                            ids_person_moto.extend(remain_ids_person_moto)
                    
                    ids_person_motos.extend(ids_person_moto)
                    ids_pers_veh.update(ids_person_moto)
                
                if n_bikes > 0 and len (ids_pers_bike) >0: # Prioritize unassigned people, if bikes still remain, assign as second vehicles to people who was already assigned 
                    ...
                    if len(elig_ids_pers) >= n_bikes:
                        ids_person_bike = np.random.choice(elig_ids_pers,size = min(n_bikes, len(elig_ids_pers)), replace = False)
                        
                    elif len(elig_ids_pers) == 0:
                        ids_person_bike = np.random.choice(ids_pers_bike, size=min(n_bikes, len(ids_pers_bike)), replace=False)
                    
                    else:
                        ids_person_bike = elig_ids_pers
                        remain_bikes = n_bikes - len(elig_ids_pers)
                        if remain_bikes > 0 and len(ids_pers_veh) > 0:
                            remain_ids_person_bike = np.random.choice(list(ids_pers_veh), size=min(remain_bikes, len(ids_pers_veh)), replace=False)
...
#######> STEP 4: ASSIGNING PREFERRED MODES TO VIRTUAL POPULATION
        ...
        for person in range(num_pop):
            modes = []
            if passenger[person] > 0: modes.append("passenger")
            if motorcycle[person] > 0: modes.append("motorcycle")
            if person in bus and bus[person]: modes.append("bus")
            if bicycle[person] > 0: modes.append("bicycle")
        ...
        target_vehicles = {mode: int(round(float(mode_share) * num_pop)) for mode, mode_share in zip(transport_modes, modes_share)}
        print('---------------------------------------------------------------------------------------------')
...
class DailyActivityFromLandUseGenerator(Process):
    def __init__(self, ident, virtualpop,  logger = None, **kwargs):
        ...
        self.travel_time_budget_method = attrsman.add(cm.AttrConf( 'travel_time_budget_method',kwargs.get('travel_time_budget_method', 2),
                                                        choices = {'Homogeneous normal distribution': 1, 'Heterogeneous normal distribution': 2, 'Log-linear regression model': 2},
                                                        groupnames = ['options'],
        ...
        self.distribution_method = attrsman.add(cm.AttrConf('distribution_method',kwargs.get('distribution_method',3),
                                                        groupnames = ['options'],
                                                        choices = {'Preferred travel distance': 1, 'Available capacity of destinations': 2, 'Available capacity of destinations and travel time budget constrained': 3, 'Distant-based Gravitational model': 4, 'Time-based Gravitational model': 5, 'Distant-based Logit model': 6, 'Time-based Logit model': 7},
...
#######> STEP 1: INITIALIZING ALL POSSIBLE DESTINATIONS AND THEIR CAPACITIES BY LAND USE TYPE
        ...
        for landusetype_key in landusetypes_key:
            ...
            # Only single zone is considered for activity location
                ...
                ids_destinations = ids_possible_fac[(facilities.ids_zone[ids_possible_fac] == landuse.zones.ids_sumo.get_id_from_index(self.select_zone))]

            # The now is to update to the dictionary
            ids_possible_destinations[landusetype_key] = ids_destinations
            print('Initialization started...Identifing facilities for landusetype: %s, ID landusetype: %s, Found: %s facilities' %(landusetype_key, landusetype_id, len(ids_destinations)))
...
#######> STEP 2: LINKING THE EDGES AND ASSOCIATED FACILITIES AND CALCULATING EDGE TIMES AND LEVEL OF ACCESS FOR FURTHER DIJKSTRA FORWARD CALCULATION
        ...
        start_time = time.perf_counter()    
        edge_to_fac = defaultdict(list)  # edge_to_fac = {id_edge1: [id_fac1, id_fac2,...], id_edge2: [id_fac2, id_fac3, id_fac4...],...}
...
#######> STEP 4: GENERATING DAILY TRAVEL DIARIES
        ...
        for id_virtual in ids_virtual:
            ...
            if self.travel_time_budget_method == 1: #Average daily travel time budget (in seconds) that individuals typically allocate for commuting purposes, without considering differences in age or gender.
            ...
            elif self.travel_time_budget_method == 2: #Additional travel time budget (in seconds) applied to specific demographic groups, such as working-age individuals or males, to reflect differentiated travel behavior.
                av_travel_time_budget = self.av_base_travel_time_budget
                std_base_travel_time_budget = self.std_base_travel_time_budget
                if 18 < age <= 65:
            ...
            print(f"➔ Home ID: {id_virtual_hsh}, Home Location: {id_fac_home}, Preferred Transport Mode: {virtual_pref_mode_name}")      
            ...
            if diary in diary_types:
                ...
                secondary_attributes = primary_diary.get('secondary', {}) # Include secondary if any, based on prob_do_secondary
                ...
                if secondary_included:
                    primary_travel_time_budget = int(np.ceil(daily_travel_time_budget * np.random.uniform(0.4, 0.5) / 60)) * 60 # For one-way, if the secondary included, assumed the out-going time occupies 40-50% daily travel time
                    secondary_travel_time_budget = int(np.ceil(daily_travel_time_budget * np.random.uniform(0.2, 0.3) / 60)) * 60 # For one-way, assumed the out-going time occupies 20-30% daily travel time
                
                else:
                    primary_travel_time_budget = int(np.ceil(daily_travel_time_budget * 0.5 / 60)) * 60 # For one-way, if the secondary not included, assumed the out-going time occupies 50% daily travel time
                ...
                for actype in activity_chain:
                    ...
                    if actype == primary_diary['primary']['actype']:
                        ...
                        if landtype_probs and len(landtype_probs) == len(landtype_choices):
                            ...
                            selected_landtype = np.random.choice(landtype_choices, p=np.array(landtype_probs)/sum(landtype_probs))
                        else:
                        ...
                        print(f"➔ Reached {len(reachable_ids_edges)} Edges & {len(reachable_ids_facs)} Facilities from Home in {primary_travel_time_budget} second")                              
                        ...
                        print(f"Primary Activity Assigned: Facility ID = {id_fac_primary}, Travel Time = {travel_time_primary:.2f} seconds")
                    ...
                    # Check if secondary activity in the activity chain
                        ...
                        print(f"➔ Reached {len(reachable_ids_edges)} Edges & {len(reachable_ids_facs)} Facilities from primary activity location in {secondary_travel_time_budget} second")                              
                       ...
                       # Step 5.7: Selecting final destination for secondary activity and get travel time
                        ...
                        print(f"Secondary Activity Assigned: Facility ID = {id_fac_secondary}, Travel Time = {travel_time_secondary:.2f} seconds")
                        ...
                        dyn_capacities[id_fac_secondary] = max(0, dyn_capacities[id_fac_secondary] - secondary_hour_capacity)
...
#######> STEP 5: SCHEDULING DAILY ACTIVITY - TIME CHOICES
                    ...
                    outgoing_travel_time = sum([activity['travel_time'] for activity in planned_activities]) / 3600  # convert to hours

                    # Step 1: Calculate durations for each activity first (primary first, then secondary)
                    for activity in planned_activities:
                        
                        activity_index = acitivity_types_key.index(activity['actype']) # To get activity duration from activity types
                    ...
                    ids_activitytype = [home_id] + [demand.activitytypes.names.get_id_from_index(activity['actype']) for activity in planned_activities] + [home_id]
                    # Create list of facilities where the activities are taken 
                    ids_facility = [id_fac_home] + [activity['facility_id'] for activity in planned_activities] + [id_fac_home]
                    # Create list min duration for the list of facilities 
                    durations_min_list = [home_morning_duration_min] + activity_durations_min + [home_evening_duration_min]
                    # Create list max duration for the list of facilities
                    durations_max_list = [home_morning_duration_max] + activity_durations_max + [home_evening_duration_max]
                    # Create list earliest time to begin the activities
                    activity_begin_earliest_list = [0.0] + activity_begin_earliest + [home_evening_begin_earliest] # Let the people leave earliest to have early morning traffic
            ...
            row = [
                    ...
                    round(travel_time_secondary, 2) if len(planned_activities) > 1 and 'travel_time_secondary' in locals() else ''
            ]
        ...
        for daily_diaries_type, number_diary in daily_diary_types.items():
            ...
            print(f"Daily Diary Type: {daily_diaries_type} - Virtual Population: {number_diary}, Percentage: ({percentage:.1f}%)")
...
## FUNCTIONS TO CALCULATE ISOCHRONE, LOCATION CHOICE, DYNAMIC CAPACITY CALCULATION, ETC.############################################################
    ...
    def isochrone_calculation(self,
        ...
        for id_edge in ids_edge_nearby:
            ...
            reachable_edges_time = {edges: times for edges, times in reachable_edges_time.items() if times >= min_travel_time}
            reachable_ids_edges = {edges: pre_edges for edges, pre_edges in reachable_ids_edges.items() if edges in reachable_edges_time}                                                          
            ...
            if id_edge in reachable_ids_edges:
                ...
                for fac_id in ids_fac:
                    ...
                    if fac_id not in reachable_facs_time or travel_time < reachable_facs_time[fac_id]:
                        reachable_facs_time[fac_id] = travel_time # Get the min travel time if a facility closing to more than one edge
        ...
        # Step 3: Otherwise, try to use any other available land-use type
            ...
            # Step 4: Use fallback if nothing is reachable
                ...
                if fallback_facs:
                    ...
                    raise Exception("No reachable facilities found for any land type. Try to avoid having many isolated land uses.")
    ...
    #-------------------------Use amenities
        ...
        if is_include_amenities:
            am_landtype = landuse.landusetypes.typekeys.get_id_from_index(landuse.landusetypes.typekeys[landtype]+'_area')
    ...
    #-------------------------Filter too small and too big facilities
        ids_possible_fac = ids_possible_fac[(facilities.areas[ids_possible_fac]>self.min_area_fac)&(facilities.areas[ids_possible_fac]<self.max_area_fac)]
    ...
    #-------------------------Spatial filter
		...
		#Use Zone
            ...
            ids_possible_fac = ids_possible_fac[(facilities.ids_zone[ids_possible_fac]==landuse.zones.ids_sumo.get_id_from_index(self.select_zone))]
    ...
    #-------------------------Distant-based and Time-based Gravitational
            ...
            attractions = (dyn_capacities[ids_possible_fac] ** self.gravitational_capacity_parameter) * (dists ** self.gravitational_dist_parameter)
