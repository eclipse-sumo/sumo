...
class OdPlots(PlotoptionsMixin,Process):
    def __init__(self, ident, demand, logger = None, **kwargs):
        ...
        for id_int, time_start, time_end in zip(ids_inter,demand.odintervals.times_start[ids_inter], demand.odintervals.times_end[ids_inter]):
            intervalchoices[str(int(time_start/60.0))+'min-'+str(int(time_end/60.0))+'min'] = (int(time_start),int(time_end))
    ...
    def show(self):
        ...
        for id_zone_orig, id_zone_dest, id_mode, time_start, time_end, n_trips, n_trips_failed, \
            ...
            if (inter in self.intervals)\
                ...
                print ('  id_zone_orig',id_zone_orig,'id_zone_dest',id_zone_dest,'inter',inter,'id_mode',id_mode,'n_trips',n_trips,'n_trips_failed',n_trips_failed)
