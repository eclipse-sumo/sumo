...
GUISHAPES = [\
    ...
    "delivery",
    "truck",
    "truck/semitrailer",# (length per carriage 13.5)
    "truck/trailer",# (length per carriage 6.75)
    "bus",
    "bus/coach",# (length per carriage 8.25)
    "bus/flexible",# (length per carriage 8.25)
    "bus/trolley",
    "rail",# (length per carriage 24.5)
    "rail/railcar",# (length per carriage 16.85)
...
TAXI_FLEETS = ['no_fleet', 'private', 'SAV']
...
class VehicleTypes(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add(cm.AttrConf(  'lanechangemodel', LANECHANGEMODELS[3],
        ...
        self.add(cm.AttrConf(    'time_waiting_memory',100,
                            name = 'Waiting time memory',
                            perm = 'rw',
                            groupnames = ['parameters'],
                            info = 'Length of time interval, over which accumulated waiting time is taken into account.',
                            #cml ='--waiting-time-memory',
                            ))
        
        self.add(cm.AttrConf(    'time_to_impatience',180,
                            name = 'Time to impatience',
                            perm = 'rw',
                            groupnames = ['parameters'],
                            info = 'Time threshold until compromising actions will be undertaken by the driver or predestrian.',
        ...
        self.add(cm.AttrConf( 'pedestrian_model', 'striping',
                                                ...
                                                choices = ['striping','nonInteracting','jupedsim','None'],
                                                perm = 'rw',
        ...
        self.add(cm.AttrConf( 'range_glosa', 100.0,
                                                groupnames = ['parameters','glosa'],
                                                name = 'Glosa range',
                                                unit = 'm',
                                                perm = 'rw',
                                                info = """maximum range from stop line at which glosa functions become active (default 100). If the current traffic light also sets this parameter, 
                                                the minimum value of the device and tls parameter is used."""
                                                # 'device.glosa.range'
                                                ))
                                                
        self.add(cm.AttrConf( 'speed_min_glosa', 5.0,
                                                groupnames = ['parameters','glosa'],
                                                name = 'Glosa min. speed',
                                                unit = 'm/s',
                                                perm = 'rw',
                                                info = """minimum speed for slow-down maneuver."""
                                                # 'device.glosa.min-speed'
                                                ))                                        
                                                
        self.add(cm.AttrConf( 'speedfac_max_glosa', 1.1,
                                                groupnames = ['parameters','glosa'],
                                                name = 'Glosa max. speedfactor',
                                                perm = 'rw',
                                                info = """maximum speedFactor for trying to reach a green light (default 1.1) Setting a value of 1 will ensure perfect compliance with the speed limit but may still cause slow drivers to speed up."""
                                                # 'device.glosa.max-speedfactor'
                                                )) 
                                                
        #self.add(cm.AttrConf( 'fac_drive_yellow_time', 5.0,
        #                                        groupnames = ['parameters','glosa'],
        #                                        name = 'Glosa drive yellow time',
        #                                        perm = 'rw',
        #                                        info = """This junction model parameter encodes willingness to continue driving at yellow depending on running duration of the yellow phase (default 0). The value will be taken into account when checking the feasibility of a speed-up maneuver."""
        ...
        self.add_col(am.ArrayConf(  'action_step_lengths', 1.0,
                                        groupnames = ['parameters','driver'], 
                                        name = 'Reaction',
                                        unit = 's', 
                                        info = "The driver's reaction time in s (actually the minimum time gap)",
                                        xmltag = 'actionStepLength',
                                        ))                                                                                                                                                               
        self.add_col(am.ArrayConf(  'taus', 1.0,
                                        groupnames = ['parameters','driver'], 
                                        name = 'Desired headway',
                                        unit = 's', 
                                        info = "The desired time headway between follower and leader",
                                        xmltag = 'tau',
                                        ))
        ...
        self.add_col(am.ArrayConf(  'have_glosa_device',  False,
                                        groupnames = ['parameters'], 
                                        name = 'has Glosa device',
                                        info = """Vehicles with Glosa devices can receive information from he next downstream  traffic light, and adjust their speed:
                                        In particular vehicles receive information on the duration of the current red/green phase .""",
                                        #xmltag = '',# tag set explicitely
                                        ))                                
        emissionclasses_xml = {}
        ...
        self.add_col(am.ArrayConf( 'impatiences', -1000.0,
                                        groupnames = ['parameters','driver'], 
                                        name = 'Impatience base',
                                        info = """
                                        If impatience grows above 1 then the driver will use any gap that is safe in the sense of collision-avoidance even if it means that another vehicle has to brake as hard as it can. 
                                        During the simulation, the impatience is calculated by
                                        MAX(0, MIN(1.0, Impatience base  + waiting time / Time to impatience))
                                        The parameter Time to impatience is a global simulation parameter.
                                        """,
                                        xmltag = 'impatience',
                                        ))
        ...
        if self.get_version()<0.5:
            self.shapes_gui.choices = GUISHAPES
    ...
    def add_vtype(self, vtype, **kwargs):
        ...
        self.set_row(       _id,
                            ...
                            action_step_lengths = kwargs.get("action_step_length",1.0),
                            ...
                            times_boarding = kwargs.get("time_boarding",0.0),
                            times_loading = kwargs.get("time_loading",0.0),
                            ...
                            areas_front_surface = kwargs.get("area_front_surface",kwargs.get("width",1.8)*kwargs.get("height",1.50)),
                            ...
                            have_glosa_device = kwargs.get("has_glosa_device",False),
                            have_reroute_device = kwargs.get("has_reroute_device",False),
                            carfollowermodels = kwargs.get("carfollowermodel",CARFOLLOWMODELS[0]),
                            )
    ...
    def add_vtypes_default(self):
        ...
        self.add_vtype('passenger1',  
                        ...
                        color = np.array((245,245,0,255),np.float32)/255.0,
                        shape_gui = 'passenger', 
                        impatience = 1.0,
        ...
        self.add_vtype('ebicycle',  
                        ...
                        impatience = 1.0,
                        emissionclass= 'HBEFA3/zero',
                        color = np.array((100,180,120,255),np.float32)/255.0,
                        shape_gui = 'bicycle', 
                        lanechange_strategy = 1.0,
                        lanechange_coop = 1.0,
                        lanechange_gain = 1.0,
        ...
        self.add_vtype('scooter',  
                        ...
                        speed_max = 25.0/3.6, 
                        deviation_speed = 0.2,
                        mode = 'scooter', # specifies mode for demand
                        impatience = 1.0,
                        emissionclass= 'HBEFA3/zero',
                        color = np.array((100,180,120,255),np.float32)/255.0,
                        shape_gui = 'scooter', 
        ...
        self.add_vtype('taxi1',
                        #TODO: make SAV it's own mode, so taxi1 share in taxi mode goes back to 1.0
                        share_in_mode = 1.0,
                        accel = 1.9, 
                        decel = 2.5, 
                        decel_apparent = 8.0,
                        ...
                        color = np.array((180,180,100,255),np.float32)/255.0,
                        shape_gui = 'passenger/sedan', 
                        impatience = 1.0,
                        emissionclass= 'HBEFA3/PC',
                        time_boarding = 00.0, # passenger pickup and dropoff times specified in taxi.py / in gui: in simulation-->taxiservice
                        time_loading = 90.0, # for containers/freight
                        sublane_alignment_lat = 'center',
                        sublane_speed_max_lat = 0.8,
                        sublane_gap_min_lat = 0.12,
        ...
        self.add_vtype('bus_flexible',
                        ...
                        impatience = 1.0,
                        emissionclass= 'HBEFA3/Bus',
                        color = np.array((255,192,50,255),np.float32)/255.0,
        ...
        self.add_vtype('tram1',  
                        ...
                        impatience = 1.0,
                        emissionclass= 'HBEFA3/zero',
                        color = np.array((255,255,150,255),np.float32)/255.0,
        ...
        self.add_vtype('rail_urban1',  
                        ...
                        emissionclass= 'HBEFA3/zero',
                        color = np.array((100,192,255,255),np.float32)/255.0,
                        shape_gui = 'rail/railcar', 
                        time_boarding = 1.5,
                        time_loading = 0.0,
                        sublane_alignment_lat = 'center',
        ...
        self.add_vtype('evehicle1',  
                        ...
                        color = np.array((0,236,255,255),np.float32)/255.0, 
                        shape_gui = 'evehicle',
                        time_boarding = 15.0,
                        time_loading = 20.0,
                        sublane_alignment_lat = 'center',
        ...
        self.add_vtype('SAV',
                        #TODO: make SAV it's own mode, so it's no longer a 0.5 share of taxi mode
                        share_in_mode = 0.0,  
                        accel = 1.9, 
                        decel = 2.5, 
                        decel_apparent = 8.0,
                        ...
                        height = 1.80,
                        width = 1.8,
                        # TODO: number_persons set to 0 here (because autonomous), but set to 1 when loaded by hybridPY...fixable? 0 good idea?
                        number_persons = 0,
                        capacity_persons = 8,
                        dist_min = 1.0, 
                        speed_max = 180.0/3.6,
                        ...
                        #TODO: make SAV it's own mode, so it's no longer a 0.5 share of taxi mode
                        mode = 'taxi', # specifies mode for demand
                        color = np.array((235,0,30,255),np.float32)/255.0,
                        #shape_gui = 'ant',
                        shape_gui = 'passenger/van',
                        impatience = 0.1,
                        time_boarding = 0.0, # passenger pickup and dropoff times specified in taxi.py / in gui: in simulation-->taxiservice
                        time_loading = 90.0, # for containers/freight
                        sublane_alignment_lat = 'center',
                        sublane_speed_max_lat = 0.8,
                        sublane_gap_min_lat = 0.12,
                        has_taxi_device = True,
                        eprofile = 'ebus',
                        carfollowermodel = CARFOLLOWMODELS[13] #CACC
                        ) 
            
        self.add_vtype('private',  
                        share_in_mode =1.0,  
                        accel = 2.9, 
                        decel = 3.0,
                        decel_apparent = 8.0,
    ...
    def _write_xml_body(self, fd,  indent, objconfigs, idcolconfig_include_tab,colconfigs, 
        ...
        for _id in ids:
            ...
            if self.have_taxi_device[_id]:
                    ...
                    # now this is a hack as these parameters are stored centrally in hybridPY scenartio.simulation.taxiservice
                    taxiservice = self.parent.parent.simulation.taxiservice
                    fd.write((indent+4)*' ' + """<param key="%s" value="%.3f"/>\n"""%('device.taxi.dispatch-period', 0)) #taxiservice.period_dispatch ))
                    
                    if taxiservice.dispatchalgorithm == 'greedyShared':
                            fd.write((indent+4)*' '+"""<param key="%s" value="%.3f"/>\n"""%('relLossThreshold',0)) #taxiservice.loss_rel))
                            fd.write((indent+4)*' '+"""<param key="%s" value="%.3f"/>\n"""%('absLossThreshold',0)) #taxiservice.loss_abs))
                    
                    fd.write((indent+4)*' ' + """<param key="%s" value="%.3f"/>\n"""%('device.taxi.pickUpDuration',0)) #taxiservice.duration_pickup ))
                    fd.write((indent+4)*' ' + """<param key="%s" value="%.3f"/>\n"""%('device.taxi.dropOffDuration',0)) #taxiservice.duration_dropoff ))

                    if taxiservice.parking:
                        fd.write((indent+4)*' ' + """<param key="%s" value="%s"/>\n"""%('device.taxi.parking',"true"))
                    else:
                        fd.write((indent+4)*' ' + """<param key="%s" value="%s"/>\n"""%('device.taxi.parking',"false"))
                    
              
            
            if self.have_glosa_device[_id]:
                    fd.write((indent+4)*' '+"""<param key="has.glosa.device" value="true"/>\n""")
                    # https://sumo.dlr.de/docs/Simulation/GLOSA.html#parameters
                    fd.write((indent+4)*' '+"""<param key="device.glosa.range" value="%.1f"/>\n"""%(self.range_glosa.get_value()))
                    fd.write((indent+4)*' '+"""<param key="device.glosa.min-speed" value="%.1f"/>\n"""%(self.speed_min_glosa.get_value()))
                    fd.write((indent+4)*' '+"""<param key="device.glosa.max-speedfactor" value="%.1f"/>\n"""%(self.speedfac_max_glosa.get_value()))
...
class VtypeReader(handler.ContentHandler):
    ...
    def startElement(self, name, attrs):
        if name == 'vType':
            ...
            #for xmltag, xmlval in self._xmlattrmap.items():
