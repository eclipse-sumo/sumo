...
def create_zones(n_zones,x_min, x_max, y_min, y_max):
    ...
    for i in range(zone_binx):
        for j in range(zone_biny):
            shape = [[x_min+i*x_bin,y_min+j*y_bin,0.],[x_min+i*x_bin,y_min+(j+1)*y_bin,0.],[x_min+(i+1)*x_bin,y_min+(j+1)*y_bin,0.],[x_min+(i+1)*x_bin,y_min+j*y_bin,0.],[x_min+i*x_bin,y_min+j*y_bin,0.]]
...
#CREATE ZONES by CLUSTER OF POINTS
    ...
    for region in regions:
        if region != []:
            ...
            print(shape)
            myscenario.landuse.zones.make(zonename = 'zone_name', coord = np.zeros(3, dtype=np.float32), shape = shape, id_landusetype = 6)
    ids_zone = myscenario.landuse.zones.get_ids()               
