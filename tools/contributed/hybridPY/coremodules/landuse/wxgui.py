...
class FacilityDrawings(ZoneDrawings):
    ...
    def update(self, is_update = True):
        ...
        for id_landusetype in landusetypes.get_ids():
            ...
            #self.colors.value[self._inds_map[inds]] = color
            #self.colors.value[self._inds_map[np.flatnonzero(ids_landusetype == id_landusetype)]] = landusetypes.colors[id_landusetype]
            self.colors[self._facilities.select_ids(ids_landusetype.value == id_landusetype)] = landusetypes.colors[id_landusetype]
        if is_update:
            self._update_vertexvbo()
            self._update_colorvbo()
...
class WxGui(ModuleGui):
    ...
    def make_menu(self):
        ...
        menubar.append_item( 'landuse/import elevations from api...',
                                self.on_import_elevations, 
                                bitmap = self.get_agileicon("Document_Import_24px.png"),
                                )
        ...
        menubar.append_item( 'landuse/edit Polygons with SUMO netedit...',
            self.on_polyedit, 
            bitmap = self.get_icon('netedit.png'),
            )
        
        menubar.append_item( 'landuse/edit Polygons with SUMO netedit on map...',
            self.on_polyedit_on_map, 
            bitmap = self.get_icon('netedit.png'),
            )
        ...
        menubar.append_item( 'landuse/zones/import from geojason...',
            self.on_import_zones_from_geojason, 
            #bitmap = self.get_icon('Files-Osm-icon_24.png'),#
            )     
        menubar.append_item( 'landuse/zones/import zonedata from CSV...',
            self.on_import_zonedata_from_csv, 
            #bitmap = self.get_icon('Files-Osm-icon_24.png'),#
            )    
        ...
        menubar.append_item(    'landuse/Points of interest/import from google...', 
                                self.on_import_google_pois, info='Import POIs from Google...',
                                #bitmap = wx.ArtProvider.GetBitmap(wx.ART_NORMAL_FILE,wx.ART_MENU)
                                bitmap = self.get_agileicon("Document_Import_24px.png"),
                                )
    ...
    def on_polyedit(self,event = None):
        """Edit polygons with SUMO's netedit.
        """
            
        
        self.get_scenario().net.call_netedit(is_maps = False, is_poly = True)
        addfilepath = self.get_scenario().get_rootfilepath()+'.add.xml'
        self._landuse.import_polyxml(filepath = addfilepath, is_pois = False)# do not import pois
        self._mainframe.refresh_moduleguis()

    def on_polyedit_on_map(self,event = None):
        """Edit polygons with SUMO's netedit. In addition to the polygons and the network, backround maps are shown. You need to download the maps before with landuse/import maps.
        """
           
        self.get_scenario().net.call_netedit(is_maps = True, is_poly = True)
        addfilepath = self.get_scenario().get_rootfilepath()+'.add.xml'
        self._landuse.import_polyxml(filepath = addfilepath, is_pois = False)# do not import pois
        self._mainframe.refresh_moduleguis()
    ...
    def on_import_zones_from_geojason(self, event=None):
        
        """
        Import zones from geojason file format.
        """

        proc = landuse.ZoneFromGeojsonImporter(self._landuse.zones, logger = self._mainframe.get_logger())
    ...
    def on_import_zonedata_from_csv(self, event=None):
        
        """
        Import zones data from CSV file.
        """

        proc = landuse.ZonePopulationImporter( self._landuse.zones, logger = self._mainframe.get_logger())
        dlg = ProcessDialog(self._mainframe, proc, immediate_apply=True)
    ...
    def on_import_google_pois(self,event = None):
        """
        Import POIs with Google API.
        """
        p = landuse.GooglePOIImporter(self.get_scenario(), logger = self._mainframe.get_logger())
    ...
    def on_import_elevations(self,event = None):
        """
        Import elevation from the Internet (currently Google API).
        """
        p = landuse.ElevationImporter(self.get_scenario(), logger = self._mainframe.get_logger())
        dlg = ProcessDialog(self._mainframe, p, immediate_apply=True)
