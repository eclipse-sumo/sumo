...
def download_googlemap_bb(filepath,bbox,proj,apikey,size = 640,filetype = 'gif',maptype = 'satellite', color = "0xff0000ff"):
    ...
    url = URL_GOOGLEMAP+"key=%s&size=%dx%d&format=%s&maptype=%s&scale=2&path=color:%s|weight:3"%(apikey,size_x,size_y,filetype.upper(),maptype,color)
    url += "|%.6f,%.6f"%(lat0,lon0)
    url += "|%.6f,%.6f"%(lat2,lon2)
...
class MapsImporter(Process):
    ...
    def get_n_tiles(self):
            ...
            if self.is_apply_bbox_explicit:
                return self._maps.get_n_tiles(self.width_tile, bbox_sumo = self.bbox_explicit)
            else:
    ...
    def do(self):
        ...
        self._maps.download(maptype = 'satellite', mapserver = 'google', 
                    ...
                    width_tile = self.width_tile,  size_tile = self.size_tile,
                    is_remove_orig = self.is_remove_orig, bbox_sumo = bbox_sumo)
...
class Maps(am.ArrayObjman):
    ...
    def _init_attributes(self):   
        ...
        self.bboxes.add_groupnames(['_private'])
    ...
    def write_decals(self, fd, indent = 4,  rootdir = None, delta = np.zeros(3,dtype=np.float32) ):
        ...
        for filename, bbox in zip(self.filenames.get_value(),self.bboxes.get_value()):
            ...
            calxml = '<decal file="%s" centerX="%.2f" centerY="%.2f" centerZ="0.00" width="%.2f" height="%.2f" altitude="0.00" rotation="0.00" tilt="0.00" roll="0.00" layer="-1000.00"/>\n'%(filepath,xc,yc,width_tile,width_tile)
    ...
    def get_n_tiles(self, width_tile, bbox_sumo = None):
        """
        Estimates number of necessary tiles.
        """      
        net = self.parent.get_net()  
        if bbox_sumo is None:
    ...
    def download(self, maptype = 'satellite', mapserver = 'google', apikey = None,
                    ...
                    is_remove_orig = True, bbox_sumo = None):
        ...
        for ix in range(nx):
            for iy in range(ny):
                ...
                id_map = self.add_row(  filenames = os.path.basename(outfilepath), 
                                        bboxes = bb,#bbox_tile,#bbox_tile_lonlat#np.array([[lon0, lat0],[lon1, lat1]],np.float32),
                                        )
