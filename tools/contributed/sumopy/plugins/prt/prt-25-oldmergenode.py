...
class Mergenodes(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add_col(am.IdsArrayConf('ids_mergenode_in1', self,
                                     name='ID mergenode in 1',
                                     info='ID of upstream mergenode at incoming line 1.',
                                     ))

        self.add_col(am.ArrayConf('distances_mergenode_in1', 0.0,
                                  groupnames=['parameters'],
                                  name='Distance mergenode in 1',
                                  info="Distance to mergenode at incoming line 1.",
                                  unit='m',
                                  ))
        ...
        self.add_col(am.IdsArrayConf('ids_mergenode_in2', self,
                                     name='ID mergenode in 2',
                                     info='ID of upstream mergenode at incoming line 2.',
                                     ))
        self.add_col(am.ArrayConf('distances_mergenode_in2', 0.0,
                                  groupnames=['parameters'],
                                  name='Distance mergenode in 2',
                                  info="Distance to mergenode at incoming line 2.",
                                  unit='m',
                                  ))

        self.add_col(am.IdlistsArrayConf('ids_mergenodes_out', net.nodes,
            ...
            if id_type == id_zippertype:
                ...
                if (len(ids_edge_from) == 2) & (len(ids_edge_to) == 1):
                    ...
                    if (al1 == 2) & (al2 == 2):
                        ...
                        ids_mergenode_out = self.search_downstream_merges(ids_edge_to[0], edges, lanes, id_prtmode)
                        self.add_row(ids_node=id_node,
                                     ids_mergenode_in1=id_node_up1,
                                     distances_mergenode_in1=dist1,
                                     ids_mergenode_in2=id_node_up2,
                                     distances_mergenode_in2=dist2,
                                     ids_mergenodes_out=ids_mergenode_out,
                                     are_station=False,
                                     )
                    ...
                    if al_in == 1:
                        ...
                        if (al1 == 1) & (al2 == 2):
                            ...
                            ids_mergenode_out = self.search_downstream_merges(id_edge2, edges, lanes, id_prtmode)
                            self.add_row(ids_node=id_node,
                                         ids_mergenode_in1=id_node_up,
                                         distances_mergenode_in1=dist,
                                         ids_mergenodes_out=ids_mergenode_out,
                                         are_station=True,
                                         )
                            ...
                            ids_mergenode_out = self.search_downstream_merges(id_edge1, edges, lanes, id_prtmode)
                            self.add_row(ids_node=id_node,
                                         ids_mergenode_in1=id_node_up,
                                         distances_mergenode_in1=dist,
                                         ids_mergenodes_out=ids_mergenode_out,
    ...
    def process_step(self, process):
        ...
        ids = self.get_ids()
        for id_merge, id_node, id_edge_out, ids_mergenode_out, ids_veh_out_sumo in\
            zip(ids,
                self.ids_node[ids],
                self.ids_merge_to_ids_edge_out_sumo[ids],
                self.ids_mergenodes_out[ids],
            ...
            if 0:
                ...
                print '  ids_mergenode_out', ids_mergenode_out
            ...
            if ids_veh_out_sumo != ids_veh_sumo:
                ...
                ids_merge_out = self.ids_node_to_ids_merge[ids_mergenode_out]
                # print '  ids_mergenode_out',ids_mergenode_out
                ...
                for id_veh, id_veh_sumo in zip(ids_veh_entered, ids_veh_entered_sumo):
                    ...
                    route_sumo = traci.vehicle.getRoute(id_veh_sumo)
                    ...
                    for id_merge_out, id_edge_sumo in zip(ids_merge_out, ids_edge_mergeout_sumo):
                        if id_edge_sumo in route_sumo:
                            ...
                            self.enter_veh(id_veh, id_veh_sumo, id_merge, id_merge_out)
                            # measure dist to merge with traci call or precalc
                            break

                self.ids_vehs_out_sumo[id_merge] = ids_veh_sumo

    def enter_veh(self, id_veh, id_veh_sumo, id_merge_from, id_merge_to):
        print 'enter_veh id_veh %d, id_merge_from %d to id_merge_to %d' % (id_veh, id_merge_from, id_merge_to)
        ...
        elif self.ids_node_to_ids_merge[self.ids_mergenode_in1[id_merge_to]] == id_merge_from:
            indpos = -1
            lineind = 1
        else:
            indpos = -1
        ...
        if lineind == 1:
            ...
            dist_mergenode = self.distances_mergenode_in1[id_merge_to]  # correct with pos

        elif lineind == 2:
            self.ids_vehs_in2[id_merge_to].append(id_veh)
            self.ids_vehs_in2_sumo[id_merge_to].append(id_veh_sumo)
            dist_mergenode = self.distances_mergenode_in2[id_merge_to]  # correct with pos
        ...
        if indpos == -1:
            ...
            ind_pos_check = 0
            is_insert_in_front = False
            id_veh_merged = -1

            print '  ids_vehs_merged_sumo', ids_vehs_merged_sumo
            print '  lineinds_vehs_merged', lineinds_vehs_merged

            for id_veh_merged, id_veh_merged_sumo in zip(ids_vehs_merged, ids_vehs_merged_sumo):
                dist = traci.vehicle.getDrivingDistance(id_veh_merged_sumo, id_edge_merge_sumo, 0.0)
                print '    check veh %d, dist %.2fm, dist_mergenode  %.2fm, ind_pos_check = %d' % (id_veh_merged, dist, dist_mergenode, ind_pos_check)
                if dist > dist_mergenode:
                    ...
                    is_insert_in_front = True
                    break

                ind_pos_check += 1

            #ind_pos_check =-1
            if is_insert_in_front:
                ind_insert = ind_pos_check - 1
                print '    insert veh %d behind veh %d, index %d' % (id_veh, id_veh_merged, ind_insert)
            else:
                ind_insert = ind_pos_check-1
                print '    append veh %d behind veh %d, last index %d' % (id_veh, id_veh_merged, ind_insert)
