...
class Decompressors(Compressors):
    ...
    def process_step(self, process):
        ...
        for id_comp,\
                ...
                for id_veh_entered, id_veh_entered_sumo, id_leader, length_plat\
                    ...
                    if (length_plat > 0.001) & (id_leader == -1):
                        ...
                        print '    dispatch id_veh_entered_sumo %s with target %s' % (
                            id_veh_entered_sumo, id_targetedge_sumo)
                        ...
                        for ind_queue, queue, capa_queue, in zip(range(n_queues), queues, capacities):
                            if id_platoonleader in queue:
                                print '      found id_platoonleader prt.%d of prt.%d at queue %d' % (
                                    id_platoonleader, id_veh_entered, ind_queue)
            ...
            if (simtime - self.releasetimes[id_comp] > 10):
                # now check if any of the vehicle in any of the queues has arrived
                # and if a queue is complete, create a platoon, send it off and
                # reset the queue
                print '  Deal with queues...'
                for ind_queue, queue, queue_alloc,\
                    ...
                    # here we coukd also test timeout conditions
                    if (simtime - releasetime_queue > 15):
                        ...
                        for i in range(len(queue_alloc)):
                            ...
                            if vehicles.reached_stop_sumo(vehicles.ids_sumo[id_veh]):
                                ...
                                ind_pl_alloc = i
                                break
                        print '      found alloc prt.%d at ind_pl_alloc %d' % (id_veh_arrived, ind_pl_alloc)
                        if id_veh_arrived > -1:
                            ...
                            if length_plat_arrived >= length_plat:
                                ...
                                for id_veh, id_veh_sumo in zip(ids_platoon[::-1], vehicles.ids_sumo[ids_platoon[::-1]]):
                                    ...
                                    vehicles.reschedule_trip_sumo(id_veh_sumo, id_targetedge_sumo)
                                    queue.remove(id_veh)
    ...
    def _alloc_queue(self, queue, queue_alloc, capa_queue,
        ...
        queue_alloc.append((id_veh, id_targetedge_sumo))
        len_queue = len(queue)
    ...
    def _alloc_queue_follower(self, queue, queue_alloc, capa_queue,
        ...
        print '_alloc_queue_follower', id_veh_sumo, id_shuntedge_sumo
        vehicles.reschedule_trip_sumo(id_veh_sumo, id_shuntedge_sumo)
        #vehicles.set_stop(id_veh, id_shuntedge_sumo, pos_stop, laneindex = 0)
        ...
        for id_node, ids_edge_from, ids_edge_to, id_type in zip(
            ...
            if True:
                ...
                if (len(ids_edge_from) == 2) & (len(ids_edge_to) == 1):
                    ...
                    if al_in == 1:
                        print '    mixed access level of incoming edge=> platform exit node'
                        id_node_up, dist = self.search_upstream_merge(id_edge_from, edges, lanes, id_prtmode)
...
class PrtVehicles(am.ArrayObjman):
    ...
    def process_step(self, process):
        ...
        fact_urgent = np.clip(dists_safe/diststomerge_min, 0.0, 1.0)
    ...
    def get_ids_sumo_sorted(self, ids_sumo):
        ...
        """
        print 'get_ids_sumo_sorted'
        ids_sumo_ordered = np.array(list(ids_sumo), dtype=np.object)
        n = len(ids_sumo_ordered)
        poss = np.zeros(n, dtype=np.float32)
        for i, id_sumo in zip(range(n), ids_sumo_ordered):
            print '  i, id_sumo', i, id_sumo, traci.vehicle.getPosition(id_sumo)
            poss[i] = traci.vehicle.getPosition(id_sumo)
        inds = np.flipud(np.argsort(poss))
        return ids_sumo_ordered[inds], poss[inds]
