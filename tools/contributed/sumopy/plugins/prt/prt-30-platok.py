...
class PrtVehicles(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.length_plat = np.ones(n_id_max, dtype=np.float32)
    ...
    def process_step(self, process):
        ...
        for id_veh, id_veh_sumo in zip(ids_update, self.ids_sumo[ids_update]):
            ...
            if 1:
                ...
                print '  %7s' % id_veh_sumo, 'ghosts', ids_ghost, self.ids_leader[id_veh], "lp=%.1fm" % self.length_plat[id_veh]
        ...
        # print '  dists',dists
        #dists_min = np.min(self.dists0[ids_contr] + deltas_ghosts - deltas_vehs, 1) - (self.length+self.length_plat[ids_contr])

        dists_min = np.min(self.dists0[ids_contr] + deltas_ghosts - deltas_vehs -
                           self.length_plat[ids_ghosts], 1) - self.length
        ...
        if 1:
            for id_sumo, velocity, velocitiy_ghost_min,\
                ...
                in zip(self.ids_sumo[ids_contr],
                       ...
                       dists_min,
                       deltas_crit,
                       inds_slow, inds_accel, self.length_plat[ids_contr]
                       ):
                ...
                print '    %7s: v=%3.1f vg=%3.1f dh=%4.1fm th=%4.1fs ds=%4.1fm dc=%4.1fm len=%4.1fm  %s' % (id_sumo, velocity, velocitiy_ghost_min, dist_min, th, dist_safe, dist_comf, length_plat, a)
    ...
    def concatenate(self, id_veh, id_veh_pre):
        ...
        self._update_concatenate(id_veh, self.length_plat[id_veh])
    ...
    def _update_concatenate(self, id_veh, length_plat):
        ...
        if self.ids_leader[id_veh] == -1:
            # first vehicle
            self.length_plat[id_veh] = length_plat
        else:
            # propagate platoon length
            self.length_plat[id_veh] = 0.0
    ...
    def decatenate(self, id_veh):
        ...
        if id_leader > -1:
            ...
            if self.ids_leader[id_leader] == -1:
                ...
                self.length_plat[id_veh] = self.length_plat[id_leader]-self.length
            else:
