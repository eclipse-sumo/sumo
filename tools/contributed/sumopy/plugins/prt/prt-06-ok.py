...
class PrtStops(am.ArrayObjman):
    ...
    def get_waitpositions(self, ids, is_alight=False, offset=-0.0):
        ...
        """
        # print 'get_waitpositions min(ids),max(ids)',min(ids),max(ids)
        positions = np.zeros(len(ids), dtype=np.int32)
        ...
        for id_stop in ids:
            ...
            # print '  id_stop,ids_berth,ind_berth',id_stop,ids_berth,ind_berth,stoppositions[ids_berth[ind_berth]]
    ...
    def prepare_sim(self, process):
        ...
        id_person_to_ids_edge_sumo_dests = {}
        ...
        for id_person, stagelist in zip(ids_persons, stagelists[virtualpop.ids_plan[ids_persons]]):
            for stages, id_stage in stagelist:
                if stages.ident == 'prttransits':
                    edge_orig_dest = (ids_edge_sumo[stages.ids_fromedge[id_stage]],
                                      ids_edge_sumo[stages.ids_toedge[id_stage]])
                    id_person_sumo = virtualpop.get_id_sumo_from_id(id_person)
                    if id_person_to_ids_edge_sumo_dests.has_key(id_person_sumo):
                        id_person_to_ids_edge_sumo_dests[id_person_sumo].append(edge_orig_dest)
                    else:
                        id_person_to_ids_edge_sumo_dests[id_person_sumo] = [edge_orig_dest]

        # print '   id_person_to_ids_edge_sumo_dests',id_person_to_ids_edge_sumo_dests
        self.id_person_to_ids_edge_sumo_dests = id_person_to_ids_edge_sumo_dests

        self.ids_vehs = np.zeros(np.max(ids)+1, dtype=np.object)
        self.ids_vehs_toallocate = np.zeros(np.max(ids)+1, dtype=np.object)

        self.times_lastboard = np.zeros(np.max(ids)+1, dtype=np.int32)
    ...
    def process_step(self, process):
        ...
        for id_prtstop, id_edge_sumo, ids_veh_sumo_prev, ids_person_sumo_prev in\
            ...
            if 1:
                ...
                # print '    ids_berth_board',self.ids_berth_board[id_prtstop]
                print '    ids_vehs_alight_allocated', self.ids_vehs_alight_allocated[id_prtstop]
                print '    ids_vehs_toallocate', self.ids_vehs_toallocate[id_prtstop]
                print '    ids_vehs_board_allocated', self.ids_vehs_board_allocated[id_prtstop]
                print '    inds_berth_alight_allocated', self.inds_berth_alight_allocated[id_prtstop]

            if 0:
                for id_veh_sumo in self.ids_vehs_sumo_prev[id_prtstop]:
            ...
            ids_veh_remove = []
            for id_veh in self.ids_vehs_board_allocated[id_prtstop]:
                # TODO: here we could also check vehicle position
                if traci.vehicle.isStopped(vehicles.get_id_sumo(id_veh)):
                    ids_veh_remove.append(id_veh)
            ...
            for id_veh in ids_veh_remove:
                self.ids_vehs_board_allocated[id_prtstop].remove(id_veh)

            # check for new person arrivals/departures
            ...
            if ids_person_sumo_prev != ids_person_sumo:
                #ids_veh_entered = vehicles.get_ids_from_ids_sumo(list(ids_veh_sumo.difference(ids_veh_sumo_prev)))
                ids_person_sumo_left = list(ids_person_sumo_prev.difference(ids_person_sumo))
                #ids_person_left = virtualpop.get_ids_from_ids_sumo()
                for id_person_sumo in ids_person_sumo_left:
                    ...
                    if traci.person.getRoadID(id_person_sumo) == id_edge_sumo:
                        ...
                        self.ids_persons_sumo_boarded[id_prtstop].append(id_person_sumo)

                self.ids_persons_sumo_prev[id_prtstop] = ids_person_sumo
            ...
            if 1:
                ...
                print '    ids_persons_sumo_boarded', self.ids_persons_sumo_boarded[id_prtstop]

            # check if boarding is completed in load area and reprogram
            ids_berth_board = self.ids_berth_board[id_prtstop][::-1]
            for id_berth_board, id_veh, stoppos in zip(
                ids_berth_board,
                berths.ids_veh[ids_berth_board],
                berths.stoppositions[ids_berth_board],
            ):
                if id_veh >= 0:  # is there a waiting vehicle
                    if vehicles.is_completed_boarding(id_veh):
                        self.times_lastboard[id_prtstop] = process.simtime
                        berths.states[id_berth_board] = BERTHSTATES['free']
                        berths.ids_veh[id_berth_board] = -1
                        print '    veh %s completed boarding' % id_veh
                        # identify whic of the boarding persons is in the
                        # vehicle which completed boarding
                        dist_min = np.inf
                        id_person_sumo_inveh = None
                        for id_person_sumo in self.ids_persons_sumo_boarded[id_prtstop]:
                        ...
                        if id_person_sumo_inveh is not None:
                            ...
                            id_edge_sumo_from, id_edge_sumo_to = self.id_person_to_ids_edge_sumo_dests[id_person_sumo_inveh].pop(
                                0)
                            print '    found person', id_person_sumo_inveh, 'from', id_edge_sumo_from, 'to', id_edge_sumo_to
                            vehicles.schedule_trip_occupied(id_veh, id_edge_sumo_to)
                            self.ids_persons_sumo_boarded[id_prtstop].remove(id_person_sumo_inveh)
                        else:
                            print 'WARNING: no person found inside vehicle', id_veh

            # check if there are passengers in the vehicles to allocate
            # TODO: can be replaced by a single instruction
            n_pax = 0
            for id_veh in self.ids_vehs_alight_allocated[id_prtstop]+self.ids_vehs_toallocate[id_prtstop]:
            ...
            if np.all(berths.states[ids_berth_board] == BERTHSTATES['free']):
                # print '    foreward all occupied id_prtstop,ids_berth_board',id_prtstop,ids_berth_board
                #self.foreward_boardzone(id_prtstop, ids_berth_board)
                pass

            # foreward if there are passengers in unallocated vehicles
            elif n_pax > 0:
                # print '    foreward pax waiting id_prtstop,n_pax',id_prtstop,n_pax
                self.foreward_boardzone(id_prtstop, ids_berth_board)

    def foreward_boardzone(self, id_stop,  ids_berth_board):
        ...
        for id_berth, state in zip(ids_berth_board, berths.states[ids_berth_board]):
            if state == BERTHSTATES['boarding']:
                id_veh = berths.ids_veh[id_berth]
                id_stop_target, id_edge_sumo_target = self.plan_emptytrip(id_stop)
                print '   empty trip for', id_veh, ' from', id_stop, 'to', id_stop_target, id_edge_sumo_target
                self.parent.prtvehicles.schedule_trip_empty(id_veh, id_edge_sumo_target)
                berths.states[id_berth] = BERTHSTATES['free']
                berths.ids_veh[id_berth_board] = -1

    def plan_emptytrip(self, id_stop):
        # print 'plan_emptytrip from',id_stop
        # search closest stop
        ids_stop = list(self.get_ids())
        ids_stop.remove(id_stop)
        id_stop_target = ids_stop[random.randint(0, len(ids_stop)-1)]
        # print '  to stop',id_stop
        return id_stop_target, self.ids_stop_to_ids_edge_sumo[id_stop_target]
    ...
    def _init_attributes(self):
        ...
        if not hasattr(self, 'prttransit'):
            ...
            self.prttransits = attrsman.add(
                cm.ObjConf(virtualpop.add_stagetype('prttransits', PrtTransits), is_child=False))
