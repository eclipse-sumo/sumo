...
BERTHSTATES = {'free': 0, 'allocated': 1, 'boarding': 2, 'alighting': 3}
...
class PrtStops(am.ArrayObjman):
    ...
    def process_step(self, process):
        ...
        for id_prtstop, id_edge_sumo, ids_veh_sumo_prev in\
            ...
            for id_veh in ids_veh_entered:
                ...
                #                        ])
            # for id_veh_sumo in ids_veh_sumo_entered:
                # unsubscribe
                # traci.vehicle.unsubscribe(id_veh_sumo)
                # print '  unsubscribe?',id_veh_sumo
                # print '  ',dir(traci.vehicle)
        # check if vehicles need to be forwarded
        inds_forward = np.flatnonzero(self.inds_berth_alight_allocated == 0)
    ...
    def enter(self, id_stop, id_veh):
        ...
        if id_berth < 0:
            ...
            self.parent.prtvehicles.control_stop(id_veh, id_stop,
    ...
    def allocate_alight(self, id_stop):
        ...
        if ind_berth == 0:
            ...
            # print '  slowDown'
            #traci.vehicle.slowDown(id_veh_sumo, 1.0, 10)
            return -1
        else:
            ind_berth -= 1
...
class PrtVehicles(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.ids_berth_alloc = -1*np.ones(np.max(ids)+1, dtype=np.int32)
    ...
    def control_stop(self, id_veh, id_stop,
                     id_edge_sumo=None,
                     position=None):
