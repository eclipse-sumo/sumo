...
get_traci_route = traci.vehicle.getRoute
...
class Mergenodes(am.ArrayObjman):
    ...
    def _init_attributes(self):
        ...
        self.add_col(am.IdlistsArrayConf('ids_nodes_out', net.nodes,
                                         name='IDs mergenode out',
                                         info='IDs of downstream mergenodes.',
                                         ))
        ...
        for id_merge, id_node, id_edge_out, ids_node_out, ids_veh_out_sumo in\
            ...
            if ids_veh_out_sumo != ids_veh_sumo:
                ...
                for id_veh, id_veh_sumo in zip(ids_veh_entered, ids_veh_entered_sumo):
                    # print '    >>entered vehicle',id_veh,id_veh_sumo
                    ...
                    if not has_merge:
                        ...
                        self.exit_veh(id_veh, id_veh_sumo, id_merge, id_edge_sumo, vehicles)
    ...
    def exit_veh(self, id_veh, id_veh_sumo, id_merge_from,  id_edge_merge_sumo, vehicles):
    ...
    def enter_veh(self, id_veh, id_veh_sumo, id_merge_from, id_merge_to, id_edge_merge_sumo, vehicles):
        ...
        if lineind == 1:
            ...
            dist_mergenode = self.distances_node_in1[id_merge_to]  # correct with pos

        elif lineind == 2:
            self.ids_vehs_in2[id_merge_to].append(id_veh)
            self.ids_vehs_in2_sumo[id_merge_to].append(id_veh_sumo)
            dist_mergenode = self.distances_node_in2[id_merge_to]  # correct with pos

        if indpos == -1:
            # vehicle is new and must be merged into   ids_vehs_merged
            # print '  merge veh %d arriving from in $d at dist %.2fm'%(id_veh,lineind,dist_mergenode)
            dist_mergenode2 = traci.vehicle.getDrivingDistance(id_veh_sumo, id_edge_merge_sumo, 0.0)
            print '  merge veh %d arriving from in %d at dist %.2fm, dist2 %.2fm' % (id_veh, lineind, dist_mergenode, dist_mergenode2)
            ...
            print '  lineinds_vehs_merged', lineinds_vehs_merged
            if (ind_insert == 0) | self.are_station[id_merge_to]:
                ...
                # really merged because aunly one in line
                ...
                for id_veh_merged, id_veh_merged_sumo in zip(ids_vehs_merged[::-1], ids_vehs_merged_sumo[::-1]):
                    dist = traci.vehicle.getDrivingDistance(id_veh_merged_sumo, id_edge_merge_sumo, 0.0)
                    print '    check veh %d, dist %.2fm, dist_mergenode  %.2fm, ind_insert = %d' % (id_veh_merged, dist, dist_mergenode, ind_insert)
                    if dist_mergenode > dist:
                ...
                if is_insert:
                    ...
                    if ind_insert == len(ids_vehs_merged):
                        ...
                        if lineinds_vehs_merged[ind_insert-1] != lineind:
                            ...
                            vehicles.add_ghost(id_veh, id_ghost, dist_mergenode, dist)
                        ...
                        if lineinds_vehs_merged[ind_insert] != lineind:
                            vehicles.add_ghost(id_ghost1, id_veh, dist_last, dist_mergenode)

                        if lineinds_vehs_merged[ind_insert-1] != lineind:
                            vehicles.add_ghost(id_veh, id_ghost2, dist_mergenode, dist)
                    ...
                    if lineinds_vehs_merged[ind_insert] != lineind:
                        ...
                        vehicles.add_ghost(id_veh_behind, id_veh, dist, dist_mergenode)
...
class PrtVehicles(am.ArrayObjman):
    ...
    def prepare_sim(self, process):
        ...
        self.odos0_vehicles = np.inf * np.ones((n_id_max, n_ghosts_max), dtype=np.float32)
    ...
    def del_ghost(self, id_veh, id_ghost):
        ...
        if id_ghost in self.ids_ghosts[id_veh]:
            ...
            self.odos0_vehicles[id_veh][ind_ghost] = 0.0
            self.odos0_ghosts[id_veh][ind_ghost] = 0.0
