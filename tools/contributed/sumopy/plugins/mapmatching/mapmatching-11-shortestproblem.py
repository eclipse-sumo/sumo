...
# @file    mapmatching-11-shortestproblem.py
...
from coremodules.demand.demand import Trips, Routes, DemandobjMixin
...
class PostMatchfilter(FilterMixin):
    ...
    def filter_ids(self):
        ...
        inds_eliminate = np.logical_or(inds_eliminate, trips.lengths_route_matched[ids_selected] < 1.0)
...
class GpxImporter(FilterMixin):
    def __init__(self,  mapmatching, logger=None, **kwargs):
        ...
        self.filepath = attrsman.add(
            cm.AttrConf('filepath', kwargs.get('filepath', rootfilepath+'.gpx'),
                        groupnames=['options'],
                        perm='rw',
                        name='filename',
                        wildcards='GPX file (*.gpx)|*.gpx|XML file (*.xml)|*.xml',
                        metatype='filepath',
                        info="""Path and file name of GPX file with GPS traces in XML format.""",
                        ))
        self._init_traceoptions(**kwargs)
...
class GpsTrips(Trips):
    ...
    def route_shortest(self, dist_modespecific=5.0, c_modespecific=0.9,
        ...
        # delete current
        ids_with_shortes = self.select_ids(np.logical_and(
            self.are_selected.get_value(), self.ids_route_shortest.get_value() >= 0))
        ...
        ids_route = routes.add_rows(ids_trip=ids_trip,
                                    ids_edges=ids_edges,
                                    costs=costs,
                                    colors=COLOR_SHORTEST_ROUTE*np.ones((len(ids_trip), 4), dtype=np.float32),
                                    )
...
class Routesresults(am.ArrayObjman):
    def __init__(self, ident, parent,
        ...
        self.add_col(am.IdsArrayConf('ids_route', parent.parent.trips.routes,
                                     groupnames=['state'],
                                     is_index=True,
                                     name='ID route',
                                     info='ID of route.',
...
class Routesanalyzer(Process):
    ...
    def do(self):
        ...
        edgesresults.speeds_average[ids_valid] = edges.lengths[edgesresults.ids_edge[ids_valid]] / \
            (edgesresults.durations_tot_matched[ids_valid]/edgesresults.numbers_tot_matched[ids_valid])
