...
# @file    mapmatching-04-improveok.py
...
class BirgilMatcher(Process):
    ...
    def match_trip_birgil(self, id_trip,  ids_point, id_mode,  fstar, costsdir):
        ...
        while (ind_point < ind_point_final):  # & is_connected:
            ...
            for routeinfo in routelist:
                ...
                if (np.isnan(dist_point_edge)) | (dist_point_edge > self.width_buffer_max):
                    ...
                    #edges_next = []
                    # for edge in edges_next_all:
                    #    if edge in self._edge_to_id_edge:
                    #        edges_next.append(edge)
        ...
        if ids_edge[-1] in ids_edge_final:  # dist!=np.Inf: DID WE ARRIVE?
            ...
            print 'WARNING: last matched edge', ids_edge[-1], ' did not reach final edges', ids_edge_final
            route = ids_edge
            #route = []
    ...
    def _get_cost_birgil(self, p, id_edge, phi_point, accesslevel, get_dist_point_to_edge):
        ...
        print '     dist =%.1f phi_point=%.2f,phi_seg=%.2f,phi_point-phi_seg=%.2f, delta_phi=%.2f' % (
            dist, phi_point/np.pi*180, phi_seg/np.pi*180, (phi_point-phi_seg)/np.pi*180, np.clip(phi_point-phi_seg, -np.pi/2, np.pi/2)/np.pi*180)
        ...
        print '     cost_birgil=%.3f, dist=%.3f,  cost_angle=%.3f, accesslevel=%d' % (
            cost, dist, cost_angle, accesslevel)
        print '     cost_birgil=%.3f, Xist=%.3f,  Xost_angle=%.3f, Xccesslevel=%.3f' % (
            cost, dist, self.weight_angle * cost_angle, self.weight_access*accesslevel)
