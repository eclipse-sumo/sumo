...
class WxGui(ModuleGui):
    ...
    def refresh_widgets(self):
        ...
        if is_refresh | self._is_needs_refresh:
            ...
            drawing.set_element('gpsroutesdraws', GpsRoutesDrawings,
                                self._results.tripresults.get_routes(), layer=149)
    ...
    def make_menu(self):
        ...
        menubar.append_item('plugins/mapmatching/browse GPS data',
        ...
        menubar.append_item('plugins/mapmatching/browse results',
        ...
        menubar.append_item('plugins/mapmatching/open results...',
                            self.on_open_results,
                            bitmap=wx.ArtProvider.GetBitmap(wx.ART_FILE_OPEN, wx.ART_MENU),
                            )

        menubar.append_item('plugins/mapmatching/safe results as...',
                            self.on_save_results_as,
                            bitmap=wx.ArtProvider.GetBitmap(wx.ART_FILE_SAVE_AS, wx.ART_MENU),
                            )

    def on_save_results_as(self, event=None):
        """
        ...
        wildcards_obj = "Python binary mapmatch result files (*.mmatch.obj)|*.mmatch.obj|Python binary files (*.obj)|*.obj"
    ...
    def on_open_results(self, event=None):
        ...
        wildcards_all = "All files (*.*)|*.*"
        wildcards_obj = "Python binary mapmatching result files (*.mmatch.obj)|*.mmatch.obj|Python binary files (*.obj)|*.obj"
        wildcards = wildcards_obj+"|"+wildcards_all
    ...
    def on_match_birgil(self, event=None):
        ...
        """
        if self._matchprocess.ident != 'birgilmatcher':
            self._matchprocess = mapmatching.BirgilMatcher('birgilmatcher',
                                                           self._mapmatching, self._results,
                                                           logger=self._mainframe.get_logger()
                                                           )
        ...
        dlg = ProcessDialog(self._mainframe, self._matchprocess, immediate_apply=True)
    ...
    def is_matchprocess(self, ident):
        # print 'is_matchprocess',self._matchprocess
        if self._matchprocess is None:
            return False
        else:
            # print '  self._matchprocess.ident',self._matchprocess.ident
    ...
    def close_match_birgil(self, dlg):
        ...
        print 'close_match_birgil', dlg.get_status(), dlg.get_status() == 'success'
        if dlg.get_status() == 'success':
            #p = self._mapmatchprocess

            self._mainframe.browse_obj(self._results.tripresults)
            self._is_needs_refresh = True
            self.refresh_widgets()
    ...
    def on_browse(self, event=None):
        """View and browse GPS data in object panel."""
        self._mainframe.browse_obj(self._mapmatching)
        if event:
            event.Skip()

    def on_browse_results(self, event=None):
        """View and browse mapmatching results in object panel."""
