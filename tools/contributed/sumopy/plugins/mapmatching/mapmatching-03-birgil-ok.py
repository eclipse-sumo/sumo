...
# @file    mapmatching-03-birgil-ok.py
...
class BirgilMatcher(Process):
    ...
    def match_trip_birgil(self, id_trip,  ids_point, id_mode,  fstar, costsdir):
        ...
        return route, length_route, duration_gps, lengthindex, err_dist, t_match
...
class PostMatchfilter(Process):
    def __init__(self,  mapmatching, logger=None, **kwargs):
        ...
        self.lengthindex_max = attrsman.add(cm.AttrConf('lengthindex_max', kwargs.get('lengthindex_max', 0.9),
                                                        groupnames=['options'],
                                                        perm='rw',
        ...
        if (lengthindex < self.lengthindex_max)\
            & (lengthindex >= self.lengthindex_min)\
            & (duration_gps > self.duration_min)\
            & (length_route > self.dist_min)\
                & (len(route) > 0):
            if log:
                log.w('  SUCCESS: %d edges, lengthind=%.2f, duration=%ds \n           len=%.0f, pnts=%d, err=%.2fm/pnt., err_march=%.2fm/pnt., t_match=%dms' %
                      (len(route), lengthindex, duration_gps, length_route, n_points_eff, err_dist, err_dist_alg, t_match*1000))
        else:
            if log:
                log.w('  FAILED:  %d edges, lengthind=%.2f, duration=%ds \n           len=%.0f, pnts=%d, err=%.2fm/pnt., err_march=%.2fm/pnt., t_match=%dms' %
                      (len(route), lengthindex, duration_gps, length_route, n_points_eff, err_dist, err_dist_alg, t_match*1000))
            route = None  # unvalidate route!


class TripGeomfilter(Process):
    def __init__(self,  mapmatching, logger=None, **kwargs):
...
class TripTimefilter(Process):
    def __init__(self,  mapmatching, logger=None, **kwargs):
        ...
        self.hour_from_morning = attrsman.add(cm.AttrConf('hour_from_morning', kwargs.get('hour_from_morning', 8),
                                                          groupnames=['options'],
                                                          perm='rw',
        ...
        self.hour_to_morning = attrsman.add(cm.AttrConf('hour_to_morning', kwargs.get('hour_to_morning', 8),
                                                        groupnames=['options'],
                                                        perm='rw',
        ...
        self.hour_from_evening = attrsman.add(cm.AttrConf('hour_from_evening', kwargs.get('hour_from_evening', 8),
                                                          groupnames=['options'],
                                                          perm='rw',
        ...
        self.hour_to_evening = attrsman.add(cm.AttrConf('hour_to_evening', kwargs.get('hour_to_evening', 8),
                                                        groupnames=['options'],
                                                        perm='rw',
...
class GpsTrips(Trips):
    ...
    def set_matched_route(self, id_trip, route,
        ...
        self.ids_route_matched[id_route] = id_route
    ...
    def get_ids_route_selected(self):
        ...
        ids_route = self.ids_route_matched[self.get_ids_selected()]
        return ids_route[ids_route >= 0]
