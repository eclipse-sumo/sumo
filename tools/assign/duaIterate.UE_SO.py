...
# @author  Behzad Bamdad Mehrabani (Contributed to SO)
# @date    2008-02-13
...
def initOptions():
    ...
    argParser.add_argument("--system-optimal", action="store_true", dest="SO",
                           default=False, help="perform system optimal traffic assignment")
    return argParser
        ...
        if not (options.skipFirstRouting and step == options.firstStep):
            ...
            for router_input, output in zip(router_demands, simulation_demands):
                ...
                if options.SO:
                    if options.logit:
                        if step > 1:
                            with open(os.path.join(str(step - 1), get_weightfilename(options, step - 1, "dump")),
                                      'r') as sumo_cur:
                                tree_sumo_cur = ET.parse(sumo_cur)
                                root_sumo_cur = tree_sumo_cur.getroot()
                            with open(os.path.join(str(step - 2), get_weightfilename(options, step - 2, "dump")),
                                      'r') as sumo_prv:
                                        ...
                                        for edge_prv in interval_prv.iter('edge'):
                                            if begin_cur == begin_prv and edge_cur.get("id") == edge_prv.get("id"):
                                                if edge_cur.get("traveltime") is not None and edge_prv.get(
                                                    ...
                                                    veh_cur = round(
                                                        float(edge_cur.get("left") + edge_cur.get("arrived")), 2)
                                                    veh_prv = round(
                                                        float(edge_prv.get("left") + edge_prv.get("arrived")), 2)
                                                    tt_cur = round(float(edge_cur.get("traveltime")), 2)
                                                    tt_prv = round(float(edge_prv.get("traveltime")), 2)
                                                    dif_tt = abs(tt_cur - tt_prv)
                                                    dif_veh = abs(veh_cur - veh_prv)
                                                    if dif_veh != 0:
                                                        margin = round((dif_tt / dif_veh), 3)
                                                        new_tt = round((margin + tt_cur), 3)
                                                        edge_cur.set("traveltime", str(new_tt))
                            tree_sumo_cur.write(
                                os.path.join(str(step - 1), get_weightfilename(options, step - 1, "dump")))
                    else:
                        argParser.error("SO assignment only works with logit route choice algorithm "
