...
optParser.add_argument("--osmfile", help="give the osm bbox file name generated by osmGet.py", default="osm_bbox.osm.xml", metavar="FILE")
optParser.add_argument("--netfile", help="give the corresponding osm-based net file name", default="osm.net.xml", metavar="FILE")
optParser.add_argument("--cost-modifier", action="store_true", default=False, help="change edge weights for priority edges/routes for evacuation")
optParser.add_argument("--timeline", type=str, help="Set the evacuation portions, seperated by comma; the ratios are decided together with the given duration")             
...
optParser.add_argument("--admin_level", type=float, default= 10, help="select the areas according to the defined admin_level")                    
...
optParser.add_argument("--simulation",action="store_true", default=False, help="directly run the simulation at the end")
...
def genPolyTypeFile(prefix):
    ...
    with open(typeFile, 'w') as outf:
        ...
        outf.write('<polygonType id="barrier"                 name="barrier"     color="1.0,.3,.3" layer="0" fill="false" discard="true"/>\n')
        ...
        outf.write('<polygonType id="boundary"                name="boundary"    color="1.0,.33,.33" layer="0" fill="false" discard="true"/>\n')
        ...
        outf.write('<polygonType id="railway:position"        name="railway.position"        color="blue"  layer="1" discard="true"/>\n')
        outf.write('<polygonType id="railway:position:exact"  name="railway.position.exact"  color="green" layer="2" discard="true"/>\n\n')
        outf.write('</polygonTypes>\n')
    outf.close()
...
def main(args=None):
    ...
    #    outf.write('</polygonTypes>\n')
    #outf.close()
    exeCall = "polyconvert -n %s --osm-files %s --type-file %s --output-file %s --osm.keep-full-type" %(netfile, options.osmfile, typeFile, tazPolyFile)
    ...
    exeCall = "od2trips -n %s -d %s --output-prefix %s --output-file %s --different-source-sink True --begin %s --end %s %s " %(tazFile, maxtrixFile, options.prefix, tripFile, options.begin, (options.begin+options.duration), timeline)
    subprocess.call(exeCall, stdout=sys.stdout, stderr=sys.stderr, shell=True)
