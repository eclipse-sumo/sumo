...
def resetParams(vehID):
    ...
    print ("new errorTimeScaleCoefficient:%s (diff=%s)"%(errorTimeScaleCoefficient, errorTimeScaleCoefficient-new_errorTimeScaleCoefficient))

    new_errorNoiseIntensityCoefficient = 0.22
    traci.vehicle.setParameter(vehID, "device.driverstate.errorNoiseIntensityCoefficient", str(new_errorNoiseIntensityCoefficient))
    errorNoiseIntensityCoefficient = float(traci.vehicle.getParameter(vehID, "device.driverstate.errorNoiseIntensityCoefficient"))
    print ("new errorNoiseIntensityCoefficient:%s (diff=%s)"%(errorNoiseIntensityCoefficient, errorNoiseIntensityCoefficient-new_errorNoiseIntensityCoefficient))
    
    new_speedDifferenceErrorCoefficient = 0.33
    traci.vehicle.setParameter(vehID, "device.driverstate.speedDifferenceErrorCoefficient", str(new_speedDifferenceErrorCoefficient))
    speedDifferenceErrorCoefficient = float(traci.vehicle.getParameter(vehID, "device.driverstate.speedDifferenceErrorCoefficient"))
    print ("new speedDifferenceErrorCoefficient:%s (diff=%s)"%(speedDifferenceErrorCoefficient, speedDifferenceErrorCoefficient-new_speedDifferenceErrorCoefficient))
    ...
    print ("new headwayErrorCoefficient:%s (diff=%s)"%(headwayErrorCoefficient, headwayErrorCoefficient-new_headwayErrorCoefficient))
    
    new_speedDifferenceChangePerceptionThreshold = 0.55
    traci.vehicle.setParameter(vehID, "device.driverstate.speedDifferenceChangePerceptionThreshold", str(new_speedDifferenceChangePerceptionThreshold))
    speedDifferenceChangePerceptionThreshold = float(traci.vehicle.getParameter(vehID, "device.driverstate.speedDifferenceChangePerceptionThreshold"))
    print ("new speedDifferenceChangePerceptionThreshold:%s (diff=%s)"%(speedDifferenceChangePerceptionThreshold, speedDifferenceChangePerceptionThreshold-new_speedDifferenceChangePerceptionThreshold))
    
    new_headwayChangePerceptionThreshold = 0.66
    traci.vehicle.setParameter(vehID, "device.driverstate.headwayChangePerceptionThreshold", str(new_headwayChangePerceptionThreshold))
    headwayChangePerceptionThreshold = float(traci.vehicle.getParameter(vehID, "device.driverstate.headwayChangePerceptionThreshold"))
    print ("new headwayChangePerceptionThreshold:%s (diff=%s)"%(headwayChangePerceptionThreshold, headwayChangePerceptionThreshold-new_headwayChangePerceptionThreshold))
...
if __name__ == "__main__":
    ...
    while not myVehicle in traci.vehicle.getIDList():
        traci.simulationStep()
