...
def subscribeParamWithKey(objectType, object, objectID):
    ...
    object.subscribeParameterWithKey(objectID, "simTime", 3, 5)
...
def getSubscriptionResults(objectType, object, objectID):
    print("%s result: %s" %
          (objectType, object.getAllSubscriptionResults()))
...
for i in range(7):
    print("step", step())
    [setParam(*x) for x in objects]
    [getSubscriptionResults(*x) for x in objects]
