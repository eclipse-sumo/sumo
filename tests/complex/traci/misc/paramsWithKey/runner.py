...
def getParamWithKey(objectType, object, objectID):
    (k, v) = object.getParameterWithKey(objectID, "bar")
    print(objectType, '(%s, %s)' % (k, v))
    object.setParameter(objectID, "bar", "42")
    (k, v) = object.getParameterWithKey(objectID, "bar")
    print(objectType, '(%s, %s)' % (k, v))
...
[getParamWithKey(*x) for x in objects]
print("step", step())
